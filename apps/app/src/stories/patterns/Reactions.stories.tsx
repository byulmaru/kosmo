...
async function assertReactionViewport(canvasElement: HTMLElement, expectedWidth: number) {
  const canvas = within(canvasElement);
  const pickerButtons = quickReactionOptions.map((option) =>
    canvas.getByRole('button', { name: `${option.label} 반응` }),
  );
  const picker = pickerButtons[0]!.parentElement!;
  const canvasRect = canvasElement.getBoundingClientRect();
  const pickerRect = picker.getBoundingClientRect();

  expect(canvasElement.ownerDocument.documentElement.clientWidth).toBe(expectedWidth);
  expect(pickerButtons.every((button) => button.getBoundingClientRect().height === 32)).toBe(true);
...
const meta = {
  component: QuickPickerInteractionCatalog,
  parameters: {
    relay: { data: { nodes: storyProfiles } },
    router: { pathname: '/@kosmo' },
  },
  title: 'KOSMO/Patterns/Reaction/Catalog',
} satisfies Meta<typeof QuickPickerInteractionCatalog>;
...
export const ProfileListStates: Story = {
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() =>
      expect(imageAvatar.querySelector('img')).toHaveAttribute('src', appleTouchIconUrl),
    );
...
export const ZeroCountSummaryIsNotRendered: Story = {
    ...
    relay: {
      operationResponses: {
        ReactionsIntegrationStoriesQuery: {
          data: {
            node: { ...post({ id: 'post-empty', reactionCounts: [] }), viewerReactions: [] },
          },
        },
      },
    },
  ...
  play: ({ canvasElement }) => {
    const canvas = within(canvasElement);
    expect(canvas.queryByTestId('reaction-summary-row')).not.toBeInTheDocument();
    expect(canvas.queryByRole('link', { name: /반응한 프로필 보기/ })).not.toBeInTheDocument();
  },
};
...
export const ReactionViewport320: Story = {
  ...
  play: ({ canvasElement }) => assertReactionViewport(canvasElement, 320),
  render: () => <QuickPickerViewportCatalog title="Reactions at 320px" />,
};

export const ReactionViewport390: Story = {
  globals: { viewport: { isRotated: false, value: 'kosmoMobile' } },
  play: ({ canvasElement }) => assertReactionViewport(canvasElement, 390),
...
export const ReactionViewport600: Story = {
  ...
  play: ({ canvasElement }) => assertReactionViewport(canvasElement, 600),
