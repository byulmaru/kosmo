...
function ProductionComposerQuotePolicyContractStory() {
  const [requests, setRequests] = useState<readonly Variables[]>([]);
  const environment = useMemo(
    () =>
      new Environment({
        network: Network.create((request: RequestParameters, variables: Variables) => {
          if (request.name === 'PostsProductionComposerAdapterStoryQuery') {
            return Promise.resolve({
              data: { composerProfile, homeTimeline },
            } as GraphQLResponse);
          }
          if (request.name === 'PostComposerCreatePostMutation') {
            setRequests((current) => [...current, variables.input as Variables]);
            return Promise.resolve({
              data: {
                createPost: {
                  post: { ...shortPost, __typename: 'Post', id: 'quote-policy-created-in-story' },
                },
              },
            } as GraphQLResponse);
          }
  ...
  return (
    <RelayEnvironmentProvider environment={environment}>
      <Suspense fallback={<Text>Production Composer fixture를 불러오는 중입니다.</Text>}>
        <ProductionComposerQuotePolicyContractContents />
      </Suspense>
      <Text testID="production-composer-request-log">{JSON.stringify(requests)}</Text>
    </RelayEnvironmentProvider>
  );
}

function ProductionComposerQuotePolicyContractContents() {
  const data = useLazyLoadQuery<PostsProductionComposerAdapterStoryQuery>(
    ProductionComposerAdapterStoryQuery,
    {},
  );
  if (data.composerProfile?.__typename !== 'Profile') {
    throw new Error('Production Composer quote policy story requires a profile.');
  }
  return (
    <Catalog>
      <PostComposer
...
function PostMediaViewerRevisionStory() {
  ...
  const originalContent = mediaViewerThreadCurrentPost.content!;
      ...
      <Pressable
        ...
        onPress={() => {
          commitLocalUpdate(environment, (store) => {
            const postRecord = store.get(mediaViewerThreadCurrentPost.id);
            if (!postRecord) {
              throw new Error('Viewer Host Post fixture가 필요합니다.');
            }
            if (phase.current === 0) {
              postRecord.setValue(null, 'content');
            } else if (phase.current === 1) {
              const contentRecord = store.get(originalContent.id);
              if (!contentRecord) {
                throw new Error('Viewer Host Content fixture가 필요합니다.');
              }
              postRecord.setLinkedRecord(contentRecord, 'content');
            } else {
              const contentRecord = store.create(
                'content-post-media-viewer-quote-revision',
                'PostContent',
              );
              ...
              contentRecord.setLinkedRecords(
                originalContent.media!.map((media, index) => {
                  ...
                  mediaRecord.setValue(`revision ${index + 1}`, 'altText');
                  mediaRecord.setValue(`viewer-revision-media-${index}`, 'id');
                  mediaRecord.setValue(postMediaImageUri, 'url');
                  return mediaRecord;
                }),
                'media',
              );
        ...
        testID="post-media-viewer-content-transition"
      >
        <Text>Viewer Content 상태 전환</Text>
      </Pressable>
    </>
  );
}
...
export const PostMediaViewerCompact: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(await viewer.findByTestId('post-media-viewer-counter')).toHaveTextContent('2 / 4');
...
export const PostMediaViewerWide: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(nestedOrigin);
    await waitFor(() => expect(screen.getAllByTestId('post-media-viewer-dialog')).toHaveLength(2));
    const nestedDialog = screen
      .getAllByTestId('post-media-viewer-dialog')
      .find((candidate) => !candidate.contains(nestedOrigin));
    expect(nestedDialog).toBeDefined();
    const nestedClose = within(nestedDialog!).getByRole('button', {
      name: '이미지 뷰어 닫기',
    });
    await userEvent.click(nestedClose);
...
export const PostMediaViewerHostContentRevision: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await userEvent.click(canvas.getByRole('button', { name: '2번째 첨부 이미지 크게 보기' }));

    const viewer = within(await screen.findByRole('dialog'));
    await viewer.findByTestId('post-media-viewer-counter');
    await userEvent.keyboard('{ArrowRight}{ArrowRight}');
    expect(viewer.getByTestId('post-media-viewer-counter')).toHaveTextContent('4 / 4');
    await userEvent.click(await viewer.findByRole('button', { name: '원문 더 보기' }));
    const transition = canvas.getByTestId('post-media-viewer-content-transition');

    transition.click();
    await expect(viewer.findByTestId('post-media-viewer-unavailable')).resolves.toBeVisible();
    transition.click();
    await waitFor(() =>
      expect(viewer.getByTestId('post-media-viewer-counter')).toHaveTextContent('4 / 4'),
    );
    expect(await viewer.findByRole('button', { name: '원문 접기' })).toBeVisible();

    transition.click();
    await waitFor(() =>
      expect(viewer.getByTestId('post-media-viewer-counter')).toHaveTextContent('2 / 4'),
    );
    expect(await viewer.findByRole('button', { name: '원문 더 보기' })).toBeVisible();
  },
  render: () => <PostMediaViewerRevisionStory />,
};
...
export const PostMediaViewerSingle: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const viewer = within(await screen.findByRole('dialog'));
    expect(await viewer.findByTestId('post-media-viewer-image')).toBeVisible();
    expect(viewer.getByTestId('post-media-viewer-image')).toHaveAttribute(
      'aria-label',
      '회색 배경의 첫 번째 첨부 이미지',
    );
...
export const ProductionComposerQuotePolicyMutationContract: Story = {
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await userEvent.click(canvas.getByRole('button', { name: /공개 범위:/ }));
    const menu = await canvas.findByRole('radiogroup', { name: '공개 범위 선택' });
    await userEvent.click(within(menu).getByRole('radio', { name: /^팔로워:/ }));
    await userEvent.type(canvas.getByRole('textbox', { name: '게시물 내용' }), '팔로워 인용 정책');
    await userEvent.click(canvas.getByRole('button', { name: '게시' }));

    await waitFor(() =>
      expect(canvas.getByTestId('production-composer-request-log')).toHaveTextContent(
        JSON.stringify([
          {
            bodyText: '팔로워 인용 정책',
  ...
  render: () => <ProductionComposerQuotePolicyContractStory />,
};
...
export const ComposerVisibilityAndSubmitInteraction: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    let menu = await canvas.findByRole('menu', { name: '게시글 공개 및 인용 설정' });
...
export const ComposerProfileDefaultVisibilitySeed: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const menu = await canvas.findByRole('menu', { name: '게시글 공개 및 인용 설정' });
...
export const ComposerVisibilityFocusLifecycle: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await pressTouch(within(menu).getByRole('menuitemradio', { name: /^모두:/ }));
...
export const ComposerVisibilityKeyboardFocusLifecycle: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(trigger).toHaveFocus();
    await userEvent.keyboard('{Enter}');
    const keyboardMenu = await canvas.findByRole('menu', { name: '게시글 공개 및 인용 설정' });
...
export const ComposerReplyMutationContract: Story = {
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(canvas.getByRole('button', { name: '조용한 공개' }));
    const menu = await canvas.findByRole('menu', { name: '답글 공개 및 인용 설정' });
    expect(within(menu).queryByRole('menuitemradio', { name: /언급한 계정만/ })).toBeNull();
...
export const ComposerReplyProfileDefaultVisibilitySeed: Story = {
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(canvas.getByRole('button', { name: '공개' }));
    const menu = await canvas.findByRole('menu', { name: '답글 공개 및 인용 설정' });
    const visibilityGroup = within(menu).getByRole('group', { name: '답글 공개 범위' });
    expect(within(visibilityGroup).getAllByRole('menuitemradio')).toHaveLength(3);
    expect(within(menu).queryByRole('menuitemradio', { name: /^언급한 계정만/ })).toBeNull();
  },
...
export const ReplyModalPresentation: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(visibilityButton);
    const visibilityMenu = await within(dialog).findByRole('menu', {
      name: '답글 공개 및 인용 설정',
    });
    ...
    await userEvent.keyboard('{Escape}');
    await waitFor(() => {
      expect(within(dialog).queryByRole('menu', { name: '답글 공개 및 인용 설정' })).toBeNull();
    });
    ...
    await userEvent.click(visibilityButton);
    await userEvent.click(
      within(await within(dialog).findByRole('menu', { name: '답글 공개 및 인용 설정' })).getByRole(
        'menuitemradio',
        { name: /^공개/ },
      ),
    );
...
export const ReplyDetailInlineIntegration: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const quoteVisibilityMenu = await canvas.findByRole('menu', {
      name: '게시글 공개 및 인용 설정',
    });
    ...
    await userEvent.keyboard('{Escape}');
    await waitFor(() => {
      expect(canvas.queryByRole('menu', { name: '게시글 공개 및 인용 설정' })).toBeNull();
    });
...
export const QuoteModalFailureLifecycle: Story = {
  ...
  play: async () => {
    ...
    await userEvent.click(visibilityButton);
    let visibilityMenu = await screen.findByRole('menu', {
      name: '게시글 공개 및 인용 설정',
    });
    ...
    await userEvent.keyboard('{Escape}');
    await waitFor(() => {
      expect(screen.queryByRole('menu', { name: '게시글 공개 및 인용 설정' })).toBeNull();
    });
    expect(visibilityButton).toHaveFocus();
    await userEvent.click(visibilityButton);
    visibilityMenu = await screen.findByRole('menu', {
      name: '게시글 공개 및 인용 설정',
    });
    await userEvent.click(within(visibilityMenu).getByRole('menuitemradio', { name: /^공개/ }));
    await userEvent.click(within(visibilityMenu).getByRole('menuitemradio', { name: /^모두:/ }));
    expect(within(dialog).getByRole('button', { name: '공개' })).toBeVisible();
    ...
    await screen.findByRole('menu', { name: '게시글 공개 및 인용 설정' });
