...
export const ExpandCollapseAndSelection: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await canvas.findByRole('tab', { name: '전구 반응 2개' });
    expect(within(getTablist()).getAllByRole('tab')).toHaveLength(7);
    expect(canvas.getByRole('tab', { name: '전구 반응 2개' })).toBeVisible();

    const selectedOutlier = canvas.getByRole('tab', { name: '전구 반응 2개' });
