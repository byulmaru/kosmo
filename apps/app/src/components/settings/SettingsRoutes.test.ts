...
describe('Settings routes', () => {
  ...
  it('full Web root는 320px master와 flexible Profile detail을 함께 표시한다', async () => {
    ...
    assert.equal(rendered('PageHeader')[1].props.leading, undefined);
  ...
  it('full Web detail은 공통 root master와 설정으로 돌아가는 back action을 표시한다', async () => {
  ...
  it('full Web mute category는 root master의 진입점을 선택한다', async () => {
