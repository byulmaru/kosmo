...
const PaginationScrollViewHost = 'PaginationScrollView' as unknown as ElementType;
...
let ReactionPeopleRouteFrame: ComponentType<{
  children?: ReactNode;
  onBack: () => void;
  scrollKey: string;
}>;
...
before(async () => {
  ({ ReactionPeopleHeader, ReactionPeopleRouteFrame, ReactionPeopleScreen } =
    await import('./ReactionPeopleScreen'));
});
...
test('Web mount keeps DOM heading focus behavior', async () => {
  ...
  await act(async () => {
    renderer = create(createElement(ReactionPeopleHeader, { onBack: () => undefined }), {
      createNodeMock: (element) => (element.type === TextHost ? webHeading : null),
    });
  });
...
test('Native Type change preserves the route scroll container', async () => {
  ...
  await act(async () => {
    renderer = create(
      createElement(
        ReactionPeopleRouteFrame,
        { onBack: () => undefined, scrollKey: props.postId },
        createElement(ReactionPeopleScreen, props),
      ),
      {
        createNodeMock: () => null,
      },
  ...
  assert.ok(renderer);
  const container = renderer.root.findByType(PaginationScrollViewHost);
  ...
  await act(async () => {
    renderer?.update(
      createElement(
        ReactionPeopleRouteFrame,
        { onBack: () => undefined, scrollKey: props.postId },
        createElement(ReactionPeopleScreen, { ...props, reactionType: '🎉' }),
      ),
    );
  });

  assert.ok(renderer);
  assert.equal(renderer.root.findByType(PaginationScrollViewHost), container);
});
