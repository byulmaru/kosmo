...
test(
  'Pair Follow는 초기 중복을 합치고 PENDING 중복을 idempotent하게 처리한다',
  { timeout: 120_000 },
  async (t) => {
    const environment = await TestWorkflowEnvironment.createLocal({
      server: { executable: { type: 'cached-download', version: 'v1.8.2' } },
    ...
    const taskQueue = KOSMO_TASK_QUEUE + '-follow-duplicate-' + process.pid;
    const openPair = {
      followerProfileId: '00000000-0000-8000-8000-000000000501',
      followeeProfileId: '00000000-0000-8000-8000-000000000502',
    };
    const openFollowId = '00000000-0000-8000-8000-000000000503';
    const pendingPair = {
      followerProfileId: '00000000-0000-8000-8000-000000000509',
      followeeProfileId: '00000000-0000-8000-8000-000000000510',
    };
    const pendingRequestId = '00000000-0000-8000-8000-000000000511';
    const calls: string[] = [];
    const transitionCalls = new Map<string, number>();
    let releaseOpenTransition!: () => void;
    const openTransitionReleased = new Promise<void>((resolve) => {
      releaseOpenTransition = resolve;
    });
    let openTransitionStarted!: () => void;
    const openTransitionStartedPromise = new Promise<void>((resolve) => {
      openTransitionStarted = resolve;
    });
    let releaseOpenEffect!: () => void;
    const openEffectReleased = new Promise<void>((resolve) => {
      releaseOpenEffect = resolve;
    });
    let openEffectStarted!: () => void;
    const openEffectStartedPromise = new Promise<void>((resolve) => {
      openEffectStarted = resolve;
    });
      ...
      effectPlan: [
        {
          ...
          input: { sourceId: openFollowId, sourceKind: 'FOLLOW' as const },
        },
      ],
    };
    const pendingCreatedExecution = {
      ok: true as const,
      ...
      effectPlan: [
        {
          kind: 'CREATE' as const,
          input: { sourceId: pendingRequestId, sourceKind: 'FOLLOW_REQUEST' as const },
        },
      ],
      pendingRequestId,
    };
    const pendingDuplicateExecution = {
      ok: true as const,
      nextState: 'PENDING' as const,
      result: {
        commandKind: 'FOLLOW' as const,
    ...
    const worker = await Worker.create({
      activities: {
        ...
        executeProfileFollowPairTransitionActivity: async (
          ...
          const count = (transitionCalls.get(input.pair.followerProfileId) ?? 0) + 1;
          transitionCalls.set(input.pair.followerProfileId, count);
          if (input.pair.followerProfileId === openPair.followerProfileId) {
            openTransitionStarted();
            await openTransitionReleased;
            return openExecution;
          }
          if (input.command.kind === 'FOLLOW') {
            return count === 1 ? pendingCreatedExecution : pendingDuplicateExecution;
          }
          return {
            ok: true as const,
        ...
        createFollowNotificationActivity: async (sourceId: string) => {
          calls.push('follow:' + sourceId);
          if (sourceId === openFollowId) {
            openEffectStarted();
            await openEffectReleased;
          }
        },
        ...
        deleteFollowRequestNotificationActivity: async (sourceId: string) => {
          calls.push('delete-request:' + sourceId);
        },
      },
      connection: environment.nativeConnection,
      namespace: environment.namespace,
    ...
    const startPair = (pair: typeof openPair) =>
    ...
    const followCommand = { kind: 'FOLLOW' as const, origin: 'LOCAL' as const };

    await worker.runUntil(async () => {
      try {
        const openStart = startPair(openPair);
        const firstOpenPromise = environment.client.workflow.executeUpdateWithStart(
          'profileFollowPairUpdate',
          { args: [followCommand], startWorkflowOperation: openStart },
        );
        await openTransitionStartedPromise;
        const openHandle = await openStart.workflowHandle();
        const concurrentOpen = await openHandle.startUpdate('profileFollowPairUpdate', {
          args: [followCommand],
          waitForStage: WorkflowUpdateStage.ACCEPTED,
        });
        releaseOpenTransition();

        const [firstOpen, duplicateOpen] = await Promise.all([
          firstOpenPromise,
          concurrentOpen.result(),
        ]);
        assert.deepEqual(duplicateOpen, firstOpen);
        await openEffectStartedPromise;
        await assert.rejects(
          openHandle.executeUpdate('profileFollowPairUpdate', { args: [followCommand] }),
        );
        assert.equal(transitionCalls.get(openPair.followerProfileId), 1);
        releaseOpenEffect();
        await openHandle.result();

        const pendingStart = startPair(pendingPair);
        const firstPending = await environment.client.workflow.executeUpdateWithStart(
          'profileFollowPairUpdate',
          { args: [followCommand], startWorkflowOperation: pendingStart },
        );
        const pendingHandle = await pendingStart.workflowHandle();
        const duplicatePending = await pendingHandle.executeUpdate('profileFollowPairUpdate', {
          args: [followCommand],
        });
        assert.deepEqual(firstPending, { ok: true, result: pendingCreatedExecution.result });
        assert.deepEqual(duplicatePending, {
          ok: true,
          result: pendingDuplicateExecution.result,
        });
        assert.equal(transitionCalls.get(pendingPair.followerProfileId), 2);

        await pendingHandle.executeUpdate('profileFollowPairUpdate', {
          args: [
            {
              kind: 'REJECT',
        ...
        await pendingHandle.result();
        assert.deepEqual(calls, [
          'follow:' + openFollowId,
          'request:' + pendingRequestId,
          'delete-request:' + pendingRequestId,
        ]);
      } finally {
        releaseOpenTransition();
        releaseOpenEffect();
...
test(
  ...
  async (t) => {
    ...
    const effectInput = {
      sourceId: '00000000-0000-8000-8000-000000000703',
    ...
    const execution = {
      ...
      result: {
        created: true,
        profileBlockId: '00000000-0000-7000-8000-000000000704',
        ownerProfileId: input.ownerProfileId,
        targetProfileId: input.targetProfileId,
      },
      unfollowInputs: [effectInput],
    };
    const calls: string[] = [];
    let releaseEffect!: () => void;
    const effectReleased = new Promise<void>((resolve) => {
...
test(
  ...
  async (t) => {
    ...
    const taskQueue = KOSMO_TASK_QUEUE + '-profile-unblock-exact-' + process.pid;
    const input = {
    ...
    await worker.runUntil(async () => {
      const startWorkflowOperation = new WithStartWorkflowOperation('profileUnblockWorkflow', {
        args: [input],
        taskQueue,
        workflowId: 'profile-unblock-test:' + process.pid + ':exact',
        workflowIdConflictPolicy: 'USE_EXISTING',
