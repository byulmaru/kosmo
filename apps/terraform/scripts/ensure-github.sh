...
gh variable set FIREBASE_IOS_APP_ID --repo "${repository}" --env "${native_environment}" --body "$(terraform_output firebase_ios_app_id)"
gh variable set FIREBASE_PROJECT_ID --repo "${repository}" --env "${native_environment}" --body "$(terraform_output firebase_project_id)"
gh variable set FIREBASE_TESTER_GROUP --repo "${repository}" --env "${native_environment}" --body native-testers
gh variable set GCP_SERVICE_ACCOUNT --repo "${repository}" --env "${native_environment}" --body "$(terraform_output gcp_service_account)"
gh variable set GCP_WORKLOAD_IDENTITY_PROVIDER --repo "${repository}" --env "${native_environment}" --body "$(terraform_output gcp_workload_identity_provider)"
