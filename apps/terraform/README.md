...
`main`을 push하면 Docker Build는 환경 중립 image를 한 번 build하고 runtime digest map을 `canonical-release-manifest`로 게시한다. Source SHA와 build identity는 해당 GitHub Actions run이 소유하며 `Deploy Dev`는 같은 run의 digest를 Argo에 전달한다. Production release는 이 push로 시작하지 않으며, Main에 저장된 release workflow를 `main` ref에서 `workflow_dispatch`로 실행할 때만 요청된다.

`target_sha`를 입력하면 repository에 존재하는 정확한 40자리 commit SHA를 사용하고, 비워 두면 preflight가 최신 `main` commit을 immutable target으로 확정한다. Preflight는 성공한 trusted canonical artifact가 완전하게 존재하는지 확인하며, 없으면 다른 tag나 SHA로 대체하지 않고 fail-closed한다. `prod` Environment required reviewer 승인 전에는 production credential·Argo mutation·image build를 수행하지 않는다. 승인 뒤에도 image를 다시 build하거나 production Sentry DSN을 조회하지 않고 같은 canonical digest를 migration과 모든 활성화 workload에 사용한다. Workflow summary에는 requester, workflow definition ref, target SHA, canonical build run과 digest를 남긴다. Git tag, `production` branch와 일반 branch push는 production source·approval·배포를 시작하지 않는다.
