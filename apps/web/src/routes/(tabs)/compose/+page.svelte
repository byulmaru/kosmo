...
<section class="grid w-[min(100%,36rem)] gap-5 self-start">
  ...
  {#if query.loading}
    <div class="border-border bg-card grid gap-3 rounded-[12px] border p-4" aria-hidden="true">
      <div class="bg-surface h-5 w-36 animate-pulse rounded-[8px]"></div>
      <div class="bg-surface h-40 animate-pulse rounded-[12px]"></div>
      <div class="flex justify-end">
        <div class="bg-surface h-10 w-30 animate-pulse rounded-[8px]"></div>
      </div>
    </div>
    <span class="sr-only" role="status">글쓰기 화면을 불러오는 중입니다.</span>
  {:else if query.error}
    <div class="border-border bg-card rounded-[12px] border p-5" role="alert">
  ...
  {:else if !session}
    <div class="border-border bg-card rounded-[12px] border p-5">
