...
>
  {#each tabs as tab}
    ...
    <button
      class={`relative text-xs font-semibold ${selected ? 'text-foreground' : 'text-muted-foreground'}`}
      type="button"
      role="tab"
    ...
    >
      {tab}
      {#if selected}
        <span class="bg-foreground absolute inset-x-8 bottom-0 h-0.5 rounded-full"></span>
      {/if}
    </button>
  {/each}
</div>
