<script lang="ts">
  const colorTokens = [
    ...
    { name: 'textPrimary', light: '#111111', dark: '#ffffff', usage: 'primary text' },
    { name: 'textSecondary', light: '#777777', dark: '#777777', usage: 'secondary text' },
...
<div class="bg-background text-foreground min-h-screen px-6 py-8">
  <div class="mx-auto grid max-w-5xl gap-10">
    <header class="grid gap-3">
      <p class="text-primary-foreground w-fit rounded-[8px] bg-primary px-3 py-1 text-sm font-bold">
        KOSMO
      </p>
      <h1 class="m-0 text-3xl leading-tight font-bold">Design foundations</h1>
      <p class="text-secondary-foreground m-0 max-w-2xl text-base leading-7">
        ...
        {#each colorTokens as token}
          <article class="border-border bg-card grid gap-3 rounded-[8px] border p-4">
            ...
            <div>
              ...
              <p class="text-secondary-foreground m-0 text-sm">{token.usage}</p>
            </div>
          </article>
        {/each}
      </div>
        ...
        <div class="mt-4 grid gap-3">
          {#each spacingTokens as [name, value]}
            <div class="grid grid-cols-[5rem_1fr_3rem] items-center gap-3 text-sm">
              ...
              ></span>
              <span class="text-secondary-foreground text-right">{value}px</span>
            </div>
          {/each}
        </div>
      </div>
        ...
        <div class="mt-4 grid grid-cols-2 gap-3">
          {#each radiusTokens as [name, value]}
            <div class="grid gap-2">
              <div
                class="border-border bg-secondary size-14 border"
                style:border-radius={value === 999 ? '999px' : `${value}px`}
              ></div>
              <div class="text-sm">
                <p class="m-0 font-semibold">{name}</p>
                <p class="text-secondary-foreground m-0">{value}px</p>
              </div>
            </div>
          {/each}
        </div>
          ...
          <article class="border-border bg-card rounded-[8px] border p-4">
            ...
            <div class="mt-3 flex flex-wrap gap-2">
              {#each group.components as component}
                <span
                  class="bg-secondary text-foreground rounded-full px-3 py-1 text-sm font-semibold"
                >
                  {component}
                </span>
              {/each}
