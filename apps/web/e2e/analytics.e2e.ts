...
test('Web runtime은 route template pageview만 전송하고 automatic telemetry와 민감 URL을 보내지 않는다', async ({
  ...
  await page.goto('/?secret_query=should-not-be-captured#secret-fragment');
  await expect
  ...
  expect(pageviews.map((payload) => payload.properties?.route_template)).toEqual(['/', '/privacy']);
...
test('Account identity는 opaque Account ID만 보내고 analytics endpoint 실패에도 인증 흐름을 유지한다', async ({
  ...
  expect(identifyPayload?.properties?.distinct_id).toBe(viewer.account.id);
