...
test('profile selection updates selected profile from mutation payload without currentSession refetch', async ({
  page,
}) => {
  ...
  await expectFollowCountLabelsInOrder(sidebarProfileSection(page));

  await page.goto('/@alpha');
  await expect(page.getByRole('heading', { name: 'alpha' })).toBeVisible();
  await expectFollowCountLabelsInOrder(profileHero(page));
...
function sidebarProfileHandle(page: Page, handle: string) {
  return sidebarProfileSection(page).getByText(`@${handle}`);
}

function sidebarProfileSection(page: Page) {
  return page.locator('section[aria-label="활성 프로필"]:visible').first();
}

function profileHero(page: Page) {
  return page.getByRole('main').locator('header').first();
}

async function expectFollowCountLabelsInOrder(locator: Locator) {
  const text = await locator.textContent();
  const followingIndex = text?.indexOf('팔로잉') ?? -1;
  const followersIndex = text?.indexOf('팔로워') ?? -1;

  expect(followingIndex).toBeGreaterThanOrEqual(0);
  expect(followersIndex).toBeGreaterThanOrEqual(0);
  expect(followingIndex).toBeLessThan(followersIndex);
}
