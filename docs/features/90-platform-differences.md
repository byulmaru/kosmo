...
이번 조사에서 기존 기능 리스트에 보강할 Mastodon 기능이다.

| 항목                  | Mastodon 기준                                       | Kosmo 결정 포인트                              |
| --------------------- | --------------------------------------------------- | ---------------------------------------------- |
| 개인 프로필 노트      | 특정 계정에 본인만 보는 note를 저장                 | 관계 메타데이터로 둘지, 프로필 기능으로 둘지   |
| Featured profiles     | 내 프로필에 추천 계정을 노출                        | 고정 게시/추천 계정/하이라이트를 묶을지        |
| Featured tags         | 내 프로필에 자주 쓰는 해시태그를 노출               | 해시태그 타임라인과 프로필을 어떻게 연결할지   |
| 팔로우별 boost 설정   | 특정 계정의 boost를 홈에서 받을지 선택              | 팔로우와 관계별 수신 설정을 분리할지           |
| 팔로우별 새 글 알림   | 특정 계정 새 게시 알림을 켤 수 있음                 | 관계 설정과 알림 설정의 경계를 정해야 함       |
| 팔로우별 언어 필터    | 특정 계정 게시를 지정 언어로만 받을 수 있음         | 언어 감지/언어 입력 정책과 함께 설계해야 함    |
| Followed hashtags     | 해시태그를 팔로우하고 관련 게시를 받을 수 있음      | 홈 타임라인 포함 여부와 별도 피드 여부         |
| 프로필 디렉터리       | 서버가 알고 있는 discoverable 프로필 목록           | 로컬 전용/원격 포함, 정렬, 제외 정책           |
| Grouped notifications | 반복 알림을 서버에서 묶어 반환                      | 알림 저장 모델과 payload 최적화 방향           |
| 사용자 preferences    | 기본 공개 범위, 민감 표시, 언어, media/CW 펼침 설정 | 계정 설정과 작성 화면 기본값의 source of truth |

## 현재 코드상 확인된 결정 메모

- 현재 게시 본문 제한은 500자다.
- 현재 게시 공개 범위 enum은 Mastodon과 유사한 `PUBLIC`, `UNLISTED`, `FOLLOWERS`, `DIRECT` 4종이다.
- 현재 작성 UI 기본 공개 범위는 `UNLISTED`다.
- 현재 `DIRECT`는 접근 정책이 완성되지 않았으므로 DM 또는 비공개 대화 기능으로 확정하지 않는다.
- 현재 프로필 handle은 3-30자, 영문/숫자/밑줄만 허용한다.
- 현재 프로필 bio는 500자 이하로 제한된다.
- 현재 팔로우 요청 상태 enum은 있으나 새 팔로우는 즉시 `ACCEPTED`로 생성된다.
- 현재 검색은 사람 탭의 정확 handle 조회만 완성 구현으로 확인된다.
- 현재 이미지 업로드 엔드포인트는 있으나 게시 첨부 연결은 확인되지 않았다.
