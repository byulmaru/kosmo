...
- [Post List 컨텍스트](../contexts/post-list.md)는 Home Post List 후보 집합에서 원본 Post와 Reply
  Post를 분리하고, Repost 원본 Author Profile과 mute 적용 위치/결정을 명시한다.
...
- [Discovery 컨텍스트](../contexts/discovery.md)는 검색 scope에 적용된 mute와 실제 moderation action만
  검색 제외 조건으로 사용하고, 해시태그 목록 후보를 공개 원본 Post로 제한한다.
