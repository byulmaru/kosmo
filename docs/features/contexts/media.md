...
Media의 업로드 주체, 사용 주체, 파일 원본, 파생 이미지, Alt Text, Media Proxy를 정의한다.

## 컨텍스트 관계

- 상위: [DDD 도메인 명세 인덱스](../README.md)
- upstream: [Identity](./identity.md)
- policy upstream: [Publishing](./publishing.md), [Identity](./identity.md), [Trust & Safety](./trust-safety.md)
- downstream: [Publishing](./publishing.md), [Discovery](./discovery.md)
- peer: [Notification](./notification.md), [Post List](./post-list.md)

## DDD 명세

- 컨텍스트 경계: 로컬 이미지 업로드, 원격 이미지, 파일 원본, 파생 이미지, Alt Text, Media Proxy를
  정의한다. 게시 본문, Post-Media 연결, 프로필 편집, Profile-Media 연결, Post 단위 민감한 미디어 상태,
  Media 접근 권한의 원본 판단은 소유하지 않는다.
- 보편 언어: Media, File Original, Derived Image, Thumbnail, Alt Text, Remote Media, Media Proxy.
- 핵심 모델: Media를 aggregate root 후보로 둔다. File Original과 Derived Image는 Media의 물리 파일
  표현으로 둔다.
- 값 객체 후보: File Validation Policy, Alt Text, Remote URL.
- 불변 조건: 팔로워 공개 또는 멘션한 프로필만 Post의 미디어 접근 권한은 URL만으로 우회할 수 없어야
  한다. 로컬 업로드와 원격 미디어는 출처를 구분해야 한다.
- 도메인 이벤트 후보: MediaUploaded, DerivedImageReady, RemoteMediaCached, MediaDeleted.
- 정책 후보: 파일 검증 정책, Alt Text 요구 수준, 원격 Media Proxy/Cache.
...
- 이미지 파일은 형식, 크기, 픽셀 캔버스 검증 정책의 대상이다.
...
- Media 접근 권한의 원본 판단은 연결된 Post Visibility와 Profile 표현 정책을 따른다.
...
- 미디어는 업로드 Account, 사용 Profile, 원본 파일, 썸네일 또는 파생 이미지, 원격 원본 위치, Alt
  Text를 가질 수 있다.
- 파일 원본은 Media 검증 정책에 필요한 메타데이터를 가진다.
