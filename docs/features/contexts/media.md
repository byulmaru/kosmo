...
- 컨텍스트 경계: 로컬 이미지 업로드, 원격 이미지, 파일 원본, 파생 이미지, 접근성 설명, 민감한
  미디어 상태를 정의한다. 게시 본문과 프로필 편집 자체는 소유하지 않는다.
...
- 정책 후보: 파일 크기/형식/픽셀 캔버스 제한, alt text 요구 수준, 민감한 미디어 표시, 원격 미디어
  proxy/cache, EXIF 제거.
...
- 첨부 개수 제한이 필요하다.
- 파일 크기, MIME type, 픽셀 캔버스 크기 제한이 필요하다.
