...
Profile 대상 소셜 알림과 Account 대상 운영 알림의 생성, 억제, 읽음 상태, 알림 설정을 정의한다.
...
- Profile이 작성한 Post에 Reaction이 생성되면 Reaction Notification 생성 대상이다.
- Profile은 Reaction Notification 설정을 끌 수 있다.
- 팔로워 공개 또는 멘션한 프로필만 Post의 반응 알림은 Post Visibility를 따라야 한다.

### 재게시

- Profile이 작성한 Post가 Repost되면 Repost Notification 생성 대상이다.
- Author Profile은 Repost Notification 설정을 끌 수 있다.
- 원본 게시가 팔로워 공개 또는 멘션한 프로필만이면 Repost 자체가 제한된다.
- thread mute는 해당 Post/thread에서 발생하는 답글, Reaction, Repost 알림을 끄는 단위로 사용한다.

### 팔로우

- Profile을 대상으로 Follow Relationship이 생성되면 Follow Notification 생성 대상이다.
- 승인제 Profile에 요청 대기 Follow Relationship이 생성되면 Follow Request Notification을 생성한다.
- 승인/거절 후 요청 알림은 처리됨 상태가 된다.

### 특정 프로필 새 게시 알림

- Social Graph의 관계별 새 Post 알림 preference가 켜져 있으면 팔로우한 특정 Profile의 새 Post
  Notification 생성 대상이다.
- Notification은 관계별 새 Post 알림 preference를 소유하지 않고 Social Graph의 설정을 소비한다.

### 운영 알림

- Operational Notification은 계정 제한, 보안 경고, 정책 변경, 서버 공지를 대상으로 한다.
...
- 알림 항목은 관련 Post, Profile, 요청 대기 Follow Relationship, 운영 메시지 중 하나를 참조할 수
  있다.
