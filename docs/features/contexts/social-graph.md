...
Profile 간 Follow Relationship과 관계 기반 권한, 게시 목록 후보 계산에 필요한 관계 상태를 정의한다.
...
- 정책 후보: 거절된 Follow Request의 재요청 허용, 관계별 새 Post 알림 preference.
...
- 서버 Domain Block에 걸린 원격 Domain의 Profile은 팔로우할 수 없고 관계 후보에서 제외한다.

### 언팔로우

- Profile은 팔로우를 해제할 수 있다.
- 언팔로우 후 해당 Follow Relationship은 Home Post List 후보 계산에 사용되지 않는다.
- 이미 받은 알림은 삭제하지 않는다.
...
- 요청 취소, 언팔로우, 차단으로 인한 제거 이력은 별도 Follow State로 두지 않는다.
- block과 Domain Block은 follow보다 우선한다. 차단 시 기존 follow는 삭제한다.

## 관계 상태와 접근 원칙

- 팔로우 관계는 요청 대기, 수락, 거절 상태를 가진다.
- 요청 취소, 언팔로우, 차단 삭제는 요청 대기/수락/거절 외의 별도 Follow State를 만들지 않는다.
- Identity의 Profile 팔로우 승인 정책은 자유와 승인제로 나뉜다.
- 팔로우 행동은 active Profile 기준으로 수행되며 자기 자신을 팔로우할 수 없다.
- 서버 Domain Block에 걸린 원격 Domain의 Profile은 Follow Relationship 생성과 관계 후보 계산에서
  제외한다.
- 자유 Profile의 새 Follow Relationship은 즉시 수락 상태가 되고, 승인제 Profile의 새 Follow
  Relationship은 요청 대기 상태가 된다.
- 거절된 요청은 다시 보낼 수 있다.
