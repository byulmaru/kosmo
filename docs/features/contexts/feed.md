...
- 정책 후보: 답글 포함 여부, Repost 포함 여부, 새 게시 자동 삽입, 로컬/연합 피드 노출
  범위, 추천 정렬 포함 여부, control별 숨김/접힘/경고 처리.
...
- Media control: 민감 미디어, 차단된 미디어, 변환 실패 미디어의 표시 정책을 적용한다.
...
뮤트, 민감 콘텐츠처럼 Profile이 숨김 사유를 보고 펼칠 수 있는 정책에 사용한다.
...
- 답글 포함 여부는 정책으로 정한다.
- Repost는 팔로우 관계와 관계별 수신 설정을 통해 홈 피드에 들어올 수 있다.
- block, Profile mute, word mute, media, reply, Repost control을 반영해야 한다.
- 새 게시가 있을 때 자동 삽입할지, 수동으로 불러오게 할지 정해야 한다.
...
모든 Feed는 기본적으로 다음 Feed Control을 적용해야 한다.

- viewer와 차단 관계가 있는 Profile의 Post는 Exclude한다.
- viewer가 뮤트한 Profile의 Post는 숨김 또는 접힘 정책을 적용한다.
- viewer가 설정한 단어, 문구, 해시태그 필터를 적용한다.
- 민감 미디어와 표시 제한 미디어의 표시 정책을 적용한다.
- upstream에서 전달된 Post Eligibility 변경은 기존 Feed Item에도 반영한다.
...
- Saved Search가 나중에 Post 목록 화면을 제공하더라도, 소유권은 Discovery 결정 이후 다시 본다.
