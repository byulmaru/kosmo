...
- 컨텍스트 경계: 검색, 원격 Profile lookup, 해시태그, 추천, 탐색 탭, trend를 정의한다.
  프로필과 게시의 원본 생명주기는 소유하지 않는다.
- 보편 언어: Search Query, Search Result, Lookup, Hashtag, Explore, Trend, Directory.
- 핵심 모델: Search Index, Saved Search를 aggregate 후보로 둔다.
...
- 도메인 이벤트 후보: SearchPerformed, RecentSearchSaved, RemoteProfileResolved, TrendApproved.
