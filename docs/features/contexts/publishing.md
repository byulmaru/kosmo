...
- 정책 후보: 본문 길이, 멘션한 프로필만 수신자 정책, 삭제와 수정 이력,
  content warning 요구 조건.
...
- Feed, Discovery, Notification은 Post Visibility를 새로 정의하지 않고 Publishing이 판단한 visible
  상태를 소비한다.
- Post Visibility 변경은 `PostVisibilityChanged` 도메인 이벤트로 downstream에 전달한다.
...
- content warning과 spoiler text는 게시 본문의 표시 정책과 별도 속성으로 다룬다.
...
인용은 확산과 괴롭힘 위험이 함께 있는 기능이므로 이름과 기본 정책을 확정하지 않는다.

- 인용 게시가 확산 기능으로 작동할 수 있으므로 작성자 통제와 동의 모델이 중요하다.

Kosmo에서 인용을 포함할지는 제품 결정으로 남긴다. 도입한다면 다음 정책이 필요하다.

- 작성자가 내 게시의 인용 허용 여부를 정할 수 있어야 한다.
- 인용된 원본은 카드로 표시하되 원본 삭제, 팔로워 공개 또는 멘션한 프로필만으로 변경, 차단
  상태를 반영해야 한다.
