...
Profile이 짧은 텍스트를 작성하고, 첨부 Media와 메타데이터를 연결해, Post Visibility에 맞게
배포하는 규칙을 정의한다.
...
- 팔로워 공개: 작성자, 작성자를 수락된 상태로 팔로우한 Profile, 멘션된 Profile에게만 보인다.
...
- 본문은 일반 텍스트 표현을 가진다. 서식 있는 작성 기능은 제공하지 않는다.
