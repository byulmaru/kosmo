...
- Reply control: 해당 Post List Definition의 답글 포함 정책을 적용하고, 원본 작성자와 관계가 없는
  답글을 제외한다.
- Repost control: 해당 Post List Definition의 Repost 포함 정책을 적용한다.
...
또는 Collapse를 만든다. Domain Limit은 전역 Post Eligibility 제외가 아니라 공개 탐색 지면인 Hashtag
Post List 후보 제한으로 적용한다.
