...
Post List별 Post/Repost 후보, 제어 정책, 정렬, 읽기 위치를 정의한다.
...
- 컨텍스트 경계: 홈, 프로필, 해시태그 게시 목록을 구성하는 visible eligible Post와 Repost 후보의
  선택 규칙, viewer/list 수준 제어 적용, 정렬, 읽기 위치를 정의한다. Post
...
- Post List Definition은 후보 항목을 제외할지, 접힘 상태로 노출할지, 경고 상태로 노출할지 결정하는
  Control Decision을 만든다.
- Post List Definition은 정렬 기준과 읽기 위치 기준을 가진다.
...
- Home Post List: viewer Profile이 작성한 visible eligible 원본 Post, viewer Profile이 수락된 상태로
  팔로우한 Profile의 visible eligible 원본 Post, Home Post List 답글 정책에 맞는 Reply Post, viewer
  Profile 또는 viewer Profile이 팔로우한 활성 Profile이 만든 Repost.
...
- Profile block control: Trust & Safety의 block 결과를 적용해 viewer가 차단한 Profile, viewer를
  차단한 Profile의 Post를 제외한다. Repost 후보에는 Repost를 만든 Profile과 원본 Author Profile 모두에
  적용한다.
- Profile mute control: Trust & Safety의 Profile mute 결과를 적용해 뮤트한 Profile의 Post를 숨기거나
  접는다. Repost 후보에는 Repost를 만든 Profile과 원본 Author Profile 모두에 적용한다.
- Word mute control: Trust & Safety의 Word Mute, Hashtag Mute 중 현재 Post List Definition의 적용
  위치와 일치하는 정책만 적용하고, 해당 정책의 hide/collapse 결정을 소비한다.
- Profile domain block control: Trust & Safety의 Profile 도메인 차단 결과를 적용해 viewer가 차단한
  원격 Domain의 Post와 Repost를 제외한다. Repost 후보에는 Repost를 만든 Profile의 Domain과 원본
  Author Profile의 Domain 모두에 적용한다.
- Domain limit control: Trust & Safety의 Domain Limit 결과를 공개 탐색 성격의 Post List 후보 제한으로
  적용한다.
- Media control: Publishing의 민감한 미디어 상태와 Media 접근 가능 상태를 목록별로 접거나 제외한다.
- Reply control: 해당 Post List Definition의 답글 포함 정책을 적용한다. Home Post List는 viewer
  Profile의 Post에 달린 답글, viewer Profile이 작성한 답글, viewer Profile이 팔로우한 Profile의 Post에
  viewer Profile이 팔로우한 Profile이 단 답글만 포함한다.
- Repost control: 해당 Post List Definition의 Repost 포함 정책을 적용하고, Repost를 만든 Profile의
  Profile State가 활성인지 확인한다.
...
삭제, 정지, Tombstone, Domain Block처럼 Post 자체 또는 Post 접근 정책에서 결정되는 상태는 Post
...
| Word/Hashtag Mute     | 적용 위치가 Home이면 Exclude 또는 Collapse      | 적용 위치가 Profile이면 Exclude 또는 Collapse       | 적용 위치가 Hashtag이면 Exclude 또는 Collapse |
...
| Repost                | 활성 Profile이 만든 Repost만 Include            | 활성 Profile이 만든 대상 Profile의 Repost만 Include | Exclude                                       |

Publishing visibility와 Post Eligibility에서 제외된 Post는 Post List Control 이전에 후보에서 제외한다.
Profile block은 모든 Post List에서 Exclude로 적용한다. Profile mute는 목록별 정책에 따라 Exclude 또는
Collapse를 만든다. Word/Hashtag Mute는 목록별 적용 위치와 upstream hide/collapse 결정에 따라 Exclude
또는 Collapse를 만든다. Profile Domain Block은 모든 Post List에서 Exclude로 적용한다. Domain Limit은
전역 Post Eligibility 제외가 아니라 공개 탐색 지면인 Hashtag Post List 후보 제한으로 적용한다.

## 핵심 게시 목록

### Home Post List

- viewer Profile이 작성한 원본 Post와 viewer Profile이 수락된 상태로 팔로우한 Profile의 원본 Post를
  포함한다.
...
- 특정 Profile이 작성한 원본 Post와 해당 Profile이 만든 Repost를 포함한다.
- Publishing의 Post Visibility 판정과 Social Graph 관계에 따라 후보가 제한된다.
- Profile Post List는 원본 Post와 Repost를 목록 후보로 사용한다.

### Hashtag Post List

- Post Visibility가 공개인 원본 Post만 포함한다.
- 답글은 포함하지 않는다.
- Repost는 포함하지 않는다.
- Hashtag lookup은 Discovery가 소유하고, Post List는 해당 Hashtag의 Post 목록 projection을 제공한다.
...
- Post List Projection은 게시 원본과 목록 노출 결과를 분리한다.
- Repost는 원본 Post를 참조하는 Engagement 행동이고, Post List는 Repost를 별도 후보로 합친다.
- Read Position과 Cursor는 Post List 후보 변경 후에도 동일 정렬 기준의 목록 위치를 표현해야 한다.
