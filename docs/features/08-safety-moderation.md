...
Mastodon의 domain blocks API를 참고한다.

- 사용자는 특정 원격 도메인을 개인적으로 차단할 수 있다.
- 개인 도메인 차단은 서버 전체 차단과 다르며 viewer profile 기준 필터로 적용한다.
- 검색, 원격 lookup, 타임라인, 알림, 추천에서 같은 도메인 차단 정책을 공유해야 한다.
- 서버 차단과 사용자 차단이 동시에 적용될 때 사용자에게 어떤 사유를 노출할지 정해야 한다.
