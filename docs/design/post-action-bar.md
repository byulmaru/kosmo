...
- 원형 hover background는 28×28px visual state layer를 유지한다. Web interactive rectangle은 Reply, Repost,
  Reaction과 Bookmark가 50×36px, More가 28×36px이며 visual slot 위아래로 4px씩 확장된다. background는 count를
...
- `PostLayout`은 Reaction Summary와 Action Bar를 포함하는 `Engagement`를 Post content grid 안의 마지막
  presentation 자식으로 렌더링한다. 일반 Text·Media `PostListItem`은 카드 상단 12px·하단 4px, 목록 전용
...
- 모든 플랫폼 구현에서 Bar와 visual/layout slot 높이 28, Reply·More endpoint의 content column 양끝 정렬, social action 너비 50, More 너비
  28, glyph 16, icon-count gap 4와 고정 순서를 검증한다. Web에서는 Bookmark+4px+More trailing group이
  82px이고 bounded interactive rectangle이 50×36px/28×36px이며 분배 여백을 덮거나 서로 겹치지 않는지도 확인한다.
- Web pointer hover에서는 glyph 중심 28×28 원형 background, social action의 50×36 click target과 More의
