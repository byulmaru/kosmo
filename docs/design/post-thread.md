...
- current row는 shell `mobile` presentation에서 왼쪽 16px·오른쪽 12px, `compact`·`full` presentation에서 왼쪽 8px·오른쪽 12px, 상하 16px/4px padding을 사용한다.
  current Before connector는 공용 Post list inset과 48px Avatar 반지름에서 파생한 `mobile` x=40, `compact`·`full` x=32 중심선에 맞추고 supplied visibility 경계에서 종료한다.
- 조상·하위 목록 row의 connector도 같은 공용 metric을 사용한다. 따라서 Web 390/767과 Native phone·tablet은 list/current 모두 x=40, Web 768 이상은 list/current 모두 x=32를 사용한다.
...
- Storybook에서 current·마지막 row 뒤 구분선 생략, 나머지 1px `theme.borderSubtle` 구분선과 64px/8px inset, current row의
  `mobile` 16px/12px·`compact`·`full` 8px/12px 좌우 padding, 48px Avatar와 full-width Body·Engagement, metadata→Reaction Summary 8px,
...
- Reply 대상 attribution이 일반 목록에서는 유지되지만 상세 thread의 조상·현재·하위 모든 행에서는
  표시되지 않는지 검증한다.
- Web Light·Dark 대표 Storybook에서 구분선 x=64, mobile 목록·current connector x=40~42와 compact·full x=32~34를 확인한다.
