...
모든 셸 단계는 selected Profile의 Unread count를 기존 알림 아이콘 우상단 badge로 표시한다. badge는 아이콘 wrapper 안에서 overlay되어 row, touch target과 label layout을 밀지 않는다.

- `< compact` Web과 Android/iOS: 하단 탭 바와 모바일 drawer의 알림 아이콘.
- `compact`~`full`: icon-only 레일의 알림 아이콘.
- `≥ full`: label을 유지하는 풀 사이드바의 알림 아이콘.
- `0`은 숨기고 `1`~`99`는 숫자, `100` 이상은 `99+`로 표시한다.
- light/dark 모두 기존 neutral color token 조합으로 대비를 확보하며 raw hex나 badge 전용 색을 추가하지 않는다.
- count는 진입점의 accessible name에 합쳐 읽고, `99+`로 시각적으로 cap해도 실제 서버 count를 읽는다. 시각 badge 자체는 별도 focus 대상으로 만들지 않는다.
