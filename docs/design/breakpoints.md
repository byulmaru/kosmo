...
- Web `/search`는 모든 breakpoint에서 중앙 컬럼 최상단에 높이 `64px`의 검색 도구막대를 표시하고, 그 안에
  높이 `56px`의 검색 입력을 둔다. route의 기존 `32px` 상단 여백과 도구막대 바깥쪽 가로 여백은 제거하며,
  도구막대 아래 콘텐츠만 기존 본문 여백을 사용한다.
...
- `compact`와 `full`은 현재 중앙 컬럼의 검색 상태와 leading action 동작을 유지하면서 도구막대 높이와 상단
  배치만 같은 `64px` 기준으로 정렬한다.
