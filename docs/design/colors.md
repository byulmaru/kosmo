...
- Default Toast는 `background/inverse`·`foreground/inverse`를 사용하고 accent rail을 두지 않는다. Info/Success/Warning/Danger Toast는 각 semantic feedback `subtle`·`on-subtle` pair와 `base`를 사용해 4px left rail을 둔다.
