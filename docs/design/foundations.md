...
| ------------------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| Sidebar logout action           | 통합      | 공용 `SidebarNavigation`이 compact·full·drawer visual과 pending·error를 표현하고 Production adapter가 `useLogout`·navigation guard를 직접 소유                                                | presentation은 PROD-852, runtime 연결은 PROD-796 |
...
| `ProfileEditImageFields`        | 전용 유지 | `ActionMenu`와 upload error semantics는 재사용하고 header/avatar aspect·overlap·camera veil은 component가 소유                                                                                | DSN-45 source, PROD-941 Production 이관          |
