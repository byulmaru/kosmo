...
- canonical Mention node는 `profileId`만 가지며, renderer는 exact match한 Profile의 `relativeHandle`에서 표시 문자열을 만든다. 서로 다른 `profileId`는 각각의 Profile로 구분하고, 표시 문자열·handle 문자열로 대상을 합치거나 선택하지 않는다.
...
- 저장 후 Profile이 unavailable 또는 deleted가 되면 `@알 수 없는 사용자`를 표시하고 Profile 이동 없이 비활성화한다. 대체 문구를 만들거나 actor URI·외부 URL로 우회하지 않으며, 비활성 상태는 click, keyboard focus와 navigation target을 제공하지 않는 text로 수렴한다.
...
- 긴 handle·본문은 본문 폭 안에서 줄바꿈하며 target identity를 잃지 않는다. Light·Dark 모두 기존 semantic link/text token과 focus indicator를 사용하고, 색상만으로 Mention 여부나 Profile availability를 전달하지 않는다.

## 검증 경계

- component 또는 Storybook 검증은 valid·repeated·서로 다른 Profile identity, 일반 link, unresolved fallback, unavailable/deleted Profile, 긴 handle, Light·Dark와 link/non-link focus 상태를 확인한다.
