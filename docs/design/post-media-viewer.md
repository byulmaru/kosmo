...
- Mobile 시각 기준은 [`Mobile compact lifecycle section`](https://www.figma.com/design/Erj975S6vVP8PlHQius801/KOSMO?node-id=6571-7605)의
  ...
  `390×844`의 software keyboard가 없는 lifecycle과 child overlay consumer는 Media 비율과 관계없이 `654px` stage와
  `y=654` detail anchor를 유지한다. 현재 HUG detail panel은 `182px`라 두 surface의 합은 `836px`이고 viewport
  안에 남는다. landscape `16:9`·portrait `3:4` 이미지는 stage 안에서 원본 비율을 보존해 가로·세로 중앙 정렬한
  `contain`으로 표시한다. 따라서 같은 viewport에서 overlay가 열려도 base Post가 위로 재배치되지 않는다.
  Loading·Error·Unavailable도 같은 anchor와 HUG detail panel을 사용한다. `654px`은 keyboard·IME 또는 더 낮은
  viewport에도 강제하는 runtime 상수가 아니다. Dark lifecycle 상태를 중복 생성하지 않고 panel은 화면의
  Light/Dark mode를 상속한다. shared source 승격과 runtime 반영은 이 Figma evidence와 별도로 검증한다.
...
Compact Web과 Native의 detail panel에는 작성자, 원문 text와 기존 Post Action Bar를 이 순서로 둔다. Panel은 내용 높이를 따르되 `preferredDetailMax = clamp(192px, viewport height의 32%, 240px)`로 계산한 뒤, `detailAvailable = max(0px, viewport height - detail anchor - platform bottom inset)`을 넘지 않도록 `min(preferredDetailMax, detailAvailable)`로 제한한다. `192px`은 충분한 가용 높이가 있을 때의 선호 하한이지 image stage와 겹치거나 viewport를 넘기 위한 강제 최소값이 아니다. `390×844` Mobile 기준의 `y=654` anchor에서는 platform bottom inset 적용 전에도 detail 상한이 `190px`이고 현재 `182px` HUG panel이 그 안에 들어간다. keyboard·IME 또는 더 낮은 viewport에서는 Product 구현이 가용 높이에 맞춰 stage와 detail anchor를 함께 다시 계산하되, 같은 viewport의 child overlay는 계산된 base anchor를 바꾸지 않는다. 짧은 원문의 panel은 내용보다 크게 늘어나지 않고 작성자·원문과 Action Bar 사이의 남는 높이를 채우지 않으며, Action Bar는 원문 바로 아래의 고정 영역을 유지한다. 원문은 처음에 3줄로 제한한다. 넘치는 경우에만 `더 보기` control을 제공하고 펼친 뒤에는 `접기`로 바꾼다. 펼친 원문은 detail panel의 text 영역 안에서만 줄어들고 scroll하며 image surface와 고정 Action Bar를 밀어내거나 가리지 않는다. Control은 펼침 상태를 접근성 state로 전달한다.
...
- Mobile Figma의 [`Post action overlays and picker`](https://www.figma.com/design/Erj975S6vVP8PlHQius801/KOSMO?node-id=6772-10989)는
  ...
  prop은 추가하지 않는다. `390×844` 비키보드 consumer는 base stage `654px`와 detail `y=654`를 유지하며, 현재
  Quick selector는 detail 안의 trigger를 따라 `y=726`에 놓인다. child overlay는 base Media·Post의 flow를
  재계산하지 않는다.
...
- Web runtime은 backdrop·modal 내부 pointer 격리, keyboard arrow, Escape, 배경 surface 비활성화, focus trap·복귀, route·history 유지와 `<768px`·`>=768px` layout을 관찰한다. Compact에서는 짧은 원문의 content-height panel과 `min(clamp(192px, 32vh, 240px), detailAvailable)` 최대 높이·낮은 viewport의 stage/detail non-overlap·text-only scroll·Action Bar 인접 배치, `768–1279px` rail Reply의 Viewer close 뒤 공용 modal open과 focus 이동·복귀를 확인한다. Full에서는 `24px` inset·`clamp(320px, 25vw, 350px)` rail과 남은 image 폭, Action Bar의 가로 overflow 방지, 오른쪽 독립 scroll, inline Composer 작성, reply pagination, Action Bar·reply interaction과 child overlay layering을 함께 확인한다.
