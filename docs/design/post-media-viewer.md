...
- Loading: spinner·상태 설명만 표시하고 navigation·counter와 Compact tray를 숨긴다. Wide rail은 유지한다.
- Error: 상태 설명·`다시 시도` action을 표시하고 navigation·counter와 Compact tray를 숨긴다. Wide rail은 유지한다.
- Unavailable: 상태 설명만 표시하고 navigation·counter와 Compact tray를 숨긴다. Wide rail은 유지한다.
...
- DSN-63 Target의 `Loading`·`Error`·`Unavailable`은 각각 canonical 상태 설명을 사용한다. `Error`만 `다시 시도` action을 제공하며, 세 상태 모두 navigation·counter와 Compact tray는 렌더링하지 않는다. Wide presentation의 346px context rail은 세 상태에서도 유지한다.
...
Close, 이전·다음, Sensitive `보기`와 Error `다시 시도`, 더 보기·접기는 keyboard·touch·Screen Reader에서 같은 기능을 제공하고 role, accessible name, disabled·expanded 상태를 전달한다. 현재 위치 변경은 이미지의 accessible name과 별도로 인지 가능하게 알린다.

## PROD-853 공용 UI·Storybook 검증

아래 정적 component·Storybook 검증은 DSN-63 Target의 disconnected shared surface만 증명한다.
DSN-63의 black 70% overlay, canonical Media frame, 48×48 interaction target, 30px icon, 2.5 stroke,
Compact 56px tray, Wide 346px full-height rail, 상태별 visibility와 비순환 boundary를 확인하지만 Gallery·permission·
...
- Component test는 Compact·Wide와 Ready·Sensitive·Loading·Error·Unavailable 상태, canonical frame·control geometry, 상태별 navigation·counter·tray·rail visibility, Sensitive reveal·Error retry, 1장·다중 위치와 비순환 경계를 확인한다.
- Storybook은 실제 Post action control과 thread 맥락 fixture, Controls/Actions, 390 Compact·1024/1440 Wide와 theme 상태, 1장·다중 이미지, 첫·중간·마지막 위치, Sensitive reveal·Error retry callback을 확인한다. Media 저장·공유처럼 계약에 없는 action은 넣지 않는다.
