...
- 공용 `IconButton`의 원형 상태 surface는 명시적으로 선택한 icon-only action에만 적용한다. Rest는 transparent, Web Hover는 약한 상태 배경, Web·Native Pressed는 더 강한 상태 배경을 사용하며 glyph color·opacity와 target geometry는 유지한다. Disabled는 배경을 표시하지 않고 기존 disabled opacity를 유지하며, reduced motion에서는 상태 전환을 즉시 반영한다. 자체 스타일이 상태를 소유하는 control과 brand mark에는 적용하지 않는다.
