...
| Composer 미디어 편집             | `Pen` / `Pen`                                                 | canonical source `24×24`; `20` inside Web `32×32`, stroke `2`                     | attachment별 ALT·민감도·향후 이미지 편집 화면 진입; editor preview에서는 숨김                                                                                               | `__ComposerMediaItem`; Figma 이관 완료, Product 구현·runtime 접근성 검증 대기                      |
| Composer 미디어 민감 상태        | `Flag` / `Flag`                                               | canonical source `24×24`; `16` inside `28px` pill, stroke `2`                     | 플래그가 설정된 attachment에만 표시; 누르면 `Tool=Sensitive`로 진입; Native target은 `44pt`/`48dp`                                                                           | `__ComposerMediaItem`; Figma 이관 완료, Product 구현·runtime 접근성 검증 대기                      |
