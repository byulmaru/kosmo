...
- `SidebarNavigation`의 Compact 글쓰기 진입점은 원형 Primary CTA 안에 canonical `SquarePen` outline을 사용한다. Rest·Hover·Pressed·FocusVisible 상태는 공용 navigation interaction 계약을 따르며, PROD-796 Production adapter가 이 presentation을 사용한다. Android·iOS 실제 runtime interaction과 접근성 검증은 별도다.
- `SidebarNavigation`의 Profile 목적지는 full·compact·drawer 모두 선택 프로필이 있으면 `28×28` Avatar를 사용하고, 프로필이 없을 때만 disabled `UserRound` `20×20`, stroke `2`를 fallback으로 사용한다. 공용 UI·Storybook과 PROD-796 Production adapter를 이 계약의 정본으로 사용한다. 2026-09-12 Figma `SidebarNavigation`의 `Profile=Available` 27개 변형에도 기존 `Avatar/Size=28`을 연결했다. 가용성을 소유하지 않는 내부 `__SidebarPrimaryNavigation`의 기본 `UserRound`와 `Unavailable` 변형의 기존 interaction 모델은 유지했으며, 후자의 disabled 상태 정렬과 Android·iOS 실제 runtime 검증은 별도다.
- Profile Avatar는 공통 `20px` 너비의 icon slot에 가운데 배치한다. `28px` 시각 크기는 유지하면서 다른 아이콘과 가로 중심선을 맞추고, full·drawer의 label 시작점도 동일하게 유지한다.
...
- Web `SearchToolbar`의 Menu·Back·Clear는 canonical `Menu`·`ArrowLeft`·`X`와 `44×44 CSS px` target을 사용한다. Rest는 no-fill, Hover·Pressed는 공용 transient state surface, FocusVisible은 `2px` focus ring이다. Leading control이 있는 variant는 왼쪽 `space/8`, 오른쪽 `space/16`을 사용하고 입력창 오른쪽 여백은 유지한다. Android/iOS에는 Web 검색 상단바 통합과 Menu를 적용하지 않으며, Native Back·Clear가 같은 glyph·state identity를 소비할 때 interaction target은 iOS `44×44pt`, Android `48×48dp` 최소값을 따른다. Figma source는 `__SearchToolbarControl`이 소유하며 실제 platform별 interaction은 Product 구현에서 검증한다.
- 공용 `IconButton`은 모든 실제 icon-only action에 원형 상태 surface를 제공한다. Rest는 transparent, Web Hover는 약한 상태 배경, Web·Native Pressed는 더 강한 상태 배경을 사용하며 glyph color·opacity와 target geometry는 유지한다. Disabled는 배경을 표시하지 않고 공통 disabled opacity를 사용하며, reduced motion에서는 상태 전환을 즉시 반영한다. 기존 selected·base background, border, shadow와 focus 표시 및 brand mark는 유지한다. Media Viewer의 흰색 glyph control은 같은 원형 surface의 inverse contrast를 사용한다.
...
| -------------------------------- | ------------------------------------------------------------- | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| 새 글 작성                       | `SquarePen` / `SquarePen`                                     | canonical source `24×24`, stroke `2`                                              | Lucide 원본 path 유지; accessible name `글쓰기`                                                                                                                              | Figma·Production adapter 이관 완료; Android·iOS runtime 검증 대기                                 |
...
| Viewer 이전 이미지               | `ChevronLeft` / `ChevronLeftIcon`                             | `24`, stroke `2`, fixed white                                                     | `48×48` `IconButton`; 공용 inverse 원형 surface; 첫 이미지에서 disabled·공통 opacity                                                                                         | `PostMediaViewer`                                                                                 |
| Viewer 다음 이미지               | `ChevronRight` / `ChevronRightIcon`                           | `24`, stroke `2`, fixed white                                                     | `48×48` `IconButton`; 공용 inverse 원형 surface; 마지막 이미지에서 disabled·공통 opacity                                                                                     | `PostMediaViewer`                                                                                 |
...
| 모든 상태에서 outline icon, 선택 상태의 상시 surface fill | 미선택=Lucide outline, 선택=`Icon/Custom/Filled/{House,Search,SquarePen,Bell}` + primary label + container no-fill; hover·pressed·focus feedback만 일시 표시; Profile Avatar 유지 | `BottomTabBar`                          | PROD-852 공용 UI·Storybook 및 PROD-796 Production adapter 연결 완료; Android·iOS runtime 검증 대기 |
