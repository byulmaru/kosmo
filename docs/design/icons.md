...
| 화면·경로 뒤로             | `ArrowLeft` / `ArrowLeft`           | Profile Edit `24`, stroke `2`; 다른 소비처는 후속 | Profile Edit `44×44` target; route·navigation history를 되돌리는 action에만 사용 | Profile Edit Figma 이관 완료; Search와 다른 route는 개별 이관·검증          |
