...
웹 화면의 1440 프레임은 레이아웃 브레이크포인트 3단계(풀 사이드바 + 피드 + 컴포저, `≥ lg`)에 대응한다. 1024 프레임(접힌 아이콘 메뉴 + 피드 + 컴포저)은 압축형 재조정 이후 단일 단계와 1:1로 맞지 않는다(코드는 `lg`부터 풀 사이드바로 전환). 단계 정의와 대응은 [breakpoints.md](./breakpoints.md)를 참고한다.
