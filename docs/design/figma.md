...
웹 화면의 1440 프레임은 레이아웃 브레이크포인트 3단계(풀 사이드바 + 피드 + 컴포저, `≥ xl`)에 대응한다. 1024 프레임(접힌 아이콘 메뉴 + 피드)은 코드의 2단계(`md`~`xl` 아이콘 레일 + 피드)에 대응한다(코드는 `xl`부터 풀 사이드바로 전환하며 우측 컴포저를 포함한다). 단계 정의와 대응은 [breakpoints.md](./breakpoints.md)를 참고한다.
