...
표시돼요. 팔로우 관계는 유지돼요.`를 안내하고 `취소`·`뮤트 해제`를 제공한다.
  ...
  해제 확인창 자체의 증거가 아니다.
- 프로필에서는 더보기 메뉴가 프로필 링크 복사와 뮤트 항목을 합성한다. 뮤트 액션은 Relay fragment·mutation·확인창과
...
`ProfileMuteAction`은 기존 ModalSheet·ConfirmationContent·ToastProvider를 재사용한다. 더보기 소비자가
ActionMenu를 소유하고, 뮤트 액션이 제공하는 메뉴 항목을 다른 항목과 합성한다.
