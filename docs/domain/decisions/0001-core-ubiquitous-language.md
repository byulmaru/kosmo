...
| 로그인 주체         | Account         | 계정          | 인증, Profile Owner/Member 관계의 주체                                        |
| 게시 접근/노출 범위 | Post Visibility | 공개 범위     | Post를 누가 볼 수 있고 어디에 노출될 수 있는지 나타내는 값 객체               |
| 게시 목록           | Post List       | 게시 목록     | Post 후보 선택, 제어, 정렬 규칙                                               |
| 반응                | Reaction        | 반응          | Profile이 Post에 이모지 단위로 남기는 반응 행동                               |
| 재게시              | Repost          | 재게시        | Post Form 값 중 원본 Post를 Repost 작성자의 Post List 후보로 다시 포함하는 값 |
...
- `Account`는 로그인, Profile Owner/Member 권한의 주체로만 사용한다.
...
- `Post Visibility`는 domain/value-object 용어로 사용하고, 화면 표시용 한국어 문구는 `공개 범위`로
  쓴다.
- `Post Visibility`는 [Post](../objects/post.md)의 속성이다. Post List는 Post Visibility를 Post List
  Control로 재정의하지 않는다.
...
- `Post Eligibility`는 데이터를 가지는 Post 속성이나 값 객체가 아니라 Post가 소유하는 후보성
  정책이다.
...
- `Post List Item`은 canonical domain term으로 사용하지 않는다. 목록에 표시되는 대상은 Post 후보와
  Control Decision으로 설명한다.
- `Reaction`과 `Repost`를 canonical domain term으로 사용한다. `Reaction`은 별도 객체이고, `Repost`는
  [Post](../objects/post.md)의 Post Form 상태 값이다. 좋아요, 부스트, 확산 계열 표현은 canonical domain
  term으로 사용하지 않는다.
- `Reaction`은 Post 하나에 Profile당 같은 이모지 1개만 허용하고, 서로 다른 이모지는 여러 개
  허용한다.
- `Bookmark`의 한국어 표현은 `북마크`로 쓴다.
- `Quote`는 현재 도메인 범위에서 제외한다.
- `Collection`은 현재 도메인 범위에서 제외한다.
...
- `Audience`와 `공개 설정`은 canonical domain term으로 쓰지 않는다. 필요하면 UI 설명 문구로만
  제한한다.
- Messaging은 현재 도메인 범위에서 제외한다. `멘션한 프로필만`은 [Post](../objects/post.md)의 Post
  Visibility 값으로 다룬다.
- Account가 주체인 행동은 인증, Profile Owner/Member 권한, 운영자 권한에 한정한다. 그 외 소셜
...
- Home Post List는 viewer Profile 또는 viewer Profile이 팔로우한 Profile이 작성한 Repost Post를 포함한다.
- Profile Post List는 대상 Profile의 Original Post와 Repost Post를 표시한다.
- Hashtag Post List는 Post Visibility가 공개인 Original Post만 표시하고, Reply와 Repost를 포함하지
  않는다.
...
- [Post](../objects/post.md)는 `Post`, `Post Form`, `Repost`, `Post Visibility`를 canonical term으로
  사용한다.
- [Post List Definition](../objects/post-list-definition.md)은 Post가 제공한 visible eligible Post를
  소비하고, viewer/list 수준 제어만 `Post List Control`로 정의한다.
- [Account](../objects/account.md)와 [Profile](../objects/profile.md)은 `Account`, `Profile`,
  `Remote Profile`을 canonical term으로 사용한다.
- [Reaction](../objects/reaction.md)은 `Reaction`을 canonical term으로 사용한다.
- 객체 문서는 `Post`, `Profile`, `Account`, `Post Form`, `Post Visibility`, `Post List`, `Reaction`,
