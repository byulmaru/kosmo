...
- ADR 0002의 “게시 후 Attached Media 관계를 바꾸지 않는다”, “Sensitive Media는 Post 속성이고 Alt Text는
  Media 속성이다” 결정을 대체한다.
- ADR 0003과 ADR 0005의 Post 작성 시 Media 관계를 만들고 게시 뒤 연결을 바꾸지 않는 결정을 대체한다.
- ADR 0013의 Media가 Alt Text와 Attached Post 관계를 소유한다는 부분을 대체한다. Media Storage Service와
...
- V1 ProseMirror body에 `media` block node를 추가한다. node의 `mediaId`, nullable `altText`와 document 안의
  위치가 revision별 Media 참조, 대체 텍스트와 표시 순서를 소유한다. document root의 `sensitiveMedia` attr는
...
- ActivityPub projection은 paragraph/text/link를 안전한 `Note.content` HTML로 직렬화하고 Media node를 문서
  ...
  `altText`는 Image의 사람이 읽을 수 있는 이름으로 제공한다. `sensitiveMedia`는 지원하는 ActivityPub
  sensitive 속성으로 투영한다.
...
- 기존 V1 document는 유효하고 의미가 변하지 않는다. 기존 node/field 의미 변경·제거·이름 변경, Media 참조
...
- Alt Text와 Sensitive Media는 같은 Media를 사용하는 다른 revision에서 독립적으로 바뀔 수 있다.
...
- [Media](../objects/media.md)는 논리 이미지와 업로드 lifecycle을 소유하고 revision별 Alt Text를 소유하지 않는다.
