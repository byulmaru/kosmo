...
- Home과 Profile Post List는 Content, Reply Parent와 Repost Source 조합에 따라 Repost와 Quote를 후보로
  사용할 수 있다.
- Hashtag Post List는 Public이고 Content가 있으며 Reply Parent가 없는 Post만 포함한다.
...
- Content 없는 Repost는 Repost Source가 될 수 없다. Repost Source는 Content가 있는 Post다.
- Quote는 Content와 Repost Source를 가진 Post다. Quote 입력은 Content가 있는 입력 Post 자체를 참조하며,
  Quote Eligibility는 Repost Source 조회 가능성도 요구한다. Quote도 Reply Parent를 가질 수 있다.
...
- [Post](../objects/post.md)는 Content와 관계 조합, Post Visibility, Content Warning, Sensitive Media를 정의한다.
