...
- Remote Profile은 Origin Instance 관계를 필수로 가진다. Remote Post의 Instance는 Author Profile에서
  파생하고 Remote Media의 Instance는 Media Profile에서 파생한다.
...
- Mute/Block/Instance 상태가 바뀌어도 기존 Notification Item의 존재와 Read State는 바꾸지 않는다.
...
- [Profile](../objects/profile.md)은 Origin Instance와 avatar/header 관계를 정의한다.
