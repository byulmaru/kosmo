...
이 문서는 2026-09-22 현재 Linear 본문과 승인 댓글을 바탕으로 복원했다.
...
- 이 지표의 앱 소유 custom event는 무작위 불투명 session 식별자와 결과·요청 단계처럼 계산에 필요한 고정
  분류값만 사용한다.
- raw Hashtag·검색어·Hashtag ID·대상 Profile ID·이름·handle·오류 원문·URL·pathname·Account ID 중복
  property는 추가하지 않는다. 현재 Account 연결은 기존 opaque Account identify/reset 경계를 따른다.
...
PROD-556은 자신의 지표 계약, 이벤트, dashboard, 합성 자료 대조와 실제 수집 검증을 맡는다.
