...
| inbound Move Follow 이전     | 시스템            | Follow Relationship | target Profile   | `System.RemoteProfileSource`        | source Followee가 검증된 inbound Move의 Remote Profile이고 Follower가 Local Profile이며 target Actor identity와 exact source alias가 확인된다. Local·Remote target 모두 target의 기존 Follow Approval Policy를 따른다 | 실행 시작 시 target Follow Relationship 또는 Pending Follow Request가 없으면 target 관계 또는 요청을 먼저 생성한 뒤 기존 source Follow Relationship을 제거한다. 이미 target 관계 또는 Pending Follow Request가 있으면 source 관계를 유지한다. Follower가 target Profile 자신이면 target→target 관계를 만들 수 없으므로 이전 대상에서 제외하고 기존 target→source Follow Relationship을 유지한다 |
...
각 source Remote Profile의 기존 established Follow Relationship 중 Follower가 Local Profile인 관계만, Follower가
target Profile 자신인 경우를 제외하고 이전한다. 실행 시작 시 같은 follower와 target 사이에 Follow Relationship 또는
Pending Follow Request가 없으면 target Follow Relationship 또는 Follow Request를 성공적으로 저장한 뒤 source Follow
Relationship을 제거한다. 이미 target Follow Relationship 또는 Pending Follow Request가 있으면 기존 lifecycle에
수렴시키고 source Follow Relationship을 유지한다. Follower가 target Profile 자신이면 target→target 관계를 만들 수
없으므로 해당 관계를 이전 대상에서 제외하고 기존 target→source Follow Relationship을 유지한다. 일반적인
target-first 순서는 remote-to-local과 remote-to-remote target에 동일하게 적용한다. target 생성 뒤 source 제거 전에
처리가 중단되어 재시도하더라도 별도 migration receipt가 없으므로 재시도 시 이미 존재하는 target 상태로 관찰되면
source 관계가 남을 수 있다. 서버 간 receipt 도착 순서는 보장하지 않으며 동시 Follow/Unfollow race는 허용한다.
중단된 이전은 기존 Temporal 재시도로 재개하며, migration 전용 generation protocol은 이 문서에서 정하지 않는다.
