...
target Profile 자신인 경우를 제외하고 target Follow Relationship 또는 Follow Request가 성공적으로 저장된 뒤 source
Follow Relationship을 제거한다. Follower가 target Profile 자신이면 target→target 관계를 만들 수 없으므로 해당 관계를
이전 대상에서 제외하고 기존 target→source Follow Relationship을 유지한다. 일반적인 target-first 순서는
