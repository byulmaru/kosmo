...
| Remote Featured collection 동기화 | 시스템         | Remote Profile      | 광고된 `featured` URI, 원격 ActivityPub Profile 표현                          | `System.RemoteProfileSource`         | Remote Profile 등록, stale refresh, 검증된 inbound Profile Update 또는 Active Local Profile이 이 Remote Profile의 established Follower가 된 시점에 advertised URI를 처리한다. Public/Unlisted는 기존 공개 fetch를 사용하고, Active local follower identity가 있으면 한 sync 시도 동안 같은 identity로 collection의 모든 page와 각 Note를 authenticated fetch·검증한다. 각 Note의 canonical `attributedTo`는 광고하는 Actor의 canonical URI와 정확히 일치해야 한다 | 성공한 authoritative sync의 지원 Post 전체를 원격 순서로 원자적으로 교체하며, 검증된 원격 표현에서 URI가 사라지면 empty set으로 교체한다. fetch·parse·검증·취소·순환·예산 초과 실패는 마지막 성공 set을 보존한다. 실패는 기존 retry-capable async effect/Workflow 경계에서 관측·재시도할 수 있어야 하며, 성공한 retry가 마지막 성공 set을 원자적으로 교체한다 |
