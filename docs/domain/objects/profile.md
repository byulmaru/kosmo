...
| 표시 이름        | 문자열, 필수       | Local은 1-40자, Remote는 원격 원본 값을 보존한다                   | 항상            | Profile 조회 정책 통과 | 없음      |
...
| Profile 편집        | Account        | Profile   | 표시 이름, bio, avatar/header, 링크, Follow Approval Policy | `Account.Active`, `Profile.Owner`    | 대상이 Account의 selected Profile이고 Origin이 Local이며 Lifecycle State가 Active이고 Suspension State가 Normal이다 | Profile 표현 속성, Policy, 선택된 Media 관계가 바뀐다                                                                                                                                                                      |
