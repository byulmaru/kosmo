...
| Local Profile current pin 교체    | Account        | Local Profile       | Post ID, current pin expected value                                           | `Account.Active`, `Profile.Owner`    | 현재 first-party UI가 관리하는 current pin slot을 교체할 때 canonical ModalSheet confirmation 뒤 수행한다. 같은 transaction에서 Local Profile Post 고정의 Profile 상태와 대상 작성자·lifecycle·Content·Visibility 자격을 동일하게 검증하며 expected value가 현재와 다르면 저장 상태를 바꾸지 않는다. 이 정책은 ordered pin set의 cardinality를 제한하지 않는다                                                                                                                                                                       | current pin slot만 원자적으로 교체하고 다른 ordered pin 관계를 보존한다. expected value 불일치는 stale/conflict 결과로 반환하고 저장 상태를 바꾸지 않는다                                                                                                                                                                          |
...
의존하지 않으며 완료 시간 SLA는 정의하지 않는다. Public/Unlisted만 조회할 때는 기존 공개 fetch를 사용할 수 있다. Active
local follower identity가 있으면 한 sync 시도 동안 같은 identity로 collection의 모든 page와 각 Note 역참조를 authenticated
...
Active Local Profile과 Remote Profile 사이의 Follow Relationship이 새로 성립하면 저장된 검증 표현이 광고한 `featured`
URI의 sync를 해당 Local Profile identity로 실행하거나 예약한다. 이 trigger는 Follow 성립 결과의 성공을 sync 완료·성공에
의존시키지 않는다.
established Follow Relationship을 보존한 Local follower identity가 Profile 재활성화 또는 정지 해제로 다시 Active/Normal이
되면, 저장된 검증 표현이 `featured` URI를 광고하는 각 Remote Profile의 sync를 해당 identity로 실행하거나 예약한다. 이
trigger는 Profile 상태 전이 결과의 성공을 sync 완료·성공에 의존시키지 않는다.
...
사라진 경우는 currentness token을 갱신해 이전 URI의 진행 중인 시도와 예약된 retry를 무효화한 뒤 authoritative empty set으로
처리한다.
...
- Profile의 고정 Post 관계와 API projection은 ordered 0..N collection이다. 현재 Local first-party UI는 server-authoritative
  ...
  segment에 표시한다. Profile 목록은 이 segment 뒤에 일반 chronology를 이어 표시한다. Home·Local·Hashtag 목록에는 이
  ordering을 적용하지 않는다.
- 현재 Local first-party UI에서 첫 visible pin만 pinned segment와 고정 표시를 사용한다. 나머지 조회 가능한 Local pin은
  Reply·Quote를 포함해 기존 chronology 위치에 일반 Post로 한 번 표시하며, pin 관계 때문에 Profile 목록에서 사라지지 않는다.
