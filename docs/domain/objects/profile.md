...
- Profile의 고정 Post 관계와 API projection은 ordered 0..N collection이다. 현재 Local first-party UI는 server-authoritative
  order의 첫 visible pinned Post만 렌더·관리하고, Remote는 검증된 Featured collection 전체를 유지한다. Profile 목록은 조회 가능한
  고정 Post를 server-authoritative ordered collection 순서로 먼저
  표시하고 일반 chronology를 이어 표시한다. Home·Local·Hashtag 목록에는 이 ordering을 적용하지 않는다.
