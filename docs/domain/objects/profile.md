...
| qualified handle | 문자열, 필수       | `@handle@host` 형식이며 전체 Host 범위에서 Profile을 식별한다   | 항상            | Profile 조회 정책 통과 | 없음      |
...
| Origin Instance     | [Instance](./instance.md)                                     | Profile -> Instance            | 1 -> 1      | Origin이 Remote  | Profile 조회 정책 통과  | 없음                                      |
...
| ------------------- | -------------- | --------- | --------------------------------------------------------------------- | ---------------------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Local Profile 생성  | Account        | Profile   | handle, 선택적 표시 이름, Follow Approval Policy                      | `Account.Active`             | 같은 Local handle이 없고 Account가 생성 결과의 Owner가 된다                         | Origin=Local, Lifecycle=Active, Suspension=Normal인 Profile과 입력 Policy, Owner Membership이 생성된다 |
| Remote Profile 등록 | 시스템         | Profile   | Origin Instance, Remote URL, qualified handle, Follow Approval Policy | `System.RemoteProfileSource` | Origin Instance가 원격 요청 허용 상태이고 같은 qualified handle의 Profile이 없다    | Origin=Remote, Lifecycle=Active, Suspension=Normal인 Profile과 원격 Policy가 생성된다                  |
...
| Profile 편집        | Account        | Profile   | 표시 이름, bio, avatar/header, 링크, Follow Approval Policy           | `Profile.Owner`              | Origin이 Local이고 Lifecycle State가 Deleted가 아니며 Suspension State가 Normal이다 | Profile 표현 속성, Policy, 선택된 Media 관계가 바뀐다                                                  |
| Profile 비활성화    | Account        | Profile   | 없음                                                                  | `Profile.Owner`              | Origin이 Local이고 Lifecycle State가 Active이며 Suspension State가 Normal이다       | Lifecycle State가 Deactivated가 된다                                                                   |
| Profile 재활성화    | Account        | Profile   | 없음                                                                  | `Profile.Owner`              | Origin이 Local이고 Lifecycle State가 Deactivated이며 Suspension State가 Normal이다  | Lifecycle State가 Active가 된다                                                                        |
| Profile 삭제        | Account        | Profile   | 없음                                                                  | `Profile.Owner`              | Origin이 Local이고 Lifecycle State가 Deactivated이며 Suspension State가 Normal이다  | Lifecycle State가 Deleted가 된다                                                                       |
| Profile 정지        | 운영자 Account | Profile   | 사유                                                                  | `Account.Operator`           | Lifecycle State가 Deleted가 아니고 Suspension State가 Normal이다                    | Suspension State가 Suspended가 되고 Lifecycle State는 유지된다                                         |
| Profile 정지 해제   | 운영자 Account | Profile   | 사유                                                                  | `Account.Operator`           | Suspension State가 Suspended다                                                      | Suspension State가 Normal이 되고 Lifecycle State는 유지된다                                            |
...
- Remote Profile은 Origin Instance의 Safety State가 Domain Block이 아니어야 한다.
...
- Remote Profile lookup은 Origin Instance의 Safety State가 Domain Block이 아니고 Reachability State가
