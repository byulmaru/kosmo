...
| Local Profile current pin 교체    | Account        | Local Profile       | Post ID, current pin expected value                                           | `Account.Active`, `Profile.Owner`    | 같은 transaction에서 Local Profile Post 고정의 Profile 상태와 대상 작성자·lifecycle·Content·Visibility 자격을 동일하게 검증하며 expected value가 current pin과 다르면 저장 상태를 바꾸지 않는다. 이 정책은 ordered pin set의 cardinality를 제한하지 않는다                                                                                                                                                                            | 대상이 다른 위치에 이미 pinned면 해당 관계를 current slot으로 이동하고 기존 current 관계를 제거한다. 그 외 pin의 상대 순서를 보존하고 중복 관계를 만들지 않는다. expected value 불일치는 stale/conflict 결과로 반환하고 저장 상태를 바꾸지 않는다                                                                                  |
