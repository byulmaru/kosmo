...
| 고정 Post           | [Post](./post.md)                                             | Profile -> Post                  | Local 0..1, Remote 0..N (ordered) | Local mutation 성공 또는 Remote Featured authoritative sync 성공 | 기존 Profile·Post 조회 정책과 원격 순서 | 없음                                      |
...
| Local Profile Post 고정/교체      | Account        | Local Profile       | Post ID, current pin expected value                                           | `Account.Active`, `Profile.Owner`    | Profile Origin이 Local이고 Lifecycle State가 Active이며 Suspension State가 Normal이다. 대상은 현재 Profile이 작성한 Active이며 Content가 있는 Post·Reply·Quote이고 Visibility가 Public, Unlisted 또는 Followers Only다. Mentioned Profiles, Content 없는 pure Repost와 타인 작성 Post는 제외한다. 교체는 canonical ModalSheet confirmation 뒤 수행하며 expected value가 현재와 다르면 저장 상태를 바꾸지 않는다 | 새 current pin을 저장하거나 같은 Post면 idempotent success를 반환한다. expected value 불일치는 stale/conflict 결과로 반환하고 현재 pin을 보존한다                                                                                   |
| Local Profile Post 고정 해제      | Account        | Local Profile       | Post ID                                                                       | `Account.Active`, `Profile.Owner`    | Profile이 Local이고 Active/Normal이다. 대상이 현재 pin과 같으면 제거하고, 이미 없거나 현재 pin과 다르면 다른 current pin을 보존하는 idempotent no-op으로 처리한다                                                                                                                                                                                                                                               | 일치하는 current pin은 제거하고, 이미 없는/different 대상은 idempotent success를 반환한다                                                                                                                                           |
| Remote Featured collection 동기화 | 시스템         | Remote Profile      | 광고된 `featured` URI, 원격 ActivityPub Profile 표현                          | `System.RemoteProfileSource`         | Remote Profile 등록, stale refresh 또는 검증된 inbound Profile Update에서 advertised URI를 처리한다. Public/Unlisted는 기존 공개 fetch를 사용하고, Active local follower identity가 있으면 한 sync 시도 동안 같은 identity로 collection의 모든 page와 각 Note를 authenticated fetch·검증한다                                                                                                                    | 성공한 authoritative sync의 지원 Post 전체를 원격 순서로 교체하며, 검증된 원격 표현에서 URI가 사라지면 empty set으로 교체한다. fetch·parse·검증·취소·순환·예산 초과 실패는 마지막 성공 set과 상위 Profile 등록·갱신 결과를 보존한다 |
...
실패시키지 않는다. 검증된 원격 표현에서 `featured` URI가 사라진 경우는 authoritative empty set으로 처리한다.
...
- Profile 목록은 조회 가능한 고정 Post를 고정 순서(Local 0..1, Remote는 검증된 Featured collection 순서)로 먼저
...
- 기존 Profile/Post visibility, lifecycle, block/domain 정책으로 조회할 수 없는 Profile 또는 Post는 pinned segment와
  ...
  unavailable, author 또는 visibility 자격을 잃은 경우 해당 관계를 제품상 current pin으로 간주하지 않아 새 pin을 막지
