...
Local Profile과 Remote Profile은 Profile Origin 상태 차원으로 구분한다. Profile Migration은 target Profile과 이전
원본인 Remote Profile을 연결하는 관계이며, inbound ActivityPub Move의 source·target 검증에 사용한다.
...
| 고정 Post           | [Post](./post.md)                                             | Profile -> Post                  | 0..N (ordered) | Local mutation 성공 또는 Remote Featured authoritative sync 성공 | 기존 Profile·Post 조회 정책과 원격 순서 | 없음                                      |
...
| Profile Migration   | Profile                                                       | target Profile -> Remote Profile | 1 -> 0..1      | 관계가 설정된 경우                                               | 관계 당사자                             | 없음                                      |
...
| Profile 편집                      | Account        | 현재 선택된 Profile | 표시 이름, bio, avatar/header, 링크, Follow Approval Policy, Profile Tag 목록 | `Account.Active`, `Profile.Owner`    | Lifecycle State가 Active이며 Suspension State가 Normal이다                                                                                                                                                                                                                                                                                                                                                                            | Profile 표현 속성, Policy, 선택된 Media 관계와 Profile Tag 목록이 원자적으로 바뀐다                                                                                                                                                                                                                                                |
| 기본 Post Visibility 변경         | Account        | Local Profile       | Public, Unlisted 또는 Followers Only                                          | `Account.Active`, `Profile.Owner`    | Origin이 Local이고 Lifecycle State가 Active이며 Suspension State가 Normal이다                                                                                                                                                                                                                                                                                                                                                         | 이후 새 Post·Reply·Quote Composer가 사용할 Profile 기본값이 바뀐다. 이미 작성된 Post와 열려 있는 Composer의 개별 Visibility는 바뀌지 않는다                                                                                                                                                                                        |
| Local Profile Post 고정           | Account        | Local Profile       | Post ID                                                                       | `Account.Active`, `Profile.Owner`    | Profile Origin이 Local이고 Lifecycle State가 Active이며 Suspension State가 Normal이다. 대상은 현재 Profile이 작성한 Active이며 Content가 있는 Post·Reply·Quote이고 Visibility가 Public, Unlisted 또는 Followers Only다. Mentioned Profiles, Content 없는 pure Repost와 타인 작성 Post는 제외한다                                                                                                                                      | 대상 Post를 server-authoritative ordered pin set의 한 위치에 추가하고 기존 pin의 상대 순서를 보존한다. 이미 같은 Post가 있으면 다른 관계와 순서를 바꾸지 않는 idempotent success를 반환한다                                                                                                                                        |
| Local Profile current pin 교체    | Account        | Local Profile       | Post ID, current pin expected value                                           | `Account.Active`, `Profile.Owner`    | Local Profile Post 고정의 Profile 상태와 대상 작성자·lifecycle·Content·Visibility 자격 및 current pin expected value를 교체와 원자적으로 검증한다. expected value가 current pin과 다르면 저장 상태를 바꾸지 않는다. 이 정책은 ordered pin set의 cardinality를 제한하지 않는다                                                                                                                                                         | 대상이 다른 위치에 이미 pinned면 해당 관계를 current slot으로 이동하고 기존 current 관계를 제거한다. 그 외 pin의 상대 순서를 보존하고 중복 관계를 만들지 않는다. expected value 불일치는 stale/conflict 결과로 반환하고 저장 상태를 바꾸지 않는다                                                                                  |
| Local Profile Post 고정 해제      | Account        | Local Profile       | Post ID                                                                       | `Account.Active`, `Profile.Owner`    | Profile이 Local이고 Active/Normal이다. 대상 Post가 ordered pin set에 있으면 해당 관계만 제거한다. 이미 없으면 다른 pin 관계를 바꾸지 않는 idempotent no-op으로 처리한다                                                                                                                                                                                                                                                               | 지정한 Post 관계만 제거하고 다른 ordered pin 관계는 보존한다. 이미 없으면 idempotent success를 반환한다                                                                                                                                                                                                                            |
| Remote Featured collection 동기화 | 시스템         | Remote Profile      | 광고된 `featured` URI, 원격 ActivityPub Profile 표현                          | `System.RemoteProfileSource`         | Remote Profile 등록, stale refresh 또는 검증된 inbound Profile Update에서 advertised URI를 처리한다. Public/Unlisted는 기존 공개 fetch를 사용하고, Active/Normal이며 사용 가능한 Local Instance에 속한 local follower identity가 있으면 한 sync 시도 동안 같은 identity로 collection의 모든 page와 각 Note를 authenticated fetch·검증한다. 각 Note의 canonical `attributedTo`는 광고하는 Actor의 canonical URI와 정확히 일치해야 한다 | 성공한 authoritative sync의 지원 Post 전체를 원격 순서로 원자적으로 교체하며, 검증된 원격 표현에서 URI가 사라지면 이전 시도를 무효화하고 empty set으로 교체한다. fetch·parse·검증·취소·순환·예산 초과 실패는 마지막 성공 set을 보존한다. 실패는 관측·재시도할 수 있어야 하며, 성공한 retry가 마지막 성공 set을 원자적으로 교체한다 |
...
| Profile Migration source 지정     | Account        | 현재 선택된 Profile | source qualified handle                                                       | `Account.Active`, `Profile.Owner`    | 현재 선택된 Profile을 target으로 사용하고 현재 context의 Owner 권한을 재사용한다. 별도 target eligibility는 적용하지 않는다. 같은 source·target pair면 no-op이고, 다른 pair와 충돌하면 거부한다                                                                                                                                                                                                                                       | source Remote Profile이 materialize되고 현재 선택된 Profile에서 Remote source로 준비 관계가 생성된다. Local Actor의 aliases는 이 관계에서 파생된다                                                                                                                                                                                 |
| inbound ActivityPub Move 처리     | 시스템         | Profile             | 인증된 source actor/object URI, target canonical Actor URI                    | `System.RemoteProfileSource`         | 인증된 actor와 object가 같은 canonical URI이고 target actor의 `alsoKnownAs`에 exact source URI가 있으며 source는 Remote Profile이다                                                                                                                                                                                                                                                                                                   | 검증된 remote-to-local 또는 remote-to-remote Move가 기존 Profile identity와 Follow lifecycle에 반영된다                                                                                                                                                                                                                            |

Profile Origin은 연결된 Instance Type과 같아야 한다. Follow Approval Policy 변경은 이미 존재하는 Pending Follow
Request의 상태나 존재를 바꾸지 않는다.

Account 삭제는 연결된 모든 Profile의 Deactivated 상태를 전제로 하지만 Profile을 삭제하거나 비활성화하지
않는다. 따라서 Profile의 상태·속성·관계는 삭제 후에도 보존된다.
...
ActivityPub Remote Profile 검색 경계는 qualified handle을 canonical actor URI로 먼저 해석한다. Remote Profile
materialization 경계는 canonical actor URI와 선택적인 Profile ID를 입력으로 받아 actor metadata나 Profile이 아직
저장되지 않은 새 actor도 materialize할 수 있다. Fedify lookup 결과의 canonical actor URI는 ActivityPub actor
metadata에 저장해 Remote Profile identity와 연결한다. 이후 stale refresh는 저장된 actor metadata의 canonical
actor URI를 known actor URI로 재사용해 같은 actor를 갱신하며 qualified handle lookup을 다시 수행하지 않는다.
refresh 결과의 actor URI가 저장된 actor identity와 다르면 Profile 또는 actor metadata를 저장하지 않는다. canonical
actor URI가 유지되는 동안 actor `preferredUsername`이 바뀌어도 새 Profile을 만들지 않고 같은 Profile의 handle,
normalized handle과 qualified handle을 갱신한다.

Profile Migration 준비 관계는 target Profile에서 이전 원본인 Remote Profile source로 향한다. target Profile은
source를 하나만 가질 수 있고, 같은 Remote source는 하나의 target Profile에서만 준비할 수 있다. 이
관계는 inbound Move 처리 완료나 전체 migration 이력을 뜻하지 않는다. source가 아직 저장되지 않은 inbound Move는
검증된 원격 actor를 Remote Profile로 먼저 materialize한다. Local Actor의 ActivityPub `alsoKnownAs` aliases는 이
관계의 Remote source canonical Actor URI에서 파생하며 별도 Profile 속성이나 사용자 입력으로 저장하지 않는다.

Profile Migration source 지정은 Settings의 Profile detail에서 feature flag가 켜져 있을 때만 노출한다. 현재 선택된
Profile을 target으로 사용하며 별도 target Profile ID를 입력받지 않는다. 현재 context의 `Account.Active`와
`Profile.Owner` 권한을 재사용하고 별도 target eligibility를 적용하지 않는다. flag는 UI 노출 조건일 뿐 권한 증거가
아니며, Profile Owner 검증은 기존 `Account.Active`와 `Profile.Owner`를 사용한다. flag가
꺼져 있거나 값을 확인할 수 없거나 로딩 중이면 source 준비 control을 노출하지 않는다. 이미 준비된 관계와 그로부터
파생된 alias, inbound Move 처리는 flag 상태로 중단하거나 제거하지 않는다. remote-to-local Move는 준비 관계의 target
Profile과 source 관계를 검증하고, 해당 target Profile의 기존 Follow Approval Policy에 따라 Follow Relationship 또는 Follow
Request를 선택한다. remote-to-remote Move는 준비 관계 없이 canonical target identity와 해당 target Profile의 기존 Follow
Approval Policy를 검증한다. Kosmo가 source가 되어 Move를 발행하는 동작은
이 범위에 포함하지 않는다.
...
기본값을 자동으로 갱신하지 않는다. Remote Profile에는 Kosmo Local 기본값을 만들거나 변경하지 않는다.
...
Remote Profile 등록, stale refresh와 검증된 inbound Profile Update에서 원격 표현이 광고한 `featured` URI가 있으면
Featured sync를 production path에서 실행하거나 별도 effect로 예약한다. 상위 Profile 결과의 성공 여부는 sync 완료·성공에
의존하지 않으며 완료 시간 SLA는 정의하지 않는다. Public/Unlisted만 조회할 때는 기존 공개 fetch를 사용할 수 있다.
Active/Normal이며 사용 가능한 Local Instance에 속한 local follower identity가 있으면 한 sync 시도 동안 같은 identity로
collection의 모든 page와 각 Note 역참조를 authenticated fetch·검증해야 Followers Only membership을 반영할 수 있다. 각
시도는 취소할 수 있어야 하고 next page 순환을 검출하며,
...
Remote Profile 등록·stale refresh·검증된 inbound Profile Update sync가 실행될 때 현재 Active/Normal이며 사용 가능한 Local
Instance에 속한 local follower identity가 있으면 Followers Only membership 검증에 사용한다.
...
유지하고, 이후 성공한 retry만 snapshot을 원자적으로 교체한다. 더 최신 trigger 뒤에 완료된 이전 시도의 성공 결과는 폐기한다.
retry timing·backoff·횟수·SLA와 최신성 판별 수단은 이 계약에서 고정하지 않는다. 검증된 원격 표현에서 `featured` URI가
사라진 경우는 이전 URI의 진행 중인 시도와 예약된 retry가 이후 결과를 덮지 못하게 하고 authoritative empty set으로 처리한다.
...
| `Profile.Owner`              | 객체 종속 | Account가 Profile의 Owner Membership을 가진다             |
...
- Profile의 고정 Post 관계와 API projection은 ordered 0..N collection이다. 현재 Local first-party UI는 server-authoritative
  order의 첫 visible pinned Post만 pinned segment에서 렌더·관리하고, Remote는 검증된 Featured collection 전체를 pinned
  segment에 표시한다. 이 별도 pinned segment는 기존 Profile chronology의 후보·순서·pagination을 바꾸지 않으며, 기존
  chronology에도 포함되는 Post는 pinned segment와 원래 위치에 모두 표시될 수 있다. Home·Local·Hashtag 목록에는 pinned
  ordering을 적용하지 않는다.
- 현재 Local first-party UI에서 첫 visible pin만 pinned segment와 고정 표시를 사용한다. 나머지 Local pin은 pinned
  presentation을 추가하지 않으며, 각 Post는 pin 관계와 무관하게 기존 Profile chronology 정책을 그대로 적용받는다.
- 기존 Profile/Post visibility, lifecycle, block/domain 정책으로 조회할 수 없는 Profile 또는 Post는 pinned segment와
  pinned count에서 숨긴다. Profile이 unavailable이면 기존 Profile mutation 권한·상태 규칙을 적용하고, Post가 Tombstone,
  unavailable, author 또는 visibility 자격을 잃은 경우 해당 관계를 제품상 visible pin으로 간주하지 않아 새 pin을 막지
  않는다. 물리 cleanup 방식은 기존 lifecycle 구현에 맡긴다. Deactivated·Suspended·unavailable Profile의 pin mutation은
  기존 Active/Normal 조건에 따라 거부한다.
...
- 프로필 이전: Profile Migration
- 이전 source: Migration Source
- 이전 target: Migration Target
- ActivityPub 이전 활동: ActivityPub Move
- 팔로워 수: Followers Count
- 팔로잉 수: Following Count
- 프로필 태그: Profile Tag
- 기본 게시 공개 범위: Default Post Visibility
- 고정 Post: Pinned Post
...
- theme, 일반 계정 이동·서버 이전 UI, 게시물·미디어·팔로잉을 포함한 전체 데이터 이전과 outgoing Kosmo Move는
  현재 범위에서 제외한다. Profile Migration은 inbound ActivityPub Move에 한정한다.
