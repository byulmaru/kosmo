...
| Repost Source     | [Post](./post.md)         | Post -> Post     | 1 -> 0..1   | Source에 Content가 있고 Post가 Repost 또는 Quote일 때 | 두 Post 조회 정책 통과                          | 없음             |
...
- Repost Source가 Tombstone이거나 조회 정책을 통과하지 못하면 Repost와 Quote 모두 후보가 아니다.
...
- Post의 공유 참조는 canonical Web origin과 `/{relativeHandle}/{postId}` 경로를 결합한 절대 URL이다.
