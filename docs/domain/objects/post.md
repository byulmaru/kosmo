...
Mentioned Profile 관계는 작성 중 Profile 검색 결과에서 사용자가 명시적으로 선택한 Profile identity로만 만든다.
본문에 같은 `@handle` 문자열이 있다는 사실만으로 Profile을 추측하거나 관계를 만들지 않는다. 작성 요청은 중복
Profile identity를 허용하지 않으며, 각 대상은 요청 시점의 `searchProfiles` 노출 정책을 통과해야 한다.
...
| ----------------------------------- | ----------------- | --------- | --------------------------------------------------------------------------------------------------------------- | ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Post 작성                           | Profile           | Post      | 본문, Post Visibility, Content Warning, Sensitive Media, Media 목록, 선택한 Mentioned Profile 목록              | `Account.Active`, `Profile.Member` | 행동 주체는 선택된 Active/Normal Profile이다. 본문과 Media가 모두 비어 있을 수 없으며 Media는 최대 4개다. Mentioned Profile 목록은 중복 없이 각 Profile의 현재 `searchProfiles` 노출 정책을 통과해야 한다                                                               | Lifecycle=Active이고 Current Content가 있으며 Reply Parent와 Repost Source가 없는 Post, 첫 Post Content, Author/Hashtag/Mentioned Profile 관계가 원자적으로 생성된다                                                 |
| Reply 작성                          | Profile           | Post      | Parent Post, 본문, Post Visibility, Content Warning, Sensitive Media, Media 목록, 선택한 Mentioned Profile 목록 | `Account.Active`, `Profile.Member` | 행동 주체는 선택된 Active/Normal Profile이고 Content가 있는 Parent를 볼 수 있다. Post Visibility는 Parent와 독립적으로 행동 주체가 선택하며 본문/Media/Mentioned Profile 조건은 Post 작성과 같다                                                                        | Lifecycle=Active이고 Current Content와 입력 Reply Parent가 있으며 Repost Source가 없는 Post, 첫 Post Content, Author/Hashtag/Mentioned Profile 관계가 원자적으로 생성된다                                            |
