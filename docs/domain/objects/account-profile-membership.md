...
| Role 변경              | Owner Account     | Membership | Role            | `Account.Active`, `Profile.Owner`      | 현재 Role과 다르고 Owner를 Member로 바꾼 뒤에도 Owner가 한 명 이상 남는다                    | Membership Role이 바뀐다                             |
| Owner 지위 양도        | Owner Account     | Membership | 대상 Membership | `Account.Active`, `Profile.Owner`      | 대상은 같은 Profile의 Member이고 행동 주체의 Membership은 Owner다                            | 대상은 Owner, 행동 주체는 Member로 원자적으로 바뀐다 |
