...
| ---------------------- | ----------------- | ---------- | --------------- | -------------------------------------- | -------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| Account 연결           | Owner Account     | Membership | Account, Role   | `Account.Active`, `Profile.Owner`      | 대상 Account가 Active이고 같은 Account/Profile Membership이 없으며 Profile이 삭제되지 않았다 | 새 Membership이 생성된다                             |
| Role 변경              | Owner Account     | Membership | Role            | `Account.Active`, `Profile.Owner`      | 현재 Role과 다르고 Owner를 Admin 또는 Member로 바꾼 뒤에도 Owner가 한 명 이상 남는다         | Membership Role이 바뀐다                             |
| Owner 지위 양도        | Owner Account     | Membership | 대상 Membership | `Account.Active`, `Profile.Owner`      | 대상은 같은 Profile의 Admin 또는 Member이고 행동 주체의 Membership은 Owner다                 | 대상은 Owner, 행동 주체는 Member로 원자적으로 바뀐다 |
| 다른 Account 연결 해제 | Owner Account     | Membership | 없음            | `Account.Active`, `Profile.Owner`      | 대상 Account가 행동 주체와 다르고 제거 후에도 Owner가 한 명 이상 남는다                      | 대상 Membership이 제거된다                           |
| 자기 연결 해제         | 대상 Account      | Membership | 없음            | `Account.Active`, `Membership.Account` | 제거 후에도 Owner가 한 명 이상 남는다                                                        | 대상 Membership이 제거된다                           |
