...
| ------------ | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------- | -------------- | --------- |
| Hashtag Name | 문자열, 필수     | 바깥 공백 제거 → 앞의 선택적 `#` 하나 제거 → Unicode NFKC → locale 비종속 Unicode full case folding 순서로 정규화한다. 정규화 결과는 Unicode code point 기준 1-20자의 문자·숫자·밑줄이어야 한다. 정규화된 이름은 대소문자를 구분하지 않으며, 같은 정규화 결과는 하나의 Hashtag identity만 가진다 | 항상      | 공개 검색 정책 | 없음      |
