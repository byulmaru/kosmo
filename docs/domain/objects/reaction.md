...
- Activity의 `actor`는 저장된 Remote Profile actor URI와 일치해야 하고, `object`는 대상 Post의 ActivityPub
  identity와 일치해야 한다. 대상 Post Author의 actor URI는 activity 수신자에 포함되어야 한다. 단, Local Post
  Author의 personal inbox로 직접 전달된 activity는 해당 inbox가 식별한 Local actor를 수신자 증거로 사용할 수
  있다. shared inbox와 Remote Post 대상 activity는 activity 수신자에서 Post Author를 식별해야 한다.
