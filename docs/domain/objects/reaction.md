...
이 목록의 나열 순서는 Reaction Type 표시 순서를 정의하지 않는다. Post의 Reaction Type 요약은 각 Type에
현재 존재하는 Reaction 중 가장 이른 생성 시각 오름차순으로 표시한다. 같은 가장 이른 생성 시각에는 제품상
Type 우선순위를 뜻하지 않는 결정적 최종 순서를 적용한다. 한 Type의 현재 Reaction이 모두 제거됐다가 다시
생성되면 새로 존재하는 Reaction의 가장 이른 생성 시각으로 순서를 정한다.
