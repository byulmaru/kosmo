...
| 상호작용 Notification 제거     | 시스템    | Notification | Type, Recipient, Related Post, Related Profile | `System.NotificationSource`                | Reaction이 제거되었거나 Repost가 Tombstone이고 입력 관계와 일치하는 Notification이 존재한다             | 일치하는 Notification 제거를 Best Effort로 시도한다. 실패해도 Reaction 제거 또는 Repost Tombstone 전이를 되돌리지 않는다 |
