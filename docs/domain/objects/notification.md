...
| Reaction Notification 제거     | 시스템    | Notification | Source Reaction                          | `System.NotificationSource`                | Reaction이 제거되고 같은 Source Reaction을 가진 Notification이 존재한다                                 | 일치하는 Notification 제거를 Best Effort로 시도한다. 실패해도 Reaction 제거를 되돌리지 않는다         |
| Repost Notification 제거       | 시스템    | Notification | Recipient, Related Post, Related Profile | `System.NotificationSource`                | Repost가 Tombstone이고 입력 관계와 일치하는 Notification이 존재한다                                     | 일치하는 Notification 제거를 Best Effort로 시도한다. 실패해도 Repost Tombstone 전이를 되돌리지 않는다 |
...
- Reply, Reaction, Repost의 Recipient Profile과 원인 행동의 Related Profile이 같으면 새 Notification을
  생성하지 않는다.
