...
| 최초 동시 중복 질문          | Codex가 Reply → Quote → Mention을 권장하고 사람이 “권장안대로” 채택했다. 순서의 상세 이유는 당시 기록되지 않았다.                                                   |
| 후속 선생성 처리 질문        | 먼저 생성된 Mention과 Reply를 이후 Quote 승인으로 추가·교체하지 않기로 했다. Reply에 대한 후속 응답은 이 규칙에 한정하며 우선순위 전체의 근거로 확대하지 않는다.    |
| 2026-09-09 우선순위 재검토   | 수신자의 글에 생긴 답글·인용을 대표 알림으로 전달하려는 현재 유지 이유를 검토하고 사람이 “PR에 반영”을 요청했다. 당시 이유로 소급하지 않는다.                       |
...
구조 허용은 알림 순서의 이유를 증명하지 않는다. 최초 질문은 Reply → Quote → Mention을 권장하고
Quote → Reply → Mention과 유형별 개별 표시를 대안으로 제시했다. 당시 상세 이유는 확인되지 않았다.

후속 조사에서 PROD-884의 2026-09-08 커밋 `08770fdb5`에 선행 Reply/Mention 분류 계약이 있음을 확인했다.
이는 최초 Quote 질문보다 먼저 작성됐고 PR #778로 main에 병합됐다. 수신자의 Post에 답하면서 같은 수신자를
Mention하면 Reply 한 건을 제공하지만 Quote의 위치까지 정하지는 않는다. 최신 main `1bb02f04c`와 정렬했다.

이번 재검토에서는 수신자의 글에 생긴 답글·인용을 대표 알림으로 전달하려는 이유로 순서를 유지하기로 했다.
Mention의 중요도나 관계의 강도를 낮게 평가하는 판단은 아니며, 알림 이유 문구와 게시글 안의 인용 내용 표시는
구분한다. 상세 이유는 [ADR 0028](../decisions/0028-quote-notification-policy.md#2026-09-09-재검토에서-확정한-유지-이유)에 기록한다.
동시 후보의 우선순위와 먼저 생성된 알림 유지 규칙은 구분하며, 현재 이유를 당시 선택 배경으로 소급하지 않는다.
...
| Reply → Quote → Mention의 근거 질문 | 선행 Reply/Mention 분류, 최초 순서 채택과 이번에 확정한 유지 이유를 구분한다. 수신자의 글에 생긴 답글·인용을 대표 알림으로 전달하는 절충을 답변한다. |

제품 행동 규칙의 미결정 사항은 없다. 당시 선택 배경의 기록상 한계는 유지하며 이번에 확정한 유지 이유와 구분한다.
