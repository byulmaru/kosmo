...
- Post Visibility는 Post 상태 차원이다. Post Eligibility는 데이터를 가지는 속성이 아니라 Post가 소유한
  조회 후보성 정책이다.
- Reply, Repost와 Quote는 별도 durable 객체나 배타적인 상태 값이 아니라 Post의 Content, Reply Parent,
  Repost Source 관계 조합이다.
