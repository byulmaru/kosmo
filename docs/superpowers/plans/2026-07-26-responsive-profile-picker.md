...
- Web full과 mobile Web drawer는 이름·chevron 내부 content의 6px 하향 광학 보정과 Web 전용 open chevron만 변경한다.
...
- **테스트 필요성:** full flow/expanded toggle·6px content geometry, compact overlay dismissal·stacking seam, 12개 목록의 430px wrapper·internal scroll·keyboard focus, mobile Web drawer의 down/up chevron·6px content geometry, 실패 유지와 명시적 close reset을 관찰 가능한 결과로 직접 증명한다.
...
| full·mobile Web chevron·6px optical shift | Task 10 | `ResponsiveProfilePickerFull`·`UniversalMobile` trigger state·text/icon geometry와 직접 시각 검증 |
...
### Task 8: Mobile Web trigger 광학 보정을 4px로 조정한다
...
- Produces: text·chevron center가 42px trigger center보다 4px 아래인 Mobile Web 전용 optical position

**테스트 코드 범위:** 기존 `UniversalMobile` play의 text·icon center assertion 두 곳만 4px로 수정한다.

**테스트 필요성:** 2px 구현이 남아 있으면 실제 Mobile Web DOM geometry가 실패하고 승인된 4px 위치만 통과하는지
...
- [x] **Step 1: canonical → Linear → OpenSpec 계약을 4px로 정렬한다**

`docs/design/breakpoints.md`와 `PROD-238`의 Mobile Web 광학 보정을 4px로 갱신한 뒤 OpenSpec current contract에
반영하고, 기존 2px 결정과 완료 기록은 historical record로 보존한다. 후속 task 1.9는 구현 전 미완료로 둔다.

- [x] **Step 2: 기존 Mobile geometry assertion을 4px로 바꾸고 RED를 확인한다**

`UniversalMobile`의 text·icon center expected literal만 `4`로 바꾼다. 2px 제품 코드에서 focused Storybook test를
실행해 expected 4, received 2의 동작 실패를 확인한다.

- [x] **Step 3: Mobile Web trigger 내부 transform만 4px로 변경한다**

`styles.mobileWebTriggerContent.transform`의 `translateY`만 2에서 4로 바꾼다. trigger root, platform/surface guard,
...
`UniversalMobile`에서 trigger center 대비 text·icon center가 실제 4px 아래이고, open 전후 trigger·navigation rect,
picker anchor와 Down/Up chevron이 유지되는지 확인한다. 4px가 아래로 처져 보이면 사용자 확인 없이 추가 보정하지 않는다.

- [x] **Step 6: OpenSpec 1.9를 완료하고 checkpoint commit·push한다**
...
### Task 10: Full Web trigger도 같은 내부 콘텐츠를 6px 보정한다

**Files:**

- Modify: `docs/design/breakpoints.md`
- Modify: `openspec/changes/add-responsive-profile-picker/{proposal.md,design.md,decisions.md,tasks.md,specs/web-app-shell/spec.md}`
- Test: `apps/app/src/stories/Shell.stories.tsx:198-228`
- Modify: `apps/app/src/components/shell/ProfileSwitcher.tsx:135-136,435-444,505-513`
- Modify: `docs/superpowers/plans/2026-07-26-responsive-profile-picker.md`

**Interfaces:**

- Consumes: `Platform.OS === 'web'`, `surface === 'full' | 'drawer'`, 기존 noncompact 이름·chevron trigger copy
- Produces: Full·Mobile Web에서 text·chevron center가 42px trigger center보다 동일하게 6px 아래인 공통 optical position

**테스트 코드 범위:** 기존 `ResponsiveProfilePickerFull` play의 이름·chevron center assertion과 open 전후 trigger rect
불변 assertion만 추가한다.

**테스트 필요성:** 현재 full Web은 box center offset이 0px이므로 mobile과 같은 6px 계약을 위반한다. 실제 full shell
surface에서 이름·chevron을 관찰하고 trigger root와 picker anchor가 이동하지 않는지 직접 증명한다.

**테스트 제외 범위:** 새 story·fixture·helper·harness, snapshot, compact avatar·Android/iOS picker test,
SidebarNavigation·UniversalShell 제품 코드, GraphQL·Relay·E2E 변경.

- [x] **Step 1: canonical → Linear → OpenSpec 계약을 full·mobile 6px로 정렬한다**

`docs/design/breakpoints.md`와 `PROD-238` Full sidebar 범위에 이름·chevron 내부 content의 6px 보정과 trigger hitbox·
picker anchor·navigation geometry 불변을 추가한 뒤 OpenSpec current contract에 반영한다. 후속 task 1.11은 구현 전
미완료로 둔다.

- [x] **Step 2: 기존 Full geometry assertion에 6px 기대값을 추가해 RED를 확인한다**

`ResponsiveProfilePickerFull`에서 이름·chevron center와 trigger center의 차이를 literal `6`으로 검증하고, picker를
연 뒤 trigger rect가 같다는 assertion을 추가한다. 현재 full 제품 코드에서 expected 6, received 0의 동작 실패를
확인한다.

- [x] **Step 3: Web full·mobile이 같은 내부 content wrapper를 공유하도록 최소 구현한다**

`fullWeb || mobileWebDrawer`일 때만 기존 이름·chevron trigger copy를 공통 `webTriggerContent` wrapper로 감싼다.
wrapper의 `transform: [{ translateY: 6 }]`은 유지하고 trigger root, `fullOverlayPosition.top=190`,
`SidebarNavigation`·`UniversalShell`은 수정하지 않는다.
...
- [x] **Step 5: Full Storybook 시각 stop gate와 geometry를 확인한다**

`ResponsiveProfilePickerFull`과 `ProfileSwitcherInteraction`에서 trigger center 대비 이름·chevron center가 실제 6px
아래이고, open 전후 trigger rect, picker anchor와 navigation rect가 유지되는지 확인한다. 아래로 처져 보이거나
anchor가 이동하면 범위를 확대하지 않고 중단한다.

- [x] **Step 6: 독립 리뷰 뒤 OpenSpec 1.11과 checkpoint commit·push를 완료한다**

Terra `implementation_reviewer`는 Web full/drawer surface guard, 공통 wrapper, trigger root·picker anchor·navigation
불변, compact avatar·Android/iOS 비누출, 기존 Full Storybook assertion과 문서 정합성을 검토한다. P0–P2 finding을
수정하고 검증을 다시 통과한 뒤 OpenSpec 1.11과 위 체크리스트를 완료 처리해 checkpoint commit·push한다. PR #362는
Draft로 유지하고 Ready 전환과 OpenSpec archive를 수행하지 않는다.
