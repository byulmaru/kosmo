...
Vault의 `secret/kubernetes/kosmo/dev|prod`에는 환경별 공개 `EXPO_PUBLIC_SENTRY_DSN`을 둔다. API, Web BFF와 Web browser는 Sentry의 `kosmo` project 하나를 공유하고 `runtime` tag로 구분한다. `byulmaru/kosmo`의 main dev build는 dev 환경 객체를, `prod` Environment 승인 뒤의 main production build는 prod 환경 객체를 읽는다. Manual full-SHA release도 `prod` Environment 승인 뒤 target SHA를 build할 때 prod 객체를 읽는다. 승인 전에는 production DSN을 읽지 않으며, tag push와 `production` branch push는 production credential과 release workflow를 선택하지 않는다. Source map upload metadata와 token은 GitHub repository 설정에서 주입한다.
...
Vault Secrets Operator는 환경별 Vault 객체 전체를 기존 `env` Kubernetes Secret으로 변환하고 API와 Web BFF는 여기서 `EXPO_PUBLIC_SENTRY_DSN`을 읽는다. Web dev build는 main의 dev 객체를, `prod` 승인 뒤 실행되는 main production build와 승인된 manual full-SHA build는 prod 객체의 같은 변수를 사용한다. Event의 dev/prod 구분은 공용 `ENVIRONMENT`와 Web의 `EXPO_PUBLIC_ENVIRONMENT`가 담당한다. Automatic release의 `SENTRY_RELEASE`는 main의 immutable full SHA를 사용하고, manual release는 dispatch의 `github.sha`가 아니라 resolved target SHA를 사용해 workflow가 `kosmo@<target commit SHA>`로 만든다. 이 release 값은 event와 source map을 연결한다. DSN, environment와 release가 모두 있으면 해당 runtime은 Sentry를 활성화한다. 로컬·테스트에는 이 배포 metadata를 기본 주입하지 않는다.
