...
- 사용자 작성 콘텐츠와 원래 exception message
- user, extra, context와 모든 자동 console·network·navigation·UI breadcrumb

Sentry의 기본 개인정보 전송도 활성화하지 않는다. 새 tag, context 또는 breadcrumb가 필요하면 허용할 값과 사용자 콘텐츠 포함 가능성을 먼저 검토하고 이 문서와 redaction test를 함께 갱신한다.
...
6. event JSON에서 request, user, extra, context, breadcrumb, 원래 exception message와 검증 요청의 인증·GraphQL·사용자 값이 없는지 확인한다.
