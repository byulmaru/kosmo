...
`byulmaru/expo-ota`는 signing key와 R2 credential을 읽어 static `multipart/mixed` manifest와
immutable asset을 발행한다. Client와 static delivery에는 private key나 publish credential을
넣지 않는다.
...
- reusable publisher는 GitHub OIDC로 Vault에 접근한다. Signing key와 R2 credential을
  workflow input, export artifact, client bundle에 넣지 않는다.
