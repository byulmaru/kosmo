...
| 값         | 의미                                                                |
| ---------- | ------------------------------------------------------------------- |
| `ACTIVE`   | 일반 조회, 게시, 팔로우, actor profile 선택이 가능한 활성 프로필.   |
| `DISABLED` | 비활성화된 프로필. 새 게시물 작성, 팔로우 요청, 공개 조회에서 제외. |
...
- 일반 공개 조회는 `Profile.state = ACTIVE`인 프로필만 반환한다. `DISABLED` 프로필은 `profile`, `profileByHandle`, `followers`, `following`, `posts` 공개 응답에서 제외한다.
