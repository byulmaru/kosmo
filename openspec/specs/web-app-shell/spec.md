...
웹 애플리케이션은 인증된 사용자가 사이드바에서 접근 가능한 프로필 사이를 전환할 수 있게 해야 한다(MUST). 프로필 전환 성공 후 앱 셸의 활성 프로필 표시는 Mearie cache의 `Session.selectedProfile` 갱신을 반영해야 하며(MUST), 앱 셸 아래 route는 자기 화면에서 필요한 active profile 필드를 자기 GraphQL operation으로 선언해야 한다(MUST). follow action UI는 viewer-relative 상태를 `Profile.viewerState`에서 읽어야 한다(MUST).

#### Scenario: Render accessible profiles

- **WHEN** 인증된 계정에 접근 가능한 활성 프로필이 있다
- **THEN** 사이드바는 260px 높이의 상단 프로필 영역에 활성 프로필 정보를 표시한다
- **AND** 활성 프로필 정보는 `currentSession.selectedProfile` 조회 결과를 기반으로 하며, 프로필 전환 성공 후에는 mutation 응답으로 갱신된 Mearie cache 결과를 반영한다
...
- **AND** 요청 성공 후 사이드바는 Mearie cache의 `Session.selectedProfile` 갱신 결과를 새 활성 프로필로 반영한다
- **AND** 이미 열린 홈과 `/compose`처럼 active profile 객체가 필요한 route는 각 route query의 `currentSession.selectedProfile` 갱신 결과를 비동기적으로 반영한다
- **AND** 검색, 프로필, followers/following 화면의 follow action UI는 `Profile.viewerState` 갱신 결과를 비동기적으로 반영한다
...
- **AND** 사이드바는 Mearie cache의 `Session.selectedProfile` 갱신 결과를 새 활성 프로필로 반영한다
- **AND** 시스템은 접근 가능한 프로필 목록이 새 프로필을 포함하도록 `createProfile.account.profiles` 또는 동등한 프로필 목록 데이터를 갱신한다
