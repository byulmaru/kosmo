...
- **AND** 모바일 Web과 Android/iOS는 화면 상단에서 44px 여백을 둔다
...
- **WHEN** 모바일 Web 또는 Android/iOS에서 Welcome heading이 여러 줄로 표시된다
