...
API와 Worker 비소유 PostgreSQL LOGIN 및 role별 Vault/VSO static SCRAM credential을 provision하고, shared `kosmo_worker` application consumer와 Secret rotation restart target을 연결하는 계약을 정의한다. 기존 `kosmo_api` provisioning은 rollback window 동안 유지하며 객체 ACL contract 제거와 production 운영 authorization은 각각 PROD-781과 별도 승인 범위다.
