...
TBD - created by archiving change add-r2-image-upload-api. Update Purpose after archive.

## Requirements

### Requirement: 인증된 이미지 업로드 endpoint

API는 이미지 업로드를 위한 루트 `POST /upload` endpoint를 제공해야 하며, 기존 bearer token 요청 context에서 파생된 유효한 인증 session과 선택된 actor profile을 요구해야 한다(MUST).
...
#### Scenario: 선택된 프로필이 없는 업로드 요청

- **WHEN** 클라이언트가 유효한 인증 session을 가지고 있지만 요청 context에 actor profile ID가 없다
- **THEN** 시스템은 HTTP 403 JSON error response를 반환한다
...
- **AND** request context의 actor profile ID를 media row에 저장한다
- **AND** media row는 업로드된 file row를 `originalFileId`로 참조한다
- **AND** file row는 R2 object key, MIME type, byte size를 저장한다
...
- **THEN** 시스템은 `source = REMOTE`, profile ID, remote URL, remote fetched timestamp를 가진 media row를 저장할 수 있다
