# system-db-execution-boundary Specification

## Purpose

기존 owner credential과 SQL 동작을 유지하면서 federation 및 향후 system 작업이 명시적인 DB handle과 transaction 수명 경계를 사용하고, 공유 core service가 caller-owned DB handle을 조합할 수 있게 하는 계약을 정의한다.
