...
- **AND** configured local instance에 저장된 `ProfileState.ACTIVE` Profile 중 정규화 handle이 입력 문자열을 포함하는 결과를 `normalizedHandle ASC` cursor page로 반환한다
...
- **AND** 해당 domain의 ActivityPub instance에 이미 저장된 `ProfileState.ACTIVE` Remote Profile 중 정규화 handle이 입력 문자열을 포함하는 결과를 `normalizedHandle ASC` cursor page로 반환한다
...
- **THEN** 시스템은 `normalizedHandle ASC` 순서의 첫 페이지와 다음 `after` cursor를 반환한다
