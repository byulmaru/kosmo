## migration-owner-postgres-credential-boundary Specification

## Purpose

모든 환경의 migration이 CNPG-managed schema owner credential로 직접 실행되고 active runtime과 분리되며, obsolete migration identity와 credential 경로가 production gate 뒤 제거되는 계약을 정의한다.
