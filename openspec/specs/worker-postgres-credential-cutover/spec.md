# worker-postgres-credential-cutover Specification

## Purpose

Web trusted federation ingress와 enabled Temporal Worker가 별도 application DB handle 없이 process 기본 `db`에서 `kosmo_worker` BYPASSRLS credential을 사용하고, API·GraphQL operation·migration·MessageQueue 경계를 침범하지 않는 전환과 rollback 계약을 정의한다.
