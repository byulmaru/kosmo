...
웹 앱은 새 글 작성 컴포넌트 사용처에서 인증과 active profile 부재 상태를 처리해야 한다(MUST). `/compose` 사용처는 `(tabs)` layout의 GraphQL active profile bridge에서 작성 프로필을 읽어야 하며(MUST), 이미 열린 상태에서 프로필 전환이 성공하면 성공 응답의 `Session.selectedProfile`을 작성 프로필로 즉시 반영해야 한다(MUST).
...
- **THEN** `(tabs)` layout query는 `currentSession.selectedProfile`에서 새 글 작성 컴포넌트가 선언한 `Profile` fragment를 spread한다
- **AND** `/compose` route는 layout의 GraphQL active profile bridge에서 전달된 selected profile fragment ref를 작성 프로필로 사용한다
...
- **THEN** 시스템은 성공 응답의 `Session.selectedProfile`을 새 글 작성 컴포넌트의 작성 프로필로 반영한다
