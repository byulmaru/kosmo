...
유니버설 앱은 새 글 작성 컴포넌트에서 nullable Content Warning과 plain-text 본문의 합산 글자수를 표시해야 한다(MUST).
...
**Authority / Provenance:** `docs/domain/objects/post.md`, `docs/domain/objects/post-content.md`, `docs/domain/objects/media.md`, PROD-461, PROD-553, PROD-554, PROD-642. 유니버설 앱은 Composer의 Plain Text, optional nullable Content Warning, Ready Media global ID와 nullable Alt Text 순서, Sensitive Media를 `createPost` mutation에 제출해야 한다(MUST).
