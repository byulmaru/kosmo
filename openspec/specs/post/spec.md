...
kosmo 게시글 capability의 현재 계약을 문서화한다. 게시글·게시글 콘텐츠 GraphQL object와 TipTap 저장 계약을 유지하면서, 유니버설 앱의 plain-text `TextInput`이 pure TypeScript adapter로 현재 doc/paragraph/text JSON을 생성하는 작성 흐름을 다룬다.
