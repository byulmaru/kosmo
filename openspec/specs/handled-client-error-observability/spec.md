# handled-client-error-observability Specification

## Purpose

Web·Native 앱이 기존 플랫폼 Sentry 경계와 runtime gate를 재사용해 처리된 오류를 안전하게 수집하고,
Post Composer와 Local Profile의 공통 이미지 업로드 실패를 한 번만 관측하는 요구사항을 정의한다.
기존 처리되지 않은 React·Native 오류 관측과 공통 이미지 업로드 lifecycle은 호환성을 확인하기 위한
참고이며, 이 capability의 처리된 오류 요구사항은 이 문서와 canonical 문서 및 PROD-929가 함께 소유한다.

호환성 참고:

- `openspec/specs/native-error-observability/spec.md`: 기존 처리되지 않은 React·Native 오류 관측 계약과의 호환성을 확인한다.
- `openspec/specs/post-composer-media-upload/spec.md`: 기존 공통 이미지 업로드 lifecycle과 오류 분류와의 호환성을 확인한다.
