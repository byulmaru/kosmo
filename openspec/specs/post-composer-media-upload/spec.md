...
**Authority / Provenance:** `docs/domain/objects/media.md`, `docs/domain/decisions/0013-media-storage-service-boundary.md`, `docs/domain/decisions/0018-media-upload-lifecycle-without-file.md`, PROD-461, PROD-553. 유니버설 앱은 Post Composer에서 선택한 각 이미지 byte를 Kosmo가 발급한 제한된 URL로 Media Storage Service에 직접 전송하고 같은 Media를 Ready로 완료해야 하며(MUST), Kosmo API를 byte proxy로 사용하면 안 된다(MUST NOT).

#### Scenario: 선택 즉시 업로드 성공

- **WHEN** 사용자가 갤러리 이미지를 선택한다
- **THEN** 앱은 `issueMediaUploadUrl`로 Uploading Media와 제한된 upload URL을 받는다
- **AND** 선택한 이미지 byte를 upload URL에 직접 `PUT`한다
- **AND** PUT 성공 뒤 같은 Media global ID로 `completeMediaUpload`을 호출한다
- **AND** Ready 응답을 받은 항목만 Post 작성 후보로 표시한다

#### Scenario: 업로드 실패와 재시도

- **WHEN** 업로드 시작, byte PUT 또는 완료 확인이 실패한다
- **THEN** 앱은 해당 이미지의 미리보기와 실패 상태를 유지한다
...
**Authority / Provenance:** `docs/domain/objects/post.md`, `docs/domain/objects/post-content.md`, `docs/design/accessibility.md`, PROD-461, PROD-553. 유니버설 앱은 Web/iOS/Android 갤러리 이미지 선택을 제공하고 현재 Composer에서 최대 4개의 항목을 선택 순서대로 관리해야 한다(MUST). 카메라 촬영을 추가하면 안 된다(MUST NOT).
...
- **AND** 실패 항목에는 재시도와 제거 action을 제공한다
