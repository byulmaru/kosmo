...
2. 업로드 입력은 multipart `image` 필드를 사용한다.
   ...
   REST 입력 검증은 route handler 내부 수동 검증이 아니라 `@hono/standard-validator`의 `sValidator('form', ...)` middleware 흐름에서 zod schema로 처리한다.
...
5. 추가 환경변수는 명시적으로 관리한다.
   ...
   필요한 환경변수는 `R2_ENDPOINT`, `R2_BUCKET`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_PUBLIC_BASE_URL`이다. 이 값들은 `packages/core/env.ts`에서 zod schema로 파싱해 export하고, API R2 helper는 파싱된 `env`를 참조한다. 업로드 최대 크기는 환경변수가 아니라 route 정책 상수로 관리한다.
