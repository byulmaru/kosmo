...
- **색/크기/radius는 토큰·기존 스케일에 바인딩(arbitrary 하드코딩 금지)**: 색·radius·`text-md`·`text-sm` 등은 토큰으로 참조한다. `@theme` 전용 토큰이 없는 Figma 크기(h1 32px·각주 13px)는 새 토큰을 만들기보다 가장 가까운 기존 스케일에 바인딩한다 — h1 → `text-3xl`(30px), 각주 → `text-xsm`(12px). 정확한 값이 꼭 필요해지면 그때 Foundation 토큰을 추가한다. 폰트는 기존 SUIT/Pretendard(Figma의 Inter/Noto는 대치 표기).
