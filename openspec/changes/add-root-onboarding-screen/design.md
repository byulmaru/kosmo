...
- **색/radius/정의된 크기는 `layout.css @theme` 토큰 사용**: 색·radius·`text-md`·`text-sm` 등 정의된 값은 토큰으로 참조한다. `@theme`에 토큰이 없는 폰트 크기(h1 32px·각주 13px)는 코드베이스 기존 관행(`text-[13px]`·`text-[17px]` 등)대로 arbitrary 값을 쓴다(폰트 크기 토큰 정비는 별도 작업). 폰트는 기존 SUIT/Pretendard(Figma의 Inter/Noto는 대치 표기).
