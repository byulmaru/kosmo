...
- [ ] 2.2 일반 프로필 선택 성공 handler가 `currentSession` 전체 수동 invalidation/refetch에 의존하지 않고, `homeTimeline`, `Profile.viewerFollow` 같은 active-profile 의존 field만 필요한 범위로 갱신하게 한다
...
- [ ] 4.3 E2E에서 일반 프로필 선택 성공 handler가 `currentSession` 전체 invalidation/refetch를 직접 요구하지 않고, 필요한 mutation과 active-profile 의존 field 갱신만 발생시키는지 확인한다
