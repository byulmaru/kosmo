...
PROD-451은 조상·현재·하위 Reply를 기존 Post rendering과 fixture용 optional 선택 adapter로 구분하는 재사용 presentation layout을 전달하고, PROD-422는 API에서 제공한 조회 가능한 조상 경로·현재 Post·하위 Reply와 route-aware Post renderer를 같은 layout에 연결한다.
...
- PROD-451은 fixture·Storybook·presentation component interaction과 fixture용 optional Post 선택 adapter만 소유한다. 기존 Post Link를 감싸거나 대체하지 않으며 실제 Reply field·Relay query/cache·route·visibility integration, Quote Source·Action Bar·Reaction/Repost UI를 추가하지 않는다.
...
- PROD-451은 fixture 기반 일반 Reply, role별 list/detail renderer 선택, 직접 connector, 직접 연결된 마지막 visible Reply에서의 supplied boundary 종료, Reply+Quote-shaped subtree 보존과 optional mock callback adapter를 Storybook/component interaction으로 검증한다. 실제 spacing·typography 밀도는 Light 390px/600px visual QA로 검증하고 Dark appearance는 미검증 위험으로 남긴다.
