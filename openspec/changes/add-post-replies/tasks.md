...
- field 이름, collection shape와 순서 방향은 Linear와 OpenSpec에서 승인되기 전 구현하지 않는다.

**Verification**

- 일반·다단계·Reply+Quote 조상, unavailable 중단, 숨은 조상 비노출과 cycle 방어를 API test로 검증한다.

- [ ] 3.1 조상 경로의 공개 GraphQL field·collection·순서 계약을 Linear에서 확정하고 OpenSpec decision을 갱신한다.
- [ ] 3.2 승인된 공개 계약에 따라 직접 Parent 기반 조상 조회를 구현한다.
...
Home은 viewer와 관련된 eligible Reply를 포함하고 Profile·Hashtag Post List는 Reply Parent가 있는 Post를 제외한다.
...
- viewer 작성·viewer Post의 Reply·상호 followee 맥락과 관련 없는 Reply, Reply+Quote, Profile·Hashtag 제외 및 기존 Post/Repost/Quote 목록 회귀를 검증한다.

- [ ] 5.1 Home의 viewer 관련 Reply 후보 정책을 구현한다.
- [ ] 5.2 Profile·Hashtag의 Reply 제외 정책을 구현한다.
- [ ] 5.3 권한·pagination limit·기존 목록 회귀 test와 관련 check를 통과시킨다.
...
- 관계 조합별 저장→직접 Parent·조상·descendant 조회→Home/Profile/Hashtag 후보→Post 상세 thread 흐름을 통합 검증한다.
