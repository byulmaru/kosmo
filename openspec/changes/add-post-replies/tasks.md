...
Hashtag Post List의 canonical Reply 제외 계약은 `reply` spec과 부모 통합 범위에 유지한다. 현재 Hashtag query/resolver 표면이 없으므로 PROD-429는 Home/Profile만 구현·검증하며, Hashtag capability를 도입하는 구현 slice가 해당 requirement와 회귀 검증을 소유해야 한다.
