...
- Content, Reply Parent와 Repost Source의 허용 조합 및 대상 유효성을 공통 core 경계에서 검증한다.
...
- 현재 `createPost`의 Local/ActivityPub overload와 반환값은 contentful 호출 계약을 가진다. optional 관계 입력을 추가하더라도 기존 호출과 ActivityPub first-write-wins 결과를 바꾸면 안 된다.
- `repostSourceId`와 partial unique index는 PROD-394가 소유한다. Reply migration이나 core 변경에서 이를 재생성·수정하지 않는다.
- Parent와 Source는 lifecycle과 무관하게 저장 ID를 유지하지만 생성 시점에는 존재하고 `currentContentId`가 있어야 한다.
- 조상 조회는 조회 불가능한 Parent에서 중단한다. descendant 조회는 구조 탐색과 viewer 필터를 분리해야 숨겨진 Parent 아래의 visible Reply를 함께 제거하지 않는다.
- PROD-399·400은 정확한 GraphQL field 이름, connection 여부, pagination과 정렬 방향을 아직 Linear에서 고정하지 않았다.
...
- `createPost` transaction은 빈 관계의 Post를 먼저 만들고 Content를 생성한 뒤, 제안된 Content/Reply Parent/Repost Source 조합과 두 대상의 존재·Content를 공통 내부 validator로 확인한다. 마지막 Post update에서 `currentContentId`, `replyParentId`, `repostSourceId`를 함께 연결한다.
- validator는 package 공개 barrel에 새 action으로 노출하지 않는다. 현재 contentful `createPost`는 Post·Reply·Quote·Reply+Quote를 저장하고, contentless Repost 조합 판정은 후속 Repost action이 같은 검증 경계를 재사용할 수 있게 유지한다.
...
- 조상·descendant 조회는 재귀 CTE나 동등한 bounded traversal을 사용할 수 있다. visibility 중단·독립 필터·cycle 방어와 query plan 검증을 동일하게 만족해야 한다.
- PROD-399·400의 GraphQL field와 collection shape는 해당 이슈 착수 전에 승인된 decision으로 확정할 수 있다. 현재 요구사항에 없는 pagination·정렬 행동을 구현 편의로 추가해서는 안 된다.

### Known Traps

- Content 없는 중간 Post를 먼저 최종 상태로 저장하면 구조 CHECK 또는 기존 workload와 충돌할 수 있다. 관계와 `currentContentId`는 최종 update에서 함께 연결한다.
- Parent/Source의 존재만 확인하고 `currentContentId`를 확인하지 않으면 contentless Repost를 관계 대상으로 허용하게 된다.
...
4. application code가 optional 관계 입력과 공통 검증을 사용하도록 배포한다.
...
- PROD-399의 조상 경로 GraphQL field 이름, 반환 collection shape와 immediate-parent-first/root-first 순서는 해당 이슈 구현 전 결정이 필요하다.
- PROD-400의 descendant GraphQL field 이름, connection·pagination과 정렬 계약은 실제 query plan과 함께 해당 이슈 구현 전 결정이 필요하다.
- PROD-422의 세부 thread 배치와 중첩 표현은 PROD-451의 presentation 책임을 침범하지 않는 범위에서 해당 이슈 구현 전 확정해야 한다.
