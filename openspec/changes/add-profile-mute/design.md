...
없음. v1 재-Mute는 기존 동일 pair row의 ID를 보존하고 `expires_at`을 `null`로 수렴시킨다. 기간 지정 Mute 생성, 만료 판정·정리와 이 v1 계약의 대체 여부는 `PROD-826`이 명시적으로 정한다.
