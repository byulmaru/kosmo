...
[tasks.md](./tasks.md)의 6장 완료 여부를 보조하며, Native runtime·최종 archive처럼 아직 통과하지 않은
검증은 완료로 취급하지 않는다.
...
SDK/emulator가 없어 runtime·native accessibility를 실행하지 못했다. 따라서 6.4는 체크하지 않는다.
...
- 6.6의 canonical·Linear·OpenSpec 최종 대조, delta sync와 archive는 아직 수행하지 않는다. Native runtime/a11y와 남은 최종 대조가 끝난 뒤 root가 재판정한다.
