...
- **THEN** the command carries the exact deleted-source ID and directed pair needed by its effects, rather than reading or deleting a later row for the same pair
...
- **THEN** no Follow or Follow Request state, source identity, or downstream effect is committed for that transition
