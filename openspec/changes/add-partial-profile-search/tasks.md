...
사용자가 사람 탭에서 기존 handle 해석 정책에 맞는 일부 문자열을 검색하면 DB에 저장된 local 또는 지정 domain의 remote Profile 중 부분 일치하는 결과 전체를 기존 프로필 항목·링크·팔로우 상태로 볼 수 있다.

**Guardrails**

- 기존 exact `profileByHandle` 단건 계약과 그 소비자를 유지하고, `profilesByHandle` 다건 계약으로 사람 검색을 제공한다.
- 사용자 입력의 escape 문자, `%`, `_`를 리터럴로 escape한 뒤 부분일치용 `%`를 양쪽에 추가하고 SQL parameter binding을 유지한다.
- 현재 단계에서는 pagination, 최대 결과 개수, 별도 정렬, 검색 index 또는 DB migration을 추가하지 않는다.
...
- API 통합 테스트로 local/remote 부분일치, 복수 결과 전체 반환, empty, 정규화, `%`·`_`·escape 문자, 검색 visibility와 remote no-materialization을 검증한다.
- 검색 페이지 E2E로 다건 `ProfileListItem`, profile link·follow 상태, empty, wildcard 입력과 기존 loading/error 상태를 검증한다.
...
- [x] 1.1 기존 exact lookup을 보존하면서 부분일치 Profile 전체를 반환하는 `profilesByHandle` GraphQL 계약을 구현한다.
...
- [x] 1.4 API 통합 테스트에 부분일치·복수·empty·정규화·wildcard·visibility·no-materialization 시나리오를 추가하고 통과시킨다.
- [x] 1.5 사람 탭 Relay query와 렌더링을 전체 결과 목록으로 전환하고 기존 링크·팔로우·loading/error/empty 계약을 유지한다.
- [x] 1.6 검색 Story/E2E를 다건·empty·wildcard 결과 shape에 맞춰 갱신하고 관련 검증을 통과시킨다.
