...
PROD-504는 현재 단계에서 일치 결과 전체 반환을 선택했다. DB schema·index·migration, pagination·상한·별도 정렬은 추가하지 않는다.

## Goals / Non-Goals

**Goals:**

- 기존 exact lookup을 유지하는 부분일치 다건 GraphQL query를 제공한다.
- 사용자 입력의 SQL `LIKE` 메타문자를 리터럴로 처리하고 parameter binding을 유지한다.
- 사람 탭에서 전체 결과를 기존 `ProfileListItem`/`FollowButton` 계약으로 렌더링한다.
- local/remote 저장 Profile의 조회·검색 노출 정책과 remote no-materialization 경계를 지킨다. Staged visibility는 exact·partial에 동일하게 적용한다.

**Non-Goals:**

- pagination, 결과 상한, 관련도 또는 안정 정렬
...
- SearchScreen의 Relay query와 Story mock은 단일 nullable object shape이므로 다건 non-null list shape에 맞춰 함께 변경해야 한다.

### Recommended Approach

- exact field는 유지하고 `profilesByHandle(handle: String!): [Profile!]!` 형태의 별도 다건 query를 추가한다. 반환 Profile row는 기존 loadable Node/fragment 경계를 그대로 사용한다.
...
- 조회에는 `.limit(1)`과 `first`를 사용하지 않고, 별도 `orderBy`, limit, cursor 없이 정책을 통과한 row 전체를 반환한다.
- 사람 탭의 colocated Relay query를 새 list field로 전환하고 결과를 map해 `ProfileListItem`을 렌더링한다. 빈 배열은 기존 empty 상태, request 실패는 기존 RouteBoundary, 각 Node의 follow state는 기존 fragment가 소유한다.
- API 통합 테스트는 local/remote 부분일치, 복수/empty, 정규화, `%`·`_`·escape 문자, 노출 정책, no-materialization을 검증한다. 검색 E2E는 다건 렌더링·empty·wildcard 입력과 기존 링크/follow 표시 계약을 검증한다.
...
- [일치 결과가 많으면 응답 크기와 UI 렌더 비용이 증가한다] → PROD-504에서 전체 반환을 명시적으로 수용하고, 실제 사용량을 근거로 pagination·상한을 후속 변경에서 추가한다.
...
- [DB 반환 순서가 실행마다 달라질 수 있다] → 현재 계약은 별도 정렬 의미를 제공하지 않으며 클라이언트·테스트가 순서에 의존하지 않게 한다.

## Migration Plan

1. 기존 exact field를 유지한 채 다건 query와 통합 테스트를 추가한다.
2. GraphQL schema/Relay artifact를 갱신하고 사람 탭·Story/E2E를 새 list shape로 전환한다.
3. API와 클라이언트를 함께 배포한다. DB migration과 backfill은 없다.
4. 롤백 시 사람 탭을 기존 exact query로 되돌리고 새 다건 field를 제거한다. 저장 데이터 변화는 없다.

## Open Questions

없음. pagination·상한·정렬과 검색 index는 현재 미결정이 아니라 명시적 후속 범위다.
