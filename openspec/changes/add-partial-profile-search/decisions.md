...
이 결정 기록은 PROD-504와 Profile canonical 조회·검색 정책을 부분일치 다건 GraphQL API와 사람 탭 목록으로 구체화한다. 기존 exact lookup 소비자 호환성, SQL `LIKE` 입력 안전성, current-stage 전체 결과 반환과 후속 pagination 경계를 구현 전에 고정한다.

## Decision Records

### 현재 단계는 일치 결과 전체를 반환한다
...
### exact lookup과 별도의 다건 GraphQL field를 사용한다
...
- 없음. pagination·상한·정렬과 검색 index는 현재 미결정이 아니라 명시적으로 제외된 후속 범위다.
