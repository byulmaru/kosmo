...
현재 사람 검색은 exact handle 단건 조회만 지원해 사용자가 handle 일부만 알고 있으면 저장된 프로필을 찾을 수 없다. PROD-504는 기존 handle 해석과 프로필 노출 정책을 유지하면서 DB `LIKE` 부분 일치 결과 전체를 목록으로 제공한다. 검색 대상은 이미 저장된 local·remote Profile이며, 검색 중 remote lookup·refresh·신규 materialization은 수행하지 않는다.

## What Changes

- 기존 exact `profileByHandle` 계약은 프로필 route와 다른 소비자를 위해 유지하고, 사람 검색용 부분 일치 다건 GraphQL 조회를 추가한다.
...
- 현재 단계에서는 일치 결과 전체를 반환하고 최대 개수, pagination, 별도 정렬 계약은 후속 확장으로 남긴다.
- 사람 검색 UI는 다건 결과를 기존 `ProfileListItem`과 팔로우 액션 정책으로 표시하며 기존 로딩·오류·빈 결과 상태를 유지한다.
...
- `profile`: 저장된 local/remote Profile을 기존 handle 해석 경계 안에서 부분 일치 다건 조회하는 API 계약을 추가한다.
- `web-app-shell`: 사람 탭의 exact 단건 결과 표시를 부분 일치 전체 결과 목록 표시로 변경한다.
