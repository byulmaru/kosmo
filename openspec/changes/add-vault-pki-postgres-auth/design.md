...
PROD-470은 이 password 경로를 기본값으로 보존하면서, 향후 PROD-369가 분리할 workload별 DB 역할이 동일한 Vault PKI issuer의 client certificate를 사용할 수 있는 선택적 경로를 추가한다. 로컬 client가 Tailnet hostname을 검증하려면 CNPG server certificate도 같은 issuer에서 발급되고 해당 hostname과 내부 read-write Service 이름을 SAN으로 가져야 한다.
...
- 현재 chart는 하나의 `kosmo` owner 연결만 가진다. PKI values는 API, web, migration과 CNPG replication의 role 및 Vault role을 분리해 표현하되, 비활성 기본값에서는 기존 manifest를 바꾸지 않아야 한다.
...
Helm values는 기능 전체를 끄는 기본값과 server, API, web, migration 및 CNPG replication별 PKI role/DB role을 제공한다. 활성화 시 server `VaultPKISecret` 하나와 workload별 client `VaultPKISecret`, `streaming_replica` client `VaultPKISecret`을 만들고, 모두 같은 mount/issuer chain을 사용한다. server destination Secret은 `tls.crt`, `tls.key`, `ca.crt`를 가져 CNPG의 server/client CA와 server TLS에 사용한다. 외부 client CA에는 private key를 포함하지 않으므로 replication 인증서도 Vault에서 발급해 `replicationTLSSecret`으로 지정한다. workload client Secret도 같은 세 key를 가지며 각 consumer에만 mount한다. API와 web certificate에는 해당 Rollout restart target을 지정한다.

server certificate 요청에는 `<cluster>-rw`, namespace-qualified Service 이름, cluster-local FQDN과 설정된 Tailnet hostname을 SAN으로 포함한다. workload의 `DATABASE_URL`에는 password를 넣지 않고 workload별 DB role을 사용한다.
...
- [workload별 Secret 증가] → role 격리와 회전 책임을 명확히 하는 비용으로 수용한다.

## Migration Plan

1. 이 PR을 PKI 비활성 기본값으로 배포해 기존 manifest와 runtime 동작을 보존한다.
2. 인프라 저장소에서 Vault PKI issuer와 제한된 server/API/web/migration/replication role 및 VSO ACL을 만든다.
3. dev values에 workload별 role, TTL과 Tailnet hostname을 설정하고 Helm render를 확인한다.
4. CNPG server TLS와 client CA가 적용된 뒤 PKI workload를 배포하고 `current_user`, server hostname 검증과 API/web smoke를 확인한다.
