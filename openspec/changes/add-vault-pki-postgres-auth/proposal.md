...
- Helm chart가 선택적으로 Vault PKI client/server 인증서 동기화, CNPG client CA와 `cert` 인증, 검증 가능한 server TLS, workload 인증서 mount와 회전 시 재시작을 구성한다. API와 web/federation은 별도 leaf key를 사용하지만 동일한 `kosmo_runtime` PostgreSQL 로그인을 사용하고 migration만 별도 로그인으로 유지한다. VSO Secret과 CNPG TLS를 먼저 준비하고 다음 sync에서 client 인증을 활성화해 기존 PreSync migration이 미준비 인증서를 참조하지 않게 한다.
