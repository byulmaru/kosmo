...
Helm chart가 선택적으로 같은 Vault PKI issuer의 server 인증서와 workload 및 CNPG replication client 인증서를 동기화하고 CNPG 및 consumer에 연결한다.

**Guardrails**

- 비활성 기본값은 기존 password manifest를 유지한다.
- server certificate는 내부 read-write Service와 설정된 Tailnet hostname을 SAN에 포함한다.
- client CN은 workload의 PostgreSQL role과 일치한다.
...
- 활성 출력에서 server/API/web/migration/replication PKI Secret, CNPG CA·server·replication TLS, 제한된 `pg_hba`, workload별 read-only mount와 restart target을 확인한다.

- [x] 3.1 PKI 비활성 기본값과 server/API/web/migration/replication별 Vault role·DB role·TTL·Tailnet SAN values를 정의한다.
- [x] 3.2 활성화 시 VSO client/server/replication certificate, CNPG TLS·`cert` 인증과 workload별 TLS mount를 render하게 한다.
- [x] 3.3 비활성 호환성과 활성 PKI manifest 계약을 증명하는 Helm render 검증을 추가한다.
...
- [x] 4.1 README와 관련 memory를 구현된 로컬·Kubernetes 인증서 동작 및 외부 선행조건에 맞춘다.
- [x] 4.2 관련 검증을 통과시키고 Linear·OpenSpec·diff 정합성을 재확인한다.
- [x] 4.3 PROD-470 브랜치를 게시하고 한국어 PR에 결정, 검증과 남은 인프라 선행조건을 기록한다.
