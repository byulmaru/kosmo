...
`PROD-783`은 main merge를 기본 production source로 만들면서 dev 자동 배포를 유지하고, main이 아닌 정확한 commit SHA도 예외적으로 승인해 배포할 수 있게 한다. Main source라도 production checkout·Vault/Sentry credential 접근·prod build는 `prod` Environment 승인 뒤로 미루며, automatic과 manual 모두 승인 뒤 하나의 gated job에서 build와 deploy를 연속 실행한다. `PROD-791`은 실제 workload가 GHCR만 사용하는 현재 구조에 맞춰 중복 ECR push·보존 경로를 제거한다.
...
- Production image는 Helm과 Argo CD가 사용하는 GHCR에만 push한다. ECR copy와 mutable `stable` tag를 release 완료 조건이나 rollback source로 사용하지 않는다.
...
3. Automatic과 manual release는 하나의 production deploy job 정의를 공유한다. Main push 경로는 event full SHA를 target으로 선택하고 production source를 checkout하거나 Vault/Sentry credential을 요청하기 전에 `prod` Environment 승인을 기다린다. 승인 뒤 shared gated job이 main full SHA를 checkout하고 prod Vault role·Sentry upload secret을 얻어 image를 GHCR에 build한다.
4. Automatic gated job은 GHCR build output digest를 같은 job의 migration-gated Argo sync에 전달하고, Argo source를 main full SHA로 고정해 revision을 검증한다.
...
6. Manual preflight가 성공하면 같은 shared gated job이 resolved target SHA를 선택한다. 승인 뒤 target SHA를 checkout하고 prod image를 GHCR에 build한 뒤 digest-pinned Argo sync와 revision 검증을 수행한다. `SENTRY_RELEASE`, checkout, Helm version과 Argo revision은 dispatch workflow의 `github.sha`가 아니라 resolved target SHA를 사용한다.
...
8. 외부 Vault prod build role은 automatic과 manual gated build가 사용하는 exact `repo:byulmaru/kosmo:environment:prod` identity만 허용한다. main ref, `refs/heads/production`, tag와 일반 branch identity는 허용하지 않는다. Image push는 GitHub Packages permission과 GHCR만 사용하며 AWS credential을 요청하지 않는다.
...
- GHCR retention을 별도 운영 정책으로 관리할 수 있다. 다만 release workflow가 배포에 사용하지 않는 두 번째 registry를 보존 목적으로 다시 도입해서는 안 된다.
...
- 승인 전에는 production image를 build하지 않으며, Argo가 mutable `main`, `stable` 또는 `production` ref를 다시 해석하게 하지 않는다.
- Workflow trigger만 바꾸고 Vault의 production branch trust, Terraform bootstrap revision과 GitHub Environment policy를 남기지 않는다.
