...
현재 production 배포는 장기 `production` 브랜치에 변경을 다시 전달해야 시작되어 `main` merge와 실제 배포 후보가 분리되고, 긴급한 비-main commit을 명시적으로 승인해 배포할 경로도 없다. `main`의 각 merge를 dev와 production 후보로 자동 준비하되 production 상태 변경은 GitHub Environment 승인으로 제한하고, 예외적으로 정확한 commit SHA를 선택하는 감사 가능한 수동 경로를 제공해야 한다.

## What Changes

- **BREAKING** 장기 `production` 브랜치 PR merge·push 기반 release를 폐기하고 `main` push의 immutable SHA를 기본 production source로 사용한다.
- `main` push에서 기존 dev image와 prod 설정의 production candidate image를 별도로 build하고, dev는 자동 배포하되 production candidate는 `prod` Environment reviewer 승인 뒤에만 배포한다.
- Main production candidate build는 승인 전에 실행하고, 승인 job은 그 build가 만든 digest와 main full SHA를 migration과 모든 production workload에 그대로 전달한다.
...
- `production-release`: 기본 source와 사람 승인 경계를 production PR에서 main candidate의 Environment 승인으로 바꾸고, 승인 뒤 임의 commit SHA를 별도 build·배포하는 수동 경로를 추가한다.
...
- 외부 Vault GitHub Actions role과 GitHub `prod` Environment: main workflow identity, required reviewer와 deployment policy
