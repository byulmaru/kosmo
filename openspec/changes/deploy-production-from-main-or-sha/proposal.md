...
- `main` push에서 기존 dev image는 자동 build·배포하고, production 경로는 `prod` Environment 승인 대기만 만든다. 승인 전에는 production source checkout, Vault/Sentry credential 접근과 prod image build를 실행하지 않는다.
...
- Dev/prod 및 automatic/manual image metadata를 분리하고, production image를 실제 workload source인 GHCR에만 push한다.
...
- Linear Implementations: `PROD-783`, `PROD-790`, `PROD-791`
...
- `apps/terraform/argocd.tf`, `apps/terraform/ecr.tf`: production bootstrap source와 release overlay ownership을 유지하면서 사용하지 않는 ECR repository·OIDC push role·lifecycle 선언을 제거
