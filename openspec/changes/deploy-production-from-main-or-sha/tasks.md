...
- Main automatic production 경로는 승인 전에 production source를 checkout하거나 Vault/ECR/Sentry credential을 요청하거나 prod image를 build하거나 Argo CD production 상태를 변경하지 않는다.
...
- 승인 전 또는 sync 실패 release는 현재 production digest의 `stable` 보존 표식을 이동시키지 않는다.
...
Argo CD, ECR, Vault와 GitHub Environment가 `prod` 승인 뒤 실행되는 automatic main과 approved manual release만 허용하고 production branch source·credential trust를 더 이상 사용하지 않는다.
...
- [ ] 3.2 ECR OIDC trust를 exact `environment:prod` identity만 허용하도록 제한하고 main ref·production branch·tag·일반 branch identity를 제거·검증한다.
...
- [ ] 5.3 Repository PR merge와 필요한 ECR/Vault/GitHub 설정 apply 뒤 첫 main release를 Environment 승인하고 gated build·배포해 SHA·digest·migration·workload·smoke 결과를 기록한다.
