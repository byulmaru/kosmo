...
PROD-645와 PROD-648은 각각 Account 외부 설정과 Profile 기본 공개 범위 기능을 소유한다. PROD-653은 두
기능을 재구현하지 않고 canonical `/settings`, 공통 section 구조, shell navigation과 페이지 통합을 소유한다.
...
- Account와 Profile 설정의 heading, 현재 대상, empty/loading/error 상태를 공통 구조로 제공한다.
- child feature가 자기 데이터·상태·오류 경계를 유지하면서 같은 page에 결합될 수 있는 좁은 통합 경계를 둔다.
...
- `UniversalShell`의 query는 shell chrome에 필요한 session·selected Profile 최소 데이터만 소유한다. settings
  page의 표시 이름, `relativeHandle`과 child fragment는 route query가 선언해야 하며 shell query 결과를 route
  전용 scalar prop으로 확장하지 않는다.
...
- mobile Web의 header는 `getWebMobileShellHeader`가 소유하지만 Native에서는 route가 header를 소유한다.
  ...
  생긴다.
- 공용 `RouteBoundary`는 route 전체 Suspense/error fallback에 적합하다. Account와 Profile child가 독립적으로
  실패할 수 있으면 가장 가까운 child boundary가 자기 section을 복구해야 하며 route boundary 하나로 두
  section을 함께 숨기지 않는다.
...
1. settings route가 current session과 selected Profile identity를 읽는 top-level Relay query를 소유하고,
   ...
   `ShellChromeContext`의 기존 Profile switcher action을 재사용한다.
2. page view는 `계정 설정`, `프로필 설정` section과 공통 spacing·heading·state 배치만 소유한다. child feature는
   각 section 내부에서 자기 fragment, mutation, pending과 오류 복구를 소유하며 공통 shell이 저장 상태를
   합치지 않는다.
...
4. route와 두 section의 통합 가능한 content가 준비된 commit에서 `SidebarNavigation`의 주요 목록에
...
- child feature는 route query에 fragment를 colocate하거나 자기 Suspense boundary 안에서 독립 query를 소유할
  수 있다. 어느 방식이든 Relay actor 전환 뒤 이전 Profile 결과를 새 identity 아래에 표시하지 않고 section
  오류 격리를 유지해야 한다.
...
- [PROD-645·PROD-648의 통합 surface가 늦어져 settings navigation 노출이 지연될 수 있음] → 공통 shell 작업은
  ...
  때까지 pending으로 유지한다.
- [route query와 child query의 Suspense 조합이 page 전체를 불필요하게 가릴 수 있음] → 공통 identity에 필요한
  최소 query만 route boundary에 두고 독립 child failure/loading은 section 가까이에서 처리한다.
...
1. canonical 디자인과 OpenSpec Gate를 승인하고 PROD-645·PROD-648의 통합 가능한 component/API 경계를
   확인한다.
2. 공통 settings page shell과 상태 catalog를 구현하되 준비되지 않은 navigation은 노출하지 않는다.
3. 두 child feature를 각 section에 연결하고 같은 commit 범위에서 `/settings` route, sidebar·rail·drawer
   navigation과 mobile Web header 분류를 활성화한다.
