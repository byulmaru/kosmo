...
홈 타임라인은 선택 프로필의 글과 `ACCEPTED` followee의 글을 함께 보여줘야 한다. accepted follower의 `FOLLOWERS` 접근 정책은 선행 change(`extend-followers-post-access`)가 공통 `Post` 접근 정책으로 제공하므로, 이 change는 해당 정책을 재사용하는 `Query.homeTimeline` 추가에 집중한다.
...
- 공통 게시글 visibility 정책을 재사용해 followee의 노출 가능한 글만 반환한다.
- 첫 페이지 조회에 필요한 cursor connection shape를 제공한다.

**Non-Goals:**

- Redis fanout, 타임라인 저장 테이블, 운영용 백필을 만들지 않는다.
- accepted follower의 `FOLLOWERS` 접근 정책은 선행 PR에서 처리한다.
...
- 후보 글에는 `postVisibilityAccessWhere`를 적용한다. 이 helper가 `PUBLIC`, `UNLISTED`, 내 글, accepted follower의 `FOLLOWERS` 글을 일관되게 처리한다.
...
- [Risk] 선행 접근 정책 PR이 없으면 followee의 `FOLLOWERS` 글이 후보에는 포함돼도 visibility helper에서 제외될 수 있다. → 이 PR은 `prod-147-post-access` 위에 stacked PR로 둔다.
