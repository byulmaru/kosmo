...
Argo CD가 dev와 격리된 `kosmo-prod` Application을 자동 동기화하고, 유효한 immutable release image가 지정되면 application workloads를 함께 render한다. 총 3 instances의 PostgreSQL, backup, runtime Vault 환경값과 CNPG-managed migration owner credential 기반이 Ready가 되어 PROD-545가 PROD-546 live restore rehearsal을 시작할 수 있다.
...
- Production runtime 환경값은 기존 환경별 Vault projection에 따라 `kubernetes/kosmo/prod`에서 동기화하며 `/prod/runtime` path를 만들지 않는다. 어떤 secret value도 repository, Terraform input/state 또는 rendered manifest에 넣지 않는다.
- PROD-712 이후 migration은 CNPG-generated owner credential을 사용하며 별도 `kosmo_migration` login/Secret을 provision하지 않는다. Active API/Web/Worker/Fedify workload는 `kosmo_runtime`을 유지하고 owner Secret을 참조하지 않는다.
...
- PROD-562의 historical migration identity/credential provisioning은 PROD-712가 supersede한다. Production migration Job의 owner 직접 연결, 실행 순서와 contract gate 및 public 사용자 journey smoke는 각 후속 change의 책임을 유지한다.

**Verification**

- Dev/prod Helm lint/render와 admission dry-run으로 environment, hostname, 명시적인 image version, runtime Vault path, prod 3 instances, backup 연결, active workload credential 격리와 migration owner Job/read-routing 경계를 검증한다.
- Production manifest의 server-side admission dry-run과 Terraform saved plan을 검토한다.
- 필수 add-on, public Gateway/TLS, production runtime Vault path와 Pod Identity 준비 상태를 확인한 뒤 Application automated sync 상태를 확인한다.
- CloudNativePG 3 Pods/PVC, runtime VSO projection, generated owner Secret의 존재와 migration-only 소비, API/Web Rollout·Service와 HTTPRoute Accepted/ResolvedRefs/readiness를 확인하고 active workload database credential이 `kosmo_runtime` 경계에 유지됨을 기록한다. Secret value는 읽거나 기록하지 않는다.
...
- [x] 1.2 Historical production migration basic-auth Secret과 `kosmo_migration` `DatabaseRole`을 runtime credential과 격리했다. 이 경계는 PROD-712가 owner 직접 연결로 대체하고 제거한다.
...
- [ ] 1.10 PostgreSQL 3 Pods/PVC, runtime projection, CNPG-generated owner Secret의 migration-only 소비와 obsolete migration role/Secret 부재를 확인하고 민감 정보 없이 PROD-545에 증거를 기록한다.
