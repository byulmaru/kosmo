...
`RouteBoundary`에 도달하며, 성공 뒤 `store-and-network` 재조회가 실패하면 Relay cache 목록을 유지하고 오류
fallback으로 전환하지 않는다. 따라서 오류 presentation을 일괄 교체하면 PROD-939에서 제외한 새로고침 동작까지
바뀐다.
...
- 성공 이력이 없는 Local 오류만 Web 빈 영역·Native 2행 skeleton과 persistent Toast로 표시한다.
- 기존 RouteBoundary 재시도와 공용 Toast lifecycle을 재사용한다.
- 재시도 중복 입력과 route·actor 전환 뒤 남는 Toast를 막는다.

**Non-Goals:**

- 새로고침 목록 보존·부분 GraphQL 응답·pagination·cursor·filtering 변경
...
cache 결과도 route에서 렌더되면 성공으로 기록한다. 성공 전 네트워크 오류에는 route 전용 fallback을 렌더하고,
성공 뒤 `store-and-network` 재조회 네트워크 오류에는 Relay cache 목록을 유지하며 `RouteBoundary` 오류
fallback이나 별도 Toast를 표시하지 않는다. 최초 오류 fallback은 Web에서 빈 영역을, Android/iOS Native에서
기존 `Skeleton` primitive로 Figma Android baseline의 두 행을 렌더하고, mount effect에서 공용 persistent
Danger Toast를 연다. effect cleanup으로 재시도 성공, route 이탈과 actor remount 때 해당 Toast만 닫는다.

재시도는 기존 `resetErrorBoundary`/`fetchKey`를 그대로 사용한다. 공용 Toast action은 현재 active Toast일
때만 닫힘과 callback을 한 번 실행하도록 좁게 보강해 exit motion 중 연속 입력도 중복 요청을 만들지 않게 한다.
...
- Local의 모든 오류를 새 fallback으로 바꾸면 새로고침 오류 계약까지 변경된다.
...
3. Storybook·단위·Web E2E와 Web 시각 검증을 실행하고 디자인 문서를 Current로 정렬한다.
4. 회귀 시 Local custom fallback과 Toast guard를 되돌리면 기존 인라인 StateView 경로로 복원된다.
