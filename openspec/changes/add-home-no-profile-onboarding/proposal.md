...
- 홈(`(tabs)/+page.svelte`)을 선택 프로필 유무로 분기한다. `TestQuery`를 `HomePageQuery`(`currentSession.selectedProfile`, `me.profiles`)로 교체한다.
...
- 의존: 기존 `(tabs)` 셸과 사이드바 내비게이션(PROD-112/사이드바 흐름) 위에 얹힌다. PROD-140(로그인 시 기존 프로필 자동 선택)과 hard block 관계가 아니며, PROD-140 머지 후에는 신규(프로필 0개) 사용자 위주로 노출된다.
