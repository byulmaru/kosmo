...
작성 기준 main에서는 source commit과 Notification projection이 분리되어 있지만 실제 Mute·Block 생성 판정은 없다. PROD-813의 Done과 PROD-814·822의 In Review 상태를 기록하되, 이슈 상태만으로 capability의 main 반영을 가정하지 않는다. Spec Gate와 선행 구현 완료 조건은 별도로 확인한다.
...
- Mute·Block capability 자체, 기간 Mute, Domain Block, Quote·Mention 생성, UI·GraphQL shape 변경.
...
| Mute / Block capability | 작성 기준 main의 `profile-mute.ts`는 mutation을 제공하며 재사용 가능한 Mute·Block 공통 read predicate는 조사 범위에서 확인되지 않았다. 최종 PROD-814·822 반영 뒤 실제 경계를 다시 읽는다. 존재하지 않는 helper 이름을 선행 계약처럼 고정하지 않는다. |
...
Mute는 Recipient→Related 방향의 영구 관계를 확인하고, Block은 두 방향 중 하나라도 존재하는지 확인한다. 최신 Profile·콘텐츠 직접 조회 policy는 이 pair 판정을 대신하지 않는다. 현재 transaction 안에서 projection을 처리하는 source는 그 경계를 유지하고, source action transaction이나 transport context를 새로 넘기지 않는다.
...
- 정책 오류를 allow로 처리하면 fail-open이고, 정상 deny로 삼키면 기존 failure/retry 계약을 바꾼다.
