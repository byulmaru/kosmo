...
- local/remote request가 동일한 pending-only profile-follow action을 사용한다.
...
- [Risk] UI가 incoming approve/reject 전체 관리 화면까지 확장될 수 있다. → 이번 사용자 흐름은 기존
  FollowButton의 outgoing request 생성/취소와 API의 자기 Profile request connection/processing 계약까지만
  제공하고, Notification 화면은 소유 이슈에 남긴다.

## Migration Plan

1. canonical Follow Request/Relationship/Notification 문장에서 terminal request 참조를 제거한다.
2. core profile-follow action에 transaction-aware lifecycle과 integration port interface를 추가한다.
3. API에 request ref/loader, 자기 Profile의 connection과 follow/approve/reject/cancel mutation을 연결한다.
...
저장 schema는 이미 pending-only이므로 migration은 없다. rollback은 API/UI/action 변경을 제거하고
