...
- [x] 3.1 current PostContent Media를 검증해 ActivityPub Image에 필요한 공개 표현으로 projection한다.
- [x] 3.2 Media node를 제외한 HTML과 ordered attachment/sensitive를 Local Note 역참조 및 최초 delivery에 연결한다.
- [x] 3.3 federation projection·권한·회귀 검증과 관련 Fedify check를 통과시킨다.
...
- [x] 4.1 세 구현 자식 결과를 실제 direct upload, 새 Post 작성과 Local Note 흐름으로 통합 검증한다.
- [x] 4.2 canonical·Linear·OpenSpec과 GraphQL/Relay schema를 재대조하고 전체 정적·테스트 검증을 통과시킨다.
- [x] 4.3 세 구현 자식과 부모 완료 조건을 확인하고 delta spec을 동기화한 뒤 change를 archive한다.
