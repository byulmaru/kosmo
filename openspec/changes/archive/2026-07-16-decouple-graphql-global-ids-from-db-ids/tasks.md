...
신규 도메인 row는 PostgreSQL 18.4 `uuidv7()` default ID를 사용하고 기존 UUIDv8 row는 data rewrite 없이 계속 유효하다.

**Guardrails**

- 기존 primary key와 foreign key 값을 삭제, backfill 또는 재작성하지 않는다.
- PostgreSQL `uuid` column type, primary key와 foreign key constraint를 변경하지 않는다.
- ID column default 외의 schema나 기존 ID 값을 변경하지 않는다.
- 같은 millisecond 안의 단조 생성 순서를 보장하지 않는다.

**Verification**

- PostgreSQL 18.4에서 신규 ID의 UUID version과 timestamp를 DB integration test로 검증한다.
- migration이 각 ID column의 default만 바꾸고 기존 UUIDv8 값을 유지하는지 확인한다.
- repository에 애플리케이션 `createId`나 table discriminator 의존성이 남지 않았는지 확인한다.

- [x] 1.1 PostgreSQL 18.4 `uuidv7()` column default와 default-only migration, 기존 UUIDv8 보존 검증을 구현한다.
- [x] 1.2 모든 신규 DB ID 생성 경로에서 애플리케이션 generator와 table discriminator 의존성을 제거하고 기존 ID data rewrite가 없음을 확인한다.
...
- 클라이언트는 global ID에 `encodeURIComponent` 같은 추가 escaping을 적용하지 않고 route와 GraphQL variable에 그대로 전달한다.
- 현재 메모리 기반 Relay store를 위해 persisted cache migration을 추가하지 않는다.

**Verification**

- Node ID가 URL-safe opaque string으로 route와 mutation variable에 동일하게 전달되는지 점검한다.
- GraphQL schema generation, Relay artifact generation과 client typecheck를 통과시킨다.

- [x] 3.1 unpadded base64url codec과 client ID 사용 경로를 구현하고 route·schema·Relay artifact를 갱신한다.
