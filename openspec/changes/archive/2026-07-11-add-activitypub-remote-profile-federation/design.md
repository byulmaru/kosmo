...
- **remote actor는 `Profile`로 materialize한다.** 별도 `RemoteProfile` GraphQL 타입을 만들지 않는다. local/remote 차이는 `Profile.instance.kind`, 소속 `instance`, `activitypub_actor` metadata, resolver 정책으로 표현한다.
- **소속 instance kind를 `Profile.instance.kind`로 노출한다.** `relativeHandle`은 표시 문자열이지 instance 종류 판별 API가 아니다. 클라이언트가 local/ActivityPub 여부를 알아야 하는 경우 `Profile.instance.kind`를 사용한다. `InstanceKind` 값은 `LOCAL`, `ACTIVITYPUB`로 유지한다.
- **`profileByHandle`은 DB-only query로 유지한다.** bare handle 또는 configured local domain의 `handle@domain`/`@handle@domain`은 configured local instance에서 조회하고, configured local domain이 아닌 `handle@domain`, `@handle@domain`, 또는 동등한 federated handle은 저장된 remote `Profile`을 `(instance.domain, profile.normalizedHandle)` 기준으로 조회한다. 이 query는 WebFinger, actor document fetch, refresh, DB materialization을 수행하지 않는다.
...
- **Fedify의 protocol primitive를 재사용한다.** remote actor lookup, JSON-LD dereferencing, ActivityPub object parsing, HTTP safety는 Fedify API와 설정을 사용한다. kosmo는 Fedify가 제공하는 현재 lookup/dereference 동작을 따르고, Fedify가 반환한 typed actor를 `Profile`로 투영하며 actor URI uniqueness, instance state, refresh timestamp 같은 제품 정책만 구현한다.
...
- **remote instance를 먼저 보장한다.** remote profile materialization은 federated handle의 normalized domain으로 ActivityPub `instance` row를 find-or-create한 뒤 진행한다. 기존 instance가 `SUSPENDED` 또는 `UNRESPONSIVE`이면 outbound lookup이 필요한 새 `Profile` 생성이나 기존 profile refresh를 진행하지 않고 실패 처리한다.
...
- **actor는 canonical URI domain에 저장한다.** WebFinger 요청 domain과 actor `id`의 normalized host가 다르면 actor `id` host의 ActivityPub instance에 `Profile`을 저장하거나 기존 alias-stored profile을 이동한다. 요청 domain은 lookup 입력일 뿐 별도 DB alias를 만들지 않으므로 DB-only handle 조회는 canonical actor domain을 사용한다.
- **remote actor 필드는 기존 profile 필드에 투영한다.** actor `preferredUsername`은 기존 `Profile.handle` 스키마를 만족해야 하며, 통과한 값만 `Profile.handle`과 `Profile.normalizedHandle`로 저장한다. actor `name`은 기존 `Profile.displayName` 스키마를 만족할 때만 `Profile.displayName`으로 저장하고, 없거나 지원하지 않는 값이면 handle을 표시 이름으로 사용한다. actor `summary`는 `Profile.bio`, actor `published`는 `Profile.createdAt`에 저장한다. actor `published`가 없을 때 현재 시각을 `createdAt`으로 쓰는 fallback은 최초 저장 시에만 적용하고, refresh에서는 기존 `createdAt`을 보존한다. `relativeHandle`은 DB에 저장하지 않고 configured local instance 기준으로 계산한다.
...
2. `Profile.instance.kind`를 GraphQL에 추가하고 소속 instance의 `InstanceKind`를 그대로 반환한다.
...
Rollback은 새 GraphQL 필드와 remote actor materialization 경로를 닫은 뒤, 새 metadata 컬럼을 되돌린다. 이미 remote actor/profile row가 생긴 뒤에는 local user data와 섞이지 않도록 `instance.kind = ACTIVITYPUB` 기준으로 정리해야 한다.
