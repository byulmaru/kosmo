...
`createPost`가 성공해도 이미 열어 둔 Home Post List의 Relay connection에는 새 Post가 반영되지 않아 사용자가 새로고침 전까지 결과를 확인하기 어렵다. 호출자 Store에서 normalized Post를 즉시 반영해 작성 성공과 현재 목록을 일치시킨다.

## What Changes

- `createPost`의 기존 `CreatePostPayload.post` 선택을 유지하고 별도 edge 공개 계약은 추가하지 않는다.
- Home Post List를 안정적인 Relay managed connection으로 식별한다.
- mutation이 Relay `@prependNode`에 Home managed connection ID를 전달해 normalized `post`를 요청 actor Environment의 이미 로드된 connection에 최신순으로 한 번 반영한다.
- Relay 기본 handler를 사용해 connection을 합성하거나 Post List 정책·cursor를 재구현하지 않고, 같은 Post Node의 edge를 중복 삽입하지 않는다.
...
- Universal client: `PostComposer` mutation, Home `@connection` identity, Relay `@prependNode`와 관련 테스트
