...
- 이전 application 재배포도 이전 commit에 새 tag를 붙이는 같은 경로를 사용한다.
...
GitHub Release publish/resolve job과 script, 별도 deploy workflow는 삭제한다. 실패 뒤 application을 되돌려야 하면 현재 DB와 호환되는 이전 commit에 새 tag를 붙여 동일한 build·approval·sync 경로를 실행한다. 이는 DB rollback이 아니다.
...
- 이전 commit 재배포가 현재 DB와 호환되는지는 해당 schema migration release가 판단한다.

## Risks / Trade-offs

- [Tag 이름에 제한이 없음] → 사람이 읽는 tag는 GitHub audit ref로만 사용하고 manifest label은 commit short SHA, container identity는 digest로 분리한다.
- [이전 commit 재배포는 기존 image 재사용이 아니라 새 build임] → 단순한 단일 경로를 우선하며 각 실행이 만든 digest를 그대로 승인·배포·감사한다.
