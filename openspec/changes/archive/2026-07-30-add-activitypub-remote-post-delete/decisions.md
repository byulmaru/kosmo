...
- Context / Problem: object URI 또는 instance origin만 비교하면 같은 instance의 다른 actor나 잘못 연결된 mapping이
  타인의 Post를 변경할 수 있다.
- Decision Outcome: stored ActivityPub Actor, ACTIVITYPUB Instance Type, exact mapping URI, Current Content가 있는
  Note 구조, Post Author Profile과 actor URI가 모두 일치해야 한다. 검증된 삭제는 Profile 또는 Instance의 현재
  가용 상태와 무관하게 반영하며 Local Post는 어떤 fallback identity로도 선택하지 않는다.
...
- Consequences: unknown/non-ActivityPub actor, missing mapping, 다른 Author/object, Content 없는 Repost의 Announce
  mapping과 Local Post는 write 없이 종료한다. unavailable remote Author의 검증된 Delete는 Tombstone으로
  전이하며 Repost 취소는 기존 `Undo(Announce)` lifecycle에 남는다.
