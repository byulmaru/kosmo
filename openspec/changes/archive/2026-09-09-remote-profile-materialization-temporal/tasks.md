...
- Latest completion evidence (2026-09-11): Standalone Worker execution with its own `TestWorkflowEnvironment` and no test-client preload passed 16/16 with cleanup complete; API profile passed 73/73, Fedify remote actor passed 39/39, Core client passed 10/10, and Core unit passed 74/74. Worker build, API/Fedify typechecks, ESLint, Prettier, `git diff --check`, active OpenSpec strict (78/78), and archived proposal/delta Validator (issues 0) also passed with dedicated PostgreSQL/Temporal environments cleaned up; caller timeout continuation and Worker restart recovery remain delegated to Temporal native semantics.
