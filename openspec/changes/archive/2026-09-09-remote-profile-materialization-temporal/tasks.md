...
- 각 Workflow는 자기 input에서 ID를 만드는 규칙을 한 곳에 정의한다. 공용 `runWorkflow`는 `workflowIdFromArgs: (...args: Parameters<T>) => string` callback에 native args를 한 번 전달해 해당 규칙을 적용하고 KOSMO task queue와 5초 bounded deadline을 붙여 native `start` 또는 `execute`만 호출하며, native result·start 반환값·error와 conflict·reuse policy를 그대로 전달하고 domain 오류 정책은 호출부에 남긴다. 이 change에서는 remote materialization Workflow만 기존 ID 규칙을 공용 wrapper에 적용하며 다른 domain ID와 UWS는 일괄 마이그레이션하지 않는다.
...
- [x] 1.3 JSON-safe actorUri/profileId 입력·Profile ID 결과와, remote materialization Workflow의 기존 input-to-ID 규칙을 공용 `runWorkflow`가 적용하는 stable Workflow identity, in-flight reuse와 완료 후 재시도 semantics를 적용한다.
