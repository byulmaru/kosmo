...
- 공용 `runWorkflow`는 KOSMO task queue와 5초 bounded deadline, `${workflowName}:${JSON.stringify(identityKeys)}` ID composition만 공유하고, conflict·reuse policy는 caller가 native options로 선택한다. 이 change에서는 remote materialization Workflow만 새 composition을 사용하며 다른 domain ID와 UWS는 마이그레이션하지 않는다.
...
- [x] 1.3 JSON-safe actorUri/profileId 입력·Profile ID 결과와, 공용 `runWorkflow`가 Workflow name 및 caller identity keys로 `${workflowName}:${JSON.stringify(identityKeys)}` ID를 조합하는 stable Workflow identity, in-flight reuse와 완료 후 재시도 semantics를 적용한다.
