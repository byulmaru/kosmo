...
- Worker에 기존 `WorkflowDefinition<T>`와 native child lifecycle을 재사용하는 Workflow-safe `runChildWorkflow<T>` primitive를 추가해 child `start`/`execute` 경계를 공통화한다. ID·args·result·handle과 native child options/error/queue inheritance를 보존하고, helper가 parent lifetime 정책을 자동으로 정하지 않게 한다.
