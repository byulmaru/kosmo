...
PROD-728의 CloudNativePG Pooler는 dev/prod에 session mode와 `DISCARD ALL`로 배포돼 있으며 direct Service와 공존한다. PROD-370은 actor setting key와 fail-closed read helper를 제공한다. API `DATABASE_URL`은 request authentication과 startup/bootstrap을 위한 현재 owner-compatible fallback으로 direct Service를 유지하며, 향후 GraphQL principal 방향은 결정하지 않는다. GraphQL operation connection만 `OPERATION_DATABASE_URL`을 통해 Pooler로 보내야 한다. `postgres.credentials.api` trio가 구성된 경우에도 rendered env의 username, database와 password Secret source, scheme, path와 query는 현재 전환에서 유지하고 operation URL의 host와 port를 포함한 authority만 in-chart Pooler Service `<release>-postgres-pooler-rw:5432`로 교체한다. Runtime operation client는 configured URL을 변경 없이 postgres.js에 전달하고 query parameter를 변경하거나 호환되지 않는 URL을 자동 보정하지 않는다. 따라서 configured URL은 이미 Pooler와 호환되어야 한다. PROD-715 Web/Worker 기본 principal은 별도 실행 경계로 이 change에서 바꾸지 않으며 `OPERATION_DATABASE_URL`에 공급하지 않는다. Migration direct endpoint는 유지한다. GraphQL principal transition은 PROD-716이 소유하고, 취소된 client-certificate/direct-rw 대안 PROD-470은 재개하지 않는다.

Merge revision `de6034d3`의 dev activation에서는 operation client가 API direct DB client의 `connection` startup options를 상속해 PgBouncer가 지원하지 않는 옵션을 전달했고, operation 초기화가 거부되어 GraphQL Query/Mutation이 HTTP 500으로 실패했다. Direct `DATABASE_URL` client의 기존 timeout startup 동작은 이 change의 범위 밖으로 두고 유지한다. Fix는 operation client 생성 시 direct `connection` options를 전달하지 않는 것이며, configured `OPERATION_DATABASE_URL`은 변경 없이 사용한다. Runtime은 호환되지 않는 URL을 자동 보정하지 않는다. Actor GUC만 session-level initialization SQL 한 번에 설정해야 한다. 이 incident에 대한 사용자 결정은 전체 activation revert가 아닌 forward fix이며 endpoint, credential/Secret selector, Pooler CR·replica·resource·capacity는 변경하지 않는다.
...
- API `DATABASE_URL`은 현재 owner-compatible direct fallback을 유지하고 `OPERATION_DATABASE_URL`만 Pooler endpoint로 전환하며 migration direct endpoint는 이 change에서 유지한다. PROD-715 Web/Worker 기본 principal은 별도 실행 경계로 제외하고 operation URL에 연결하지 않는다.
...
- direct postgres.js client의 기존 server timeout startup 동작은 이 change의 범위 밖으로 두고 유지한다. Pooler operation client는 direct DB client의 `connection` startup options를 상속하지 않고 configured `OPERATION_DATABASE_URL`을 변경 없이 사용해야 하며, actor GUC만 initialization SQL round trip에서 session-level로 설정해야 하고 성공 전에는 resolver를 실행하지 않는다. 호환되지 않는 configured URL은 runtime이 자동 보정하지 않는다.
...
- API `DATABASE_URL`은 current owner-compatible direct fallback helper로 보존하고 GraphQL operation 전용 `OPERATION_DATABASE_URL`만 Pooler helper를 사용하도록 endpoint 표현을 분리해야 한다. PROD-715 Web/Worker 기본 principal은 이 change에서 소비하거나 operation URL에 연결하지 않는다.
...
- Helm에 API current fallback direct `DATABASE_URL`과 GraphQL operation 전용 Pooler `OPERATION_DATABASE_URL` helper를 추가하고 API Rollout만 operation URL을 사용한다. 기존 API-role Secret 참조와 trio의 username/database/password source는 현재 전환에서 유지하며, configured trio에서도 rendered operation URL의 host와 port를 포함한 authority만 in-chart Pooler Service `<release>-postgres-pooler-rw:5432`로 교체하고 scheme, path와 query를 보존한다. Runtime operation client는 configured URL을 변경 없이 전달하며 호환되지 않는 query parameter나 URL을 자동 보정하지 않는다. Migration URL helper는 direct Service를 계속 가리킨다. PROD-715 Web/Worker 기본 principal은 별도 실행 경계로 이 change에서 소비하지 않으며 operation URL에 연결하지 않는다. 새 credential selector는 만들지 않는다. PROD-716은 GraphQL principal credential source/cutover를 후속으로 소유하고, Role/Secret provisioning, grant와 RLS policy는 각각의 별도 issue 경계로 남긴다.
...
- Direct DB client의 `connection` startup options를 operation client factory에 그대로 전달하면 frontend connection 자체가 초기화 단계에서 거부되어 모든 GraphQL operation이 HTTP 500으로 실패한다. Operation client는 해당 options를 상속하지 않아야 하며, configured URL은 변경 없이 사용하므로 호환되지 않는 URL을 지원한다고 가정하지 않는다.
...
- [direct DB `connection` startup options 상속으로 dev GraphQL 초기화가 실패함] → operation client에 direct `connection` options를 전달하지 않고 configured URL을 변경 없이 사용하며 actor GUC만 한 session SQL round trip으로 설정한다. current API Pod 로그의 unsupported startup-parameter 오류가 없고 기존 GraphQL smoke가 성공할 때까지 live gate를 닫지 않는다.
...
3. Helm render에서 API `DATABASE_URL`은 current owner-compatible direct fallback, `OPERATION_DATABASE_URL`만 Pooler, migration은 direct URL이며 모든 해당 Secret 참조가 기존 값임을 확인한다. PROD-715 Web/Worker 기본 principal은 이 render/operation 검증에서 소비하지 않는 별도 경계로 기록한다.
...
8. 실패 시 application credential이나 out-of-band Kubernetes resource를 변경하지 않고 전체 activation merge/squash revision을 Git revert해 pre-activation tree를 배포한다. API `DATABASE_URL`은 current owner-compatible direct fallback을 유지하고 `OPERATION_DATABASE_URL` env와 operation plugin/code는 제거한다. PROD-715 Web/Worker 경계, migration, Pooler와 Cluster는 유지하며 Pooler manifest를 prune하지 않는다.
9. PROD-715 Web/Worker 기본 principal, PROD-716 GraphQL principal credential source/cutover와 별도 Role/Secret provisioning, grant, RLS policy issue는 이 change의 live gate나 rollback에 포함하지 않는다.
