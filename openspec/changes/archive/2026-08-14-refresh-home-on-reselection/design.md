...
Home route handler는 Web에서 activation마다 document scroll을 최상단으로 이동하고 기존 visible Home query의 `fetchKey`를 증가시킨다. Home query reader와 data renderer를 분리하고, reader가 `useLazyLoadQuery(HomePageQuery, {}, { fetchKey, fetchPolicy: 'store-and-network' })`에서 반환한 데이터를 actor revision과 함께 Home route ref에 동기적으로 기록한다. reader를 감싼 Home-local error boundary는 같은 actor의 마지막 성공 data를 stale timeline으로 렌더하고, stale data가 없거나 actor revision이 다르면 local blocking error와 retry를 렌더한다. local blocking fallback이 소비한 오류는 기존 unexpected-error reporter와 console에 한 번 기록한다. 수동 retry는 visible `fetchKey`를 한 번 증가시키고, current-home 재선택의 오류 reset은 기존 `resetKeys`가 처리한다. actor Relay environment revision이 바뀌면 Home-local boundary만 revision key로 remount해 새 actor query를 시작한다. 정상 current-home 재선택에서는 성공한 PostList를 remount하지 않는다. outer `RouteBoundary`는 inner fallback 자체에서 다시 발생하거나 inner boundary 밖에서 전파된 오류의 최종 fallback을 맡는다. Home screen이 유지되는 동안 현재 environment에 해당 operation을 `retain`하고 cleanup하며, 재검증 전용 `fetchQuery` subscription·token lock은 두지 않는다. Relay의 동일 environment·operation·variables in-flight dedupe가 진행 중 중복 요청을 억제한다. actor environment 전환으로 이전 Store와 새 Store가 분리되고 stale ref도 revision으로 격리되지만 일반 query의 실제 network cancellation은 보장하지 않는다.
