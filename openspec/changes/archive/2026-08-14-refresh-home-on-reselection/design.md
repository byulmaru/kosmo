...
- Home의 `fetchKey` 증가는 query를 다시 실행할 수 있지만 완료·실패 시점을 알 수 없어 정확한 in-flight 잠금 해제에 사용할 수 없다.
...
Home route handler는 Web에서 activation마다 document scroll을 최상단으로 이동한다. Home 재선택 요청이 진행 중이면 scroll 뒤 종료한다. 진행 중이 아니면 현재 actor의 Relay environment에서 `HomePageQuery`를 `network-only`로 한 번 실행하고 observable의 `complete`와 `error`에서 모두 잠금을 해제한다. environment 전환 또는 unmount에서는 subscription을 dispose하고 해당 environment의 잠금을 정리한다. 새로고침 오류는 기존 Relay store 데이터를 제거하거나 RouteBoundary 오류 상태로 바꾸지 않는다.
