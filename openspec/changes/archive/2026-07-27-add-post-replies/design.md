...
- `PostThreadLayout`의 현재 소유권은 `docs/design/post-thread.md`를 따른다. 이 archived change가 확정한 item, role, supplied 순서와 direct connector metadata 소유권은 유지되며, 2026-07-31 PROD-593에서 row boundary presentation이 canonical design에 추가됐다. fixture caller는 `renderPost` 안에서 local state를 close over하여 mock 선택 action을 붙일 수 있다. 기존 `PostListItem`·`PostLayout` renderer는 자신의 Profile/Post Link를 그대로 유지하며 presentation이 이를 다른 `Pressable`로 감싸지 않는다.
