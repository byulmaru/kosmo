...
- PROD-422는 조상·현재·하위 Reply 조합, Reply+Quote Source sibling과 Source 부재, unavailable 조상 경계, 각 Post 상세 이동과 fragment·route integration을 client test로 검증한다. descendant pagination은 20개 초기 page, Web document near-end append, 요청 중 중복 방지, 짧은 content auto-fill, page 종료와 부분 실패 후 기존 항목 유지·retry를 검증하고 Native `ScrollView` event가 같은 판정을 사용하는 최소 단위 test를 포함한다. PR #358에서 승인된 웹 우선 출시 gate에 따라 iOS/Android 실기기 검증은 이 change의 완료 증거로 주장하지 않으며, 저장소가 보장하지 않는 20개 초과 Native seed도 새로 추가하지 않는다.
...
- [x] 6.6 Reply+Quote Source 존재·부재, 권한 경계, fragment integration, Web document 다음 page append·중복 방지·종료·auto-fill·부분 실패 retry와 Native `ScrollView` 공통 판정 test 및 관련 client check를 통과시킨다. iOS/Android 실기기 확인은 PR #358의 승인된 웹 우선 출시 gate에 따라 완료 증거에서 제외한다.
