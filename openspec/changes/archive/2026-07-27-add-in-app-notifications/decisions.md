...
- Confirmation / Follow-up: `PROD-324`가 `web-app-shell` delta, shell/Storybook/Relay cache test와 platform smoke를 소유한다. `PROD-372`는 Read 결과의 `recipientProfile.unreadNotificationCount` cache 반영을 소유하고 계약 부모 `PROD-271`이 최종 Follow→badge→Read 수렴을 Web E2E로 검증한다.
