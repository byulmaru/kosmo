...
- PROD-645의 Account 외부 진입점과 PROD-667의 Profile 내부 설정을 root/detail 구조에 배치한 뒤 자동화된
  Web 페이지 수준 통합을 검증한다. 실제 Web 보조기술과 Android·iOS runtime QA는 PROD-727에서 후속 검증한다.
- PROD-645의 canonical URL·플랫폼별 외부 이동과 PROD-667의 Profile 데이터·저장 계약은 재정의하지 않는다.
  ...
  Settings 통합과 이 change의 완료·archive 판단을 소유한다. PROD-727은 실제 Web 보조기술과 Android·iOS
  runtime QA를 후속 소유하며 이 change의 sync/archive를 차단하지 않는다. PROD-653은 완료된 정보 구조 선행 이슈다.
...
- Linear Implementations: `PROD-685`가 route·공통 shell·navigation·PROD-645/667 조립과 page-level 검증을
  ...
  `PROD-727`은 실제 Web 보조기술과 Android·iOS runtime QA를 비차단 후속 이슈로 소유한다.
