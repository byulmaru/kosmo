...
Account와 선택된 Profile actor context를 PostgreSQL setting에서 안전하게 읽는 공통 helper contract를 정의한다. 이 capability는 setting key와 read-only UUID 해석만 소유하며, setting 공급·connection lifecycle과 실제 policy 적용은 downstream이 소유한다.
