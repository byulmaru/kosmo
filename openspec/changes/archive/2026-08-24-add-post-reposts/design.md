...
- 순수 Repost의 presentation·직접 상세 URL과 Action Bar target은 direct Source를 사용하고, Quote는 자체
  canonical detail과 Source preview navigation을 분리한다.
- 실제 Post surface의 마지막 sibling에 Action Bar를 배치하고 Repost menu·생성·취소·실패 toast를 제공한다.
...
- Notification outbox/backfill·범용 cleanup worker, Repost 외 action의 production 조립과 최종 공통 정책
...
- PROD-414 당시 `deletePost` payload는 삭제된 Repost의 `postId`만 반환해 Source Post의 최신 count/viewer relation을 정규화할 수 없었다. PROD-471은 기존 `postId`를 유지하면서 nullable `repostSource` Post를 추가하고, 서버 확정 count/viewer relation이 같은 actor Relay Store에 정규화되게 한다.
...
2. PROD-401의 Local GraphQL entry는 공통 `usingProfile` 인증이 검증한 `ctx.session.profileId`를 별도 역할 제한 없이 public `repostPost` action에 전달한다. 이 action은 `origin = LOCAL | ACTIVITYPUB` 입력을 받아 자체 transaction을 소유하고 Source visibility/eligibility, derived visibility와 duplicate/concurrent idempotency를 유지한다. ActivityPub 입력은 검증된 actor/source/activity identity와 timestamp를 같은 action에 전달하며, action이 일반 `createPost`와 같은 저장 경계에서 Repost와 `ActivityPubPosts` mapping을 기록한다. insert conflict가 발생하지 않게 미리 조회하는 것만으로 동시성을 보장하지 않고, DB conflict 뒤 기존 Active Repost를 다시 조회해 성공 결과로 정규화한다. 새로운 `materializeActivityPubRepost`, `materializeRepostInTransaction`, `undoActivityPubRepost` 또는 caller-owned Repost transaction을 만들지 않는다. PROD-669가 연결한 caller-owned lifecycle은 PROD-725가 대체하며, verified Undo는 Fedify가 mapping과 actor를 read-only로 해석한 뒤 공통 `deletePost`에 전달한다. PROD-495가 정한 current Announce identity/generation과 no-lock 동작은 유지한다.
...
6. PROD-453의 fixture-only 단계는 production fragment shape를 따르는 typed fixture adapter와 mock navigation으로 Repost·Quote 상태를 검증했다. PROD-415 production integration에서 Content 없는 Repost는 `PostListItem`이 Repost Author attribution을 정확히 한 번 표시한 뒤 direct Source를 일반 Post와 같은 비재귀 표준 목록 행 leaf의 실제 표시 대상으로 사용한다. Quote와 Reply+Quote는 Quote Author와 자체 Content를 먼저 표시하고 direct Source를 compact bordered preview로 표시한다. Quote Source preview는 자체 Action Bar를 갖지 않으며 nullable Source인 Quote는 preview와 Source 이동 affordance만 생략한다. 순수 Repost의 direct Source 자체가 Quote인 조합의 preview 정책은 2026-07-27 PROD-415 creator reply에 따라 후속으로 미룬다. `PostSourcePresentationView`와 `PostSourcePreview`는 presentation data에서 canonical Expo Router Link와 body shortcut을 직접 만들고 caller-injected renderer·navigation callback을 받지 않는다. Storybook은 production과 같은 Router decorator에서 실제 pathname을 검증하고 raw object를 fragment key로 cast하지 않는다. 실제 관계 field를 읽는 Relay operation·fragment와 generated type은 PROD-415가 소유한다. PROD-414는 #341로 main에 들어온 단일 공개 `PostActionBar` 경계에서 `PostActionBar_post`가 `RepostAction_post` child fragment를 spread하게 한다. private `RepostAction`은 fragment read, create/delete mutation, pending, actor 격리와 선택 상태·label·정확한 delete identity 파생을 함께 소유하고 공통 private control을 렌더한다. `PostListItem`·`PostLayout`은 Action Bar를 content grid의 마지막 sibling이자 모든 navigation link 밖에 렌더링하고, 순수 Repost에서는 direct Source fragment를 Action Bar target으로 공급한다. Repost trigger는 항상 menu를 열며 Web은 anchored popup, Android·iOS는 bottom action sheet를 사용한다. 현재 상태에 따라 `재게시하기` 또는 `재게시 취소`만 표시하고 `인용하기`는 PROD-431 전까지 노출하지 않는다. 실제 surface는 action별 error callback을 하나의 공용 transient toast host에 연결하고 정확한 한국어 실패 문구·약 3초 자동 dismiss·latest replace·alert semantics를 제공한다. 새 외부 dependency는 추가하지 않는다. 최종 disabled 정책과 나머지 action의 production 조립은 PROD-432가 설계하고 통합 검증한다. 취소 성공 뒤 Source cache는 PROD-471의 `DeletePostPayload.repostSource` 선택 결과로 서버 확정 Source 상태를 같은 actor Store에 정규화한다.
...
8. PROD-416의 idempotent cleanup 저장 경계는 PROD-725의 accepted delete effects Workflow가 Activity로 재시도한다. source transaction과 caller는 cleanup을 직접 await하지 않는다. terminal failure 뒤 남은 row는 Active pure-Repost 구조와 Recipient 기준 관계 visibility를 검증하는 predicate로 모든 API surface에서 숨긴다.
...
- 기존 contentful `createPost` 입력·반환을 nullable Repost 용도로 넓히거나 API context 전용 visibility helper를 core service에서 import하지 않는다.
...
- Source preview나 Action Bar를 전체 Post Link 안에 중첩하지 않는다.
...
- Repost trigger에서 mutation을 즉시 실행하거나 선택 상태에 따라 menu와 즉시 실행을 섞지 않는다.
- Repost의 선택 상태·label·delete identity·mutation callback을 독립 scalar config로 분해하거나, 독립 공개 Repost action leaf, `PostActionBar` 중복 구현, PROD-433 branch 코드 복사, persistent 오류·재시도 UI 또는 success toast를 PROD-414에 추가하지 않는다.
...
- [UI action의 실제 surface 부재] → PROD-414가 main의 공개 `PostActionBar` 경계를 `PostListItem`·`PostLayout`에 처음 배치하고 Repost menu·toast까지 연결한다. PROD-432는 나머지 action 조립·최종 disabled 정책과 전체 통합 검증을 소유한다.
...
- [취소 성공 뒤 오래된 client 상태] → PROD-414는 client count 산술·광범위한 invalidation·임시 refetch를 추가하지 않고 당시 제한을 검증했다. PROD-471은 서버 결과 기반 `repostSource` 정규화로 같은 actor Store의 제한을 해소하며, actor 간 격리를 회귀 테스트로 유지한다.
