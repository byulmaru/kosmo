...
- Home과 Profile Post List가 Repost와 Source 양쪽의 조회 정책을 적용해 후보를 선정한다.
- Repost·Quote 프레젠테이션, 목록·상세 renderer별 direct Source 표시와 canonical Source 이동을 제공한다. 순수 Repost의 body·timestamp와 직접 상세 URL은 direct Source의 구조와 관계없이 Source detail로 이동하고, 그 아래 Action Bar도 화면에 표시한 direct Source를 대상으로 동작한다. 단, direct Source가 Quote인 순수 Repost의 표시 깊이는 PROD-828로 분리한다.
...
- External Completion Dependencies: [PROD-432](https://linear.app/byulmaru/issue/PROD-432) / PR #437, [PROD-725](https://linear.app/byulmaru/issue/PROD-725) / PR #618·#626
- Historical Superseded Implementation: PROD-669의 process-local Repost lifecycle은 PROD-725의 Temporal effects Workflow로 대체됐다.
- Deferred Follow-ups: [PROD-827](https://linear.app/byulmaru/issue/PROD-827) Hashtag Post List 구현, [PROD-828](https://linear.app/byulmaru/issue/PROD-828) 순수 Repost-of-Quote 표시 Domain Gate
...
- `post`: Post 구조 조합, Repost Source·count·현재 Profile 상태와 Home/Profile 목록 후보 계약 확장
- `notification`: Repost source의 생성·조회·inbox 표시·읽음 처리와 PROD-725 effects Workflow의 멱등 생성·정리 계약 추가

## Impact

- Linear: PROD-394, PROD-401, PROD-402, PROD-403, PROD-411, PROD-412, PROD-414, PROD-430, PROD-415, PROD-416, PROD-453, PROD-471과 부모 PROD-389. PROD-432·725는 완료된 external dependency이고, PROD-669는 superseded history다.
