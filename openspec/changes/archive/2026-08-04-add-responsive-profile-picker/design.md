...
- Web full과 mobile Web drawer의 이름·chevron trigger에서 chevron 상태와 hitbox·anchor·navigation geometry를
  보존한다. 이 change에서 도입했던 6px 하향 transform은 후속 `PROD-541`에서 제거됐으므로 복원하지 않는다.
- 가장 가까운 Storybook surface에서 responsive trigger와 긴 목록 회귀를 검증한다.

**Non-Goals:**

- Android/iOS profile picker를 재설계하지 않는다. Web full과 mobile Web drawer는 trigger의 chevron 외 content,
  dismissal과 transient-state lifecycle을 재설계하지 않는다.
...
trigger ReactNode를 공개 계약으로 노출하지 않는다. `ProfileSwitcher`는 full summary와 같은 root의 absolute layer를
...
Web full과 mobile Web drawer는 open 상태에 위 방향 chevron을 사용하되 trigger root와 picker surface를 이동하지
않아 hitbox, absolute anchor와 navigation geometry를 유지한다. 후속 `PROD-541`은 이름·chevron 내부 content의
6px 하향 transform을 제거하고 trigger 수직 중심 정렬을 복원했으므로, 이 change의 archive와 spec sync는 해당
transform을 다시 요구하거나 복원하지 않는다. compact avatar trigger와 Android/iOS geometry는 유지한다.
...
검증으로 실행한다. 기존 full·mobile story에서는 trigger의 down/up chevron과 이름·icon이 별도 하향 transform
없이 trigger 수직 중심을 유지하는지 검증한다. Storybook viewport preset이나 새 test harness는 추가하지 않고
...
- 후속 `PROD-541`에서 제거한 6px 하향 transform을 trigger content·root·picker wrapper에 다시 적용하지 않는다.
...
- [archive sync가 후속 중심 정렬을 되돌릴 위험] → delta에서 superseded 6px transform을 제거하고 `PROD-541`의
  transform 없는 중심 정렬, picker anchor·navigation 불변과 Android/iOS 비변경을 유지한다.
...
3. Web full과 mobile Web drawer trigger의 chevron 상태를 제공한다. 후속 `PROD-541`에서 제거된 6px 내부
   transform은 archive sync에서 복원하지 않는다.
