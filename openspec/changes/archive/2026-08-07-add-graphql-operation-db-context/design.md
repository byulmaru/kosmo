...
PROD-708은 이 구조를 인증 identity와 operation 실행 context로 분리하고, 후속 PROD-371이 점진적으로 소비할 transaction DB handle을 추가한다. 기존 resolver SQL을 한꺼번에 옮기지 않으므로 기반만 독립 배포·rollback할 수 있다.

## Goals / Non-Goals

**Goals:**

- Query/Mutation마다 actor setting과 resolver용 `ctx.db`가 같은 primary transaction을 사용한다.
...
별도 Yoga execution plugin은 Query/Mutation의 execute 함수를 Drizzle transaction으로 감싼다. transaction 시작 직후 parameterized `set_config(..., true)` 호출로 account/profile 값을 설정하고 context의 `db`를 transaction handle로 교체한 다음 원래 execute 함수를 호출한다. Query에는 `read only`, Mutation에는 `read write` access mode를 사용한다.
...
- [모든 Query/Mutation이 아직 빈 primary transaction을 하나씩 추가해 pool 사용 시간이 늘어난다] → operation timeout과 pool 수명 테스트를 추가하고, 이 비용을 후속 RLS SQL 이전에 필요한 명시적 기반 비용으로 제한한다.
...
2. Yoga execution plugin과 `ctx.db` 타입을 owner credential 상태로 배포한다.
3. lifecycle, batch cache, actor setting과 pool 누출 검증 뒤 PROD-371의 점진 SQL 이전을 unblock한다.
4. rollback은 execution plugin/context DB handle 추가만 되돌린다. schema, credential과 기존 resolver SQL이 변하지 않아 data rollback은 없다.
