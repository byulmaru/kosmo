> **Reconciliation (PROD-780, 2026-08-16):** The owner `kosmo` source for API/Fedify application workloads and the `BYPASSRLS` assertion below are historical PROD-715/724 baseline claims. The current application-runtime contract is shared `kosmo_worker` standard `PG*` for API/Web/Temporal Worker/Fedify application consumers with `rolbypassrls=false`; migration owner and queue credentials remain separate. Existing `kosmo_api` role/ACL/default ACL/Vault/CNPG Secret provisioning remains until PROD-781, and owner `kosmo` credential retirement remains PROD-712. This note records reconciliation only; it does not sync or archive main specs.
