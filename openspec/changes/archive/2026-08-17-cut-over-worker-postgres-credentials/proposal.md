> **Reconciliation (PROD-780, 2026-08-17):** This archived change records the historical shared-Worker transition. The current application-runtime contract supersedes that target: API, Web, Temporal Worker and Fedify application consumers use the new `kosmo_runtime LOGIN NOBYPASSRLS` standard `PG*` source. Legacy `kosmo_worker BYPASSRLS` and `kosmo_api` role/ACL/default ACL/Vault/CNPG Secret provisioning remain rollback-compatible until PROD-782/PROD-781 respectively; migration owner, Fedify MessageQueue and owner `kosmo` boundaries remain separate. The historical body below is preserved as implementation evidence.
