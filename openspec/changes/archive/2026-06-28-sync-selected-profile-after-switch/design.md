...
4. **route boundary는 layout의 GraphQL active profile 값을 전달한다.**
   이미 열린 `/compose` 화면은 자체 `currentSession` query를 다시 실행하지 않는다. `(tabs)` layout은 최초 표시에는 `TabsLayoutQuery.currentSession.selectedProfile`을 쓰고, 프로필 전환 성공 후에는 mutation 응답의 `Session.selectedProfile` fragment ref를 최신 active profile 값으로 사용한다. `/compose` route는 이 값을 Svelte context로 읽어 작성 프로필에 사용한다. 이 context는 독립 프로필 저장소가 아니라 layout이 가진 GraphQL 결과와 mutation 응답 fragment ref를 route 경계 너머로 전달하는 bridge다.
