...
- Reply mode open 직후의 dirty close, 일반 Post의 pristine close, dirty 취소 확인, pending close 차단, 실패 상태 유지,
  성공 close·focus 복원과 Web modal focus trap·배경 scroll lock을 surface lifecycle로 제공한다.
...
- Reply mode open 직후 dirty·일반 Post pristine·pending·실패·성공 close, focus trap·복원·배경 scroll lock, single central
  scroll과 selected Profile 없는 surface의 unchanged partial rollout을 검증한다.
...
- [x] 2.4 direct Parent preview와 기존 composer를 조립해 Web 목록 modal·좁은 Web/Native 전체 화면·상세 thread 행별 inline surface,
      Reply mode open 직후 dirty·일반 Post pristine·pending·실패·성공 lifecycle과 focus·scroll 계약을 구현한다.
...
- Parent Content Warning 유무에 따른 Reply 초기값, 사용자 수정·제거, Reply mode open 직후 dirty, discard/reset, 실패 유지와
  Parent·Profile·Relay Environment 전환을 검증한다.
