...
- reveal 상태를 Post card·modal·thread row 각각의 local state로 두거나 route/surface remount 때 같은 Post의 상태를 초기화하지 않는다.
