...
- 기존 bearer token context에서 파생된 인증 session과 선택된 actor profile을 요구한다.
...
- 로컬 업로드를 `source = LOCAL`인 `media` row로 저장하고 account, required profile, original file ref를 연결한다.
- 리모트 ActivityPub 이미지는 `source = REMOTE`인 `media` row로 URL/fetchedAt metadata를 저장할 수 있게 하며, 리모트 actor/profile 정체성은 `Profile`이 표현한다.
...
2. 업로드 입력은 multipart `image` 필드를 사용한다.
   ...
   REST 입력 검증은 route handler 내부 수동 검증이 아니라 `@hono/standard-validator`의 `sValidator('form', ...)` middleware 흐름에서 zod schema로 처리한다. 다만 인증 session과 선택된 actor profile 확인은 multipart parsing보다 앞선 middleware에서 먼저 수행한다.

   전체 request body는 Hono `bodyLimit` middleware로 64MiB까지 허용한다. 이는 malformed 또는 과도한 multipart body가 인증 이후 parser 비용을 무제한 유발하지 않게 하는 방어선이다. 원본 이미지 파일 자체는 zod schema에서 10MiB로 다시 제한한다.

3. 현재 변경은 허용 MIME type 기반의 가벼운 이미지 입력 검증만 수행한다.

   API는 endpoint 정책 상수에 포함된 이미지 MIME type이 아닌 입력을 거부한다. 실제 디코딩 가능 여부, 애니메이션 처리, EXIF 정규화, 변환 실패 처리는 후속 이미지 처리 워커에서 다룬다.
...
5. 추가 환경변수는 명시적으로 관리한다.
   ...
   필요한 환경변수는 `R2_ENDPOINT`, `R2_BUCKET`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_PUBLIC_BASE_URL`이다. 이 값들은 `packages/core/env.ts`에서 zod schema로 파싱해 export하고, API R2 helper는 파싱된 `env`를 참조한다. 요청 body와 원본 이미지 파일 최대 크기는 환경변수가 아니라 route 정책 상수로 관리한다.

6. `file`과 `media`를 분리한다.

   `file`은 우리 R2에 존재하는 물리 객체다. 이번 upload endpoint는 storage key, MIME type, byte size를 채운다. public URL은 storage key와 런타임 public base URL에서 계산하고 DB에는 저장하지 않는다. width, height, SHA-256은 후속 이미지 처리/계측 단계 전까지 nullable이다.

   `media`는 제품에서 참조하는 논리 미디어다. source, owner, required profile, file refs, remote metadata, thumbhash를 가진다. 로컬 업로드 직후에는 `originalFileId`만 채우고 `thumbnailFileId`와 `thumbhash`는 비워둘 수 있다.

7. 객체 key는 서버가 생성한다.

   사용자 filename을 storage path로 신뢰하지 않는다. 객체 key는 `uploads/yyyy/mm/<uuid>` 형식을 사용하며 파일 확장자를 보존하지 않는다.

## 위험 / 트레이드오프

- MIME type 검증만으로는 실제 이미지 디코딩 가능성을 보장하지 않는다. 이는 업로드 API 범위를 작게 유지하기 위한 결정이며, 워커 처리에서 보강한다.
- multipart 요청이 API memory를 많이 사용할 수 있으므로 인증 이후 64MiB body limit과 10MiB file limit을 둔다.
