...
shared navigation row로 해석해 full·compact·drawer에 별도 항목을 추가했지만, 실제 제품 의도는 Figma가 향후
멀티프로필 전환 cluster 아래에 예약한 위치의 작은 노란 `편집` action이다. 현재 production Profile 요약에는
그 thumbnail visual이나 switching 동작이 없으므로 이 change는 action만 같은 좌표에 복원한다.
...
- action은 Figma `UserInfo`가 오른쪽 mini-profile cluster 아래에 예약한 좌표에 우측 정렬하며, 기존 Profile
...
- `/profile-edit` form·API·DB·Media, 공개 ProfileHero 편집 button, generic `/menu`, production에 없는
  mini-profile thumbnail visual·전환 기능과 새 Profile 권한 정책은 변경하지 않는다.
...
- `apps/app/src/components/shell/shellLayout.ts`와 `shellLayout.test.ts`의 platform target·label 대비 계약
