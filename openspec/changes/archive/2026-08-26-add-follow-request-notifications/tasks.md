...
- visibility 확인과 concrete source field resolve 사이의 source 삭제를 DB-backed regression으로 재현해 Node·connection이 non-null source 오류 없이 같은 snapshot을 사용하거나 item을 숨기는지 검증하고, 정상 Read payload가 공통 source loader로 concrete field를 resolve하는지 확인한다.
