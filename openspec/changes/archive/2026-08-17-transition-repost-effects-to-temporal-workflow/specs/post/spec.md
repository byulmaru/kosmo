...
#### Scenario: 모든 최초 Tombstone이 공통 Delete Workflow를 시작함

- **WHEN** Active Post가 처음 Tombstone으로 전환되어 commit된다
- **THEN** 시스템은 committed relation shape에서 `postKind=POST | REPLY | QUOTE | REPLY_QUOTE` 또는 `postKind=REPOST`를 도출해 `post-delete:{postId}` identity의 공통 Delete Workflow start를 시도한다
- **AND** start 또는 effects 실패는 Tombstone 전환과 GraphQL 성공 payload를 바꾸지 않는다

#### Scenario: Content Post deletion uses the common workflow

- **WHEN** Content가 있는 ordinary Post, Quote, Reply 또는 Reply이면서 Quote가 Tombstone으로 전환된다
- **THEN** 시스템은 `postKind=POST | REPLY | QUOTE | REPLY_QUOTE`인 공통 Delete Workflow를 시작한다
