...
이 기록은 PROD-477의 공통 production 오류 관측 목표와 PROD-483의 Android·iOS 오류 수집, 개인정보 최소화, release·source map·debug symbol 및 production build 설정 범위를 Expo CNG와 현재 배포 workflow에 적용한 선택을 정리한다.
...
- Confirmation / Follow-up: 설정 단위 테스트에서 사용자·인증·콘텐츠 context가 추가되지 않는 경계를 확인한다. 실제 Android·iOS 검증 event는 이 change에서 실행하지 않는다.
...
- Confirmation / Follow-up: clean Android/iOS prebuild 결과와 workflow 설정을 확인한다. 실제 production build log와 Sentry release artifact·원본 위치는 이 change에서 생성하거나 검증하지 않는다.
...
- Confirmation / Follow-up: workflow 정적 검증에서 release가 build의 full SHA를 사용하고 token이 repository·generated project·artifact에 남지 않는 경계를 확인한다. 실제 Sentry event/artifact는 이 change에서 검증하지 않는다.
