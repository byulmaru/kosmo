...
- [x] 1.4 Define the V1 `{ version, summary, body }` JSON envelope, body node types, version registry constants and runtime-independent client type guard for doc/paragraph/text/hard_break/link.
...
- [x] 2.1 Replace Drizzle `PostContents.bodyText` and `contentWarning` with one non-null typed jsonb `document` column.
...
- [x] 3.1 Add an output-only `PostContentDocument` scalar exposing the canonical `{ version, summary, body }` JSON.
- [x] 3.2 Resolve `PostContent.bodyText` and `contentWarning` from document projections instead of stored DB fields while preserving visibility access boundaries.
