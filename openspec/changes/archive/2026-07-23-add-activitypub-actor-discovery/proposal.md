...
- canonical 웹 origin의 federation 요청은 일반 Hono BFF route보다 먼저 federation protocol 경계에서 처리하고, 처리되지 않은 요청만 기존 BFF 동작으로 이어간다.
- WebFinger `acct:{handle}@{localDomain}` 조회가 local active profile을 actor URI `{localOrigin}/ap/actor/{profile.id}`로 연결하도록 정의한다.
- canonical actor URI도 WebFinger resource로 발견하며, 누락·malformed resource는 400, unknown/non-local resource는 404로 응답한다.
- actor document는 `Person`의 `id`, `preferredUsername`, `name`, `url`, `published`, `inbox`, `outbox`, `publicKey`, `assertionMethod`를 보장한다.
...
- `followers`, `following`은 광고하지 않고, outbox submission/collection과 followers/following collection 동작은 이 capability에서 제공하지 않는다.
...
- `apps/web`: federation 요청을 기존 BFF route와 SPA fallback보다 먼저 처리하고, federation protocol 경계가 처리하지 않은 요청만 기존 동작으로 이어간다.
