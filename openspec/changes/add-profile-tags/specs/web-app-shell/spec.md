...
- **AND** Profile Tag는 bio 다음이자 팔로우 수와 콘텐츠 목록보다 앞에 저장 순서로 표시한다
...
- **THEN** 시스템은 각 정규화된 이름 앞에 `#`를 한 번 붙인 chip을 저장 순서로 표시한다
- **AND** Profile Tag 검색이 전달되기 전에는 chip을 링크나 버튼으로 표현하지 않는다

#### Scenario: Wrap Profile Tags without overflow

- **WHEN** 좁은 화면에서 긴 허용값 또는 5개의 Profile Tag를 모두 표시한다
