...
- 공개 조회 가능한 Local Profile에만 Profile Tag를 노출하고, 비활성화·정지 때 관계를 보존한 채 숨기며, 선행 `PROD-532`가 제공하는 Deactivated→Deleted lifecycle 경계에 해당 Profile 관계 cleanup을 통합한다.
...
- `data-model`: canonical Hashtag identity를 저장하고 Profile과의 identity 관계, 유일성, 삭제 생명주기와 migration 계약을 추가한다.
...
- Core/DB: Hashtag가 소유하는 Name 정규화·저장 경계, Profile-Hashtag identity 관계, migration, transaction과 `PROD-532` lifecycle 경계의 관계 cleanup 통합
