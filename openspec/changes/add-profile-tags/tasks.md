...
- Deactivated Profile과 Suspended Profile의 관계 보존은 공개 visibility에서 숨기는 정책과 분리한다. Lifecycle State가 `Deleted`로 전이될 때 service/lifecycle transaction은 해당 Profile의 `profile_hashtag` 관계만 제거하고 canonical Hashtag row와 다른 Profile/Post 관계는 유지한다. Profile row 물리 삭제의 FK cascade는 별도 DB safety invariant/test로 검증한다. Remote Profile은 빈 tags를 반환하며 actor fetch를 수행하지 않는다.
...
- fresh/upgrade migration, unique/check/foreign-key, 빈 기존 Profile, Deactivated/Suspended 관계 보존과 물리 Profile row 삭제 FK cascade safety를 `@kosmo/core` migration·DB test로 검증한다.
- Active Account Owner의 Local Profile(Deactivated 포함) 성공과 Member·비-Owner·inactive Account·Remote·Deleted·Suspended 거부, omitted·null·empty·ordered tags, rollback과 concurrent replacement를 service·GraphQL database integration test로 검증한다. Deleted lifecycle transition에서 service/lifecycle이 `profile_hashtag` 관계만 제거하고 canonical Hashtag row와 다른 Profile/Post 관계를 보존하는 통합 검증을 FK cascade safety test와 별도로 수행한다.
