...
**Authority / Provenance:** `docs/domain/objects/profile-block.md`의 관계·행동·조회 정책, `docs/domain/decisions/0005-domain-boundary-followup-clarifications.md`, `PROD-818`의 2026-09-06 전달 결과·선행 조건. 시스템은 검증된 Remote Owner → Local Target의 inbound Block을 기존 Profile Block으로 생성해야 한다(MUST). 같은 방향 pair에는 관계가 하나만 존재해야 하며(MUST), 별도 제품 차단 상태를 만들지 않아야 한다(MUST NOT). Block 생성은 기존 Owner 권한과 양방향 Follow Request·Follow Relationship 및 직접 원인 Notification cleanup 경계를 따라야 하고(MUST), 해당 생성이 포착한 필수 정리를 완료하기 전에 Block 성공을 확정해서는 안 된다(MUST NOT). Unblock은 입력한 exact Profile Block ID의 관계만 제거해야 하며(MUST), Follow Request·Follow Relationship·Notification을 추가로 정리하거나 제거된 Follow를 복구해서는 안 된다(MUST NOT). 기존 Reaction·Repost Post·Bookmark를 삭제해서는 안 된다(MUST NOT).
...
#### Scenario: Block 생성 cleanup이 실패한다
...
- **WHEN** Undo가 저장된 Block과 같은 URI만 참조하거나 embedded `Like`·`Follow` 등 non-Block Activity를 포함한다
- **THEN** 해당 입력을 Profile Block Undo로 소비하거나 저장된 URI를 type oracle로 사용하지 않는다
- **AND** Profile Block과 protocol generation을 변경하지 않고 기존 해당 Undo 처리기로 넘긴다
...
#### Scenario: protocol metadata 없이 정확한 generation을 해제한다

- **WHEN** 발신 대상인 Local Owner → Remote Target의 exact `profileBlockId`를 해제하지만 protocol metadata가 없다
- **THEN** exact ID로 stable original identity를 구성해 `Undo(Block)`만 queue에 인계한다
- **AND** 원본 `Block`을 먼저 생성·발신하지 않으며 metadata 부재를 legacy/no-send 상태로 해석하지 않는다
...
- **THEN** Block 효과의 미정산 실패는 관찰·복구할 수 있게 보존하고 exact generation의 Undo는 별도로 인계한다
- **AND** Undo를 위해 Block을 새로 발신하거나 로컬 차단·해제 결과를 되돌리지 않는다
...
- **AND** dispatcher 정상 no-op을 handoff 성공이나 terminal skip으로 정산하지 않되 exact generation의 Undo B1은 대기시키지 않는다
...
- **AND** B1의 실제 인계 정산과 무관하게 Undo B1을 진행하며 Undo도 같은 admission을 적용한다
...
### Requirement: metadata-independent Local Unblock generation

**Authority / Provenance:** `docs/domain/objects/profile-block.md`의 연합 행동, `docs/domain/decisions/0031-profile-block-federation.md`의 exact generation 처리, `PROD-818`의 2026-09-17 리뷰 결정. Profile Block과 연합 기능은 함께 사용 가능해졌으므로 시스템은 protocol original 없이 먼저 존재한 Profile Block을 legacy rollout 상태로 모델링해서는 안 된다(MUST NOT). 모든 발신 대상 Local Owner → Remote Target Unblock은 exact `profileBlockId`에서 stable original identity를 재구성하고 동일한 Undo-only 경로를 사용해야 한다(MUST). metadata 부재를 이유로 선행 Block을 만들거나 Undo를 생략해서는 안 된다(MUST NOT). product row 삭제는 exact generation에만 적용해야 하며(MUST), 이후 replacement Block을 삭제해서는 안 된다(MUST NOT). Follow·Request·Notification cleanup은 Unblock 경계에 포함하지 않는다(MUST NOT).

#### Scenario: metadata 없이 Local Unblock한다

- **WHEN** exact `profileBlockId`의 발신 대상 관계를 해제하지만 protocol metadata를 찾지 못한다
- **THEN** stable original identity를 구성해 `Undo(Block)`만 인계한다
- **AND** 선행 Block 발신이나 legacy/no-send 분기를 만들지 않는다

#### Scenario: 상대 서버가 original Block을 모른다

- **WHEN** Kosmo가 exact generation의 Undo를 인계했지만 상대 서버에 해당 original Block 상태가 없다
- **THEN** remote no-op을 허용하고 로컬 해제 결과를 rollback하지 않는다
- **AND** remote 적용 여부를 로컬 성공 조건으로 주장하지 않는다

#### Scenario: 해제 retry와 재차단이 겹친다

- **WHEN** 이전 generation의 해제 retry가 진행되는 동안 같은 pair에 replacement Block이 성립한다
- **THEN** 이전 exact-row 삭제와 Undo는 보존한 exact `profileBlockId`만 대상으로 삼고 replacement Block을 해제하지 않는다
- **AND** replacement Block은 새로운 stable original identity를 사용한다
