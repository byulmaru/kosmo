...
- `docs/domain/objects/profile-block.md`, `docs/domain/decisions/0031-profile-block-federation.md`.
...
- `docs/domain/objects/profile-block.md`의 관계·행동·Owner 권한,
  `docs/domain/decisions/0031-profile-block-federation.md`의 단일 관계·rollout.
...
- embedded object가 실제 `Block`일 때만 Profile Block Undo로 처리한다. URI-only와 non-Block은 저장된 URI로
  type을 추론하지 않고 기존 해당 Undo 처리기로 넘긴다.
...
- embedded Block·URI-only·embedded non-Block, cleanup 실패·재시도, 반대 방향 유지와 Follow 비복구를 확인한다.
...
- [x] 3.3 잘못된 입력, embedded Block, URI-only·embedded non-Block의 실제 처리 결과를 검증하고 기존 Undo 회귀를 통과시킨다.
...
- `docs/domain/objects/profile-block.md`의 연합·실패 경계,
  ...
  `docs/domain/decisions/0031-profile-block-federation.md`의 Mastodon 호환 결정.
...
- 원본 삭제·metadata 부재 후 Undo retry, exact generation identity, queue 수락 응답 유실과 policy 제외를 검증한다.
- B1 인계 응답 유실이나 retry 소진 중에도 exact generation의 Undo가 선행 Block 생성 없이 독립적으로 인계되는지 확인한다.
- unavailable 후 복구·계속 unavailable·잘못된 URI·mapping 부재 각각에서 queue 호출 수와 pending 사유를 검사한다.
  미인계 정상 no-op과 실제 수락을 같은 성공으로 처리하지 않는지 확인한다.
...
## 5. PROD-818 exact generation Undo와 Worker 복구

**Authority / Provenance**

- `docs/domain/objects/profile-block.md`의 Undo identity·실패 경계,
  `docs/domain/decisions/0031-profile-block-federation.md`의 exact generation 처리.
- PROD-818의 2026-09-17 리뷰 결정과 Worker restart·실패·재시도 범위.

**Deliverable**

metadata 유무와 무관하게 exact generation의 Undo-only 경로를 사용하고, 재시작·rollback 뒤에도 관계와 미완료 효과가 보존된다. group 3·4에 의존한다.

**Guardrails**

- D3·D4·D5: metadata 부재를 legacy/no-send 상태로 해석하지 않고 exact `profileBlockId`에서 original identity를 재구성한다.
- Local Unblock에서 `Block`을 먼저 생성·재발신하지 않고 Undo만 인계한다.
...
- metadata 없는 exact generation, 삭제된 row 뒤 재시도, 해제 후 재차단의 DB 상태와 Undo-only/새 Block 결과를 확인한다.
...
- [x] 5.1 metadata 유무별 exact generation Undo와 해제·재차단 보호를 검증한다.
...
- `docs/domain/objects/profile-block.md`의 관계·cleanup·조회·해제,
  `docs/domain/decisions/0031-profile-block-federation.md`의 상호운용 범위.
...
- D1~D5·D9: 같은 관계 재사용·Owner 방향·원본 identity·대상 한정·echo 방지·실패 격리·embedded Block type 경계를 모두 확인한다.
...
- Local→Local 무발신, Kosmo→Mastodon Block/Undo, Mastodon→Kosmo Block/Undo, 양방향 Block 공존,
  duplicate·out-of-order·URI-only/non-Block·실패 서버·restart·metadata 없는 Undo를 실행한다.
...
- `docs/domain/decisions/0031-profile-block-federation.md`의 책임 경계.
