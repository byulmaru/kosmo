...
- `selectedProfileForEdit` consumer와 공개 schema를 호환성 확인 뒤 제거한다.
...
6. repository operation·fixture·generated artifact와 공개 schema consumer를 검색해 기존 두 main consumer가
   전환됐음을 확인한 뒤 `selectedProfileForEdit` resolver와 schema field를 제거한다.
...
- [공개 GraphQL field 제거가 미확인 consumer를 깨뜨릴 수 있음] → removal 전 repository·generated operation과
  알려진 PR #529 consumer를 확인하고 breaking change를 PR에 명시한다. PR #529는 선행 merge 뒤 별도로 갱신한다.
...
2. main의 공개 Profile route와 `ProfileEditRoute`를 새 projection으로 전환하고 Relay artifacts와 component
   tests를 갱신한다.
3. main repository와 공개 schema consumer 호환성을 확인한 뒤 `selectedProfileForEdit` schema/resolver,
   operation과 생성물을 같은 선행 migration에서 제거한다. archived OpenSpec은 수정하지 않는다.
...
5. 선행 migration이 merge되면 PROD-660이 PR #529를 stack/rebase하고 ProfileSwitcher consumer만 새 계약으로
   전환한다. 이 change의 archive는 PROD-705 선행 PR이 전체 검증과 canonical/spec 정합성을 확인한 뒤 소유한다.
...
두 기존 consumer operation을 같은 revert에서 복원한다. 어떤 단계에서도 client-side Owner 추측이나 public
