...
### consumer 전환 뒤 selectedProfileForEdit을 제거한다
...
- Decision Outcome: Membership field를 additive하게 제공하고 main의 first-party consumer를 먼저 전환한 뒤
  `Query.selectedProfileForEdit` schema/resolver와 관련 artifact를 같은 선행 migration에서 제거한다.
  public/runtime schema를 함께 동기화한다.
- Alternatives Considered: query를 deprecated 또는 영구 alias로 유지하는 방식은 관계 중심 계약 전환을
  완료하지 못해 제외한다. consumer 전환 전 즉시 제거는 deploy/compile 호환성을 깨뜨려 제외한다.
- Consequences: schema removal은 breaking change로 명시하고 rollback 시 schema/resolver와 필요한 consumer
  operation을 한 계약 단위로 복원한다. archived `add-local-profile-edit`은 수정하지 않는다.
...
- Decision Outcome: PROD-705 선행 PR은 schema/resolver/loader, main의 first-party consumer, tests, canonical 및
...
- archived `openspec/changes/archive/2026-08-06-add-local-profile-edit/decisions.md`의 guest-safe nullable selected
  ...
  및 consumer-first `selectedProfileForEdit` 제거 결정이 query source를 대체한다.
