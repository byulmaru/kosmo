...
- Follow Request Notification payload의 concrete source field는 다른 Notification kind와 같은 source loader로 resolve한다. Batch resolver에 Follow Request 전용 source join·snapshot Map을 추가하지 않는다.
...
3. visible 후보 전체를 하나의 `UPDATE` statement에서 `readAt = coalesce(readAt, now())`로 갱신하고 row를 반환한다. 정상 DML이 획득하는 row lock 외의 명시적 pessimistic lock은 추가하지 않는다.
