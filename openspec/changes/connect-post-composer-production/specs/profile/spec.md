...
**Authority / Provenance:** `docs/domain/objects/profile.md`, `PROD-816`, `PROD-878`, PROD-797 — 이 요구사항을 MUST
준수한다.
...
- 앱 최상위 정적 route namespace: `bookmarks`, `feedback`, `hashtags`, `home`, `local`, `notifications`,
  `search`, `settings`
...
배포 단위에서 System Reserved 목록과 공용 검증을 함께 갱신해야 한다(MUST). Route를 retired할 때도 해당
segment의 영구 예약 여부를 함께 결정하고 목록과 공용 검증에 반영해야 한다(MUST).
...
#### Scenario: Keep a retired route segment permanently reserved

- **WHEN** 로그인한 계정이 retired `/compose`의 bare `compose` 또는 대소문자만 다른 값을 Local Profile handle로 제출한다
- **THEN** 시스템은 해당 segment가 현재 정적 route가 아니어도 Profile을 생성하지 않고 handle field 오류를 반환한다
- **AND** retired namespace를 동적 Profile route로 재사용하지 않는다
