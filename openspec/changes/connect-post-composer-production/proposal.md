...
- **BREAKING** 직접 `/compose` 호환 route를 제거한다. 직접 접근은 404가 될 수 있으며, shell의 composer trigger만 지원되는 작성 진입점으로 유지한다.
...
- 영향 코드: 앱 shell/route의 composer host와 direct route tree, 기존 `PostComposer`, `PostComposerTarget`, Media presentation/editor와 관련 Storybook Tests.
