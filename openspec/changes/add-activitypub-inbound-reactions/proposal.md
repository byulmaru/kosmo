...
- 저장된 active Remote Profile actor, Post 조회 가능성과 local·remote ActivityPub Post identity를 검증한다.
  `Like`·`EmojiReact` activity audience는 Post Visibility나 수신 권한을 정의하지 않으므로 유효성 검증에 사용하지
  않는다. `Create(Note)`의 audience 기반 Visibility projection은 변경하지 않는다.
...
- Linear Implementations: PROD-498이 OpenSpec, schema, core action, Fedify handler, 테스트와 PR을 직접 소유한다.
  PROD-567은 active change에서 inbound Reaction audience 검증 제거를 소유한다. PROD-494는 완료된 Post
...
- `packages/fedify`: typed inbox listener, local·remote Post URI lookup, actor 검증과 Undo parsing
