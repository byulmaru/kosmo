...
- Overseas transfer disclosure approval: [Linear `PROD-795`](https://linear.app/byulmaru/issue/PROD-795)의 `2026-09-16 PostHog 국외 처리 고지 방식 결정` 댓글(`fe9c8467-3ebb-4a15-bdbf-872810de964a`) — 별도 동의 UI가 아닌 개인정보처리방침 개정·사전 공지 경로와 개인정보 보호법 제28조의8 제1항 제3호 가목의 계약 체결·이행에 필요한 처리위탁·보관 경로를 선택했다. 일반 이벤트 12개월 자동 삭제는 고지하지 않고, Session Replay 30일과 정확한 시행일은 각각 실제 설정·별도 공지에 한정한다. 선택적 분석을 해당 처리위탁으로 보는 법적 판단의 잔여 위험은 인지된 전제이며, 이 결정은 실제 계약·DPA 체결 사실을 대신 증명하지 않는다.
