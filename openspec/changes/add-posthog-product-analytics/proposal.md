...
- 초기 PostHog 기반 단계(PROD-819·PROD-795)에서는 broad autocapture와 Session Replay를 비활성화하고 Expo Router의 안정적인 route template 변화만 `$pageview`로 한 번 수집한다. 후속 PROD-741 활성화 단계에서는 production canonical origin의 Web Session Replay를 10% sample로 켠다.
...
- `web-product-analytics`: PostHog Web 초기화, phased Session Replay와 retention, 최소 수집 pageview와 명시 이벤트, Account identity 수명주기, 장애 격리와 Native no-op 경계
