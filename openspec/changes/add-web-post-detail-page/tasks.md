...
- [x] 2.3 작성자 영역을 `/@{handle}`로 링크한다 (초기 placeholder, `4.2`에서 컴포넌트로 교체됨)
...
## 4. 실데이터 연결 (같은 prod-89 브랜치, 블로커 머지 후)

- [ ] 4.1 본문 더미 상수 → `post` 단건 조회 `createQuery`로 교체한다 (PROD-93 머지 후)
- [x] 4.2 작성자 영역 → `PostAuthorProfile` + `PostAuthorProfile_profile` fragment로 교체한다. 작성자 데이터는 라우트 핸들 기준 `profileByHandle` 실쿼리에서 가져오고, 로딩/오류/없음 상태도 이 query에 매핑한다 (PROD-97 머지 후)
- [ ] 4.3 본문까지 실데이터로 HEAD가 동작하면 Draft → Ready로 전환한다
