...
게시글 URL로 단일 게시글을 직접 볼 수 있는 웹 게시글 디테일 페이지가 필요하다(PROD-89). 작성자 표시 컴포넌트(PROD-97)는 머지되어, 작성자 영역은 라우트 핸들 기준 `profileByHandle` 실쿼리 + `PostAuthorProfile`로 실데이터를 표시한다. 다만 단건 조회 query(PROD-93)가 아직 없어, **게시글 본문(Plain Text·작성 시각·공개 범위·상태)은 더미로 두고** 블로커가 머지된 뒤 **같은 브랜치(prod-89)에서 `post` query로 교체**한다. 본문 더미 상태가 main에 머지되지는 않는다. 이 페이지는 PROD-91이 추가한 핸들 기반 프로필 라우트(`(tabs)/@[handle]`) 위에 얹힌다.
...
- 작성자는 `profileByHandle` 실쿼리에서 공급하고, 게시글 본문(`Post`/`PostContent`, PR #67 shape)은 단건 조회 query가 없어 로컬 더미 상수 한 곳에서 공급한다. 분기 조건은 작성자 query의 `loading`/`error`/데이터 null과 본문 `state`에 매핑하며, PROD-93의 `post` query가 머지되면 본문 더미를 1:1로 교체한다.
- (Non-goals) 답글·반응·리포스트·ReplyComposer(Figma 풀 스레드 뷰), 실제 게시글 본문 조회 query 연결(PROD-93 후속·같은 브랜치), 모바일 `(tabs)` 셸 헤더와 back 헤더 통합(별도 웹 레이아웃 이슈)은 본 체인지의 머지 범위 밖이다.
...
- 소비 API: `profileByHandle`(작성자, 기존 스키마). 게시글 본문은 더미이며 PROD-93 머지 후 `post` 단건 조회 query를 같은 브랜치에서 연결한다. 백엔드 변경 없음.
