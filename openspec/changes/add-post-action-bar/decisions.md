...
- Decision Class: Human Decision
- Authority / Provenance: `PROD-977`, `docs/design/colors.md`, `docs/design/post-action-bar.md`, 2026-09-14 KST 사용자 승인 “플랫폼 가리지 않고” 및 Phase 0 범위 승인, 2026-09-16 KST 사용자 PR 분리 승인
- Status: Active
...
- Decision Outcome: Text·Media·PureRepost·Quote의 카드 root는 별도 card fill 없이 semantic `backgroundCanvas`의 feed canvas를 유지한다. Web pointer hover에서는 `stateHover`, Web·Android·iOS pointer 또는 touch press에서는 넓은 surface용 `statePressedSubtle` overlay를 카드 전체에 적용하며 pressed가 hover보다 우선한다. `statePressedSubtle`은 Light black 6%, Dark white 10%로 공용 `statePressed`보다 완화한다. release·cancel·leave 뒤에는 남은 입력 상태 또는 resting canvas로 돌아간다. 행 자체에 새 navigation, Pressable, role이나 focus target을 추가하지 않고 작성자·시간·본문·미디어·Action Bar의 기존 입력을 유지한다.
...
### Post presentation root가 canvas 배경을 직접 소유한다

- Decision Date: 2026-09-18
- Decision Class: Human Decision
- Authority / Provenance: `PROD-936`, `docs/design/colors.md`, `docs/design/post-action-bar.md`, 사용자 원인 정정과 수정 요청
- Status: Active
- Context / Problem: Native의 neutral 계열 배경 노출은 pressed·hover나 Navigation 기본색보다 공용 Post presentation root가 투명해 host 배경을 그대로 보이는 데서 발생했다. 부모 container의 배경 상속에 의존하면 같은 `PostListItem`·`PostLayout`이 Web·Native의 서로 다른 host에서 다른 기본 표면을 보인다.
- Decision Outcome: `PostListItem`의 Text·Media·PureRepost·Quote root와 `PostLayout` root는 semantic `backgroundCanvas`를 직접 적용한다. direct Quote Source preview와 state overlay처럼 내부에서 의도적으로 투명한 요소는 별도 resting fill 없이 root canvas를 보인다.
- Alternatives Considered: raw `#fff`는 Dark semantic과 token source를 우회하므로 제외했다. 모든 내부 요소에 fill을 반복하면 Quote Source와 overlay 계약을 깨므로 제외했다. Navigation 또는 route host만 교정하면 공용 presentation이 다른 consumer에서 다시 투명해질 수 있어 presentation root가 소유한다.
- Consequences: Light에서는 흰색, Dark에서는 검은색 canvas가 Web·Native에 일관되게 적용된다. row를 `backgroundSurface`·`backgroundElevated` card로 올리거나 hover·pressed·navigation·layout 계약을 바꾸지 않는다.
- Confirmation / Follow-up: 실제 `PostListItem`과 `PostLayout` renderer에서 Web·iOS·Android root의 `backgroundCanvas`를 검증한다. Native runtime paint는 별도 release gate에 남긴다.
