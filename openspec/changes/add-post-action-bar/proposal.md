...
- PROD-866은 Post action presentation semantic과 공용 presentation geometry를 최신 canonical source에 맞춘다. Reaction active·hover는 `#F97066`, Repost hover·selected는 Light `#16794A`, Dark `#409667`을 사용하고 전역 Success semantic은 변경하지 않는다. PROD-882는 미선택 Repost의 glyph·count와 hover count를 중립색으로 복구해 selected 상태와 구분한다. Web control은 28px visual row를 유지하면서 action 사이 분배 여백이 아닌 50×36px/28×36px bounded interactive rectangle을 제공한다. `PostLayout` Engagement는 full-width border·padding을 소유하며 current thread row 뒤 generic divider는 생략한다. direct Quote Source preview는 resting fill 없이 주변 Post background를 유지하고 interactive Web hover에서만 semantic state overlay를 사용한다.
