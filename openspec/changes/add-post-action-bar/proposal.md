...
- PROD-866은 Post action presentation semantic과 공용 presentation geometry를 최신 canonical source에 맞춘다. Reaction active·hover는 `#F97066`을 사용하되 selected HeartPlus는 fill 없이 outline stroke만 의미색으로 표시하고, Repost hover·selected는 Light `#16794A`, Dark `#409667`을 사용하며 전역 Success semantic은 변경하지 않는다. PROD-882는 미선택 Repost의 glyph·count와 hover count를 중립색으로 복구해 selected 상태와 구분한다. Web control은 28px visual row를 유지하면서 count가 있으면 숫자 `0`을 포함해 `6 + 16 + 4 + 렌더된 count 너비 + 6`을 HUG하고, count가 없으면 28×36px actual target을 제공한다. social layout slot은 최소 50px이고 target이 더 넓을 때만 확장하며, leading Reply target만 slot 시작점에 두고 나머지 social target은 가운데에 유지한다. More slot은 28px, Bookmark+More trailing group은 82px을 유지하고 action 사이 분배 여백은 덮지 않는다. `PostLayout`은 metadata 아래 8px에 Reaction Summary를 두고, Summary 아래 4px부터 시작하는 full-width 상·하 border와 8px padding 사이에는 Action Bar만 배치한다. Summary가 없으면 metadata에서 border까지 8px을 유지하고 current thread row 뒤 generic divider는 생략한다. direct Quote Source preview는 resting fill 없이 주변 Post background를 유지하고 interactive Web hover에서만 semantic state overlay를 사용한다. 공용 Post 본문과 Quote Source의 외부 링크는 mode별 `actionLinkBase`를 사용하며 Source 작성자 행의 44×44px 생성 시각 target 안에서 시간 label을 오른쪽 정렬한다.
...
- 시각 참고: [PostActionControl](https://www.figma.com/design/Erj975S6vVP8PlHQius801/KOSMO?node-id=3801-8494), [PostActionBar](https://www.figma.com/design/Erj975S6vVP8PlHQius801/KOSMO?node-id=6604-48270); canonical spacing source: [PostListItem component set](https://www.figma.com/design/Erj975S6vVP8PlHQius801/KOSMO?node-id=1924-1992)
