...
- Reply·Repost·Reaction·Bookmark의 지속 처리 상태(default·pending·disabled), 선택적 count, callback 및 접근성 metadata 계약을 정의한다. 공개 제품 상태는 Reply의 controlled `expanded`, Repost의 `hasReposted`, Reaction의 `hasReacted`, Bookmark의 `hasBookmarked`로 표현하고 범용 `selected` prop을 노출하지 않는다. More는 callback과 접근성 label만 소비한다.
