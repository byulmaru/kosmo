...
- Reaction active·hover와 Repost hover·selected에 action별 semantic color를 사용하고, 미선택 Repost는 중립색으로 활성 상태와 구분하며 전역 Success semantic과 분리한다.
...
- 일반 Text·Media 목록 Post의 카드 상단 12px·하단 4px, 마지막 presentation(본문·미디어 또는 Reaction Summary)에서 Action Bar까지 8px 리듬을 canonical Figma target으로 기록하고, production 적용은 관련 Product 이슈와 별도 OpenSpec lifecycle로 분리한다.
...
- 게시글 렌더 경로는 route → `PostLayout`/`PostList` → `PostListItem`이며, `PostActionBar`는 `PostListItem`·`PostLayout` 내부의 final sibling으로 production surface에 배치돼 있다. `PostList`, route 또는 `actionBar?: ReactNode` seam을 추가하지 않는 기존 경계를 유지한다.
...
현재 production의 일반 Text·Media 목록 `PostListItem`은 카드 상단 `spacing.sm` 8px과 final slot 상단 0·하단 `spacing.xs` 4px을 유지한다. canonical Figma target은 Action Bar 자체 28px geometry를 유지하면서 카드 상단 `spacing.md` 12px·하단 `spacing.xs` 4px을 사용한다. content column의 기존 `spacing.xs` 4px gap 뒤 final slot 상단에 `spacing.xs` 4px을 추가하고 slot 하단은 0으로 두어 마지막 presentation(본문·미디어 또는 Reaction Summary)에서 Action Bar까지 8px, Action Bar에서 구분선까지 4px을 만든다. 이 OpenSpec·Figma 변경에서는 production 규범 spec과 코드를 수정하거나 runtime 완료를 주장하지 않는다. 같은 target의 production migration은 관련 Product 이슈를 확인하고 별도 OpenSpec spec·task를 연결한 뒤 진행한다. Quote와 순수 Repost는 기존 final slot 상단 0·하단 4px을 유지한다. Quote 목록은 nested Source preview 내부 하단 padding을 `spacing.xs` 4px로 줄이고 border 밖에서 Action Bar까지 `spacing.sm` 8px 간격을 유지한다. 상세 thread의 현재 Post는 current row 상단 `spacing.lg` 16px을 유지하고, Reaction Summary가 있으면 Summary와 Action Bar 사이를 `spacing.xs` 4px로 둔다. inline Reply Composer가 닫힌 상태에서는 빈 Composer wrapper를 렌더링하지 않으며 Action Bar 아래부터 다음 thread divider까지 current row wrapper가 `spacing.xs` 4px을 제공한다. 카드 구분선 두께는 1px을 유지하고 입력·메뉴 외곽선용 `border`보다 낮은 강도의 light/dark semantic `divider` color를 사용한다. 순수 Repost attribution은 `typography.sm`의 20px line box를 사용하고 Source 표준행과의 추가 gap을 두지 않는다. Web attribution Profile text link는 inline target 예외를 사용하며, Native target 복구와 인접 Source link 비중첩은 출시 전 runtime gate로 남긴다.
