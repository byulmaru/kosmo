...
- selected를 하나의 처리 상태 값으로 합치면 selected+pending을 표현할 수 없고 요청 중 선택 의미가 사라진다. 반대로 selected 색상을 disabled·error보다 우선하면 처리 가능 여부와 실패 상태가 가려지므로 처리 상태의 시각 표현을 우선한다.
...
- Reaction count 집계와 selected 의미는 PROD-417·PROD-418에서 결정한다. PROD-432의 실제 연결과 최종 archive는 해당 공개 계약 확정 뒤 진행한다.
