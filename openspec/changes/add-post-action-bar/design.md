...
- 같은 React Native 구현이 Android·iOS·Web에서 28 logical unit visual geometry와 접근성 metadata를 제공한다. Web actual target은 count가 있으면 숫자 `0`을 포함해 content를 HUG하고 count가 없으면 28×36px이며, social layout slot은 최소 50px으로 target이 더 넓을 때만 확장한다. Native target 복구는 출시 gate로 분리한다.
...
Bar와 각 control의 visual/layout row는 높이 28을 사용한다. Bar는 별도 좌우 inset 없이 Reply layout slot의 왼쪽 경계와 More slot의 오른쪽 경계를 PostBody가 사용하는 content column의 양끝에 맞추고, 고정 순서의 나머지 action을 `space-between`으로 분배한다. Reply·Repost·Reaction·Bookmark의 layout slot은 최소 50, More는 28이며 모든 glyph visual box는 16×16이다. 실제 Web target은 slot 가운데에 놓고 count가 있으면 숫자 `0`을 포함해 `왼쪽 6 + glyph 16 + gap 4 + 렌더된 count + 오른쪽 6`을 HUG한다. count가 없으면 target은 28×36이고 target이 50보다 넓을 때만 social slot도 함께 늘어난다. hover·pressed의 visible state layer는 count를 감싸지 않고 glyph 주위 28×28 원을 유지한다. pending spinner, selected·pressed·disabled visual은 같은 28px row 안에서 layout을 바꾸지 않는다. Bookmark 50 + gap 4 + More 28의 trailing group은 exact 82이고 모든 target은 action 사이 분배 여백이나 인접 target을 덮지 않는다. Android·iOS도 공용 visual geometry를 사용하지만, Native release 전에 iOS 44×44pt와 Android 48×48dp target을 복구하고 touch·VoiceOver·TalkBack runtime을 별도 검증한다. count는 저장소에 이미 사용 중인 `Intl.NumberFormat` compact notation 관례를 재사용해 실행 환경의 기본 locale에 맞게 표시하고, K/M 반올림·단위 승격·상한을 수동 구현하지 않는다. 색상·typography는 기존 theme token을 사용하고 icon은 기존 `lucide-react-native`에서 시각적으로 대응하는 glyph를 선택한다. 폭 검증은 기존 Storybook의 `kosmoMobile` 390px, `kosmoCompact` 900px, `kosmoFull` 1400px viewport에서 실제 목록·상세 surface 콘텐츠 폭과 한국어·영어 대표 compact fixture를 사용한다.
...
- [가변 폭 36px Web target에서 pending·selected·disabled 상태가 28px visual row나 인접 action 배치를 바꿀 수 있음] → 모든 상태에서 exact 28px visual row, count 기반 HUG 또는 count 없는 28×36 target, 50px minimum slot과 16px glyph slot을 Storybook geometry assertion으로 검증한다.
