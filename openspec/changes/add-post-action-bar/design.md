...
PROD-433의 Storybook은 Reply `expanded`, Repost `hasReposted`, Reaction `hasReacted`, Bookmark `hasBookmarked`와 각 액션의 기본·pending·disabled 조합, active Reaction·Bookmark의 채워진 icon, Reaction·Bookmark count 제외, 한국어·영어 compact count, count 없음, optional 액션, More callback-only와 390px·900px·1400px 폭을 각각 검토할 수 있게 한다. 처리 상태의 시각 표현은 도메인 상태의 primary 표현보다 우선하되 의미와 접근성 상태는 유지한다. 상호작용 검증은 default의 callback 호출, pending·disabled의 차단, controlled `expanded`, More callback, keyboard/touch activation, `액션 바` toolbar 이름과 내부 button 탐색을 확인한다. 이 단계에서는 `PostListItem`, `PostLayout`, route나 Relay 생성물을 수정하지 않는다.

PROD-434는 `PostListItem`과 `PostLayout`에 단일 presentation-only seam `actionBar?: ReactNode`를 추가하고 준비된 `PostActionBar`를 게시글 content column의 마지막 child로 배치한다. seam은 `PostActionBarProps`, action callback, Relay data 또는 adapter 상태를 받거나 구성하지 않는다. 일반 목록은 header와 `PostBody` Link 뒤, PROD-415의 Repost 목록과 Quote 목록은 각각 전체 `PostSourcePresentationView` 뒤, 상세는 `ProfileNameBlock`과 body·metadata wrapper 뒤에 Action Bar를 둔다. Action Bar wrapper는 본문·Source Link/Pressable과 body·metadata wrapper 밖의 sibling이며 `actionBar`가 `null` 또는 `undefined`이면 wrapper 자체를 렌더하지 않는다. 이 배치로 기존 게시글 navigation과 action Pressable이 중첩 활성화되지 않게 하고, 실제 action data를 연결하기 전 production caller에는 불완전한 Action Bar를 노출하지 않는다. production surface는 실제 adapter 연결 뒤 다섯 액션을 모두 제공하며, PROD-432의 surface adapter가 전달하는 disabled 상태를 optional prop 생략 없이 표현할 수 있게 한다. `PostList`의 production forwarding, 실제 Action Bar 상태·callback과 Relay fragment는 PROD-432에 남긴다. 기존 `Posts.stories.tsx` browser harness에서 일반·Repost·Quote·상세와 390px·900px·1400px 조합을 검증하며 새 test harness를 만들지 않는다. 각 조합은 Action Bar container·button과 인접 Profile·timestamp·본문·Source Link/Pressable의 bounding box가 양의 면적으로 교차하지 않고, Action Bar 활성화가 해당 mock callback만 호출하며 pathname·Linking·router navigation을 바꾸지 않는지 관찰한다. 구현 편집 전 local `HEAD`와 live PR #357 head의 일치를 확인하고, PR #358은 PROD-434에 합치지 않는다. #358 reconciliation은 live diff·conflict·명령·영향 파일·재검증 범위를 제시해 별도 사용자 승인을 받은 뒤 수행하며, 승인된 조정 뒤 상세 wrapper 폭과 전체 app 검증을 다시 실행한다.
