...
#### Scenario: Reply 제출 pending 소유 경계

- **WHEN** Reply Composer가 답글을 제출 중이다
- **THEN** Composer의 `답글 게시` 버튼이 spinner·입력 차단을 소유한다
- **AND** Action Bar Reply는 별도 pending spinner를 표시하지 않고 controlled `expanded`와 default·disabled 계약만 유지한다
...
- **WHEN** 사용자가 pending을 소유하는 Repost·Reaction·Bookmark 액션을 다시 활성화한다
...
- **AND** Web leading Reply actual target은 최소 50px slot의 시작에 맞고 Repost·Reaction·Bookmark actual target은 각 slot 가운데에 있다
...
- **AND** source navigation이 활성인 preview는 touch가 아닌 pointer hover 동안 root 전체에 semantic `stateHover` overlay를 표시하고 leave 뒤 resting 상태로 돌아간다
- **AND** 클릭 가능한 외부 링크는 mode별 semantic `actionLinkBase`와 밑줄을 사용하고 바깥 Source navigation 입력과 격리된다
- **AND** `interactive=false` preview와 touch pointer는 hover fill을 표시하지 않는다

#### Scenario: Post content timestamp alignment

- **WHEN** outer Post 또는 nested Source preview가 `PostSourcePresentationView`의 생성 시각 Link를 렌더한다
- **THEN** timestamp Link는 최소 44×44 target을 유지하고 label은 target 내부에서 오른쪽 정렬된다
- **AND** 별도 `PostListItem` timestamp의 정렬과 target 계약은 변경하지 않는다
...
#### Scenario: Reply 제출 처리 상태 연결

- **WHEN** Reply Composer가 답글을 제출한다
- **THEN** Composer의 `답글 게시` 버튼이 pending·spinner·중복 제출 차단을 표현한다
- **AND** production surface는 Action Bar Reply에 별도 pending 상태를 합성하지 않는다

#### Scenario: 액션별 pending 경계

- **WHEN** Repost·Reaction·Bookmark 중 한 액션 요청이 pending이다
...
- **THEN** production surface는 해당 액션의 pending을 종료하고 요청 직전의 확정된 `hasReposted`·`hasReacted`·`hasBookmarked`와 제공된 Repost count를 유지한다
- **AND** Reply 제출 실패는 Composer 계약에 따라 입력과 controlled `expanded`를 유지하며 Action Bar에 pending을 남기지 않는다
...
- **WHEN** component test가 Reply의 default·disabled와 pending을 소유하는 config action의 default·pending·disabled를 활성화한다
