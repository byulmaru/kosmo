...
- Gate fixture에서 phase, digest, recovery chain, rehearsal overdue 상태, restore point WAL archive, workload identity와 rollback window 성공/실패 경로를 검증한다.
...
- [x] 1.1 Immutable digest, phase, schema-change authority, rollback window, compatibility allowlist와 restore evidence를 받고 별도 approval input을 요구하지 않는 fail-closed migration gate interface를 구현한다.
...
- [x] 1.7 Credential/preflight/SQL/lock/timeout 실패, 같은 digest 재시도, forward recovery와 restore 판단 경계를 중복 contract 승인 없이 운영 runbook에 기록한다.
- [x] 1.8 Dev/prod render와 모든 gate failure fixture를 추가하고 별도 approval workflow 제거 후 관련 test, lint, format과 OpenSpec strict validation을 통과한다.
