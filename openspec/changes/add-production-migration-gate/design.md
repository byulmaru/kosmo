...
- Contract migration에만 별도 승인과 강화된 evidence gate를 적용한다.
...
- GitHub repository에는 아직 production environment가 없으므로 contract 전용 protected environment 설정은 구현·운영 prerequisite다.

### Recommended Approach

1. PROD-563의 일반 release workflow가 호출할 수 있는 독립 gate command 또는 reusable workflow interface를 둔다. 이 interface는 immutable image digest, migration phase, schema-change authority, rollback window 종료 시각, compatibility allowlist와 restore rehearsal evidence reference를 입력으로 받는다.
...
4. `contract`는 recovery window 안의 base backup과 연속 WAL chain, 월간 restore rehearsal의 overdue 여부, live active/preview/rollback workload digest와 rollback window를 검사한다. 이후 고유한 named restore point를 만들고 해당 target WAL의 archive 성공을 확인한다. 모든 자동 검사 뒤 별도 protected approval job을 통과해야 migration을 실행한다.
...
- Gate는 reusable GitHub Actions workflow 또는 repository script와 얇은 workflow adapter로 구현할 수 있다.
- Live workload 검사는 `kubectl` 또는 동일한 Argo Rollouts API 조회로 구현할 수 있다.
- Restore evidence는 승인된 workflow artifact나 deployment record로 대체할 수 있다. 단, evidence reference, 월간 rehearsal overdue 판정, 비민감 로그와 contract approver 검토 가능성을 유지해야 한다.
...
- Contract 전용 environment는 GitHub repository 설정이 필요하다. 코드만 병합하고 reviewer protection을 구성하지 않으면 contract path는 fail-closed 상태로 유지해야 한다.
...
2. PROD-564에서 gate interface, production migration Job 경계, phase/evidence/workload 검사, contract approval와 runbook/test를 구현한다.
