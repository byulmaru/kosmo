...
- Contract phase에 일반 production release 승인과 분리된 명시적 승인을 요구한다.
...
- `production-migration-gate`: Production release의 migration identity, credential, phase, backup/restore evidence, workload compatibility, contract 승인과 실패 차단을 검증하는 배포 계약.
...
- Operations: contract 승인 입력, backup/restore evidence reference, rollback window와 workload compatibility 확인 절차.
