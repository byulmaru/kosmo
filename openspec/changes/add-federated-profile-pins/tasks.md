## 1. PROD-809 Local pin domain and atomic mutation
...
Local Profile Owner가 승인된 Post를 최대 하나 고정·해제·교체할 수 있고, stale confirmation이 현재 pin을 훼손하지 않는다.
...
- 교체는 ModalSheet 확인 후 current expected value를 검증한 원자적 결과여야 한다.
- replacement expected-current 불일치는 저장 상태를 보존한 stale/conflict 결과여야 한다.
- 동일 pin과 이미 없거나 different target인 unpin은 idempotent no-op이어야 하며 다른 current pin을 제거하지 않는다.

**Verification**

- DB/core/API 테스트로 권한·자격, 단일 cardinality, atomic replacement, stale concurrent request, same-pin/unpin no-op을
  입력·결과·저장 상태로 검증한다. replacement stale/conflict 결과가 idempotent success와 구별되고 저장 상태를 보존하는지도 검증한다.

- [ ] 1.1 Local Profile pin/unpin의 eligibility, Owner 권한과 최대 1개 invariant를 구현한다.
- [ ] 1.2 교체·해제 mutation에서 current expected value를 검증하고 원자성·idempotent no-op을 보장한다.
- [ ] 1.3 approved replacement 확인과 실패·동시성 결과를 API 계약에 연결하고 focused DB/core 테스트를 통과시킨다.

## 2. PROD-809 Profile pinned-first list and pagination
...
Profile 목록이 visible pinned Post를 고정 순서(Local 0..1, Remote는 원격 collection 순서)로 먼저 표시하고, 일반 chronology와 결합된 서버 cursor/page
결과에서 중복·누락을 만들지 않는다.
...
- [ ] 2.1 Profile 목록의 pinned-first combined ordering과 Remote ordered set 소비를 구현한다.
...
## 3. PROD-809 Outbound ActivityPub Featured projection
...
## 4. PROD-809 Inbound Featured traversal and authoritative sync
...
- [ ] 4.2 Followers Only authenticated fetch에서 Active local follower identity와 author·audience·Follow 관계를 검증한다.
- [ ] 4.3 bounded authoritative sync·failure preservation·remote removal을 inbound 테스트로 검증한다.

## 5. PROD-809 Lifecycle, compatibility and runtime verification
...
Profile/Post lifecycle과 moderation 정책에 맞는 visible pin cleanup이 유지되고, Mastodon 호환 서버 기준 양방향 runtime
검증으로 공개·비공개와 pin lifecycle의 관찰 가능한 결과가 증명된다.

**Guardrails**

- deactivated/suspended/unavailable Profile, Tombstone/unavailable Post와 block/domain policy는 기존 조회 정책으로 숨긴다.
- Local current pin이 Tombstone/unavailable/author 또는 visibility eligibility 상실 상태가 되면 제품상 current pin으로 간주하지
...
- canonical domain validator, `openspec validate add-federated-profile-pins --strict`, 변경 파일 Prettier check와 `git diff --check`를
  스펙 PR에서 통과시킨다.
- 구현 PR에서 Mastodon 호환 서버 양방향 runtime으로 Public/Unlisted, Followers Only signed fetch, unsigned/non-follower
  denial, pin/unpin/update/sync/unfollow와 Profile/Post unavailable cleanup을 검증한다.

- [ ] 5.1 Profile/Post lifecycle, block/domain filtering과 성공 sync 이후 visible pin cleanup을 기존 조회 경계에 연결한다.
- [ ] 5.2 Mastodon 호환 서버 기준 양방향 federation runtime을 검증한다.
- [ ] 5.3 스펙·구현 PR의 검증 증거를 PROD-809 완료 조건에 연결한다.
