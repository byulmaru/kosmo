...
set을 서로 다른 lifecycle로 다뤄야 한다. Local pin은 기본적으로 set에 추가하고 지정한 항목만 제거한다. 임의 삽입·재정렬·
current-slot replacement는 reorder UI 계약이 생길 때 별도 도입한다. Followers Only는 일반
...
- Local pin/unpin의 자격·ordered add/remove와 idempotent no-op을 제공한다.
...
1. 기존 core/domain action 경계에서 Local pin 후보를 검증하고, pin은 ordered set에 추가하고 unpin은 지정한 항목만 제거한다.
   같은 대상과 이미 없는 해제는 현재 상태를 유지하는 idempotent 결과로 정규화한다. 새 pin의 앞·뒤 배치와 별도 재정렬 UX는
   고정하지 않으며, 임의 삽입·재정렬·current-slot replacement는 reorder UI 계약 이후 별도 도입한다.
...
3. Local Actor 표현에는 기존 Profile representation 경계에서 `featured` link를 추가하고, collection item은 기존
   Local Note projection과 authorization을 호출한다. pin/unpin transaction commit 후에는 기존 Profile Update(Person)
...
- Local pin 관계의 저장은 기존 Post/Profile persistence 계층에 맞는 별도 관계 또는 Profile projection 중 하나를
  사용할 수 있다. 어느 쪽이든 ordered 0..N API shape, additive add/unpin과 Remote ordered set 보존을 외부 동작으로 증명해야 한다.
...
- 임의 삽입·재정렬을 저장 계약에 선행하면 UI가 정하지 않은 순서 의미가 고정된다. reorder UI와 함께 별도 계약으로 도입한다.
...
- [pin과 unpin이 동시에 도착] → 각 mutation은 지정된 관계만 변경하고 unique profile/post 관계를 DB에서 보호한다.
