...
- Worker selector의 atomic URL·password Secret trio를 Web Rollout의 trusted federation ingress와 Temporal Worker Deployment에만 주입한다.
- PROD-710이 제공하는 명시적 Worker connection seam과 이전된 SQL callsite에 `WORKER_DATABASE_*` source를 wiring해 Web trusted federation ingress와 Temporal Worker DB Activity가 `kosmo_worker` credential을 사용하게 한다.
...
- `kosmo_worker` source만 독립적으로 선택하거나 owner fallback으로 되돌릴 수 있게 하며, production 전환은 PROD-369 역할·Secret, PROD-724 객체 GRANT, PROD-710 명시적 connection/SQL 경계가 준비된 뒤 별도 사용자 승인으로 수행한다.
...
- Temporal domain Workflow, Fedify MessageQueue runtime(PROD-448), API/Web BFF `kosmo_api` cutover(PROD-716), 역할·Secret·객체 GRANT 생성과 production sync/apply는 포함하지 않는다.
...
- Related foundation: 완료된 `PROD-706`, `PROD-730`

## Capabilities

### New Capabilities

- `worker-postgres-credential-cutover`: Web trusted federation ingress와 Temporal Worker DB Activity의 명시적 `kosmo_worker` connection, 독립 cutover·rollback 및 production 검증 계약.
...
- Web trusted federation ingress bootstrap과 Temporal Worker DB Activity bootstrap: PROD-710이 제공한 명시적 Worker connection factory/handle에 `WORKER_DATABASE_URL` source를 wiring한다. Connection 경계와 SQL callsite 이전 자체는 PROD-710의 선행 결과다.
...
- PROD-369/724/710 blocker가 남아 있는 동안 OpenSpec·Helm seam migration처럼 독립 검증 가능한 준비만 진행하며 실제 credential cutover와 live connection 검증은 완료 처리하지 않는다.
