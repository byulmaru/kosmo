...
- Chart가 기존 direct read-write Service를 가리키는 `kosmo_worker` URL과 PROD-369의 release별 `*-postgres-worker` / `password` Secret ref를 생성해 Web과 Temporal Worker workload의 기본 `DATABASE_*` source로 사용한다.
...
- Web/Worker workload 기본 `DATABASE_*` source, Secret 변경 시 Web/Worker restart, API 비주입, migration·queue·GraphQL operation PgBouncer 불변, Git revert rollback과 live role 검증.
