...
- Web/Worker process 기본 DB를 chart-derived 표준 `PGHOST`/`PGPORT`/`PGUSER`/`PGDATABASE`/`PGPASSWORD` Worker source로 전환한다.
...
- Worker credential에는 values 입력이 없다. Chart가 `PGHOST`를 기존 direct read-write Service로 생성하고 `PGPORT=5432`, `PGUSER=kosmo_worker`, `PGDATABASE=kosmo`를 고정하며 `PGPASSWORD` Secret ref는 PROD-369의 release naming을 재사용한다.
- Web process의 비GraphQL trusted 경로는 process 기본 표준 `PG*` env/전역 `db`를 공유한다.
