...
계획을 복원한다. 실제 지표 수집과 대시보드 구성은 구현 이후에 검증한다.

## What Changes

- production Web에서 인증된 화면 조회 시 선택 Profile과 멀티 Profile 자격을 함께 관측한다.
...
- 현재 canonical 정책과 Linear의 계산식·귀속·제외·개인정보 경계를 유지한다.
- PROD-795는 Done으로 확인했지만 PROD-555 자체 production 대조는 별도로 남는다.
- OpenSpec은 수정 가능한 세션 메모이며 새 요구사항이나 구현 승인 gate를 만들지 않는다.

## Verification

- Spec 단계: 복원 해시, 윤문 전후 계약·구조, OpenSpec strict validation, 포맷과 링크를 확인한다.
...
- Canonical: `docs/domain/policies/multi-profile-usage.md`,
  `docs/domain/records/2026-08-31-multi-profile-metrics-contract.md`
...
- Spec 복원과 현행화 이후 구현·실수집 검증은 별도 세션에서 계속한다.
