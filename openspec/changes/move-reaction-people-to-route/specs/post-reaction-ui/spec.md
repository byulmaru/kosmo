...
- **AND** Web 중앙 600px column, Full RightRail, Mobile BottomTabBar를 유지하고 route entry의 최초 header mount에서 제목으로 focus를 옮긴다
...
- **THEN** router history가 있으면 이전 화면으로 돌아가고 없으면 canonical Post 상세로 이동한다
- **AND** People 목록에서 프로필을 방문했다가 돌아오면 기존 navigation lifecycle을 통해 같은 Type과 People scroll 위치를 복원한다
