...
- E2E는 390px Home→People→Type 변경→프로필 방문→Back과 Home scroll 복원, guest 직접 진입의 순수 Repost·handle·Type 정규화를 확인한다.
...
- Web Back은 router history가 있으면 이전 화면으로 이동하고 history가 없으면 canonical Post 상세로 이동한다. People 목록의 프로필 방문 복귀는 기존 browser/navigation lifecycle의 Type·scroll 복원을 사용한다.
...
- E2E는 Type을 바꾼 뒤 People 목록을 실제 non-zero scroll 위치로 이동해 프로필을 방문하고, browser Back으로 Type·행·scroll 위치가 복원되는 경로를 포함한다.
- 앱 unit 585개와 route/screen focused unit, 변경 파일 ESLint, Prettier, `git diff --check`, OpenSpec strict validation이 통과했다. 전체 TypeScript check는 기존 `SettingsLinkRow.test.ts`의 unrelated `href` 타입 오류로 완료하지 못했다.

## 2026-09-16 route-owned 최초 제목 focus 후속 검증

- route component가 private `useRef` claim callback을 loading·error·success Header에 전달하고, route entry의 최초 Header mount만 Web·Native 제목 focus를 소비한다.
- 현재 worktree에서 `node scripts/test-db.mjs run -- pnpm test:e2e:database -- reaction-people.e2e.ts`를 실행해 단순화된 3개 시나리오가 통과했다.
- 같은 claim callback을 공유하는 Header remount의 1회 focus와 route Back/fallback은 focused app unit으로 확인하며, 실제 느린 조회 전환과 iOS·Android screen reader runtime은 실행하지 않는다.

## 2026-09-18 Stack 재구성 검증

- 최신 `PROD-938-route` 위로 `PROD-938` 변경을 재구성하고 앱 unit 622개, TypeScript, 관련 Storybook 26개, OpenSpec strict validation을 통과했다.
- `reaction-people.e2e.ts`는 두 번 실행했으나 두 실행 모두 테스트 시작 전 Playwright `webServer` 60초 대기에서 종료됐다. assertion 실패는 관찰되지 않았으며 E2E 통과 증거는 CI에서 다시 확인한다.
- Type 변경은 선택 tab의 focus와 현재 People scroll을 유지하고, 프로필 방문 후 Back은 기존 route scroll 위치를 복원하는 계약으로 문서와 E2E를 정렬했다.
