...
- 최상위 Profile Home의 loading·query error에서도 빈 제목 PageHeader와 기존 뒤로가기 action을 유지하고, 기존 ProfileHero skeleton·StateView retry 본문을 그대로 표시한다.
- 모바일 Web에서는 route PageHeader와 셸의 메뉴 전용 헤더를 중복 렌더링하지 않는다.
- 기존 ProfileHero, 게시물 본문, query lifecycle·retry, 관계 목록과 일반 PageHeader의 여러 줄 reflow는 유지한다.
...
- Profile query/API schema, ProfileHero·게시물·관계 목록 데이터와 query lifecycle에는 영향이 없으며, loading·query error는 route chrome만 추가한다. 외부 의존성에는 영향이 없다.
