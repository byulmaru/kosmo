...
#### Scenario: Hide request Node with unavailable participant

- **WHEN** request participant가 비활성 상태이거나 remote participant의 instance가 `SUSPENDED`이다
- **THEN** 시스템은 해당 request Node를 `null`로 반환한다
...
API는 request 승인·거절·취소 결과가 Relay cache에서 삭제된 request를 제거하고 영향받은 Profile을 갱신할 수 있는 payload를 반환해야 한다(MUST).
