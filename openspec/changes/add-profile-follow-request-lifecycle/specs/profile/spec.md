...
- **WHEN** active profile이 pending request의 followee이고 요청 거절을 실행한다
- **THEN** 시스템은 request를 삭제한다
- **AND** 삭제된 request ID와 follower/followee Profile을 반환한다

#### Scenario: Cancel outgoing request

- **WHEN** active profile이 pending request의 follower이고 요청 취소를 실행한다
- **THEN** 시스템은 request를 삭제한다
- **AND** 삭제된 request ID와 follower/followee Profile을 반환한다
...
#### Scenario: Reject transition with unavailable participant

- **WHEN** request participant가 비활성 상태이거나 remote participant의 instance가 `SUSPENDED`인 상태에서 승인·거절·취소를 실행한다
- **THEN** 시스템은 요청된 전이를 거부한다
- **AND** request, relation과 저장 count를 변경하지 않는다
...
#### Scenario: Hide request with unavailable participant

- **WHEN** pending request의 participant가 비활성 상태이거나 remote participant의 instance가 `SUSPENDED`이다
- **THEN** 시스템은 해당 request를 incoming/outgoing connection에서 반환하지 않는다
