...
- participant 가용성과 차단 상태는 승인에서만 요구하고, 거절·취소는 unavailable counterpart가 있어도 올바른 actor가 pending row를 삭제할 수 있게 한다.
...
- 삭제된 request Node를 payload로 반환하지 않는다. 승인은 opaque global ID와 영향받은 Profile을, 거절·취소는 opaque global ID만 반환한다.
...
- Integration test로 participant/non-participant access, 다른 Profile의 nullable connection, unavailable counterpart cleanup visibility, pagination, mutation 권한과 삭제 ID를 검증한다.
...
- [ ] 2.4 승인·거절·취소 mutation의 역할 검증, transition별 payload, unavailable counterpart cleanup, relation/count 결과와 not-found 재처리를 테스트하고 구현한다.
