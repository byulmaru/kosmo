...
- API `DATABASE_URL` direct client의 기존 PostgreSQL timeout startup 동작은 이 change의 범위 밖으로 두고 변경하지 않는다. Operation Pooler client는 PgBouncer가 지원하지 않는 server timeout startup/query parameter를 보내지 않으며, actor GUC만 하나의 session-level initialization SQL round trip에서 설정한 뒤 resolver를 시작한다. 연결 대기는 별도 숫자를 선택하지 않고 postgres.js의 기본 bounded connection timeout 동작에 맡긴다.
...
- API `DATABASE_URL`은 현재 request/auth/startup에서 사용하는 owner-compatible fallback인 direct `kosmo-postgres-rw`를 유지하며 향후 API/Web principal 방향을 결정하지 않는다. GraphQL operation 전용 `OPERATION_DATABASE_URL`만 `kosmo-postgres-pooler-rw:5432`를 사용한다. 기존 `postgres.credentials.api` trio가 구성된 경우에도 rendered env의 username, database와 password Secret source, scheme, path와 query는 현재 전환에서 재사용하고 operation URL의 host와 port를 포함한 authority만 chart 내 Pooler Service `:5432`로 교체한다. Runtime operation client는 URL query에서 세 server timeout key만 제거하고 unrelated query parameter는 유지한다. Web BFF baseline과 migration direct endpoint는 이 change에서 유지한다. #564의 CloudNativePG PgBouncer TLS/Vault/VSO static SCRAM 기반 선택적 trusted Worker `WORKER_DATABASE_*` seam은 이 change 범위 밖이며 `OPERATION_DATABASE_URL`에 절대 사용하지 않는다. API/Web principal transition은 PROD-716이 소유하고, 취소된 client-certificate/direct-rw 대안 PROD-470은 재개하지 않는다.
...
- Merge revision `de6034d3`의 dev activation에서 PgBouncer가 지원하지 않는 operation startup parameter가 거부되어 GraphQL이 초기화 단계에서 HTTP 500을 반환한 incident를 확인했다. 사용자 결정에 따라 전체 activation revert가 아닌 forward fix를 적용하며, endpoint·credential/Secret selector·Pooler CR 변경은 포함하지 않는다.
...
- `postgres-session-pool`: 기존 additive Pooler를 API의 `OPERATION_DATABASE_URL`만 사용하게 하고 API `DATABASE_URL`은 현재 owner-compatible direct fallback으로, Web BFF baseline과 migration은 direct endpoint로 유지한다. #564의 trusted Worker `WORKER_DATABASE_*` seam은 별도 실행 경계로 제외하고 `OPERATION_DATABASE_URL`에 연결하지 않는다. 실패 시 전체 activation merge/squash revision을 Git revert해 pre-activation tree로 되돌리며, PROD-728 Pooler 리소스는 유지한다.
...
- `apps/helm`: API current fallback direct `DATABASE_URL`, operation 전용 Pooler `OPERATION_DATABASE_URL`, Web BFF/migration direct baseline과 별도 Worker `WORKER_DATABASE_*` seam의 경계
