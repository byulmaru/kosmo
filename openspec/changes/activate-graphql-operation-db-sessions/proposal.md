...
- API `DATABASE_URL` direct client는 기존 PostgreSQL timeout startup 옵션을 유지하고, operation Pooler client는 PgBouncer가 지원하지 않는 server timeout startup parameter를 보내지 않는다. operation client는 actor GUC와 `idle_in_transaction_session_timeout`, `lock_timeout`, `statement_timeout`을 하나의 session-level initialization SQL round trip에서 설정한 뒤 resolver를 시작한다.
...
- API `DATABASE_URL`은 direct `kosmo-postgres-rw`를 유지하고 operation 전용 `OPERATION_DATABASE_URL`만 `kosmo-postgres-pooler-rw:5432`를 사용한다. 기존 `postgres.credentials.api` trio가 구성된 경우에도 rendered env의 username, database와 password Secret source, scheme, path와 query는 그대로 재사용하고 operation URL의 host와 port를 포함한 authority만 chart 내 Pooler Service `:5432`로 교체한다. Runtime operation client는 URL query에서 세 server timeout key만 제거하고 unrelated query parameter는 유지한다. 새 credential selector는 만들지 않으며 Web BFF, worker와 migration workload도 direct endpoint를 유지한다.
...
- Merge revision `de6034d3`의 dev activation에서 operation startup parameter가 PgBouncer에 거부되어 GraphQL이 초기화 단계에서 HTTP 500을 반환한 incident를 확인했다. 사용자 결정에 따라 전체 activation revert가 아닌 forward fix를 적용하며, endpoint·credential/Secret selector·Pooler CR 변경은 포함하지 않는다.
...
- runtime: CloudNativePG PgBouncer session pool, postgres.js connection 생성·종료, actor GUC·session timeout initialization과 cleanup 관찰
