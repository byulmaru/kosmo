...
- **AND** migration workload는 `<release>-postgres-rw`를 계속 사용한다
- **AND** PROD-715 Web/Worker 기본 principal은 `OPERATION_DATABASE_URL`에 공급되지 않는다
...
- **AND** PROD-715 Web/Worker 경계, migration workload, Cluster, direct Service와 PROD-728 Pooler 리소스는 유지된다
- **AND** GraphQL principal transition(PROD-716)은 변경되지 않는다
...
- **AND** PROD-715 Web/Worker 경계, migration, endpoint/credential Secret ref와 Pooler resource 설정은 변경되지 않는다
