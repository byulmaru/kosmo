...
PROD-728의 CloudNativePG Pooler는 dev/prod에 session mode와 `DISCARD ALL`로 배포돼 있으며 direct Service와 공존한다. PROD-370은 actor setting key와 fail-closed read helper를 제공한다. API `DATABASE_URL`은 request authentication과 startup/bootstrap을 위한 현재 owner-compatible fallback으로 direct Service를 유지하며, 향후 API/Web principal 방향은 결정하지 않는다. GraphQL operation connection만 `OPERATION_DATABASE_URL`을 통해 Pooler로 보내야 한다. `postgres.credentials.api` trio가 구성된 경우에도 rendered env의 username, database와 password Secret source, scheme, path와 query는 현재 전환에서 유지하고 operation URL의 host와 port를 포함한 authority만 in-chart Pooler Service `<release>-postgres-pooler-rw:5432`로 교체한다. Runtime operation client는 configured URL을 변경 없이 postgres.js에 전달하고 query parameter를 변경하거나 호환되지 않는 URL을 자동 보정하지 않는다. 따라서 configured URL은 이미 Pooler와 호환되어야 한다. Web BFF baseline과 migration direct endpoint는 이 change에서 유지한다. #564의 CloudNativePG PgBouncer TLS/Vault/VSO static SCRAM 기반 선택적 trusted Worker `WORKER_DATABASE_*` seam은 별도 실행 경계로 이 change에서 바꾸지 않으며 `OPERATION_DATABASE_URL`에 공급하지 않는다. API/Web principal transition은 PROD-716이 소유하고, 취소된 client-certificate/direct-rw 대안 PROD-470은 재개하지 않는다.
...
- API/Web이 현재 `kosmo.apiDatabaseUrl` helper를 사용하고 Worker에는 #564의 선택적 `WORKER_DATABASE_*` seam이 별도로 존재한다. API `DATABASE_URL`은 current owner-compatible direct fallback helper로 보존하고 GraphQL operation 전용 `OPERATION_DATABASE_URL`만 Pooler helper를 사용하도록 endpoint 표현을 분리해야 한다. Worker seam은 이 change에서 소비하거나 operation URL에 연결하지 않는다.
...
- Helm에 API current fallback direct `DATABASE_URL`과 GraphQL operation 전용 Pooler `OPERATION_DATABASE_URL` helper를 추가하고 API Rollout만 operation URL을 사용한다. 기존 API-role Secret 참조와 trio의 username/database/password source는 현재 전환에서 유지하며, configured trio에서도 rendered operation URL의 host와 port를 포함한 authority만 in-chart Pooler Service `<release>-postgres-pooler-rw:5432`로 교체하고 scheme, path와 query를 보존한다. Runtime operation client는 configured URL을 변경 없이 전달하며 호환되지 않는 query parameter나 URL을 자동 보정하지 않는다. Web BFF baseline과 migration URL helper는 direct Service를 계속 가리킨다. #564의 optional trusted Worker `WORKER_DATABASE_*` helper는 별도 실행 경계로 이 change에서 소비하지 않으며 operation URL에 연결하지 않는다. 새 credential selector는 만들지 않는다. PROD-716은 API/Web principal credential source/cutover를 후속으로 소유하고, Role/Secret provisioning, grant와 RLS policy는 각각의 별도 issue 경계로 남긴다.
...
3. Helm render에서 API `DATABASE_URL`은 current owner-compatible direct fallback, `OPERATION_DATABASE_URL`만 Pooler, Web BFF baseline과 migration은 direct URL이며 모든 해당 Secret 참조가 기존 값임을 확인한다. #564의 optional trusted Worker `WORKER_DATABASE_*` seam은 이 render/operation 검증에서 소비하지 않는 별도 경계로 기록한다.
