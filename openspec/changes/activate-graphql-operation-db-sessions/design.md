...
7. 실패 시 application credential이나 out-of-band Kubernetes resource를 변경하지 않고 전체 activation merge/squash revision을 Git revert해 pre-activation tree를 배포한다. API `DATABASE_URL`은 direct Service를 유지하고 `OPERATION_DATABASE_URL` env와 operation plugin/code는 제거한다. Web/worker/migration, Pooler와 Cluster는 유지하며 Pooler manifest를 prune하지 않는다.
8. PROD-716 credential·role·RLS policy/grant transition은 이 change의 live gate나 rollback에 포함하지 않고 별도 작업으로 남긴다.
