...
PROD-728의 CloudNativePG Pooler는 dev/prod에 session mode와 `DISCARD ALL`로 배포돼 있으며 direct Service와 공존한다. PROD-370은 actor setting key와 fail-closed read helper를 제공한다. API `DATABASE_URL`은 request authentication과 startup/bootstrap을 위한 현재 owner-compatible fallback으로 direct Service를 유지하며, 향후 API/Web principal 방향은 결정하지 않는다. GraphQL operation connection만 `OPERATION_DATABASE_URL`을 통해 Pooler로 보내야 한다. `postgres.credentials.api` trio가 구성된 경우에도 rendered env의 username, database와 password Secret source, scheme, path와 query는 현재 전환에서 유지하고 operation URL의 host와 port를 포함한 authority만 in-chart Pooler Service `<release>-postgres-pooler-rw:5432`로 교체한다. Runtime operation client는 URL query에서 `idle_in_transaction_session_timeout`, `lock_timeout`, `statement_timeout` 세 key만 제거하고 unrelated query parameter는 보존한다. Web BFF baseline과 migration direct endpoint는 이 change에서 유지한다. #564의 CloudNativePG PgBouncer TLS/Vault/VSO static SCRAM 기반 선택적 trusted Worker `WORKER_DATABASE_*` seam은 별도 실행 경계로 이 change에서 바꾸지 않으며 `OPERATION_DATABASE_URL`에 공급하지 않는다. API/Web principal transition은 PROD-716이 소유하고, 취소된 client-certificate/direct-rw 대안 PROD-470은 재개하지 않는다.

Merge revision `de6034d3`의 dev activation에서는 operation client가 PgBouncer가 지원하지 않는 server timeout startup parameter를 전달해 초기화 단계에서 거부됐고, GraphQL Query/Mutation이 HTTP 500으로 실패했다. Direct `DATABASE_URL` client의 기존 timeout startup 동작은 이 change의 범위 밖으로 두고 유지한다. Pooler operation client는 지원되지 않는 server timeout startup/query parameter를 보내지 않고 actor GUC만 session-level initialization SQL 한 번에 설정해야 한다. 이 incident에 대한 사용자 결정은 전체 activation revert가 아닌 forward fix이며 endpoint, credential/Secret selector, Pooler CR·replica·resource·capacity는 변경하지 않는다.
...
- direct postgres.js client의 기존 server timeout startup 동작은 이 change의 범위 밖으로 두고 유지한다. Pooler operation client는 PgBouncer가 지원하지 않는 server timeout startup/query parameter를 제거하고 actor GUC만 initialization SQL round trip에서 session-level로 설정해야 하며, 성공 전에는 resolver를 실행하지 않는다.
...
- core DB 모듈에 기존 postgres.js connection 옵션과 Drizzle schema를 재사용하는 좁은 operation client factory를 둔다. factory는 `max: 1`과 postgres.js 기본 bounded connection timeout 동작을 사용하는 새 postgres.js client, 그 client의 Drizzle `Database`, idempotent async close를 함께 반환한다. application-selected timeout 숫자는 추가하지 않는다.
...
- 없다. Pooler 호환성을 위해 PgBouncer가 지원하지 않는 server timeout startup/query parameter를 제거하고 actor GUC만 하나의 initialization SQL round trip에서 설정한다. 연결 대기는 postgres.js 기본 bounded connection timeout 동작에 맡기며 application-selected timeout 숫자는 추가하지 않는다.
...
- [operation startup parameter 호환성 오류가 dev GraphQL을 초기화 단계에서 실패시킴] → operation client에서 PgBouncer가 지원하지 않는 server timeout startup/query parameter를 제거하고 actor GUC만 한 session SQL round trip으로 설정한다. current API Pod 로그의 unsupported startup-parameter 오류가 없고 기존 GraphQL smoke가 성공할 때까지 live gate를 닫지 않는다.
