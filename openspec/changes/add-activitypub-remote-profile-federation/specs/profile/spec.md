...
로그인한 계정은 자신과 연결된 활성 `LOCAL` instance profile만 현재 세션의 active profile로 선택할 수 있어야 한다(MUST).

#### Scenario: Select accessible active local profile

- **WHEN** 로그인한 계정이 자신과 연결된 활성 `LOCAL` instance profile 선택을 요청한다
...
- **THEN** 시스템은 양쪽 프로필이 모두 노출 가능한 활성 profile인 follow 관계만 집계한다
...
active profile이 있는 인증자는 기존 `LOCAL` profile follow 관계를 해제할 수 있어야 한다(MUST).
