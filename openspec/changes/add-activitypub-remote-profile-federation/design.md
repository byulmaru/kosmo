...
- **remote lookup의 HTTP 자원 한계는 Fedify 설정으로 통일한다.** remote actor materialization은 10초 취소 신호와 1 MiB 응답 본문 한계를 Fedify의 WebFinger, actor document, JSON-LD context loader에 전달한다. alternate document 추적도 같은 취소 신호를 유지하며, kosmo는 content-type·redirect·SSRF 검증이나 별도 HTTP parser를 중복 구현하지 않는다.
