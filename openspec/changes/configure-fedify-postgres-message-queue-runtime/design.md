...
최신 main의 PROD-715 PR #564는 legacy `postgres.credentials.fedify`와 `FEDIFY_DATABASE_*`를 제거하고 Web trusted ingress 및 Temporal Worker DB Activity 전용 `postgres.credentials.worker`와 `WORKER_DATABASE_*` source로 교체했다. PROD-715는 Fedify MessageQueue를 명시적으로 제외하고, dev `kosmo_worker`는 아직 domain table 권한이 없으므로 queue transport는 release의 기존 CloudNativePG direct read-write Service와 전용 database에서 파생한 queue connection을 사용하되 consumer domain listener는 현재 Web trusted ingress와 같은 owner `DATABASE_URL`을 별도로 유지한다. `kosmo_worker` GRANT와 explicit handle 전환은 PROD-724/710/715에 남긴다. 현재 GraphQL API process도 outbound core effect를 직접 호출하므로 API producer에도 같은 전용 queue connection이 필요하다. PROD-706은 취소됐고 PR #543도 merge 없이 닫혔으므로 generic Fedify DB execution-context seam을 구현 근거로 사용하지 않는다.
...
- dev와 production queue transport는 각 namespace의 기존 CloudNativePG cluster 안의 별도 `kosmo_fedify_queue` database와 전용 login/Secret을 사용한다. chart는 environment 분기나 database 준비 flag 없이 같은 리소스를 선언하고, adapter는 schema option을 제공하지 않으므로 domain database 안의 custom schema, `search_path` helper 또는 adapter table migration을 만들지 않는다. production resource apply와 최초 adapter initialization은 별도 승인 전 시작하지 않는다.
...
5. 장기 실행 process와 probe/signal lifecycle은 `apps/fedify-consumer`가 소유한다. 공통 image에 이 app을 실행하는 별도 `fedify-queue` command를 추가하고 Helm의 기존 application workload bootstrap 경계 안에 Web/Temporal Worker와 독립된 Fedify consumer Deployment를 둔다. Service/Ingress는 만들지 않고 replica/resource/probe를 별도로 render한다.
6. exact adapter의 implicit `initialize()`가 queue connection 대상 database 안의 기본 table/index를 소유하게 하고 Drizzle domain migration, custom DDL 또는 transport ledger를 만들지 않는다. chart는 각 namespace의 기존 CloudNativePG cluster에 별도 `kosmo_fedify_queue` database와 전용 login/Secret을 항상 선언하고 queue URL·Secret ref를 파생하며, test는 격리 DB에서 초기화를 검증한다. production resource apply와 최초 producer/consumer 활성화는 별도 승인한다.
...
8. Helm은 queue 전용 environment/database/producer/consumer flag 없이 queue resource를 선언하고, 기존 `workloads.enabled` application bootstrap 경계 안에서 API/Web connection과 consumer Deployment를 함께 렌더한다. package-level runtime mode, custom config parser와 startup `getDepth()` probe를 추가하지 않고 official adapter의 enqueue/listen lazy initialization과 오류를 그대로 사용한다.
...
- [implicit adapter DDL이 의도하지 않은 database를 변경] → dev와 production 모두 별도 `kosmo_fedify_queue` database와 전용 login/Secret으로 범위를 격리하고, production resource apply와 최초 activation은 하나의 명시적 승인 대상으로 제시한다.
...
- [direct와 queue producer가 동시에 활성화될 수 있음] → chart가 API/Web에 queue connection을 항상 하나만 주입하고 direct fallback을 두지 않아 이중 발송을 막는다.
...
3. local/CI에서 adapter implicit initialization, dequeue 전 queued persistence, 기존 ordering option 보존, shared inbox recipient 병합, retry/failure, graceful shutdown과 분기 없는 dev/prod Helm render를 검증한다.
4. 구현 PR은 production mutation이나 domain Workflow 완료 없이 Ready로 만들고 PR completion evidence와 미실행 dev live/production verification을 구분한다. dev sync 전 Activity는 기존 delivery request 실패를 Temporal retry 경계에 남길 수 있다.
...
6. production chart의 queue database/credential과 runtime 선언을 먼저 review 가능한 diff로 제공한다. 실제 production에서는 현재 database/credential과 backlog 상태를 다시 확인하고, 별도 승인된 Vault value 준비 뒤 chart sync로 queue database apply·consumer 최초 listen·producer queue connection을 함께 전환한다. 전환 뒤 Activity 성공은 queue 수락이며 remote HTTP retry/order는 Fedify만 소유한다. rollback은 이전 Git revision으로 chart 전체를 되돌리되 queue table/message를 임의 삭제하지 않는다.
