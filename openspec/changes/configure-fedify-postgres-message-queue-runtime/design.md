...
- dev와 production queue transport는 각 환경의 기존 CloudNativePG cluster 안의 별도 `kosmo_fedify_queue` database와 전용 login/Secret을 사용한다. adapter는 schema option을 제공하지 않으므로 domain database 안의 custom schema, `search_path` helper 또는 adapter table migration을 만들지 않는다. production 선언은 default-off이며 실제 resource apply와 최초 adapter initialization을 시작하지 않는다.
...
6. exact adapter의 implicit `initialize()`가 queue connection 대상 database 안의 기본 table/index를 소유하게 하고 Drizzle domain migration, custom DDL 또는 transport ledger를 만들지 않는다. dev와 production chart는 각 환경의 기존 CloudNativePG cluster 안에 별도 `kosmo_fedify_queue` database와 전용 login/Secret을 default-off로 선언하고, test는 격리 DB에서 초기화를 검증한다. production resource apply와 최초 producer/consumer 활성화는 별도 승인한다.
...
- [implicit adapter DDL이 의도하지 않은 database를 변경] → dev와 production 모두 별도 `kosmo_fedify_queue` database와 전용 login/Secret으로 범위를 격리하고, production declaration은 default-off로 두며 resource apply와 최초 activation은 하나의 명시적 승인 대상으로 제시한다.
...
6. production chart의 default-off queue database/credential 선언을 먼저 review 가능한 diff로 제공한다. 실제 production에서는 현재 database/credential과 backlog 상태를 다시 확인하고, 별도 승인된 Vault value sync와 queue database apply → consumer 최초 activation/implicit initialization → producer queue cutover 순서로 진행한다. producer cutover 뒤 Activity 성공은 queue 수락이며 remote HTTP retry/order는 Fedify만 소유한다. 각 단계는 이전 direct path 또는 disabled consumer로 rollback 가능해야 한다.
