...
최신 main의 PROD-715 PR #564는 legacy `postgres.credentials.fedify`와 `FEDIFY_DATABASE_*`를 제거하고 Web trusted ingress 및 Temporal Worker DB Activity 전용 `postgres.credentials.worker`와 `WORKER_DATABASE_*` source로 교체했다. PROD-715는 Fedify MessageQueue를 명시적으로 제외하고, dev `kosmo_worker`는 아직 domain table 권한이 없으므로 queue transport는 release와 session PgBouncer에서 파생한 전용 queue connection을 사용하되 consumer domain listener는 현재 Web trusted ingress와 같은 owner `DATABASE_URL`을 별도로 유지한다. `kosmo_worker` GRANT와 explicit handle 전환은 PROD-724/710/715에 남긴다. 현재 GraphQL API process도 outbound core effect를 직접 호출하므로 API producer에도 같은 전용 queue connection이 필요하다. PROD-706은 취소됐고 PR #543도 merge 없이 닫혔으므로 generic Fedify DB execution-context seam을 구현 근거로 사용하지 않는다.
