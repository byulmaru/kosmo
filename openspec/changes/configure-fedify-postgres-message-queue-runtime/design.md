...
5. 장기 실행 process와 probe/signal lifecycle은 `apps/fedify-consumer`가 소유한다. 공통 image에 이 app을 실행하는 별도 `fedify-queue` command를 추가하고 Helm에 Web/Temporal Worker와 독립된 Fedify consumer Deployment를 둔다. Service/Ingress는 만들지 않고 replica/resource/probe를 별도로 render한다.
...
8. Helm은 queue 전용 environment/database/producer/consumer flag 없이 queue resource, API/Web connection과 consumer Deployment를 함께 렌더한다. package-level runtime mode, custom config parser와 startup `getDepth()` probe를 추가하지 않고 official adapter의 enqueue/listen lazy initialization과 오류를 그대로 사용한다.
