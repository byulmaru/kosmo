...
- queue runtime은 명시적으로 활성화될 때 공식 adapter가 connection 대상 database 안의 queue table/index를 implicit하게 초기화하게 한다. dev와 production은 각 환경의 기존 CloudNativePG cluster 안에 별도 `kosmo_fedify_queue` database와 전용 login/Secret 선언을 제공하고, queue connection은 domain/API DB 및 Worker execution credential과 분리한다. production 선언은 default-off이며 실제 resource apply, Vault value sync와 최초 활성화는 별도 승인에 남긴다.
...
- dev와 production Helm에는 default-off `kosmo_fedify_queue` Database/DatabaseRole과 환경별 VSO basic-auth Secret 동기화 선언이 추가되며 실제 Vault value, resource apply와 workload activation 전에는 database를 생성하지 않는다.
...
- 취소된 PROD-706/PR #543 코드를 cherry-pick하거나 재구현하지 않는다. queue consumer의 domain listener는 현재 main의 trusted ingress DB 동작을 보존하며, API/Worker runtime role·credential cutover는 제외한다. production default-off 선언은 포함하지만 Argo CD sync, Helm apply, Vault value write와 DB production apply는 이 change의 자동 실행 범위가 아니다.
