...
- 단위 테스트는 기준 초과 landscape/portrait, 정사각형, 기준 이하와 정확한 경계값을 포함하고, 변환 adapter의 호출 인자와 PUT body/header 연결을 검증한다. 실제 플랫폼 decode·WebP 결과는 Web와 Android/iOS 검증에서 별도로 확인한다.
...
3. Web 실제 업로드와 Android/iOS 실제 이미지 변환·업로드를 확인한 뒤 Stack layer를 Ready로 전환한다.
