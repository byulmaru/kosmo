...
- remote actor outbox traversal/backfill은 이번 change에서 다루지 않는다. durable/shared Fedify KV backend 교체나 activity ID의 별도 DB 저장, 복수 `Create.object`, reply/thread reconstruction, private/followers-only/direct addressing, duplicate Create의 visibility 변경, attachment/media 저장 또는 proxy, boost/announce/like, Note content warning projection은 후속 변경으로 남긴다.
