...
- remote Note content는 PROD-259가 media type에 따라 PROD-341의 versioned canonical ProseMirror document JSON과 Content Warning으로 projection하고, Fedify `LanguageString`은 locale을 저장하지 않고 문자열 값만 사용한다. remote HTML 원본과 파생 Plain Text는 저장하지 않으며 content가 없는 Note도 canonical empty document로 저장한다.
