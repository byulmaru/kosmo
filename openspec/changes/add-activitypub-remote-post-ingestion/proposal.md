...
- 같은 object URI가 같은 remote actor에서 다시 전달되면 기존 `Post`를 재사용하고 content, visibility를 갱신하되 `Post.createdAt`은 최초 materialization 이후 수정하지 않는다.
...
- shared inbox로 온 public `Create(Note)`도 actor, attribution, public top-level 검증을 통과하면 저장 대상이다.
- remote Note content에서 plain text와 단순 TipTap JSON projection을 저장하고 remote HTML 원본은 저장하지 않는다.
...
- remote actor outbox traversal/backfill은 이번 stack에서 다루지 않는다. reply/thread reconstruction, private/followers-only/direct addressing, media proxy, boost/announce/like, Note content warning projection은 후속 변경으로 남긴다.
...
- `activitypub-actor-discovery`: actor-scoped inbox와 shared inbox가 verified `Create(Note)` delivery도 Fedify listener로 받아 post ingestion handler로 위임하도록 discovery 404 경계를 좁힌다.
...
- `packages/fedify`: actor-scoped inbox와 shared inbox에서 Fedify inbox listener가 verified typed `Create(Note)`를 post materialization handler에 전달한다.
