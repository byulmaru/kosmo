...
- public/unlisted Note는 local recipient 없이 최초 저장하고, followers/direct Note는 접근 가능한 active local follower 또는 mentioned Profile이 있을 때만 최초 저장한다. 이미 저장된 same-visibility Note에서는 새 addressing에 local relevance가 없어져도 stale mention 접근을 제거한다.
...
- remote actor outbox traversal/backfill은 이번 change에서 다루지 않는다. durable/shared Fedify KV backend 교체나 activity ID의 별도 DB 저장, unknown actor/mention lookup 스팸 제한, 복수 `Create.object`, reply/thread reconstruction, `bto`/`bcc`와 임의 collection addressing, duplicate Create의 visibility 변경, `Update(Note)`/`Delete(Note)`, attachment/media 저장 또는 proxy, boost/announce/like, mention notification, local compose mention, Note content warning projection은 후속 변경으로 남긴다.
