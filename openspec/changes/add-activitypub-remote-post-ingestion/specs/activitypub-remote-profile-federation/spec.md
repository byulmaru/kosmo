...
시스템은 federation 내부 actor materialization 흐름에서 federated handle을 Fedify lookup으로 해석하거나, verified inbound activity actor URI를 Fedify `ctx.lookupWebFinger(actorUri)` URL-resource lookup으로 검증한 뒤 remote ActivityPub actor를 kosmo `Profile`로 materialize해야 한다(MUST).
...
#### Scenario: Materialize verified inbound activity actor URI

- **WHEN** Fedify inbox listener가 verified inbound activity를 전달하고 activity author URI가 저장된 ActivityPub actor로 조회되지 않는다
- **THEN** 시스템은 Fedify `ctx.lookupWebFinger(inboundAuthorUri)` URL-resource lookup으로 inbound author URI에 대응하는 `acct:{handle}@{domain}` subject를 조회한다
- **AND** WebFinger 결과의 ActivityPub self link는 inbound author URI와 URL `.href` 문자열로 정확히 일치해야 한다
- **AND** URL actor document를 직접 fetch할 수 있는 `ctx.lookupObject(inboundAuthorUri)`만으로는 이 WebFinger 검증을 대체하지 않는다
- **AND** 검증된 `acct:` identity와 federated handle을 기존 remote actor materialization service의 입력으로 사용한다
- **AND** materialization 결과 actor의 canonical URI도 inbound author URI와 정확히 일치해야 한다
- **AND** `activitypub-remote-post-ingestion`은 local recipient 또는 기존 follow 관계가 없어도 object hydration 전에 이 author materialization을 시도할 수 있다
- **AND** verified inbound author의 existing instance가 `UNRESPONSIVE`이면 시스템은 이를 reachability signal로 보고 instance를 `ACTIVE`로 갱신한 뒤 materialization을 계속할 수 있다
- **AND** existing instance가 `SUSPENDED`이면 시스템은 lookup/materialization을 수행하지 않는다

#### Scenario: Materialize unknown mentioned actor URI

- **WHEN** verified inbound Note의 `toIds` mention actor URI가 저장된 local 또는 ActivityPub actor로 조회되지 않는다
- **THEN** 시스템은 author와 같은 Fedify `ctx.lookupWebFinger(mentionActorUri)` URL-resource lookup, `acct:` subject, ActivityPub self-link 검증과 remote actor materialization 경계를 사용한다
- **AND** mention actor lookup은 best-effort이며 개별 lookup 실패가 author 또는 Note materialization 전체를 실패시키지 않는다
- **AND** mention actor는 activity author가 아니므로 existing `UNRESPONSIVE` instance를 inbound reachability signal로 `ACTIVE` 전환하지 않는다
- **AND** existing instance가 `SUSPENDED` 또는 `UNRESPONSIVE`이면 해당 mention actor materialization을 수행하지 않는다

#### Scenario: Reject actor URI without federated handle lookup

- **WHEN** remote actor URI만 주어지고 federated handle lookup 또는 verified inbound URI의 WebFinger/self-link 검증을 통과하지 않았다
...
- **AND** verified inbound activity author의 reachability 예외가 적용되지 않는다
- **THEN** 시스템은 Fedify lookup을 수행하지 않고 remote actor materialization을 실패 처리한다
...
시스템은 remote ActivityPub actor를 기존 kosmo `Profile` 필드와 ActivityPub actor metadata로 투영해야 한다(MUST).
...
- **AND** 시스템은 actor `followersId?.href`를 nullable `ActivityPubActor.followersUri`로 저장한다
- **AND** 시스템은 followers collection URI를 actor URI path에서 추론하지 않는다
