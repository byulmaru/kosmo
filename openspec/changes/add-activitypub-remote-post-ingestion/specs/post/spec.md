...
#### Scenario: mentioned profile의 비공개 게시글 object 조회

- **WHEN** 현재 active profile이 게시글의 `post_mention` 관계에 연결되어 있고 `FOLLOWERS` 또는 `DIRECT` 공개 범위의 활성 게시글 Node를 조회한다
- **THEN** 시스템은 `Post` object를 반환한다

#### Scenario: 접근 권한 없는 viewer의 비공개 게시글 object 조회

- **WHEN** 인증되지 않은 클라이언트가 `FOLLOWERS` 또는 `DIRECT` 공개 범위의 게시글 Node를 조회한다
- **OR** 현재 active profile이 게시글 작성자나 `post_mention` 대상이 아니고, `FOLLOWERS` 게시글에서는 작성자를 established `ProfileFollow`로 팔로우하지도 않는다
...
API는 활성 게시글의 current 또는 historical 콘텐츠 revision을 GraphQL `PostContent` Node로 노출하고 versioned canonical document, 파생 Plain Text와 선택적 Content Warning을 제공해야 한다(MUST). `Post.content`는 current revision을 가리키되 `PostContent` Node 직접 조회는 parent `Post.state`가 `ACTIVE`인 경우 historical revision도 반환할 수 있어야 한다(MUST).
...
- **THEN** 시스템은 `post_mention`을 포함한 viewer의 공통 parent `Post.visibility` 접근 조건과 작성자 profile/instance visibility를 적용한다
...
- **WHEN** 클라이언트가 `ACTIVE`가 아닌 게시글의 current 또는 historical `PostContent` Node를 직접 조회한다
- **THEN** 시스템은 해당 `PostContent` object를 노출하지 않는다

#### Scenario: SUSPENDED instance 리모트 게시글 콘텐츠 직접 조회

- **WHEN** 클라이언트가 remote ActivityPub Note에서 materialized된 게시글의 `PostContent` Node를 직접 조회하고 해당 게시글 작성자 instance 상태가 `SUSPENDED`이다
- **THEN** 시스템은 해당 `PostContent` object를 노출하지 않는다
- **AND** 이 조건은 current와 historical `PostContent` revision에 모두 적용된다

#### Scenario: 비활성 작성자 프로필의 게시글 콘텐츠 직접 조회

- **WHEN** 클라이언트가 작성자 `Profile.state`가 `ACTIVE`가 아닌 게시글의 current 또는 historical `PostContent` Node를 직접 조회한다
- **THEN** 시스템은 해당 `PostContent` object를 노출하지 않는다
- **AND** 이 조건은 parent `Post.state`와 `Post.currentContentId` 일치 여부와 관계없이 적용된다

### Requirement: 프로필 게시글 목록 connection

API는 local profile 또는 ActivityPub remote profile이 작성한 활성 게시글을 `Post.id` 기반 최신순 Relay connection `Profile.posts`로 노출해야 하며, viewer와 작성자의 관계에 따라 공개 범위를 제한해야 한다(MUST). `Profile.posts`는 게시글 node 목록 공용 wrapper인 `PostConnection`을 반환해야 한다(MUST).

#### Scenario: 공개 로컬 프로필 게시글 목록 조회

- **WHEN** 인증되지 않았거나 현재 active profile이 조회 대상 local profile과 다르고, 조회 대상을 established `ProfileFollow`로 팔로우하지 않으며, 해당 프로필 게시글의 `post_mention` 대상도 아닌 클라이언트가 local profile의 `posts` connection을 조회한다
...
- **AND** `DIRECT` 공개 범위는 현재 active profile이 해당 게시글의 `post_mention` 대상인 게시글만 반환한다
- **AND** 게시글은 최신순으로 정렬된다
- **AND** connection은 cursor 기반 페이지네이션을 지원한다

#### Scenario: mentioned viewer의 프로필 게시글 목록 조회

- **WHEN** 현재 active profile이 local 또는 ActivityPub remote profile의 `posts` connection을 조회한다
- **AND** 현재 active profile이 해당 프로필이 작성한 `FOLLOWERS` 또는 `DIRECT` 게시글의 `post_mention` 대상이다
- **THEN** 시스템은 해당 게시글을 반환한다
- **AND** 이 접근은 현재 active profile이 작성자를 established `ProfileFollow`로 팔로우하지 않아도 허용된다
...
- **AND** 게시글은 기존 connection 계약과 같이 `Post.id` 기준 최신순으로 정렬된다
- **AND** connection은 cursor 기반 페이지네이션을 지원한다
- **AND** connection cursor는 기존 계약과 같이 `Post.id`를 기준으로 하고 remote `Post.createdAt` 또는 ActivityPub 원본 `published`를 ordering/cursor primitive로 사용하지 않는다
...
- **WHEN** 인증되지 않았거나 현재 active profile이 조회 대상 프로필과 다르고, 조회 대상을 established `ProfileFollow`로 팔로우하지 않으며, 해당 게시글의 `post_mention` 대상도 아닌 클라이언트가 프로필의 `posts` connection을 조회한다
- **THEN** 시스템은 `FOLLOWERS`, `DIRECT` 공개 범위의 게시글을 반환하지 않는다

### Requirement: Home timeline connection

API는 현재 active profile 기준 홈 타임라인을 `Post.id` 기반 최신순 Relay connection `Query.homeTimeline`로 노출해야 한다(MUST). `Query.homeTimeline`은 게시글 node 목록 공용 wrapper인 `PostConnection`을 반환해야 한다(MUST). active profile이 없거나 인증되지 않은 조회에는 요청을 거부하지 않고 `null`을 반환해야 한다(MUST).
...
- **THEN** 시스템은 해당 remote followee에서 inbox delivery로 materialized된 `PUBLIC`, `UNLISTED`, `FOLLOWERS` 공개 범위의 `ACTIVE` 게시글을 반환한다
- **AND** `DIRECT` 공개 범위는 현재 active profile이 해당 게시글의 `post_mention` 대상인 게시글만 반환한다
- **AND** 게시글은 기존 connection 계약과 같이 `Post.id` 기준 최신순으로 정렬된다
- **AND** connection cursor는 기존 계약과 같이 `Post.id`를 기준으로 하고 remote `Post.createdAt` 또는 ActivityPub 원본 `published`를 ordering/cursor primitive로 사용하지 않는다
...
- **WHEN** active profile이 있는 인증자가 `homeTimeline` connection을 조회하고 현재 active profile이 아닌 다른 작성자를 established `ProfileFollow`로 팔로우하지 않는다
- **THEN** 시스템은 해당 작성자의 게시글을 반환하지 않는다

#### Scenario: 비팔로우 author의 mention 게시글 제외

- **WHEN** active profile이 있는 인증자가 다른 remote author의 materialized 게시글 `post_mention` 대상이지만 작성자를 established `ProfileFollow`로 팔로우하지 않는다
- **THEN** 시스템은 mention 관계만으로 해당 게시글을 `homeTimeline` 후보에 추가하지 않는다
- **AND** 시스템은 mention notification 또는 별도 mention 목록을 이번 capability에서 제공하지 않는다
