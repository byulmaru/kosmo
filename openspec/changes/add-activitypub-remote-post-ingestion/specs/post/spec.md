...
API는 게시글의 current 또는 historical 콘텐츠 revision을 GraphQL `PostContent` Node로 노출하고 TipTap JSON 원본, Plain Text projection, 선택적 스포일러 텍스트를 제공해야 한다(MUST). `Post.content`는 current revision을 가리키되 `PostContent` Node 직접 조회는 historical revision도 반환할 수 있어야 한다(MUST).
...
- **WHEN** 클라이언트가 current revision이 아닌 historical `PostContent` Node를 직접 조회한다
- **THEN** 시스템은 parent `Post.state` 또는 `Post.currentContentId`와 관계없이 해당 `PostContent` object를 반환할 수 있다
...
- **AND** 게시글은 최신순으로 정렬된다
- **AND** connection은 cursor 기반 페이지네이션을 지원한다
- **AND** 이번 capability는 `Profile.posts`의 기존 connection ordering/pagination primitive 또는 cursor tie-breaker를 재정의하지 않는다
...
- **AND** `DIRECT` 공개 범위의 게시글은 반환하지 않는다
- **AND** 게시글은 최신순으로 정렬된다
- **AND** 이번 capability는 `homeTimeline`의 기존 connection ordering/pagination primitive 또는 cursor tie-breaker를 재정의하지 않는다
