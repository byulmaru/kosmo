...
API는 local profile 또는 ActivityPub remote profile이 작성한 활성 게시글을 최신순 Relay connection `Profile.posts`로 노출해야 하며, viewer와 작성자의 관계에 따라 공개 범위를 제한해야 한다(MUST).

#### Scenario: 공개 로컬 프로필 게시글 목록 조회

- **WHEN** 인증되지 않았거나 현재 active profile이 조회 대상 local profile을 `ACCEPTED` 상태로 팔로우하지 않는 클라이언트가 local profile의 `posts` connection을 조회한다
...
#### Scenario: accepted follower의 로컬 프로필 게시글 목록 조회

- **WHEN** 현재 active profile이 조회 대상 local profile을 `ACCEPTED` 상태로 팔로우하고 해당 프로필의 `posts` connection을 조회한다
...
- **THEN** 시스템은 remote actor outbox에서 materialized된 `ACTIVE` 게시글을 반환한다
- **AND** materialized 게시글이 없거나 stale이면 시스템은 Fedify dereference/vocabulary API를 통해 remote outbox refresh를 시도할 수 있다
- **AND** 반환되는 게시글은 viewer가 볼 수 있는 공개 범위로 제한된다
- **AND** 게시글은 최신순으로 정렬된다
- **AND** connection은 cursor 기반 페이지네이션을 지원한다

#### Scenario: 리모트 outbox refresh 실패

- **WHEN** 활성 ActivityPub remote profile의 `posts` connection 조회 중 Fedify outbox refresh가 실패한다
- **THEN** 시스템은 이미 materialized된 노출 가능한 remote posts를 반환할 수 있다
...
- **THEN** 시스템은 Fedify outbox refresh를 시도하지 않는다
...
#### Scenario: accepted local followee 게시글 포함

- **WHEN** active profile이 있는 인증자가 `homeTimeline` connection을 조회하고 현재 active profile이 다른 활성 local profile을 `ACCEPTED` 상태로 팔로우한다
- **THEN** 시스템은 해당 followee가 작성한 `PUBLIC`, `UNLISTED`, `FOLLOWERS` 공개 범위의 `ACTIVE` 게시글을 반환한다
- **AND** `DIRECT` 공개 범위의 게시글은 반환하지 않는다
- **AND** 게시글은 최신순으로 정렬된다

#### Scenario: accepted remote followee 게시글 포함

- **WHEN** active profile이 있는 인증자가 `homeTimeline` connection을 조회하고 현재 active profile이 활성 ActivityPub remote profile을 `ACCEPTED` 상태로 팔로우한다
- **THEN** 시스템은 해당 remote followee에서 materialized된 `PUBLIC`, `UNLISTED`, `FOLLOWERS` 공개 범위의 `ACTIVE` 게시글을 반환할 수 있다
- **AND** `DIRECT` 공개 범위의 게시글은 반환하지 않는다
- **AND** 게시글은 최신순으로 정렬된다
