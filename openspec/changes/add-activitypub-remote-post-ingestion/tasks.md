...
- object mapping은 object type, actor FK와 actor URI를 중복 저장하지 않고 Post CASCADE를 사용한다.
...
- [ ] 2.1 PostgreSQL `uuidv7()` primary-key default와 최소 ActivityPub Post mapping table의 Drizzle schema/export를 추가한다.
- [ ] 2.2 unique URI/Post, Post CASCADE, receivedAt과 nullable publishedAt metadata를 migration에 반영한다.
...
actor-scoped/shared Fedify listener가 지원 delivery를 검증하고 최초 object mapping, Post와 canonical PostContent를 하나의 PostgreSQL transaction 결과로 저장한다.
...
- [ ] 4.6 object mapping/Post/first PostContent/currentContent를 하나의 transaction에서 commit/rollback한다.
