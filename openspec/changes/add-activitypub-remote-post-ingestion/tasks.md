...
- [ ] 2.1 `ActivityPubObjectType.NOTE`, PostgreSQL UUIDv7 primary key와 mapping table의 Drizzle schema/export를 추가한다.
- [ ] 2.2 unique URI/Post, actor index+RESTRICT, Post CASCADE와 nullable published metadata를 migration에 반영한다.
- [ ] 2.3 DB migration/catalog test와 reset/push 검증을 통과시키고 receipt나 미래 policy가 추가되지 않았는지 확인한다.
