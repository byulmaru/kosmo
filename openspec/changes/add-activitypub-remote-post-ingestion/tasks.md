...
- [ ] 2.1 remote-profile foundation이 저장한 remote actor outbox endpoint metadata를 읽어 Fedify vocabulary/dereference API로 public Note 또는 Create(Note)를 순회하는 `packages/fedify` adapter를 구현한다.
...
- [ ] 4.2 GraphQL post test로 remote `Profile.posts`, outbox refresh 실패 stale read, `UNRESPONSIVE` instance outbox refresh 미시도, `homeTimeline` 포함 여부를 검증한다.
