...
모든 implementation slice가 object URI 기반 public first-ingestion 계약으로 통합되고, 실제 materialized row가 기존 GraphQL authorization과 zero-network read 계약을 지키는지 검증한 뒤 canonical specs와 archive를 완료한다.
...
- PROD-260이 실제로 materialize한 mapping/Profile/Post/PostContent row로 Post Node, current/historical PostContent, Profile.posts와 homeTimeline을 검증한다.
- ACTIVE/UNRESPONSIVE allow, SUSPENDED/inactive deny, local visibility/order/cursor와 remote fetch 0회를 검증한다.
- object URI당 Post 하나, loser rollback, canonical spec sync, 전체 task 완료, strict validation과 workspace required checks를 확인한다.

- [ ] 5.1 PROD-341/255/259/260 PR 병합과 scoped validation 증거를 확인한다.
- [ ] 5.2 personal/shared concurrent delivery가 같은 object URI에서 Post side effect를 한 번만 commit하는지 검증한다.
- [ ] 5.3 실제 materialized row와 network-fail spy로 네 GraphQL surface의 parent authorization, current/historical content visibility와 zero-network read를 검증한다.
- [ ] 5.4 remote followee home 포함, non-followee 제외와 `Post.id DESC` ordering/cursor 회귀를 검증한다.
- [ ] 5.5 PROD-341 canonical document/Content Warning과 initial timestamp가 existing GraphQL schema로 조회되는지 smoke 검증한다.
- [ ] 5.6 canonical activitypub-remote-post-ingestion/data-model specs와 delta를 동기화하고 기존 Post 계약에 diff가 없으며 receipt/duplicate-update 계약이 남지 않았는지 확인한다.
- [ ] 5.7 proposal 전체 scope 완료 후 change를 archive하고 archive 후 strict validation을 통과시킨다.
