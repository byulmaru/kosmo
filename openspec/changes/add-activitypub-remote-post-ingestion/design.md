...
- **ActivityPub object URI가 remote post identity다.** object URI를 unique로 저장하고, 같은 remote actor에서 다시 발견하면 기존 `Post`를 재사용하면서 content와 visibility를 갱신한다. 최초 `Post`, `PostContent`, object mapping 생성은 하나의 transaction으로 수행하고 object URI unique conflict가 발생하면 부분 생성 row를 남기지 않는다. 이미 저장된 object mapping의 actor가 이번 delivery actor와 다르면 기존 `Post`, `PostContent`, object mapping을 갱신하지 않는다. kosmo `Post.id`는 내부 GraphQL/DB identity로 유지한다.
...
- **PostContent는 projection이다.** remote HTML 원본은 저장하지 않고 `bodyHtml`은 `null`로 둔다. HTML 또는 source content에서 만든 plain text projection을 `bodyText`로 저장하고, `bodyJson`은 plain text 기반 단순 TipTap document로 채운다. 같은 actor의 동일 object URI가 content 변경과 함께 재전달되면 새 `PostContent` revision을 만들고 `Post.currentContentId`를 교체하며, content projection이 같으면 기존 revision을 재사용한다. 최초 object mapping의 수신 시각, 원본 published 시각과 기존 `Post.createdAt`은 재전달로 갱신하지 않는다.
