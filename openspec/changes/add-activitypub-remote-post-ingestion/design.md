...
- **public top-level Note만 materialize한다.** `as:Public` 대상이 명확하고 `inReplyTo`가 없는 Note만 저장한다. `as:Public`이 primary audience이면 `PUBLIC`, secondary audience이면 `UNLISTED`로 저장할 수 있다. followers-only, private, direct, reply, addressing이 불분명한 Note는 저장하지 않는다.
