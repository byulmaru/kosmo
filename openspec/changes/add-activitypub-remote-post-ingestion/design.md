...
- **published fallback은 `Post.createdAt` projection에만 적용한다.** Note `published`가 있으면 `Post.createdAt`으로 저장하고 ActivityPub object mapping에도 원본 published 시각을 저장한다. 새 remote post 최초 저장 시 `published`가 없으면 수신 시각을 `Post.createdAt` fallback으로 사용하되, ActivityPub object mapping의 원본 published 시각은 `null`로 둔다. 이후 같은 object URI가 재전달될 때 `published`가 없으면 기존 `Post.createdAt`을 유지한다.
