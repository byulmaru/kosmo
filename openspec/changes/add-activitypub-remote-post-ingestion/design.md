...
- **PostContent는 revision projection이다.** 새 remote `Post`는 `ACTIVE`로 만들고, 최초 및 변경 revision의 `PostContent.createdAt`은 해당 delivery 수신 시각으로 둔다. primary Note `content`가 없으면 빈 `bodyText`와 빈 TipTap document를 저장한다. content가 있고 `mediaType`이 없거나 `text/html`이면 `@kosmo/core/tiptap`의 server-side HTML helper가 `@tiptap/html` `generateJSON()`과 기존 `Document`/`Paragraph`/`Text` extensions로 TipTap document를 만든 뒤 정규화하고 `bodyText`를 추출한다. `text/plain`이면 trim한 text와 기존 plain-text-to-TipTap helper를 사용하고, 그 외 media type 또는 TipTap 변환 실패이면 Note를 skip한다. attachment는 이번 change에서 저장하지 않으므로 attachment-only Note도 빈 본문으로 materialize한다. remote HTML 원본은 저장하지 않고 `bodyHtml`은 `null`로 둔다. 같은 actor의 동일 object URI가 재전달되면 persisted `bodyText`가 달라진 경우에만 새 `PostContent` revision을 만들고 `Post.currentContentId`를 교체한다. `bodyText`가 같으면 기존 revision을 재사용한다. current와 historical revision은 모두 GraphQL `PostContent` Node로 직접 조회할 수 있고, 직접 조회는 parent `Post.state` 또는 `Post.currentContentId`를 따르지 않는다. 최초 object mapping의 수신 시각, 원본 published 시각, visibility와 기존 `Post.createdAt`은 재전달로 갱신하지 않는다.
