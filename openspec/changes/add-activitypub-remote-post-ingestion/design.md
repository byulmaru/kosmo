...
- 최초 ActivityPub Post mapping, Post와 PostContent를 PostgreSQL에서 원자적으로 저장한다.
...
- PROD-354의 초기 전달은 spec-only였으며 코드, DB migration, generated GraphQL schema와 runtime behavior를 변경하지 않았다. 이후 구현 PR은 자기 코드와 scoped verification을 소유하고, 실제 materialized row GraphQL smoke, canonical spec sync와 archive는 PROD-256이 소유한다.
...
- PostgreSQL unique ActivityPub Post URI constraint는 object URI당 mapping을 하나로 제한할 수 있다.
- mapping은 materialization 결과만 나타내므로 raw ActivityPub type을 저장하지 않고, 작성자는 `Post.profileId`에서 도출한다.
...
Fedify inbox listener
  ...
  -> identity/visibility/timestamps + canonical document를 PROD-260 transaction
       ...
       unique ActivityPub Post mapping
...
- PROD-255는 raw type과 actor FK가 없는 unique ActivityPub Post URI/Post mapping을 제공한다.
...
- remote Post 전용 resolver, ActivityPub Post mapping join과 request-time fetch를 추가하지 않는다.
- PROD-256은 PROD-260이 실제로 materialize한 mapping/Profile/Post/PostContent row로 existing authorization compatibility matrix와 DB-only smoke를 검증한다.
...
- PROD-256의 actual-row smoke와 spy 구성은 달라질 수 있지만 production resolver/schema diff 없이 DB-only read를 검증해야 한다.

### Known Traps

- object URI unique constraint가 있는데 별도 activity receipt를 추가하면 같은 중복 문제를 두 identity로 소유하게 된다.
- 지원 여부를 validation에서 이미 결정한 뒤 raw ActivityPub type을 mapping enum으로 다시 저장하면 확장 가능한 protocol vocabulary와 materialization 결과를 불필요하게 결합한다.
- 작성자를 `activityPubActorId`로 mapping에 다시 저장하면 `Post.profileId -> ActivityPubActor.profileId` 관계와 같은 identity를 중복 소유한다.
...
3. PROD-255/259 구현 PR은 각 schema/projection 결과를 전달한다.
4. PROD-260이 known-actor public Create validation과 PROD-255/259 결과를 최초 materialization transaction으로 통합한다.
5. PROD-256이 실제 materialized row GraphQL smoke, 전체 slice, canonical spec sync와 archive를 검증한다.
...
- **기존 resolver 결함**: PROD-256 actual-row smoke가 결함을 발견하면 implementation scope를 다시 열고 integration gate에서 조용히 수정하지 않는다.

## Migration Plan

- schema 변경은 PROD-255의 additive ActivityPub Post mapping migration만 소유한다.
