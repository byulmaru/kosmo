...
Kosmo repository의 approved app export와 deploy channel handoff가 Kosmo repository secret `EXPO_OTA_SIGNING_PRIVATE_KEY`를 local reusable workflow의 required `signing_private_key` input으로 전달하고, local workflow가 이를 `byulmaru/expo-ota` multipart publisher에 전달해 signed immutable release를 선택된 `dev` 또는 `prod` channel에 발행하며, static R2 read-only delivery가 이를 제공하는 pipeline과 runbook. Vault는 signing private key의 원본 보관소로 유지하고 Vault와 Kosmo repository secret은 rotation 때 동기화한다. 최초 repository secret 등록은 2026-09-10 10:06:24 UTC에 확인했다. 이후 rotation 동기화와 publish 실행 결과는 남은 운영 evidence로 관리한다. public `byulmaru/expo-ota` repository와 static R2에는 private key를 저장하지 않고 common publisher는 signing storage path·field·provider·backend를 고정하거나 직접 조회하지 않는다. release promotion과 known-good recovery reissue는 이번 구현에서 보류한다.
