...
- `runtimeVersion`은 자동 fingerprint가 아닌 명시적으로 관리하는 수동 호환성 세대 값으로 사용하며, 초기 세대는 문자열 `"0.2"`이다. Native compatibility가 변경될 때마다 세대를 올리고 새 native binary를 배포하며, JS/assets-only OTA는 같은 세대를 유지한다. 호환되지 않는 OTA는 새 native binary와 새 runtime generation 없이는 publish하지 않으며 `EXPO_UPDATES_FINGERPRINT_OVERRIDE`를 사용하지 않는다.
...
- publisher는 `multipart/mixed` manifest의 JSON part를 정확한 바이트로 서명하고 새 asset을 먼저 사전 계산한 SHA-256 표준 Base64를 R2 `PutObject`에 전달해 서버 검증하는 방식으로 업로드한다. 이미 존재하는 immutable asset은 `IfNoneMatch: "*"`의 412 응답으로 재사용하며, 모든 asset의 upload 성공 또는 기존 존재 확인 뒤 고정 tuple manifest object를 갱신한다. 이번 구현에서는 channel별 직접 발행 연결까지만 다루며, release promotion과 known-good recovery reissue는 후속 운영 범위로 보류한다. offline·실패 검증을 운영 결과로 남긴다.
