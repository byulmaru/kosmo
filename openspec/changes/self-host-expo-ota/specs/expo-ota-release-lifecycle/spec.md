...
**Authority / Provenance:** `PROD-331`, `PROD-332`, `PROD-334`, `PROD-335`; the publisher contract is owned by public `byulmaru/expo-ota`. Each deploy-selected release MUST use the same canonical project/platform/safe-channel-segment/manual-runtime-generation tuple, MUST sign the exact multipart manifest JSON part, and MUST verify every referenced asset before replacing the fixed tuple manifest object. The initial manual runtime generation is the string `"0.2"`; native compatibility changes MUST increment it and require a new native binary, while JS/assets-only OTA changes MUST keep the current generation. An incompatible OTA MUST NOT be published without a new native binary and incremented generation, and `EXPO_UPDATES_FINGERPRINT_OVERRIDE` MUST NOT be used. An incomplete artifact or failed verification MUST not be published as an applicable OTA update; native requirements go through a new Store binary.
...
- **AND** the evidence identifies the platform, binary, manually assigned runtime generation, OTA channel, and release identity used
