...
- Local Delete가 Create Activity보다 먼저 commit되면 기존 Delete handoff가 삭제된 Post의 보존된 projection으로 Create를 먼저 enqueue하고 같은 Note ordering key로 Delete를 뒤이어 enqueue한다. Create queue handoff 중 Delete가 commit되면 row lock으로 외부 I/O를 감싸지 않고 handoff 뒤 상태를 재확인해 같은 ordering key의 보정 Create/Delete 쌍을 덧붙인다.
