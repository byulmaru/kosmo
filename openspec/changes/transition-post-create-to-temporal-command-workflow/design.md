...
공통 core module은 필수 `TEMPORAL_ADDRESS`와 `TEMPORAL_NAMESPACE`를 import/startup 시 검증하고, `Connection.lazy`를 사용하는 process-global 실제 Temporal `Client`를 직접 export한다. 설정 누락은 요청을 받기 전 startup failure이며 Post별 start failure로 격리하지 않는다. `createPost(input)` action은 자체 transaction을 commit한 뒤 실제 생성 결과에만 이 client와 공유 start policy를 사용해 committed Post ID의 effects Workflow start를 직접 시도하고 최종 Post 결과를 반환한다. client getter나 start 전용 helper는 두지 않는다. explicit `origin`은 Workflow input으로 전달한다. 실행 중인 동일 ID와 충돌하면 기존 execution으로 수렴하고, 종료된 동일 ID는 상태와 관계없이 재사용하지 않는다. startup이 완료된 뒤의 연결 또는 `workflow.start()` 실패는 action 내부에서 관측하고 격리해 기존 Post/GraphQL/ActivityPub 성공을 유지한다. API resolver와 Fedify handler는 database handle이나 후속 callback을 전달·호출하지 않는다.
...
Accepted Workflow는 Reply Notification과 Local-origin Fedify queue handoff를 별도 Activity로 실행한다. 두 효과가 모두 적용될 수 있는 경우 서로 독립적으로 시작하고 각 Activity의 retry/최종 실패가 다른 효과를 선행 차단하지 않게 결과를 함께 수집한다. Reply Notification materialization은 Temporal SDK나 Worker runtime state에 의존하지 않는 transport-neutral state change이므로 전용 Core service 파일의 함수 하나로 둔다. 이 함수가 process 기본 `db`로 committed Post를 다시 조회하고 공통 Post visibility policy를 재사용해 기존 recipient·self suppression·visibility·uniqueness 정책과 멱등 insert를 직접 소유한다. Worker는 별도 wrapper나 Activity 전용 Core 계층 없이 같은 함수를 Activity 이름으로 alias해 직접 등록한다. Delivery Activity는 기존 canonical Local Note/Activity identity, audience/target과 queue producer를 재사용하며 `origin=ACTIVITYPUB`이면 생성하지 않는다. Activity가 Post를 다시 읽을 때 이미 `DELETED`이면 Create handoff는 no-op이다. 기존 Delete handoff는 삭제된 Post의 보존 content를 과거 Create로 합성하지 않으며, Create handoff 뒤 상태를 다시 읽어 보정 Create/Delete 쌍을 추가하지 않는다. 별도 delivery receipt가 없는 현재 범위에서는 수신자가 Create를 관측했는지 판별하거나 queue-level exactly-once를 보장하지 않는다. acknowledgement가 모호하면 같은 Activity ID의 중복 queue message나 HTTP request가 가능하고, 수신 측 ActivityPub idempotency로 의미상 수렴한다.

Production Worker entrypoint는 compile-time registration을 별도 module 없이 inline으로 소유하고 process-global host를 한 번 시작한다. 별도 `runWorker`/`startWorker` callable lifecycle은 두지 않는다. health server, Temporal connection, Worker와 signal handler도 같은 entrypoint가 단독 소유한다. Helm은 유효한 release image가 주어지는 모든 환경에서 application workloads를 항상 render하고, chart-wide 또는 Worker별 activation key를 사용하지 않는다. Workflow producer인 GraphQL API와 ActivityPub queue consumer는 같은 환경별 Temporal endpoint·namespace를 받으며, shared Fedify registration이 inbound handler를 정적으로 import하는 Web runtime도 direct Client의 startup 계약을 충족하도록 같은 설정을 받는다. Web 자체는 Workflow를 시작하지 않는다. API에는 Worker database credential을 주입하지 않는다. Worker DB principal·Secret wiring은 이 change에서 바꾸지 않고 표준 process `db`를 소비한다.
...
- [Create Activity 전이나 queue handoff 중 Local Delete가 commit됨] → Activity 조회 시 이미 deleted면 Create를 보내지 않는다. active projection이 이미 queue handoff를 시작한 뒤의 경쟁은 별도 delivery receipt 없이 복구하지 않으며, deleted projection·보정 Create/Delete 쌍·외부 queue I/O 동안의 Post row lock을 추가하지 않는다.
