...
- [Create Activity 전이나 queue handoff 중 Local Delete가 commit됨] → 이미 deleted 상태면 Create Activity와 Delete handoff가 보존된 projection으로 Create와 Delete를 순서대로 넣는다. active projection의 Create handoff 중 삭제되면 handoff 뒤 상태 재확인이 같은 ordering key의 보정 Create/Delete 쌍을 덧붙이고, 보정 전 종료되면 Temporal retry가 같은 쌍을 append한다. 외부 queue I/O 동안 Post row lock은 유지하지 않는다.
