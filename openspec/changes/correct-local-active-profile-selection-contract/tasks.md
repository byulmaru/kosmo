...
active profile 선택 계약이 정상 제품 경로에서 계정에 연결되는 local profile만을 대상으로 하며, 런타임 동작은 변경하지 않는다.
...
- [x] 1.1 canonical profile spec과 actor-discovery 전체 산출물을 local-only 계약으로 동기화하고 spec-only 검증을 통과시킨다.
