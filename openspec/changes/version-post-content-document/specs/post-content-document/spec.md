...
### Requirement: V1 canonical PostContent document schema

시스템은 `PostContent` 본문을 schema version이 식별된 ProseMirror document JSON으로 저장하고 V1을 실제 `prosemirror-model` schema로 검증해야 한다(MUST).

#### Scenario: V1 schema 구성

- **WHEN** 시스템이 schema version `1`인 PostContent document를 검증한다
...
- **WHEN** 서버가 외부 또는 저장 경계에서 V1 document JSON을 받는다
- **THEN** 시스템은 V1 `Schema.nodeFromJSON()`으로 node를 생성하고 `Node.check()`로 구조를 검증한다
...
시스템은 의미가 같은 V1 입력이 같은 canonical document JSON을 만들도록 line ending, 빈 node, 인접 text, mark와 URL을 결정적으로 정규화해야 한다(MUST).
...
시스템은 canonical document에서 Plain Text를 결정적으로 파생하고 schema version, canonical document와 Content Warning 의미로 revision equality를 판정해야 한다(MUST).
...
- **AND** projection은 읽기, 검색, 접근성, 500자 validation에 사용할 수 있지만 별도 canonical 저장값이 아니다

#### Scenario: 같은 version revision equality

- **WHEN** 두 revision의 schema version이 모두 `1`이다
- **THEN** 시스템은 두 document를 canonicalize한 구조적 결과와 nullable Content Warning을 비교한다
- **AND** raw JSON 문자열, key ordering 또는 별도 Plain Text projection을 identity로 사용하지 않는다
- **AND** canonical document와 Content Warning이 모두 같을 때만 같은 의미로 판정한다

#### Scenario: schema version이 다른 revision equality

- **WHEN** 두 revision의 schema version이 다르다
...
- **WHEN** 앱이 schema version `1`의 유효한 document를 표시한다
...
- **WHEN** 앱이 알 수 없는 schema version, node, mark, attr 또는 안전하지 않은 link를 받는다
