...
Profile은 Local·Remote origin과 기존 Follow lifecycle을 제공하지만, target Profile이 이전 Remote source를 준비 관계로 가리키거나 inbound ActivityPub `Move`로 follower를 새 target에 옮기는 경계는 아직 없다. 현재 구현은 Core의 Profile identity·Follow 서비스, Fedify의 Remote actor materialization과 inbound handler, Temporal directed Follow pair Workflow, App Settings Profile detail이 각각 이 경계를 소유한다.
...
- source Followee의 Local established follower만 target Follow 또는 Request로 target-first 이전하고, 기존 target·실제 중복 admission·실패·중단 뒤에는 source를 보존한다.
...
- Profile 저장 모델과 Core service에는 현재 Profile Migration 관계가 없다. 관계를 추가하더라도 Profile의 기존 lifecycle·origin·policy를 재사용해야 한다. Account.Active와 Profile.Owner authorization은 GraphQL `withAuth({ profileRole: OWNER })` 경계가 소유하며, Core service는 `ctx.session.profile.id`에서 파생된 target Profile ID와 source Profile ID만 받아 source·pair 조건을 검증하고 Account·membership authorization이나 추가 target eligibility를 다시 수행하지 않는다.
...
- `packages/core/services/profile-follow*`와 `packages/core/temporal/follow-command.ts` 및 Worker pair Workflow는 directed Follow lifecycle을 소유한다. target Follow/Request가 실행 시작 시 이미 있으면 exact pair 상태에 수렴시키고 source cleanup을 시작하지 않는다. 사전 조회 뒤 concurrent transition이 `created: false`를 반환해도 source cleanup을 시작하지 않는다. source removal은 같은 실행에서 target transition이 `created: true`를 반환한 경우에만 기존 Unfollow·Undo lifecycle로 호출한다. Remote target의 Open policy도 기존 Local-to-Remote Follow effect semantics를 사용하고, HTTP receipt은 source cleanup의 별도 완료 조건이 아니다. 이를 위해 새 durable row·ledger를 기본값으로 만들 필요는 없다.
...
- App Settings는 `SettingsProfileDetail`에서 selected Profile과 membership role을 조회한다. source 등록 mutation은 이 selected Profile을 target으로 사용하고 별도 target Profile ID를 제출하지 않는다. Settings navigation과 Account `featureFlags` 조회 경계는 현재 별도이므로, 구체적인 flag key나 새 범용 registry를 이 change가 미리 고정하지 않는다.

### Recommended Approach

1. GraphQL mutation은 기존 `withAuth({ profileRole: OWNER })`로 Account.Active와 selected Profile Owner 권한을 확인하고 `ctx.session.profile.id`를 target으로 고정한다. source qualified handle은 기존 remote actor materialization으로 검증·구체화하고, materialization context는 selected target의 `canonicalOrigin`에서 만든다. Core service에는 target/source Profile ID만 전달해 source validation과 target/source pair uniqueness를 확인하고 준비 관계를 저장한다. 별도 target eligibility preflight나 Core target 재검증은 추가하지 않는다. 같은 pair는 기존 결과를 반환하는 no-op으로, 다른 pair는 conflict로 분류한다. 저장 관계는 기존 Profile identity를 참조하는 additive row와 source·target 양쪽 uniqueness 제약을 최소 단위로 삼고, 기존 Profile/Follow row를 재작성하지 않는다.
...
3. Fedify inbound listener에 Move 처리 경계를 연결한다. 인증된 actor/object의 canonical equality와 target canonical actor의 exact alias를 먼저 확인하고, source Remote Profile을 materialize한 뒤 target origin별 policy를 평가한다. remote-to-local은 준비된 Local target의 기존 Follow Approval Policy를, remote-to-remote는 기존 Remote target policy를 사용한다.
4. Move coordinator는 source Followee의 기존 established Follow 중 Local follower를 한 번에 제한된 batch/cursor로 admission한다. 각 pair의 실행 시작 시 target Follow/Request가 없을 때 기존 Follow lifecycle로 target 상태를 먼저 확정하고, transition 결과의 `created`가 `true`일 때만 기존 removal/Unfollow·Undo lifecycle로 source Follow를 제거한다. 기존 target 상태, concurrent transition의 `created: false`, target 저장 실패에서는 source를 유지한다. target 생성 뒤 source 제거 전에 실행이 중단되면 Temporal retry가 existing target으로 수렴해 source cleanup을 재개하지 않을 수 있다. 새 migration ledger나 operation receipt를 전제로 하지 않는다.
...
- handler는 prepared Local target이면 exact source/target 관계와 Local target identity/state를 확인하고, Remote target이면 수신 context에서 canonical target Actor를 조회해 exact source alias를 확인한다. source는 기존 `findOrMaterializeRemoteProfileActorByUri` 경계와 실제 context로 materialize한 뒤 기존 Temporal start 경계로 넘긴다. embedded target 표현만으로 Remote target의 alias를 신뢰하지 않는다.
- Move Workflow state는 source/target Profile ID와 선택적인 `afterSourceFollowId` cursor로 제한된다. 한 번에 최대 50개의 active Local established follower를 `ProfileFollows.id` 오름차순 keyset으로 읽고, 각 항목은 기존 Follow transition과 exact-row removal 경계를 사용한다. target row/request가 이미 있거나 transition이 `created: false`를 반환하면 다시 만들지 않고 source를 보존하며, 같은 실행에서 `created: true`인 경우에만 source removal을 시도한다. self-follow는 건너뛰되 cursor는 진행한다.
...
- source Follow를 target 저장보다 먼저 삭제하지 않는다. 기존 target relation/request, concurrent `created: false`, PENDING·terminal 재시도에서는 source cleanup을 시작하지 않고 보존한다. 같은 실행에서 `created: true`인 경우에만 source removal을 호출한다.
...
- [source materialization, alias projection과 Follow transfer가 서로 다른 경계에서 실패할 수 있다] → identity validation과 target-first 결과를 단계별로 검증하고, target admission 결과가 `created: true`일 때만 같은 실행에서 source removal을 호출한다. 중단 뒤 Temporal retry가 existing target을 관찰하면 source를 남길 수 있는 경계를 수용한다.
- [현재 Follow pair command가 PENDING·terminal 재시도를 거부할 수 있다] → 이미 저장된 target Follow/Request와 concurrent `created: false`를 source 보존 결과로 판별하고, `created: true`가 아닌 결과에서는 source cleanup을 시도하지 않는다. command sequence를 바꾸면 history replay·배포 호환성을 검증한다.
...
3. 반복 수신, target 저장 실패, target 저장 뒤 중단·재시작, 기존 target relation/request 또는 concurrent `created: false`에서 source 보존과 Remote target effect/removal lifecycle을 검증한다. HTTP receipt과 Follow/Unfollow race를 별도 완료 조건으로 만들지 않는다.
