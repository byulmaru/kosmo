...
Expo migration의 `(tabs)` shell은 `apps/app/src/components/shell/UniversalShell.tsx`가 Android/iOS/Web 공용 column 구조를 소유한다. Web에서는 document/window scroll, fixed bottom chrome, sticky mobile header와 desktop rail 계약을 유지하고, native에서는 platform `ScrollView`와 safe area를 사용한다.
...
- 검색 query navigation, route 이동, back/forward를 Expo Router/browser document scroll 정책과 충돌하지 않게 둔다.
...
- native pull-to-refresh 또는 플랫폼별 overscroll 정책 확정.
- 미래 multi-column/timeline workspace 레이아웃 설계.

## Approach

React Native Web `UniversalShell`은 document scroll을 막지 않는다. 최상위 shell은 layout flow를 유지하고, 중앙 route outlet은 page content를 렌더링하되 web 전체에서 유일한 internal scroller가 되지 않는다. 모바일 web에서는 header가 document scroll 위에서 sticky로 동작하고, 하단 탭은 fixed bottom chrome으로 유지한다. 콘텐츠 하단은 bottom tab과 safe-area를 고려한 padding 또는 scroll padding으로 겹침을 피한다. Android/iOS route는 platform `ScrollView`를 사용할 수 있지만 web document scroll 계약을 바꾸지 않는다.

`compact` 이상 web에서는 좌측 rail wrapper를 layout column 안에 둔 채 sticky viewport 경계를 적용한다. `compact`~`full`의 icon rail과 `full+`의 full sidebar는 같은 `SidebarNavigation` API를 유지하며, 실제 높이와 overflow style만 필요한 만큼 보정한다.

`full` 이상 web에서는 우측 column 역시 layout flow에 남긴다. 우측 rail wrapper는 sticky viewport 경계를 유지하되, column 자체가 layout width 계산에 참여해야 한다. 우측 rail content가 viewport보다 길어질 가능성은 rail 내부 overflow로 처리하고, 중앙 content를 별도 internal scroller로 만들지 않는다.

web route scroll은 별도 internal-scroller helper를 만들지 않고 Expo Router/browser document scroll과 맞춘다. 검색 화면의 query-only `router.replace`는 현재 scroll/focus를 보존한다. 프로필/게시글 상세 route wrapper는 sticky rail 전환 때문에 깨지는 경우에만 최소 보정한다.
...
- iOS Safari와 Expo native safe-area/overscroll 대응이 공용 shell 기본 구현만으로 충분한지 구현 검증 후 판단한다.
