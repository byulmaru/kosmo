...
- Consequences: 기존 reply connection pagination을 오른쪽 scroller에 연결하는 seam과 원본 Media 생략 입력이 필요하다. 상세 route는 Viewer open 동안 배경 document pagination effect를 중지해 Viewer 오른쪽을 단일 owner로 만들어야 한다. Viewer 뒤 원래 Post surface는 focus와 interaction 대상에서 제외해야 한다.
