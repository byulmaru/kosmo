...
- [x] 2.2 글쓰기 버튼을 우측 레일이 없는 `md`~`xl` 아이콘 레일 단계에만 두고, 풀 사이드바(`xl`+)와 mobile drawer에는 중복 글쓰기 버튼을 두지 않는다
...
- [ ] 4.2 767/768 · 1279/1280 경계 폭에서 단계가 압축형으로 전환되고, 각 단계의 내비·글쓰기 진입(모바일=하단탭, md~xl=사이드바 버튼, xl+=우측 레일 컴포저)이 동작하는지 확인한다(시각 확인은 사용자)
