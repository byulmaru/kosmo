...
- `(tabs)` 셸을 `lg` 단일 경계에서 `md`/`xl` 3단계 압축형으로 바꾼다: 모바일(`<md`) → 아이콘 레일 + 피드(`md`~`xl`) → 풀 사이드바 3분할(`≥xl`).
...
- `apps/web/src/routes/(tabs)/+layout.svelte` — 그리드 `md`(아이콘+피드)/`xl`(풀 사이드바 3분할) 압축형, 모바일 셸 토글 `lg`→`md`
