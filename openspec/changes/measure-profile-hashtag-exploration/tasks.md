...
- [ ] 2.1 관측 시점의 인증을 증명하는 WAA와 session별 첫 결과·선택·종료 집계를 구현한다. 환경·제외 목록·중복·Asia/Seoul 주차·지연 수신을 검증한다. 일요일 오류→성공 없이 월요일 이탈과 일요일 오류→월요일 retry 성공을 독립 자료로 검증하고 최초 오류/성공 주 귀속·분모 0·종료 누락을 확인한다. 같은 확정 결과를 `hashtag_id`·주별로 묶어 기존 첫 성공 Account 수·확정 session 수·세 비율을 계산하고 ID 누락과 전체 집계를 분리한다. 1.1~1.4 이후 수행한다.
...
- [ ] 2.3 `hashtag_id`별 distinct 첫 성공 Account 수·확정 session 수·결과 선택률·Empty 비율·Error 비율과 주간 추세를 실제 PostHog 저장 Insight·dashboard에 연결한다. design의 H1/H2·W1/W2 합성 자료와 각 분자/분모를 대조하고 중복·제외·여러 Hashtag의 Account 중복·분모 0·ID 누락·주 경계 회복/오류를 검증한다. raw text/name을 추가하지 않고 실제 breakdown을 재현하고 query·Insight·dashboard ID와 검토 책임을 기록한다. payload 확인만으로 완료하지 않는다. 2.1~2.2 이후 수행한다.
...
- [ ] 3.2 PROD-795의 실제 개인정보·운영 선행 증거를 확인한 뒤 production 수집·custom payload·표준 metadata·dashboard를 대조한다. 관측 시작점, 제외 목록 관리, 종료 누락 등 한계와 주간 검토 책임을 개인정보 없는 증거로 남긴다. 2.2·2.3·3.1 이후 수행하며 선행 증거가 없으면 인수만 pending으로 남긴다.
...
- Limits: 실제 PostHog query·Insight·dashboard는 아직 없다. Spec validation과 구현·production 검증을 구분한다. Hashtag별 breakdown은 2.3의 승인된 전달·검증 범위다. 새 도달률·TagChip impression·노출→탐색 funnel은 후속 후보이며 task에 추가하지 않는다.

## Progress

- Status: Active
- Completed: Linear 관계·Hashtag identity 조사, 수집 계약·Review Packet P1·P2 수정과 Hashtag별 성과 breakdown 포함 결정 반영.
- Next: 한국어 윤문·계약 대조·strict validation 결과에 문제가 없으면 사용자의 조건부 최종 승인에 따라 approval snapshot과 handoff를 기록한다. 구현은 별도 요청 뒤 새 세션에서 시작한다.
