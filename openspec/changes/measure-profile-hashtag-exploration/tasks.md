...
- [ ] 1.1 현재 main·PROD-556·상위 PostHog 상태를 재확인하고 TagChip 진입부터 이탈·다른 Hashtag·Account 전환까지 불투명 session 관측을 연결한다. 실제 route·guard·새 탭·키보드 흐름에서 경계를 검증한다.
...
- [ ] 1.3 typed custom payload, Web fail-open과 Native no-op을 검증한다. forbidden raw 값 부재, 기존 표준 metadata·identify/reset·Replay 상태 보존을 실제 호출·payload로 확인한다. 1.1~1.2와 함께 수행한다.
- [ ] 2.1 관측 시점의 인증을 증명하는 WAA와 session별 첫 결과·선택·종료 집계를 구현한다. 환경·제외 목록·중복·Asia/Seoul 주차·지연 수신·주 경계를 넘긴 최초 오류 귀속·분모 0·종료 누락을 검증한다. 1.1~1.3 이후 수행한다.
- [ ] 2.2 초기 PostHog Insight·dashboard에 네 비율, 각 분자·분모, WAA, 기간·집계 시각·규칙·제외 목록 버전과 pagination 품질 항목을 연결한다. 합성 자료의 50%·100%·25%·25% 및 경계 자료를 실제 집계와 대조하고 저장 query·Insight·dashboard ID와 검토 책임을 기록한다. 2.1 이후 수행한다.
...
- Limits: 실제 PostHog query·Insight·dashboard는 아직 없다. Spec validation과 구현·production 검증을 구분한다.

## Progress

- Status: Active
- Completed: Linear 승인 내용·최신 상위 결정·현재 코드 조사와 spec 초안 작성.
