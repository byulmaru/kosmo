...
1. PROD-268 준비 PR에서 `spoiler_text`를 `content_warning`으로 값 보존 rename하고, Plain Text validation과 `bodyText` 직접 조회를 추가한다. 기존 `body_json NOT NULL`, `TipTapDocument` scalar, create mutation과 composer write path는 유지한다.
2. PROD-268을 독립적으로 검증·머지한다. 이 시점의 main은 기존 TipTap write/storage와 새 Plain Text read/validation 경계를 함께 가진다.
3. PROD-267 cutover PR에서 Drizzle schema의 `bodyJson`과 `bodyHtml`을 제거하고 생성된 migration으로 `body_json`, `body_html`을 drop한다. 기존 `body_text`는 update하지 않는다.
