...
- crawler와 browser navigation 요청에서 status, content type, XML 구조, 정확한 URL 집합과 SPA fallback 비적용을 검증한다.
- Expo Web export 산출물에 동일한 `sitemap.xml`이 포함되는지 확인한다.
- 변경 diff에 sitemap 전용 DB query와 동적 Hono route가 남지 않았는지 검토한다.

- [x] 1.1 현재 동적 구현에서 실패하는 회귀 테스트를 먼저 작성해 `/sitemap.xml`이 정확한 세 URL만 가진 정적 XML이고 browser navigation에서도 SPA HTML을 반환하지 않는 계약을 고정한다.
- [x] 1.2 `apps/app/public/sitemap.xml`에 XML declaration, Sitemap protocol namespace와 승인된 세 `loc` entry만 추가해 1.1 테스트를 통과시킨다.
- [x] 1.3 기존 sitemap 전용 Hono route, DB loader, XML serializer와 동적 Profile·Post eligibility·용량 경계 테스트 및 격리 DB E2E를 제거한다.
- [x] 1.4 Web static asset 회귀 테스트를 실행해 crawler와 browser navigation 요청, `application/xml`, 정확한 세 URL과 금지 metadata 부재를 확인한다.
...
저장소 자동 검증이 정적 sitemap의 공개 범위, XML 표현, export 포함과 Web 제공 경계를 재현 가능하게 증명한다.

**Guardrails**

- 실제 사용자 데이터, production credential 또는 DB seed에 의존하지 않는다.
- 정적 파일의 source text 존재 여부만 검사하지 않고 Web 요청·Expo export 결과처럼 소비자에게 보이는 경계를 검증한다.
- `robots.txt`, 검색 순위와 개별 URL 색인 여부는 저장소 자동 검증 범위가 아니다.

**Verification**

- Web 단위 테스트, app·web typecheck, lint, Prettier, Expo Web export와 `openspec validate add-public-sitemap --strict`를 통과시킨다.
...
- [x] 2.2 `@kosmo/app`과 `@kosmo/web`의 관련 test·typecheck, workspace lint와 Prettier를 실행한다.
...
| ---------------------------------------------------------- | ------------- | ------------------ |
| 정적 XML sitemap 응답                                      | 1.1, 1.2, 1.3 | 1.4, 2.1, 3.2      |
| 현재 공개 URL 집합                                         | 1.1, 1.2      | 1.4, 2.1, 3.2      |
| DB와 분리된 정적 제공                                      | 1.2, 1.3      | 1.4, 2.2           |
| 보수적인 metadata                                          | 1.1, 1.2      | 1.4, 2.1           |
| 프로덕션 무인증 접근                                       | 1.2, 1.3      | 1.4, 3.2           |
| `web-platform` static asset·BFF 책임 수정                  | 1.2, 1.3      | 1.4, 2.1           |
