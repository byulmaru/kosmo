...
API는 source가 저장 Recipient와 일치하고 Related Profile을 조회할 수 있는 Notification Item만 Kosmo UUID v8 ID 순서의 stable Relay connection에 포함해야 한다(MUST). 같은 millisecond에 생성된 item 사이의 생성 순서와 concurrent insert snapshot은 보장하지 않는다.
...
- **WHEN** 둘 이상의 visible Notification Item ID가 같은 millisecond timestamp prefix를 가진다
...
- **WHEN** 첫 페이지 조회 뒤 같은 millisecond timestamp prefix를 가진 새 Notification Item이 생성되고 클라이언트가 기존 cursor를 사용한다
