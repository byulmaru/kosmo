...
- **WHEN** `account`, `account_profile`, `application`, `application_authorization`, `file`, `media`, `notification_item`, `oauth_authorization_code`, `oauth_token`, `post`, `post_content`, `profile`, `profile_follow`, `profile_follow_request`, `session` 행이 생성된다
- **THEN** 시스템은 해당 테이블의 `TableDiscriminator` 값을 포함한 UUID 문자열을 기본 키로 생성한다
- **AND** 테이블 식별자는 12비트 범위 안에 있어야 한다

## ADDED Requirements

### Requirement: Notification Item 단일 projection 저장

시스템은 Profile-scoped Notification Item의 identity, Recipient, kind, loose source reference, kind-specific data, 생성 시각과 최초 Read 시각을 하나의 `notification_item` table에 저장해야 한다(MUST).

#### Scenario: Follow Notification row 저장

- **WHEN** Follow Notification Item을 생성한다
- **THEN** 시스템은 `notification_item`에 UUID v8 `id`, non-null `recipient_profile_id`, `kind = FOLLOW`, non-null `source_id`, `data = {}`, `created_at`과 nullable `read_at`을 저장한다
...
- **AND** API의 `NotificationType.FOLLOW`는 `notification_item.kind = FOLLOW`에서 파생한다
- **AND** 아직 구현하지 않는 Notification Type 값을 선제 추가하지 않는다

#### Scenario: 의도적인 loose source reference

- **WHEN** `notification_item.source_id`를 정의한다
...
- **WHEN** 같은 `(kind, source_id, recipient_profile_id)`로 두 번째 Notification Item을 직접 저장하려 한다
...
- **THEN** database는 Recipient가 다른 Notification Item을 각각 저장할 수 있다

#### Scenario: Recipient Profile row 물리 삭제

- **WHEN** Recipient Profile row가 물리적으로 삭제된다
- **THEN** 해당 Profile의 `notification_item` 행은 `recipient_profile_id` foreign key cascade로 삭제된다
...
후속 Profile-scoped Notification kind는 같은 `notification_item` table에 자신의 source mapping과 최소 data shape를 추가해야 하며(MUST), 별도 extension framework를 선제 요구해서는 안 된다(MUST NOT).

#### Scenario: 후속 Profile-scoped Notification kind 추가

- **WHEN** 후속 change가 새 Profile-scoped Notification kind를 구현한다
- **THEN** 그 change는 `notification_kind`와 GraphQL `NotificationType`에 값을 추가하고 해당 kind의 `source_id` 의미와 `data` validation을 함께 정의한다
...
- **WHEN** 애플리케이션이 지원하지 않는 kind나 해당 kind의 schema와 맞지 않는 `data`로 Notification Item을 생성하려 한다
