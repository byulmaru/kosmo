...
- Confirmation / Follow-up: `PROD-275`가 `node(id:)`에서 FOLLOW row를 `FollowNotification`으로 resolve하고 hidden·unsupported row를 노출하지 않는지 검증하며, `PROD-352`가 같은 concrete resolution을 connection에서 검증한다. 후속 kind는 자신의 concrete object와 route mapping을 추가하며 공통 interface에는 모든 kind가 공유하는 field만 추가한다.
