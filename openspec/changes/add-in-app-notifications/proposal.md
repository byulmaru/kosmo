...
- `api-platform`: 하나의 table discriminator를 공유하는 polymorphic Node가 row를 load한 뒤 kind별 concrete GraphQL object로 resolve되는 Node routing 계약을 추가한다.

## Impact

- `packages/core`: Notification enum, ID registry, 단일 Drizzle table/migration, 저장·정책 경계와 ProfileFollow 생성·삭제 action 연결.
- `apps/api`: shared discriminator를 위한 kind-aware Node routing, Notification interface/concrete object/connection, membership 기반 Profile 목록·visible Unread count, Read mutation, schema와 resolver/service test.
