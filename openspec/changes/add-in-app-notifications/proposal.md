...
- `PROD-277`이 정상 item의 표시·navigation·refresh를, `PROD-372`가 navigation과 독립적인 best-effort Read 및 성공 payload 기반 item/count cache 동기화를, `PROD-324`가 badge UX를 소유한 뒤 `/notifications` 목록과 모든 shell badge를 구현한다.
- Notification 저장·정리 실패가 Follow 결과를 바꾸지 않는 best-effort 실패 경계를 추가한다.
- Follow Request Notification 자체는 제외하되, 승인으로 새 established `ProfileFollow`가 생성될 때의 Follow Notification integration은 포함한다. 다른 Notification kind, 실제 Mute/Block 정책 연결, invalid source·unavailable Related Profile item의 비동기 물리 삭제와 Recipient inactivity cleanup 정책, ActivityPub ingress, historical backfill, outbox/message queue/retry, Push/realtime은 제외한다.
...
- `apps/app`: `PROD-277`의 `/notifications` 정상 Follow 목록·Related Profile navigation, `PROD-372`의 best-effort Read와 Recipient payload 기반 Relay cache 동기화, `PROD-324`의 shell badge.
