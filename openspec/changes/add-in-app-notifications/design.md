...
`PROD-277`은 정상 Follow item의 loading/error/empty/read 상태, item activation과 Read/navigation 순서, fetch·refresh·pagination·Profile 전환 cache 경계를 소유한다. `PROD-324`는 badge surface, Unread presence dot, 접근성, loading/error/stale, lifecycle과 Profile cache 경계를 소유한다. 두 UI slice 모두 API에서 반환한 visible item과 count만 소비하며 unavailable item을 client가 다시 판정하거나 generic fallback으로 표현하지 않는다.
