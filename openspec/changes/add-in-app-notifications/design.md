...
`PROD-324`는 기존 셸의 내비게이션 구조를 바꾸지 않고 알림 아이콘 wrapper에 selected Profile의 `unreadNotificationCount` badge를 겹쳐 표시한다. 대상은 Android/iOS와 좁은 Web의 하단 탭 바, 모바일 drawer, 중간 Web의 compact 아이콘 레일, 넓은 Web의 full 사이드바다. 모바일 하단 탭과 label이 있는 full/drawer 항목은 현재 코드의 label을 유지하고 compact 레일은 icon-only 구조를 유지한다. badge를 별도 label 옆 pill이나 독립 진입점으로 만들지 않는다.

count `0`은 숨기고 `1`부터 `99`까지는 정확한 숫자, `100` 이상은 `99+`로 표시한다. badge는 light/dark theme의 기존 neutral token 조합으로 foreground/background 대비를 확보하고, 숫자 길이에 따라 너비만 늘어나며 내비게이션 layout이나 touch target을 밀지 않는다. 알림 control의 accessible name은 count가 양수일 때 capped 표시값이 아닌 실제 서버 count를 사용한 `알림, 읽지 않은 알림 N개`, `0` 또는 최초 성공 전에는 `알림`이다. 시각적 badge는 별도 focus나 중복 announcement 대상이 아니다.
...
controller와 표시 component는 normalized `Profile.unreadNotificationCount`를 구독해 mutation payload가 같은 Profile record를 갱신하면 모든 셸 surface가 같은 값을 반영할 수 있게 한다. 전용 controller는 ad hoc HTTP fetch나 별도 network stack을 만들지 않고 Relay operation/store를 사용한다. Read mutation, optimistic decrement와 최초 visible Read 전이의 cache orchestration은 `PROD-372`가 소유한다. `PROD-324`는 목록 길이, hidden item 또는 client-side visibility 판정으로 count를 계산하거나 보정하지 않는다.

### UI slice 책임 경계

`PROD-277`은 정상 Follow item의 loading/error/empty/read 상태, item activation과 Read/navigation 순서, fetch·refresh·pagination·Profile 전환 cache 경계를 소유한다. `PROD-324`는 badge surface, count cap, 접근성, loading/error/stale, lifecycle과 Profile cache 경계를 소유한다. 두 UI slice 모두 API에서 반환한 visible item과 count만 소비하며 unavailable item을 client가 다시 판정하거나 generic fallback으로 표현하지 않는다.
...
- `PROD-324`: 하단 탭 바, drawer, compact/full sidebar의 badge를 Storybook interaction/a11y로 검증하고 formatter·Profile 격리·마지막 성공 상태와 normalized Relay count 반영을 단위/cache test로 검증한다. 최초 count-only 실패에는 badge와 전용 retry가 없고, 성공 count `7` 뒤 같은 Profile의 count 조회가 실패해도 셸 진입점과 `7` badge가 유지되며, 다음 기존 refresh에서 수렴하는 흐름을 포함한다.
...
현재 공통 저장·API 계약과 `PROD-324` badge 구현을 막는 열린 질문은 없다. 목록의 세부 UX는 `PROD-277`에서 결정되어 이 design/decisions에 기록됐다. 실제 Mute/Block 연결은 `PROD-327`, invalid source·unavailable Related Profile item의 비동기 삭제 방식과 Recipient inactivity cleanup 여부는 `PROD-328`, delivery queue/retry와 다른 Notification kind는 각각 별도 Issue → OpenSpec 흐름에서 결정한다.
