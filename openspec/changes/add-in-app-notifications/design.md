...
`PROD-277`의 `/notifications`는 모바일과 Web이 공유하는 `모두` 단일 목록이다. 탭, 날짜별 heading, inline 맞팔로우, 본문 snippet과 언급 전용 상태는 추가하지 않는다. Follow item은 API가 반환한 Related Profile 한 명의 initials Avatar, `OOO님이 팔로우했습니다` 문구와 상대 시각을 표시한다. 현재 `FollowNotification.profile`이 단수이고 Profile image field가 없으므로 복수 사용자 집계와 이미지 avatar를 client에서 합성하지 않는다. 복수 사용자 문구와 겹친 avatar는 server aggregation 계약이 생기는 후속 이슈가 소유한다.
...
route query는 selected Profile을 target으로 `store-and-network` fetch를 수행한다. 초기 loading/error/retry/empty를 구분하고 native pull-to-refresh와 Web의 keyboard-accessible refresh action을 제공한다. 목록은 한 번에 20개씩 Relay connection pagination을 사용하며, 다음 page 요청 중 중복 호출을 막고 실패해도 기존 item을 유지한 채 같은 위치에서 재시도한다. Relay가 edge 누적과 중복 제거를 소유하고 route state는 edge를 수동 병합하지 않는다. selected Profile 전환은 actor별 Relay Environment/Store 재생성과 query target 변경으로 격리한다.
