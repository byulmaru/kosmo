...
- [ ] 9.2 전체 셸 query의 Suspense/Error boundary와 분리된 non-suspending count controller와 공용 badge 표시 경계를 만들고, 모든 지원 shell 알림 아이콘이 selected Profile의 서버 제공 `unreadNotificationCount`를 표시하며 Profile 전환 시 다른 Recipient count를 재사용하지 않게 한다.
- [ ] 9.3 normalized `Profile.unreadNotificationCount` 변경을 모든 셸 surface가 반영하고 기존 actor revision/refetch에서 서버 count로 수렴하게 하되 Read updater, speculative decrement와 hidden item client 보정 로직을 만들지 않는다.
