...
1. `PROD-549`에서 `ap-northeast-2`의 `byulmaru-kosmo-prod-postgresql-backups-822638974464` bucket과 `kosmo-prod/` prefix 전용 `byulmaru-kosmo-prod-postgres-backup` role을 만든다. Bucket에는 SSE-S3, public access block, TLS-only policy, versioning, current 10일/non-current 30일/incomplete multipart 1일 lifecycle, `prevent_destroy = true`, `force_destroy = false`를 적용한다.
...
4. runbook은 on-demand backup, 상태·접근 장애 확인, 격리된 PITR restore, 검증과 정리를 포함한다. Restore workload는 source prefix를 읽을 수 있지만 ScheduledBackup 또는 WAL archiver destination을 구성하지 않는다.
...
- restore manifest는 복사 가능한 문서 template 또는 별도 예제 manifest로 제공할 수 있다. 두 방식 모두 target time과 source server name을 운영자가 명시하고 source에 쓰지 않아야 한다.
