...
4. runbook은 on-demand backup, 상태·접근 장애 확인, 격리된 PITR restore, 검증과 정리를 포함한다. Target time과 비교 기준은 application write pause 중 같은 snapshot에서 기록하고 이후 현재 production count를 기준으로 사용하지 않는다. Restore workload는 source prefix를 읽을 수 있지만 ScheduledBackup 또는 WAL archiver destination을 구성하지 않는다.
