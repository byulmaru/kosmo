...
- Confirmation / Follow-up: Static Helm render에서 role name, attributes와 inRoles를 확인한다. 실제 catalog 확인은 non-prod live gate로 남긴다.
...
- Confirmation / Follow-up: Helm render에서 source path, destination name/type, transformation과 role passwordSecret 참조를 확인한다. 실제 Vault sync/password readiness는 unchecked live gate다.
...
- Confirmation / Follow-up: Render 음성 경계와 existing manifest diff를 확인한다. 실제 connection current_user 검증은 후속 cutover/live gate다.
...
- Confirmation / Follow-up: Static manifest assertion을 수행하고 실제 catalog/membership/ownership은 승인된 live gate로 남긴다.

### Production apply와 live evidence를 별도 gate로 둔다

- Decision Date: 2026-08-11
- Decision Class: Derived Contract
- Authority / Provenance: Linear PROD-369; docs/operations/production-migrations.md; memory/database-migrations.md; 사용자 결정
- Status: Active
- Context / Problem: CI와 Helm render는 Secret 값이나 실제 Vault/CNPG role 상태를 증명하지 않으며 production identity 변경은 명시적 승인이 필요하다.
- Decision Outcome: Static validation은 password value를 노출하지 않고 수행한다. Non-prod apply/actual Vault-role checks와 production preflight/sync/apply/live checks는 unchecked tasks로 남긴다. 사용자의 별도 승인 없이는 실행하지 않는다.
- Alternatives Considered: PR merge 또는 green CI를 production approval로 해석하는 것은 live preflight와 rollback 검토를 건너뛴다.
- Consequences: 이 pass에서 change는 active 상태로 남으며, archive ownership은 live evidence를 확보한 PROD-369 담당자에게 있다.
- Confirmation / Follow-up: Strict OpenSpec와 static Helm validation을 확인하고, live task가 완료되기 전 archive하지 않는다.
