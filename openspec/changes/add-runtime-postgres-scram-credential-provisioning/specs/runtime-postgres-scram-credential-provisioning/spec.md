...
### Requirement: 검증과 production 적용 승인을 분리한다

**Authority / Provenance:** Linear PROD-369; docs/operations/production-migrations.md, memory/database-migrations.md; 2026-08-11 사용자 결정. Static Helm lint/render와 OpenSpec 검증은 password 값을 노출하지 않고 수행해야 한다(MUST). Non-prod live apply와 실제 Vault/role 상태 검증은 별도 gate로 남겨야 하며(MUST), 사용자의 명시적 승인 없는 production sync/apply/live 검증은 수행해서는 안 된다(MUST NOT).

#### Scenario: 정적 검증만 완료한 상태

- **WHEN** role과 VSO manifest 변경을 CI 또는 로컬에서 검증한다
- **THEN** dev/prod render, role 이름/attribute, Secret shape/path와 음성 범위를 확인할 수 있다
- **AND** password 값·connection string·private credential material은 출력하지 않는다
- **AND** 이 결과는 non-prod cluster 적용이나 실제 Vault/role 상태의 증거로 해석되지 않는다

#### Scenario: 승인 없는 production 상태

- **WHEN** PR, Helm lint/render와 strict OpenSpec validation이 통과했지만 production apply 승인이 없다
- **THEN** production Secret sync, DatabaseRole apply, workload cutover와 live role/credential 확인을 수행하지 않는다
- **AND** 기존 production owner/migration/PgBouncer 경계만 유지한다
