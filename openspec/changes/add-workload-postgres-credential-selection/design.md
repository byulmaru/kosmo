...
API outbound Fedify 직접 호출을 Temporal durable intent/workflow와 Worker Fedify Activity로 바꾸는 일은 PROD-448이 소유한다. Worker foundation과 namespace는 후속에서 준비됐지만 credential 소비·주입과 실제 DB Activity 전환은 PROD-715/448이 소유한다. 실제 BYPASSRLS identity는 PROD-369의 `kosmo_worker`이며 이 change의 `fedify`/`FEDIFY_DATABASE_*`는 구현 당시의 legacy selector seam일 뿐 role/Secret 이름 계약이 아니다. 이 change는 selector와 Helm env만 추가하며 실제 Secret·role·DB client/connection·Temporal 전환은 수행하지 않는다.
...
- [Worker foundation은 있지만 business credential이 아직 연결되지 않음] → selector/env 명칭 migration과 Worker credential consumption은 PROD-715, API direct-call 제거와 capability 활성화는 PROD-448/719의 downstream gate로 명시하고 이 change에서는 연결을 수행하지 않는다.
...
4. `api` trio를 API Rollout과 Web BFF 기본 연결에 opt-in한다. Worker credential과 selector/env 명칭 전환은 PROD-715, API outbound Fedify와 Worker/Temporal capability 활성화는 PROD-448/719에서 별도 진행한다.
