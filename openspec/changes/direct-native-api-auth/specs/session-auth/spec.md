...
- **THEN** API는 민감한 입력을 echo하지 않는 GraphQL error를 반환한다
...
- **THEN** API는 GraphQL validation 또는 민감한 입력을 echo하지 않는 GraphQL error를 반환한다
...
- **AND** 사용되지 않은 fragment를 포함한 GraphQL 문서 전체에서 native input에 연결된 variable coercion, variable default 또는 inline literal validation failure도 authorization code, PKCE verifier, access token 또는 ID token을 echo하지 않는다
