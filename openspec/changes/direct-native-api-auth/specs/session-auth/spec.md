...
Kosmo API는 네이티브 공개 OIDC client가 client secret을 보유하지 않고 authorization code와 PKCE verifier를 Kosmo session token으로 교환할 수 있는 `POST /login/native/session` endpoint를 제공해야 한다(MUST). 새 Expo native bundle은 이 endpoint를 공개 API origin에서만 호출해야 한다(MUST).

#### Scenario: Exchange a valid native code

- **WHEN** 공개 API origin의 `/login/native/session`이 code, PKCE verifier, `kosmo://login/callback` redirect URI를 포함한 POST 요청을 받는다
...
- **AND** 응답 body로 Kosmo session token을 반환하고 session cookie를 설정하지 않는다
...
- **THEN** 응답, client bundle, log, 새 Kosmo session record에 OIDC client secret, authorization code, PKCE verifier, access token 또는 ID token을 포함하지 않는다
