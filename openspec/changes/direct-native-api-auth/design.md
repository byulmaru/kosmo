...
- API origin의 `POST /login/native/session`이 public-client authorization code + PKCE verifier를 Kosmo session token으로 교환한다.
...
- `apps/api`의 REST router에 `POST /login/native/session`을 추가한다. path는 BFF legacy endpoint와 같지만 host가 public API origin이므로 앱 migration의 URL surface를 최소화한다.
- 기존 BFF 입력 경계를 재사용한다: JSON body 16 KiB, code 최대 2048자, PKCE verifier 형식, `kosmo://login/callback` exact match, 모든 응답의 `Cache-Control: no-store`와 `Pragma: no-cache`.
...
- 예상된 OIDC/입력 실패는 token·code·verifier를 echo하지 않는 400 응답으로 바꾸고, 예상 밖 오류도 request body나 upstream error를 기록하지 않는 generic 응답으로 처리한다.
...
- native login은 `EXPO_PUBLIC_OIDC_NATIVE_CLIENT_ID`로 AuthSession PKCE S256 request를 만들고, validated API origin의 same-path session endpoint로 code/verifier를 보낸다.
...
- existing Playwright OIDC mock은 confidential web client와 public native client를 구분해 PKCE/no-secret flow를 제공한다. API origin에 대해 valid exchange, invalid verifier/redirect, invalid signature, payload limit, no-store/no-cookie를 검증한다.
...
2. API public-client endpoint와 upstream-token-free shared writer를 배포한다. 이 단계는 기존 BFF native endpoint를 유지하므로 이전 bundle과 호환된다.
