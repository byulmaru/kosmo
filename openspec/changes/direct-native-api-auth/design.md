...
- mutation input의 shape와 format은 GraphQL schema와 Pothos validation plugin으로 검증한다. 유효하지 않은 code나 token response처럼 예상된 OIDC code exchange 실패는 upstream 오류 세부 정보와 token material을 노출하지 않는 generic GraphQL error로 바꾼다. OIDC token endpoint의 5xx를 포함한 upstream 장애와 예상 밖 오류는 기존 internal error 분류 및 production masking을 따른다. GraphQL validation/coercion 오류는 기존 표준 동작을 유지한다.
