...
- `@kosmo/core/auth`에 verified `{ displayName, oidcSubject }`만 입력으로 받는 server-side session writer를 둔다. 이 writer는 account upsert, active profile 선택, session token 생성만 수행한다. `@kosmo/core/db`는 이 use case가 쓰는 DB client, schema, relation, DB 전용 utility만 제공한다.
