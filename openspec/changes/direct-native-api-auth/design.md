...
- 예상된 OIDC/입력 실패는 token·code·verifier를 echo하지 않는 generic GraphQL error로 바꾸고, native input에 연결된 variable coercion·variable default·inline literal validation failure도 resolver 전에 같은 방식으로 마스킹한다. 예상 밖 오류도 request body나 upstream error를 기록하지 않는 masked error로 처리한다.
- GraphQL validation은 사용되지 않은 fragment를 포함한 문서 전체를 검사하므로, 문서 안의 native input이 참조한 변수는 같은 문서 내에서 credential-sensitive로 취급한다. 이 보수적 범위는 malformed document에서 같은 이름의 다른 operation 변수 기본값도 generic error로 바꿀 수 있지만, OIDC credential echo를 우선 차단한다.
