...
- `@kosmo/core/services`에 verified `{ displayName, oidcSubject }`만 입력으로 받는 `createOidcSession` use case를 둔다. service는 선택적으로 전달된 core transaction에 합류하고, 없으면 core `db`로 transaction을 시작해 account upsert, active profile 선택, session token 생성을 원자적으로 수행한다. `@kosmo/core/db`는 DB client, schema, relation, DB 전용 utility만 제공한다.
