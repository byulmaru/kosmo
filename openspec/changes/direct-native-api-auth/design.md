...
- 예상된 OIDC/입력 실패는 token·code·verifier를 echo하지 않는 generic GraphQL error로 바꾼다. 프로덕션 GraphQL endpoint는 operation별 AST 추적 없이 validation과 variable coercion failure를 공통 `Invalid input` error로 마스킹한다. 개발 환경은 schema diagnostics를 유지하고, 예상 밖 오류는 기존 production masking을 따른다.
