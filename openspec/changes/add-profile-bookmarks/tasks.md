...
- UUIDv7 식별자와 `(createdAt, id)` 안정 순서를 사용한다.
- Target Post가 Tombstone이 되거나 조회 불가능해져도 Bookmark 관계를 삭제하지 않는다.
- 생성·삭제 API, 목록 GraphQL과 UI를 이 구현에 포함하지 않는다.
- additive migration만 사용하고 기존 데이터 backfill을 만들지 않는다.

**Verification**

- 빈 DB와 기존 migration chain에서 schema migration을 검증한다.
- 동일 Profile/Post 중복 거부, 서로 다른 Profile의 독립 저장, 최신순 index tuple, Target Post 상태 변경 뒤 관계 유지와 FK lifecycle을 DB 수준에서 검증한다.

- [ ] 1.1 Bookmark 저장 모델에 필수 관계·생성 시각·Profile/Post 유일성과 안정적 최신순 index를 구현한다.
- [ ] 1.2 기존 schema에 Bookmark 저장 구조를 추가하는 forward-only additive migration과 schema metadata를 생성한다.
- [ ] 1.3 Bookmark 제약·index·관계 lifecycle을 증명하는 focused DB regression 검증을 추가한다.
- [ ] 1.4 core migration·database test와 관련 정적 검사를 실행해 PROD-396 결과를 검증한다.
...
- Account 활성 상태, Profile membership·상태·locality와 Target Post 조회 정책을 우회하지 않는다.
- 같은 Profile/Post 조합은 순차·동시 요청에서도 하나만 유지하고 기존 생성 시각을 바꾸지 않는다.
- Bookmark 생성은 Post Author Notification을 만들지 않는다.
- duplicate create 응답과 공유 GraphQL shape는 구현 전에 Remaining Decisions에서 확정한다.

**Verification**

- 정상 생성, 사용할 수 없는 Profile, 조회 불가 Target, 순차·동시 중복, Profile 격리, transaction rollback과 무알림을 core/API 수준에서 검증한다.

- [ ] 2.1 duplicate create 응답과 생성 mutation의 공유 GraphQL 계약을 사용자와 확정하고 specs·decisions에 반영한다.
- [ ] 2.2 Bookmark 생성의 권한·가시성·유일성·transaction 결과를 core 동작으로 구현한다.
- [ ] 2.3 Relay가 현재 Profile/Post 관계를 정규화할 수 있는 생성 mutation 계약을 구현한다.
- [ ] 2.4 성공·권한 실패·가시성 실패·순차/동시 중복·rollback·무알림 검증을 추가하고 관련 check를 통과시킨다.
...
- 순서는 `createdAt DESC, id DESC`이며 ID-only cursor를 사용하지 않는다.
- 숨겨진 Target을 client post-filter로 처리하거나 Bookmark를 자동 삭제하지 않는다.

**Verification**

- owner 격리와 Node ID 우회 방지, 동률 생성 시각 composite pagination, 숨겨진 최신 row 뒤의 조회 가능한 row로 page limit 충족, 숨김·관계 유지·재노출을 API/DB 수준에서 검증한다.
...
- [ ] 4.4 owner 격리·Node 보안·composite cursor·visibility-before-limit·재노출 검증을 추가하고 관련 check를 통과시킨다.
