...
시스템은 Bookmark를 UUIDv7 primary key, 필수 Owner Profile 관계, 필수 Target Post 관계와 불변 생성 시각을 가진 별도 PostgreSQL row로 저장해야 한다(MUST). Profile/Post 조합은 데이터베이스 제약으로 유일해야 하며(MUST), Profile별 생성 시각·ID 내림차순 목록을 지원하는 index를 가져야 한다(MUST).
...
#### Scenario: Profile별 최신순 목록을 조회함

- **WHEN** 시스템이 한 Profile의 Bookmark를 생성 시각 내림차순과 ID 내림차순으로 조회한다
- **THEN** 데이터베이스는 Profile과 안정적 최신순 tuple을 포함한 index로 조회를 지원한다
