...
- 가시성 필터 이후에도 page limit과 `(createdAt, id)` cursor 순서가 정확한 최신순 목록을 제공한다.
...
- `packages/core/db/tables.ts`는 UUIDv7 primary key와 `createdAt()` helper, 명시적 FK·unique·index를 사용한다. UUIDv7만으로 같은 millisecond의 생성 순서를 보장할 수 없으므로 활성 `data-model` spec은 immutable timestamp와 ID tie-breaker를 요구한다.
- Post는 Tombstone과 가시성 정책으로 조회 가능성을 바꾸면서 row를 유지한다. Bookmark가 Post 상태나 Block 변화에 연쇄 삭제되면 canonical 관계 유지 계약을 위반한다.
...
1. **PROD-396 저장 계층**: 별도 Bookmark table에 UUIDv7 ID, Owner Profile FK, Target Post FK와 생성 시각을 저장한다. `(profileId, postId)` unique 제약과 `(profileId, createdAt DESC, id DESC)` index를 둔다. Owner Profile FK는 소유 관계 관례에 맞춰 cascade를 기본으로 검토하고, Target Post FK는 상태 변화나 암묵적 삭제가 Bookmark를 제거하지 않도록 cascade를 사용하지 않는다. additive Drizzle migration과 focused DB 검증을 함께 추가한다.
2. **PROD-408/409 생성·삭제**: core action이 transaction 안에서 행동 주체 Account/Profile 상태, membership, Target Post 가시성, owner 권한과 DB unique 경쟁 조건을 검증한다. GraphQL mutation은 요청 input의 임의 Profile ID보다 현재 `usingProfile` actor context를 owner로 사용하는 경로를 기본으로 한다. 응답에는 Relay가 영향받은 Post와 Bookmark 관계를 정확히 정규화할 수 있는 Node 정보를 포함한다.
3. **PROD-410 목록**: owner-scoped connection query에서 Bookmark와 Target Post·작성자 Profile·Instance를 결합하고 기존 Post 가시성 predicate를 SQL `WHERE`에 적용한 뒤 `(createdAt, id)` composite cursor, order, limit을 적용한다. Bookmark Node 자체는 Owner가 숨겨진 Target의 관계도 삭제할 수 있도록 owner 권한과 Target 표시 권한을 분리하되, `Bookmark.post`와 목록 edge는 Target 가시성을 우회하지 않는다.
...
- GraphQL은 `Profile.bookmarks`와 Post의 viewer-relative Bookmark 관계를 기본으로 권장하지만, owner 격리, Relay 정규화, exact deleted edge 식별과 specs의 pagination 계약을 동일하게 충족하는 다른 field/payload shape도 가능하다. 여러 구현 slice가 shape를 공유하기 전에 `decisions.md`에 확정해야 한다.
...
- Post FK에 cascade를 사용하거나 Block/Tombstone 처리에서 Bookmark를 지우면 관계 유지와 재노출 요구사항을 위반한다.
- Bookmark Node ID 또는 삭제 mutation으로 owner가 아닌 사용자에게 관계 존재를 구분 가능하게 만들면 안 된다.
- ID-only cursor는 같은 millisecond의 UUIDv7 순서를 보장하지 못한다.
...
- **[가시성 변화 중 pagination]** cursor 사이의 Target 가시성이 달라지면 사용자가 보는 항목 수가 변할 수 있다. → cursor tuple은 Bookmark 자체에 고정하고 매 요청 시 현재 가시성을 limit 전에 적용하며 중복·누락 경계를 통합 테스트한다.
- **[동시 생성 경쟁]** 사전 존재 확인만으로는 같은 Profile/Post 중복을 막지 못한다. → DB unique 제약을 최종 보장으로 사용하고 선택한 API 의미로 unique violation을 정규화한다.
...
- **PROD-408**: 중복 생성 요청을 기존 Bookmark를 반환하는 idempotent success로 할지, 존재를 노출하지 않는 명시적 conflict로 할지 확정해야 한다. 현재 follow core의 idempotent 선례를 기본안으로 검토한다.
- **PROD-409**: 이미 없거나 비소유인 Bookmark 삭제를 구분 불가능한 idempotent success로 할지, 구분 불가능한 not-found 의미로 할지 확정해야 한다.
- **PROD-408/410**: `usingProfile` actor context, `Profile.bookmarks`, Post의 viewer-relative Bookmark 관계와 mutation payload의 exact GraphQL shape를 두 구현 slice가 시작되기 전에 함께 확정해야 한다.
