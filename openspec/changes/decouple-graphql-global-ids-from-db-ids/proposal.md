...
- **BREAKING** GraphQL Node ID를 raw DB UUID에서 concrete GraphQL typename과 DB UUID를 담은 opaque global ID로 변경한다.
...
- 신규 DB row의 ID를 table discriminator가 없는 표준 UUIDv7으로 생성하고 `TableDiscriminator` registry와 호출부 의존성을 제거한다.
- 기존 UUIDv8 primary key와 foreign key는 재작성하지 않고 신규 UUIDv7과 함께 계속 조회한다.
- Relay client가 ID를 opaque 값으로 취급하는지 검증하고 schema, generated artifact, API·client 테스트와 ID 관련 문서를 새 계약에 맞춘다.
...
- Data: PostgreSQL column/constraint 변경이나 기존 primary key·foreign key data migration은 없다. 기존 UUIDv8과 신규 UUIDv7이 함께 존재한다.
