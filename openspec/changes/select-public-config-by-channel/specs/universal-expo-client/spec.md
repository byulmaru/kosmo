...
**Authority / Provenance**: 현재 canonical `openspec/specs/universal-expo-client/spec.md`, [PROD-891](https://linear.app/byulmaru/issue/PROD-891/webnative-공개-설정을-배포-채널로-선택한다)

Relay network layer는 웹에서는 same-origin BFF를, Android/iOS에서는 공개 API origin을 사용해 GraphQL 요청을 보내야 한다(MUST). Native 인증은 Web과 공유하는 OIDC issuer와 client ID 설정에 결합하며, OIDC client secret은 API 서버가 보유하는 server-side 경계에만 남아야 한다(MUST NOT be part of the universal client bundle).
...
- **WHEN** Android 또는 iOS 클라이언트가 GraphQL operation을 실행하고 SecureStore에 현재 configured API origin 및 공유 OIDC issuer/client ID 설정과 일치하는 session token envelope이 있다
...
- **AND** Native bundle과 request에는 OIDC client secret이 포함되지 않는다
