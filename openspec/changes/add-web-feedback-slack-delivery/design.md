...
- Web shell의 모든 sidebar surface에서 보호된 `/feedback` form으로 진입한다.
...
- `/feedback`은 `(tabs)/(protected)` 아래의 universal route로 두어 current-session query를 통한 보호 route 계약을 재사용한다. 별도 Web route tree나 BFF-owned UI를 만들면 universal route 정책을 어긴다.
...
7. API test에서는 network를 호출하지 않고 fetch 경계를 stub해 auth, validation, payload, timeout, non-success와 concurrent flow를 검증한다. App story/test는 idle, invalid, bug-ID, pending, success, failure와 retry를 다루고 Web E2E는 shell surface에서 `/feedback` 이동과 Relay request state를 검증한다.
...
- Feedback form은 `/feedback` route component 안에 둘 수도 있고 stateful child component로 분리할 수도 있다. Relay colocation, Storybook 상태 검증과 Web-only 노출 경계를 유지해야 한다.
...
2. Web `/feedback` form, shell navigation, Relay operation과 Storybook/E2E를 연결한다.
