...
- 피드백 내용을 DB에 저장하지 않고, client의 진행 중 반복 제출 차단과 server의 계정별 비영속 rate limit·동시 전송 차단을 적용한다.
...
- `web-feedback-delivery`: 인증된 Web 피드백 입력, 검증, GraphQL 제출, Slack 전달, 중복·rate 제한, 상태·재시도와 운영 검증 계약
...
- `apps/api`: 로그인 scope GraphQL input/enum/mutation, 비영속 rate·동시 전송 제어, Slack Incoming Webhook adapter와 단위·GraphQL 통합 테스트
...
- 운영 검증: 인증 실패, 입력 제한, 성공·실패·rate limit, secret 비노출과 production Slack smoke
