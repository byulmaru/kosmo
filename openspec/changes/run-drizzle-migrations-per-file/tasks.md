...
- [ ] 1.1 현재 Drizzle migration parsing과 history shape를 재현하는 compatibility fixture를 추가하고 기존 history에서 pending suffix를 식별하는 검증을 고정한다.
- [ ] 1.2 specs와 active decisions를 만족하도록 runtime migration 실행 경계를 파일별 SQL+history transaction으로 변경한다.
- [ ] 1.3 적용된 history의 prefix, name과 hash 무결성 검증 및 변경·누락·순서 오류의 실행 전 실패를 구현한다.
- [ ] 1.4 기존 no-op 재실행, advisory lock, PostgreSQL 환경 연결과 migration role ownership 검증을 새 runner에서 통과시킨다.
...
- [ ] 2.1 여러 pending 파일의 중간 실패에서 file 단위 schema/history atomicity와 뒤 파일 미실행을 검증한다.
- [ ] 2.2 중간 실패 후 재실행이 이미 성공한 파일을 건너뛰고 실패했던 파일부터 완료되는지 검증한다.
- [ ] 2.3 PostgreSQL enum value 추가 파일 다음 파일에서 새 값을 사용하는 production-equivalent integration test를 추가한다.
- [ ] 2.4 한 migration 파일 내부 statement 실패가 해당 파일의 모든 변경과 history를 rollback하는지 검증한다.
...
- [ ] 3.1 빈 database 전체 migration replay와 두 번째 no-op 실행을 새 runner로 검증한다.
- [ ] 3.2 기존 database history에서 incremental migration을 적용하고 history count/hash가 예상과 일치하는지 검증한다.
- [ ] 3.3 production migration runbook과 database migration memory를 파일 단위 atomicity, partial apply와 forward retry 계약으로 갱신한다.
- [ ] 3.4 OpenSpec strict validation, formatting, typecheck와 관련 integration suite를 실행하고 결과를 PROD-656 및 PR에 기록한다.
