...
- PROD-593에서 상세 thread의 row 경계를 `PostThreadLayout`이 소유하는 N-1 단일 divider로 통합하고 renderer 내부 border 중복을 제거한다. Home timeline과 같은 `theme.divider`·1px 무게를 사용하되 thread 전용 64px/8px inset으로 connector 교차를 피한다.
...
- Linear Thread UI: [PROD-593](https://linear.app/byulmaru/issue/PROD-593)
...
- Linear: PROD-394, PROD-401, PROD-402, PROD-403, PROD-411, PROD-412, PROD-414, PROD-430, PROD-415, PROD-416, PROD-453, PROD-471, PROD-593과 부모 PROD-389
...
- Excluded systems: Quote 작성 action·전용 composer와 `인용하기` 메뉴 항목, ActivityPub federation delivery·Repost ingestion, Mentioned Profiles Repost, Post Media, 범용 Notification 재설계, retry/outbox/backfill, 공통 `PostActionBar` 구현, Repost 외 action의 production 조립과 최종 공통 정책
