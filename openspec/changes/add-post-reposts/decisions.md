...
- Confirmation / Follow-up: 현재·조상·하위 Reply Quote의 Source가 정확히 한 번만 표시되고 다음 Source depth와 CTA가 없는지, pure Repost가 attribution 뒤 동일 표준 행·단일 article을 갖는지, Source null Quote가 자체 Content를 유지하는지, Content 없는 Repost 경로가 Source canonical route로 replace되는지, 기존 thread 순서·pagination·오류 복구가 유지되는지 검증한다. production Storybook은 48px 목록형 avatar와 40px 현재 avatar 모두 connector 위·아래 간격이 4px이며 끝이 둥글고, 아래 결정의 단일 thread divider가 renderer 내부 border와 중복되지 않는지도 검증한다.

### 스레드 행 경계는 PostThreadLayout이 소유한다

- Decision Date: 2026-07-31
- Decision Class: Implementation Choice
- Authority / Provenance: `PROD-593`, `PROD-422`, 2026-07-31 사용자 Web 시각 확인
- Status: Active
- Context / Problem: 상세 thread에서 각 renderer의 상·하단 border를 유지하면 인접 행 경계가 중복되어 두껍게 보이고, 화면 가장자리까지 이어진 가로선이 reply connector와 교차해 시각적으로 경쟁한다. Home timeline과 같은 `theme.divider`·1px 무게는 유지하되 thread 관계 표현에 맞는 단일 소유자가 필요하다.
- Decision Outcome: `PostThreadLayout`은 N개 thread row 사이에 N-1개의 `theme.divider` 색상 1px 구분선을 렌더링하고 마지막 행과 단일 행에는 렌더링하지 않는다. thread 안의 `PostListItem` renderer는 자체 row divider를 끄며 thread 밖 Home·Profile·Bookmark의 기본 divider는 유지한다. 구분선은 행 왼쪽 64px과 오른쪽 8px inset을 사용하고, 왼쪽은 현재 상세 Post의 본문 열에 맞추며 목록형 Post의 68px 본문 시작점과 완전히 정렬하지 않는다. 기존 connector의 위치·길이·둥근 끝·스레드 순서·들여쓰기는 유지하고 가로 구분선은 connector 오른쪽에서 시작한다.
- Alternatives Considered: renderer별 기존 border를 유지하는 방식, 현재 Post의 상·하단 border와 목록 row border를 함께 유지하는 방식, thread 가로 구분선을 모두 제거하는 방식. 기존 border 방식은 중복·전체 폭 선을 남기고, 두 종류의 border를 함께 유지하면 이중 선이 생기며, 전부 제거하면 인접 행 경계가 약해진다.
- Consequences: `PostListItem`은 기본값이 켜진 optional divider 표시 경계를 제공하고 thread caller만 이를 끈다. thread divider geometry는 좌우 비대칭이며 Home timeline과 동일한 geometry가 아니라 동일한 token·1px 시각 무게만 공유한다. data loading·pagination·Relay fragment와 connector geometry는 변경하지 않는다.
- Confirmation / Follow-up: Storybook에서 row N-1 divider, 마지막·단일 행 없음, 1px `theme.divider`, 왼쪽 64px·오른쪽 8px, 현재 상세 본문 열 정렬, 모든 connector와 비중첩, renderer 내부 border 제거 및 thread 밖 기본 divider 유지를 검증한다. Web 시각 확인을 기록하고 Dark theme와 Native 실기기 paint 확인은 출시 gate에 남긴다.
