...
5. `PROD-822-graphql`은 기존 Mute 문법에 맞춘 `blockProfile`·`unblockProfile`, `ProfileBlock` 관계와 Owner connection을 기본안으로 검토한다.
   ...
   보장된다고 추정하지 않는다. mutation은 부모 layer #726이 제공하는 transaction action을 호출하며 cleanup 구현을 `PROD-822`에 복제하지 않는다.
...
| GraphQL ingress·관리 | guest, selected Profile 없음에서 기존 Account 인증·공개 후보 결과를 유지하고 Block predicate·selected Local Profile을 요구하지 않는 기대 결과, membership mismatch, Remote selected actor, arbitrary Owner ID, 타인 Block ID와 A/B selected Profile 전환을 검증한다. exact remote materialization 완료 뒤 현재 selected Profile을 viewer로 사용한 `searchProfiles` 후보 filtering 순서와 Owner 관리 성공, 일반 Profile·nested field 우회 실패를 함께 확인한다.                                                                                                                                                                                                  |
