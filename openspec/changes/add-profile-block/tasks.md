...
관계와 durable Temporal cleanup orchestration을 구현한다. Block policy/admission을 적용하고 기존 Follow removal
transition/effect-plan을 양방향에 재사용하며, 필수 cleanup 완료 전에는 Block action을 성공으로 확정하지 않는다.

**Guardrails**

- Owner → Target 방향의 단일 관계, `createdAt`, Owner/Target unique·foreign-key·self-block 불변식과 no-backfill를 유지하고 별도
  state·expiry·복제 column을 추가하지 않는다.
...
- GraphQL ingress는 검증된 Session의 selected Local Profile actor 및 Owner scope를 사용하고, DB actor GUC·operation 전용 session·client-only
  filter로 권한을 대체하지 않는다. remote ActivityPub ingress는 `PROD-818`에 남긴다.
...
- [ ] 2.5 양방향 policy·interaction rejection·GraphQL actor isolation·Post List consumer·Notification visibility를 검증하는 API/Core integration test와
      schema check를 통과시킨다.
...
분리된 관리 목록과 selected Profile별 Relay/cache 수렴을 제공한다. 구체 route presentation을 선결하지 않으며 보호된
...
- selected Profile A/B와 Session 전환에서 Relay Environment/Store·connection·cursor·optimistic state isolation 및 Block 성공 후 보호된 Profile/Post/
  Notification hide를 검증한다.
...
- [ ] 3.3 selected Local Profile actor 경계 안에서 Block/Unblock 성공·실패 payload에 따라 관리 connection과 표시 중 Profile·Post·Notification cache를
      수렴시킨다.
...
- [ ] 4.2 cross-slice UI/API 결과와 selected Profile actor/cache isolation, 보존·비복구 및 후속 범위 경계를 플랫폼별 evidence로 기록한다.
