...
- 공통 객체 GRANT/default privilege는 후속 PROD-724에, API RLS policy는 PROD-713에, API/Fedify credential transition은 PROD-715/716에, workload credential selector는 PROD-709에 남긴다.
...
- 후속 관계: PROD-724, PROD-713, PROD-715/716과 PROD-709가 이 change가 고정한 역할 이름을 참조한다. 이 change는 객체 privilege, workload credential 전환이나 도메인 RLS policy를 포함하지 않는다.
