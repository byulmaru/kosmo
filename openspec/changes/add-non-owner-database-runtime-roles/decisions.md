...
- Decision Outcome: API는 `kubernetes/kosmo/prod/api-database`, Worker는 `kubernetes/kosmo/prod/worker-database`를 사용하고 각각 `<release-prefix>-postgres-api`, `<release-prefix>-postgres-worker` basic-auth Secret으로 투영한다. `<release-prefix>`는 가장 긴 runtime 접미사를 보존하도록 먼저 제한한다.
