...
Production Helm chart는 CloudNativePG bootstrap owner `kosmo`의 `-app` Secret을 API/Web에 계속 주입하고, 기본 비활성 Worker도 활성화 시 같은 owner fallback credential seam을 사용한다. 별도 `kosmo_migration` DatabaseRole과 basic-auth Secret에서 `kosmo_migration`은 owner membership을 받아 migration runner가 `SET ROLE kosmo`로 객체 ownership을 보존한다.
...
- Production에 `kosmo_api`, `kosmo_worker` 비소유 LOGIN을 별도 credential로 선언한다.
...
- CloudNativePG는 같은 이름의 기존 role도 DatabaseRole 선언에 맞춰 조정할 수 있으므로 live 적용 전에 `kosmo_api`, `kosmo_worker`의 선행 존재 여부와 현재 속성·membership을 민감 정보 없이 확인해야 한다.
...
1. Production에서만 두 VaultStaticSecret을 렌더하고, 각각 `kubernetes/kosmo/prod/api-database`, `kubernetes/kosmo/prod/worker-database`의 `username`/`password`만 접미사를 보존한 `<release-prefix>-postgres-api`, `<release-prefix>-postgres-worker` basic-auth Secret으로 투영한다.
...
4. Helm lint와 dev/prod render에서 dev 비렌더, production role/Secret isolation, 기존 API/Web와 기본 비활성 Worker owner fallback, API의 Worker credential 부재 및 migration manifest 무변경을 검증한다.
