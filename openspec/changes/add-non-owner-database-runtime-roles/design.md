...
Dev와 production Helm chart는 CloudNativePG bootstrap owner `kosmo`의 `-app` Secret을 API/Web에 계속 주입하고, 기본 비활성 Worker도 활성화 시 같은 owner fallback credential seam을 사용한다. Production의 별도 `kosmo_migration` DatabaseRole과 basic-auth Secret에서 `kosmo_migration`은 owner membership을 받아 migration runner가 `SET ROLE kosmo`로 객체 ownership을 보존한다.
...
- Dev와 production에 `kosmo_api`, `kosmo_worker` 비소유 LOGIN을 환경별 별도 credential로 선언한다.
- Production apply 전에 dev 실제 credential로 role identity·attribute·membership과 객체 권한 부재를 검증한다.
...
- Dev와 production role password Secret은 `kubernetes.io/basic-auth`의 `username`, `password`를 포함하고 `cnpg.io/reload: "true"` label이 있어야 CloudNativePG가 회전을 즉시 반영한다.
...
- Role CR을 삭제해도 dev/production DB identity를 자동 삭제하지 않는 것이 선택된 rollback 정책이다.
...
1. Dev와 production에 두 VaultStaticSecret을 렌더하고, 각각 `kubernetes/kosmo/<env>/api-database`, `kubernetes/kosmo/<env>/worker-database`의 `username`/`password`만 접미사를 보존한 `<release-prefix>-postgres-api`, `<release-prefix>-postgres-worker` basic-auth Secret으로 투영한다.
...
4. Helm lint와 dev/prod render에서 양쪽 환경의 role/Secret isolation, 환경별 Vault path, 기존 API/Web와 기본 비활성 Worker owner fallback, API의 Worker credential 부재 및 migration manifest 무변경을 검증한다.
5. Dev 적용 뒤 Secret value를 읽거나 출력하지 않고 VSO destination Secret type/key 존재, DatabaseRole applied 상태와 각 credential의 `current_user`·role attribute·membership·객체 ownership 부재를 검증한다. Production은 같은 절차와 별도 수동 승인 뒤에만 적용한다.
...
- [환경별 Vault path에 credential이 아직 없으면 VSO/DatabaseRole readiness가 실패한다] → workload는 기존 owner Secret을 계속 사용하므로 서비스 영향 없이 provisioning 상태만 실패하며, dev path부터 준비·검증한 뒤 production 승인을 진행한다.
...
1. Dev/prod render에 각각 환경별 두 Vault path/basic-auth Secret/DatabaseRole이 있고 기존 API/Web와 기본 비활성 Worker의 `-app` owner fallback 및 migration role/Secret이 동일함을 확인한다.
2. Dev Vault에 두 path의 `username`, `password`를 준비하고 동명 PostgreSQL role의 선행 존재 여부·속성·membership을 확인한 뒤 dev에 적용한다.
3. Dev VSO destination Secret과 DatabaseRole applied 상태를 확인하고 실제 credential로 role identity·attribute·membership·객체 ownership 부재를 민감 정보 없이 검증한다.
