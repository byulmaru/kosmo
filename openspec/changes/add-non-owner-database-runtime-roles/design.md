...
- API/Web/Worker 기본 DB 경로나 Web/Worker Fedify connection의 Secret 선택 및 실제 credential 전환.
...
- API/Web 기본 DB 경로와 기본 비활성 Worker credential seam은 계속 `<postgres-name>-app` owner fallback을 사용한다. Web inbound와 후속 Temporal Worker의 Fedify credential 선택은 PROD-715에 남기고 API Rollout에는 Fedify credential을 주입하지 않는다. 새 Secret을 rollout restart target이나 workload env에 연결하면 PROD-709 selector 및 PROD-715/716 transition 범위를 침범한다.
...
4. Helm lint와 dev/prod render에서 dev 비렌더, production role/Secret isolation, 기존 API/Web와 기본 비활성 Worker owner fallback, API의 Fedify credential 부재 및 migration manifest 무변경을 검증한다.
...
- 새 Secret을 API/Web/Worker env, migration Job, 공용 `env` Secret이나 rolloutRestartTargets에 연결하지 않는다. API Rollout에는 Fedify credential을 주입하지 않는다.
