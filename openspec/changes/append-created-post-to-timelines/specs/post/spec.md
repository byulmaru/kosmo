...
- **AND** payload는 생성 Post가 현재 selected Profile의 Home 또는 작성자 Profile Post List 후보이며 Control Decision이 `Exclude`가 아닌 각 surface에 nullable `homeTimelineEdge` 또는 `profilePostsEdge`를 반환한다
...
- **AND** payload edge 판정에는 같은 selected Profile을 viewer로 사용한다
...
- **WHEN** `createPost`가 Reply Parent가 있는 Post를 생성하고 서버가 `homeTimelineEdge`만 반환한다
...
#### Scenario: 서버가 제외한 surface를 변경하지 않음

- **WHEN** Post List Policy의 후보 또는 Control Decision 결과로 payload의 Home 또는 Profile edge가 `null`이다
