...
- mutation이 Relay `@prependNode`에 Home/Profile managed connection ID를 전달해 normalized `post`를 요청 actor Environment의 이미 로드된 connection에 최신순으로 한 번 반영한다. Reply는 Home ID만 전달한다.
...
- Universal client: `PostComposer` mutation, Home/Profile `@connection` identity, Relay `@prependNode`와 관련 테스트
