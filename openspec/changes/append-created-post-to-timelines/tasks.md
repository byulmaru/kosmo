...
`createPost`의 normalized `post`가 요청 actor의 이미 로드된 Home managed connection에 최신순·중복 없이 반영된다.

**Guardrails**

- 기존 `CreatePostPayload.post`와 API 작성 transaction을 유지한다.
- Home은 Original·Quote·Reply를 포함한다.
- connection이 없으면 record를 만들지 않으며 Post List 정책·cursor를 합성하지 않는다.
- actor Environment 전환과 Composer lifecycle guard를 유지한다.

**Verification**

- Relay compiler가 `post { id ...PostListItem_post }`와 Home connection handle을 통과하는지 확인한다.
- Relay compiler가 목록 fragment를 선택한 `@prependNode`와 client-only connection 변수를 처리하는지 확인한다.
- Composer Storybook 시나리오가 committed Post와 GraphQL 오류가 함께 온 응답을 성공으로 처리하고, Post가 없는 실패 응답에서는 입력을 보존하는지 확인한다.
- API schema/resolver 테스트는 `CreatePostPayload.post`만 유지하는지 확인한다.

- [x] 1.1 Home managed connection identity를 유지하고 Composer mutation의 Relay `@prependNode` directive에 필요한 connection ID만 전달한다.
