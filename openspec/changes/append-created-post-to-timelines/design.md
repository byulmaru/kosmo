...
`PostComposer`는 `createPost` 응답의 `post`를 Relay normalized Store에 기록하지만, 이미 렌더링한 Home 목록 membership은 자동으로 바뀌지 않는다. Home Query root의 managed connection ID를 mutation에 전달하고 `@prependNode`로 응답 Node를 반영한다.
...
- 모든 Post 작성 유형에서 Home connection ID만 전달한다.
...
1. Home Query에 안정적인 `@connection` handle을 유지한다.
...
3. commit 시 `ConnectionHandler.getConnectionID`로 Home managed connection ID만 계산한다. 이 client directive 변수는 서버 API input으로 전달되지 않는다.
