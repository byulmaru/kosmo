...
- 단일 `addReaction`은 `LOCAL`·`ACTIVITYPUB` origin으로 outbound provenance를 명시하고 optional caller transaction에는
  origin과 독립적으로 참여한다. caller transaction이 있으면 post-commit side effect는 caller가 소유하고, top-level
  actual create의 Notification은 origin과 무관하며 Fedify delivery만 Local origin으로 제한한다.
