...
- 단일 `addReaction`은 `LOCAL`·`ACTIVITYPUB` origin을 domain input으로 명시하고 transaction 인자 유무나 generic
  execution mode로 caller 의미를 표현하지 않는다. ActivityPub origin은 필수 caller transaction에 참여하며 outbound
  side effect를 일으키지 않는다.
- Local origin이 actual 변화를 commit하고 immutable Reaction row를 Fedify에 넘긴다. Fedify가 저장된 target
