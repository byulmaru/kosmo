...
#### Scenario: pure Repost만 Tombstone 이후 Delete Workflow를 시작함

- **WHEN** Content와 Reply Parent가 없고 Repost Source가 있는 Active pure Repost가 처음 Tombstone으로 전환되어 commit된다
- **THEN** 시스템은 committed Repost Post ID로 Delete Workflow start를 시도한다
...
- **THEN** pure Repost 전용 Delete Workflow를 시작하지 않는다
