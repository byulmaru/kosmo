...
- **THEN** 시스템은 committed relation shape에서 `effectKind=CONTENT` 또는 `effectKind=REPOST`를 도출해 `post-delete:{postId}` identity의 공통 Delete Workflow start를 시도한다
...
- **THEN** 시스템은 `effectKind=CONTENT`인 공통 Delete Workflow를 시작한다
