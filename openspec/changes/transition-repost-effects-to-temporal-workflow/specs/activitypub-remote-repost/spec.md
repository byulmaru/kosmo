...
- **AND** commit 뒤 Notification-only create effects Workflow start를 시도한다
...
- **AND** effects Workflow를 시작하지 않는다
...
- **AND** duplicate 결과는 새 effects Workflow를 시작하지 않는다
...
- **AND** Repost create transition이 아니므로 새 effects Workflow를 시작하지 않는다
...
- **AND** commit 뒤 Notification-only delete effects Workflow start를 시도한다

#### Scenario: superseded Announce Undo

- **WHEN** Announce A의 Repost mapping이 Announce B URI로 교체 완료된 뒤 Undo A가 도착한다
- **THEN** 시스템은 현재 Repost를 삭제하거나 effects Workflow를 시작하지 않는다
...
- **THEN** 시스템은 어떤 Repost도 삭제하거나 effects Workflow를 시작하지 않는다
