...
- [x] 1.4 Account·선택 Profile·인증 상태·PostHog session 변경을 귀속 종료에 연결한다. 네 가지 변경 자체는 새 journey를 만들지 않고, 종료 뒤 같은 검색 결과의 같은 대상을 다시 명시적으로 선택할 때만 새 `search_profile_journey_id`를 만들며 종료만 발생한 경우에는 분모를 만들지 않는지 검증한다. 기존 SDK identity·표준 metadata·fail-open·Native no-op을 유지하고 `search_profile_journey_id`가 Account·Profile·검색어에서 파생하지 않은 opaque 값인지, custom payload에 금지 속성이 없는지 검사하는 테스트를 작성한다. 실행은 1.5에 남긴다.
