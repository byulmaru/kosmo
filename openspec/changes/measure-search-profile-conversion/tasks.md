...
- [ ] 1.2 대상별 journey의 시작·중복 제거·성공 판정·30분 및 종료 경계를 구현하고 fake clock으로 입력·출력·상태 변화를 검증한다.
...
- [ ] 1.4 Account·선택 Profile·인증 상태·PostHog session 변경을 귀속 종료에 연결한다. 기존 SDK identity·표준 metadata·fail-open·Native no-op을 유지하고 custom payload의 금지 속성 부재를 확인한다.
- [ ] 1.5 관련 단위·Storybook·웹 통합 테스트와 Relay/typecheck/lint를 실행한다. 실제 UI 표시·mutation 응답·전송 payload로 실패·중복·시간 경계를 증명한다.
- [ ] 1.6 PostHog 조회·Follow funnel과 기준 dashboard를 구성해 design의 6개 journey와 추가 경계 fixture를 대조한다. 분모 6·전체 4·조회 3·Follow 2, 시작일·Asia/Seoul·잠정치·분모 0을 확인한다.
- [ ] 1.7 구현·검증 결과, dashboard/insight URL·query·환경·시각·남은 책임을 PR 또는 handoff에 남긴다. 필요하면 사용한 하네스를 같은 구현 PR에서 `--skip-specs`로 정리한다.
...
- Status: Active
- Completed: 승인된 계산 계약과 현재 실행 경로 조사, 구현·검증 계획 작성.
