...
**Migration:** 먼저 application을 URL index 유무에 호환되게 만든 뒤 별도 contract에서 `source=REMOTE` URL
partial unique index를 제거한다. 프로덕션은 아직 실서비스 전이므로 workload 배수나 rollback window 대기는
적용하지 않으며 기존 Media row와 참조는 재작성하지 않는다.
