...
- **GraphQL Fragment colocation**: `ProfileHero`는 자신이 필요한 필드를 `ProfileHero_profile` fragment로 직접 선언하고, 상위 `+layout.svelte`의 `ProfileLayoutQuery`는 `...ProfileHero_profile`를 spread해 필드 목록의 이중 관리를 없앴다. Storybook은 mearie client가 없으므로 `.storybook/mocks/mearie-svelte.ts`에서 `createFragment`를 fragment ref 패스스루로 모킹해 동일 스토리(`Default` / `No bio` / `Loading`)를 유지한다.
