...
- System Reserved Handle 목록을 소문자 handle 전체의 정확 일치로 검사한다.
...
- 현재 Local Instance의 기존 Profile handle 충돌을 점검하되 충돌 Profile을 자동으로 변경하지 않는다.
...
- 배포 전 기존 Local Profile handle을 새 정책으로 감사하고 결과에 따라 별도 migration 판단이 필요할 수 있다.
