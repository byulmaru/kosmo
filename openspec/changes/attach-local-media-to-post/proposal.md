...
- Local Media 완료 시 저장 서비스가 확정한 Original URL과 MIME을 Ready state와 함께 저장하고 기존 Ready
  Media를 backfill한다.
...
- `media-original-representation`: Local Media 완료 시 원본 공개 URL과 MIME을 Ready state와 함께 저장하고 기존
  Ready row를 backfill하는 계약
...
- Storage: 기존 `media.document` JSONB를 additive하게 사용하고 `media` table에 nullable Original URL·MIME
  column을 추가해 배포 중 호환성과 기존 Ready row backfill을 지원한다.
