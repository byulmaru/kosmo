...
edge에서 실제 차단하되 정상 ActivityPub 연합 요청은 그 AI 차단 규칙에 한정한 최소 예외로 보호한다.
...
- 저장소 응답에 canonical sitemap URL인 `https://kos.moe/sitemap.xml`을 게시하고, PROD-731이 제공할 배포된
  sitemap과의 통합을 검증한다.
- Cloudflare Managed `robots.txt`는 동적으로 관리되는 AI 학습·대량 수집 agent 차단과 Content Signal을
  소유하고, 저장소는 그 agent 목록을 중복 소유하지 않는다.
- 일반 검색 및 사용자가 요청한 조회용 크롤러는 허용하고, AI 학습·대량 수집 크롤러는 운영에서 합성되는
  Cloudflare Managed `robots.txt`와 edge 강제 차단으로 거부한다. 학습과 검색을 겸하는 crawler도 학습
  용도가 확인되면 강제 차단 범위에 포함한다.
- Cloudflare AI Crawl Control/WAF가 Training behavior 또는 공식 운영사 근거상 AI 대량 수집 용도로 확인된
  crawler의 일반 Web 요청을 차단한다. Search behavior를 함께 가진 crawler도 Training이 확인되면 포함하고,
  Search 전용 crawler와 사용자 요청 기반 Agent는 허용한다.
- ActivityPub discovery·actor·object·collection·inbox 경로는 일반 크롤러 규칙으로 차단하지 않는다. 정상
  federation 요청은 AI crawler 차단 규칙의 host·method·path·protocol 특성에 한정한 예외로 통과시키되 다른
  WAF·rate limit·보안 검사를 광범위하게 우회하지 않는다.
- 배포 후 최종 합성 `robots.txt`의 content type, 저장소 규칙, Cloudflare 관리 규칙, sitemap 선언과 edge의
  차단·ActivityPub 예외를 함께 검증하고 Cloudflare 설정 rollback 절차를 기록한다.
- `robots.txt`는 정책 표명, Cloudflare edge 규칙은 알려진 crawler의 강제 통제로 취급하며 미식별·위장
  crawler까지 완전 차단한다고 주장하지 않는다.
...
- Linear Implementations: PROD-736이 저장소 정책·자동 검증, Cloudflare 강제 차단·최소 ActivityPub 예외,
  운영 합성 응답과 edge 검증, rollback, OpenSpec 정합성 및 archive를 소유한다. PROD-731은 sitemap
...
- `apps/web`: `/robots.txt` 정적 파일 제공과 SPA fallback·federation-first routing 회귀 검증
- Cloudflare: Managed `robots.txt`/Content Signal, AI Crawl Control/WAF 강제 차단, 최소 ActivityPub 예외,
  Security Events 관찰과 rollback
- ActivityPub: WebFinger, actor, object, collection, inbox 요청이 AI crawler 강제 차단을 통과하되 기존
