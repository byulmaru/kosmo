...
- API, Web BFF와 Web browser는 Sentry project 하나를 공유하고 `runtime` tag로 구분한다. GitHub Actions는 branch build에 dev role, 정식 SemVer tag build에 prod role을 사용해 OIDC로 각각 Vault의 `secret/kubernetes/kosmo/dev|prod`를 읽고 `EXPO_PUBLIC_SENTRY_DSN`만 추출한다. GitHub repository variables의 조직·project slug와 repository secret의 upload token을 DSN과 임시 env 파일 하나로 합쳐 BuildKit secret으로 전달한다. API와 Web BFF는 Vault Secrets Operator가 환경별 객체를 변환한 기존 `env` Kubernetes Secret에서 같은 `EXPO_PUBLIC_SENTRY_DSN`을 읽는다.
