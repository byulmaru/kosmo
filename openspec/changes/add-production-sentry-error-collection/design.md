...
- strict allowlist 성격의 event 정제로 request·GraphQL·사용자 콘텐츠와 자동 breadcrumb를 제거한다.
...
- Web은 `.web` 관측 module에서 browser SDK를 초기화하고 React boundary의 `componentDidCatch`에서 수동 capture한다. `.native` module은 no-op으로 두어 Android·iOS 동작과 dependency linking을 바꾸지 않는다.
- event processor는 request/user/extra/context/breadcrumb를 제거하고 exception value를 오류 type 기반 일반값으로 바꾸며, environment/release/runtime과 stack frame만 남긴다. 자동 breadcrumb는 전부 비활성화한다.
