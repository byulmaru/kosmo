...
**Authority / Provenance:** `docs/domain/objects/profile.md`, `docs/domain/objects/post.md`, `docs/design/reply-composer.md`, `PROD-667`, [PROD-580](https://linear.app/byulmaru/issue/PROD-580/direct-%EA%B5%AC%ED%98%84-%EC%A0%84-composer%EC%9D%98-%EC%96%B8%EA%B8%89%ED%95%9C-%EA%B3%84%EC%A0%95%EB%A7%8C-%EC%98%B5%EC%85%98%EC%9D%84-%EC%9E%84%EC%8B%9C%EB%A1%9C-%EC%88%A8%EA%B8%B4%EB%8B%A4) (PROD-462 완료 전 Composer 임시 계약) MUST: 유니버설 앱은 현재 존재하는 새 일반 Post·Reply Composer에서 selected Local Profile의 기본 Post Visibility를
초기값으로 사용하고(MUST), 게시글 공개 범위를 platform에 맞는 menu 또는 modal control로 개별 변경할 수
있게 해야 한다(MUST). PROD-462가 Mentioned Profile recipient 입력·저장과 DIRECT 조회 권한을 완료하기
전까지 Composer는 `PUBLIC`, `UNLISTED`, `FOLLOWERS`만 선택·제출할 수 있게 해야 하며(MUST), `DIRECT`는 기존
enum과 서버 계약을 유지한 채 Composer 표면에서 숨겨야 한다(MUST).
...
- **WHEN** 새 일반 Post 또는 Reply Composer가 `PUBLIC`, `UNLISTED`, `FOLLOWERS` 중 하나인 selected Profile
  기본값과 함께 처음 표시된다
- **THEN** Composer는 해당 값을 선택한 공개 범위로 시작한다
- **AND** 공개 설정 control은 해당 값의 label과 icon을 표시한다
- **AND** Reply Visibility는 Parent Visibility와 독립적이다
...
- **WHEN** 사용자가 Composer의 공개 설정 surface 또는 키보드 탐색을 통해 새 공개 범위로 `DIRECT`를
  선택하거나 제출하려 한다
