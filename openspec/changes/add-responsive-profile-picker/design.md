...
- Android/iOS 또는 mobile Web drawer의 profile picker를 재설계하지 않는다.
...
- Web full sidebar와 mobile Web drawer는 현재 모두 `compact=false` 경로를 사용한다. `compact` boolean만으로 inline full surface를 선택하면 제외 범위인 mobile drawer 동작까지 바뀔 수 있다.
- `SidebarNavigation`의 navigation `ScrollView`는 picker를 감싸지 않으므로 긴 picker 목록의 scroll owner로 재사용할 수 없다.
- full profile header는 고정 높이와 absolute child 배치에 의존한다. inline picker를 absolute overflow로 붙이면 navigation을 자연스럽게 아래로 배치한다는 계약을 만족하지 못한다.
...
picker의 Relay data/action state와 반복 content를 유지하되, `full`·`compact`·`drawer` surface를 `compact` boolean과 별개로 명시적으로 전달한다. `ProfileSwitcher`는 trigger, Relay action state, list/footer content를 소유하고 full surface에서 `SidebarNavigation`이 제공하는 `renderSummary(trigger)` seam을 호출한다. `SidebarNavigation`은 기존 cover·avatar·profile detail과 전달받은 trigger를 정확히 260px 높이의 summary View 안에 렌더하며, `ProfileSwitcher`는 그 summary 다음에 inline picker를 실제 flow sibling으로 렌더한다. 따라서 닫힌 summary의 고정 높이와 mobile Web drawer·Android/iOS의 기존 fixed/absolute header 경로는 그대로 두면서 navigation만 열린 picker 뒤로 배치된다.

desktop compact에서는 새 portal이나 dependency를 추가하지 않고 `ProfileSwitcher`의 Web absolute layer를 80px rail 오른쪽에 배치한다. `UniversalShell`은 compact picker가 열린 동안 sidebar sibling의 stacking만 본문보다 높이고 column width는 80px로 유지한다. 현재 shell ancestor에는 overflow clipping이 없다는 전제에서 시작하되 768·1024·1279px paint 검증에서 clipping 또는 sibling stacking 실패가 확인되면 이 checkpoint에서 중단하고 portal/layer host 대안을 별도 승인받는다.
...
Storybook fixture는 기존 shell query fixture builder 안에서 10개 이상의 typed profile fixture를 제공한다. full과 compact surface의 trigger·expanded 상태·drawer/inline 위치, keyboard focus 가시성, 목록과 footer 접근성을 가장 가까운 story에서 검증하고, 기존 선택·생성 interaction과 Web E2E는 회귀 검증으로 실행한다. Storybook viewport preset이나 새 test harness는 추가하지 않고 768·1024·1279·1280·1440px는 browser resize로 직접 확인해 story URL과 관찰 결과를 기록한다.

### Allowed Alternatives

- compact overlay는 현재 shell hierarchy의 React Native Web absolute layer와 open-state sidebar stacking을 사용한다. exact-width paint 검증에서 이 경계로 clipping을 피할 수 없다고 확인될 때만 구현을 중단하고 Web 전용 portal/layer host를 후속 대안으로 다시 승인받는다.
...
- [inline picker가 짧은 viewport에서 navigation 접근성을 낮출 위험] → picker root를 viewport에 맞게 제한하고 목록만 줄어들도록 한다.
...
2. Web full/compact surface와 list/footer layout을 구현한다.
3. 10개 이상 fixture와 최소 interaction 검증을 추가하고 기존 선택·생성 E2E를 실행한다.
4. Storybook browser를 768·1024·1279·1280·1440px로 직접 조절해 responsive surface와 stacking을 시각 검증한다.
5. `add-shell-responsive-breakpoints`의 이전 compact popover 문구가 최종 active spec에 남지 않도록 change 적용 순서와 archive sync 결과를 확인한다.
