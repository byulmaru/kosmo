...
- [x] 1.1 `lib/components/ProfilePostList.svelte`를 추가한다. `HTMLAttributes` + `loading?: boolean` props를 받는 fragment 없는 프레젠테이션 컴포넌트로 시작하고, PROD-124에서 목록 fragment가 추가될 자리임을 주석으로 남긴다
...
- [x] 2.1 `(tabs)/@[handle]/+page.svelte`의 placeholder 문구를 제거하고 `ProfilePostList`를 렌더한다
- [x] 2.2 미래 목록 query 결과와 같은 모양의 더미 상수(기본 loading)를 한 곳에 두고, `TODO(PROD-124)` 주석으로 `createQuery` 1:1 교체 지점과 빈 상태 분기의 선제 구현임을 명시한다

## 3. Storybook

- [x] 3.1 `lib/components/ProfilePostList.stories.svelte`를 추가한다(`KOSMO/ProfilePostList`): `Playground`(loading 컨트롤)와 `States`(로딩·빈 상태 동시 표시) 스토리
