...
- 저장된 remote profile은 GraphQL Node와 domain-qualified handle 조회 대상으로 열어두되, bare/local handle 조회, active profile 선택, follow/unfollow/viewerFollow 동작, `Profile.posts` 확장은 local profile 중심으로 유지한다.
...
- `profile`: `Profile.relativeHandle`과 저장된 remote profile의 Node/domain-qualified handle 조회 계약을 추가하고, bare/local handle 조회와 follow graph/mutation은 local profile 중심으로 명확히 한다.
...
- `apps/api`: GraphQL `Profile.relativeHandle` 필드, remote profile Node/domain-qualified handle 조회 정책, remote profile active selection/follow/unfollow/viewerFollow 차단 정책, remote profile posts 빈 connection 정책을 반영한다.
