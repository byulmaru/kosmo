...
- 저장된 remote profile은 GraphQL Node 조회 대상으로 열고, active profile 선택과 session restore는 account ownership 및 active/non-`SUSPENDED` visibility를 기준으로 instance kind와 무관하게 처리한다. 기존 handle 조회와 신규 remote follow 생성, UI 연결, `Profile.posts` 확장은 후속 capability로 유지하되, 이미 저장된 visible follow 관계의 조회와 unfollow는 instance kind로 차단하지 않는다.
...
- `apps/api`: GraphQL `Profile.relativeHandle` 필드, remote profile Node 조회, ownership/visibility 기반 active profile 선택·session restore, visible follow 관계 조회·unfollow, remote 신규 follow 차단, remote profile posts 빈 connection 정책을 반영한다.
