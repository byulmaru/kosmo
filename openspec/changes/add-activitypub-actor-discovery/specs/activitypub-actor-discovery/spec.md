...
웹 애플리케이션은 canonical 웹 origin에서 ActivityPub discovery 요청을 받을 수 있도록 `packages/fedify`가 제공하는 Fedify federation instance를 `@kosmo/fedify/sveltekit` hook adapter로 SvelteKit hook에 연결해야 한다(MUST).
