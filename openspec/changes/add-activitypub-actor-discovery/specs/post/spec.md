...
- **WHEN** 인증되지 않았거나 현재 active profile이 조회 대상 configured local profile을 팔로우하지 않는 클라이언트가 프로필의 `posts` connection을 조회한다
...
- **WHEN** 현재 active profile이 조회 대상 configured local profile을 팔로우하고 해당 프로필의 `posts` connection을 조회한다
