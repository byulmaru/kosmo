...
- **remote profile에 지원되지 않는 workflow가 붙은 것처럼 보일 수 있음** → 신규 remote follow 생성은 profile not found 오류로 닫아 delivery 없는 관계를 만들지 않는다. 선택·session restore는 ownership/visibility, 기존 follow 관계 조회·unfollow와 저장된 Post 조회는 기존 visibility를 실제 invariant로 사용한다. `Profile.posts`는 kosmo DB에 이미 저장된 Post만 반환하고 federation fetch나 materialization을 수행하지 않아 remote post ingestion이 구현된 것처럼 보이지 않게 한다.
