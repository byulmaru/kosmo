...
- **remote profile은 Node 직접 조회까지만 연다.** 저장된 remote profile은 GraphQL `Profile` object로 읽을 수 있지만, 기존 local entry point인 handle 조회, 검색, 목록, active profile 선택, follow graph, follow/unfollow mutation은 local profile 기준을 유지한다. remote target follow/unfollow는 profile not found 오류로 닫고 ActivityPub Follow/Undo를 발송하지 않는다. remote profile의 `Profile.posts`는 빈 connection으로 응답하고 remote actor/post/collection fetch를 시도하지 않는다. 이 제한은 이번 cycle의 구현 범위를 단순화하기 위한 API surface 경계이며, remote profile 저장 모델에 별도 account/session 불변식을 새로 강제하는 목표는 아니다.
