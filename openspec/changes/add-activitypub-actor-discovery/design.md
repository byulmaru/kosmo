...
- **ActivityPub profile의 API 경계는 기능별 실제 invariant로 정한다.** 저장된 ActivityPub profile은 GraphQL `Profile` object로 읽을 수 있지만, remote profile을 `AccountProfile`이나 `Session`에 연결하는 production write path는 이번 capability에 포함하지 않는다. 정상 제품 경로의 active profile 선택과 session restore는 local profile membership만 다루며 기존 account membership·visibility 동작을 유지한다. 제품 경로 밖에서 만든 synthetic remote membership의 선택·복원·거부 동작은 새 계약으로 정의하지 않고 별도 kind guard도 추가하지 않는다. 기존 `profileByHandle`은 configured local 조회로 유지하고, `followProfile`은 ActivityPub delivery 없이 신규 remote 관계를 만들지 않도록 local target만 허용한다. 반면 follow graph와 `viewerState.follow`는 저장된 visible 관계를 그대로 반영하고, `unfollowProfile`은 visible target의 기존 관계를 instance kind와 무관하게 제거할 수 있다. 저장된 active Post는 작성 profile의 instance kind와 무관하게 기존 게시글 공개 범위를 적용해 Node와 `Profile.posts`에서 읽을 수 있다. 이 GraphQL read path는 remote actor/post/outbox/collection fetch나 Post materialization을 수행하지 않는다. ActivityPub `Follow`/`Undo`도 발송하지 않는다.
...
- **remote profile에 지원되지 않는 workflow가 붙은 것처럼 보일 수 있음** → remote profile을 계정이나 session에 연결하는 제품 write path와 신규 remote follow 생성은 remote profile capability에 포함하지 않는다. synthetic membership의 active profile 선택·session restore 동작은 이번 capability에서 보장하거나 거부하지 않는다. 기존 follow 관계 조회·unfollow와 저장된 Post 조회는 기존 visibility를 실제 invariant로 사용한다. `Profile.posts`는 kosmo DB에 이미 저장된 Post만 반환하고 federation fetch나 materialization을 수행하지 않아 remote post ingestion이 구현된 것처럼 보이지 않게 한다.
