...
- 저장된 remote profile의 검색/목록/UI 연결, active profile 선택, remote profile 대상 local follow graph 확장, remote profile posts fetch.
...
- **Fedify 연결은 `apps/web` hook adapter, federation 로직은 `packages/fedify`가 소유한다.** `apps/web`은 `hooks.server.ts`에서 `packages/fedify`가 export한 federation instance 또는 hook factory를 SvelteKit `handle`에 연결한다. federation 요청 판별, WebFinger resource parsing/JRD 응답 조립, actor assembly, key dispatch는 package로 분리해 Svelte component, SvelteKit route handler, web hook 본문에 ActivityPub 세부를 넣지 않는다.
...
- **remote profile은 Node 직접 조회까지만 연다.** 저장된 remote profile은 GraphQL `Profile` object로 읽을 수 있지만, 기존 local entry point인 handle 조회, 검색, 목록, active profile 선택, follow graph, follow/unfollow mutation은 local profile 기준을 유지한다. remote target follow/unfollow는 profile not found 오류로 닫고 ActivityPub Follow/Undo를 발송하지 않는다. remote profile의 `Profile.posts`는 빈 connection으로 응답하고 remote actor/post/collection fetch를 시도하지 않는다.
