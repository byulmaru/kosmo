...
- 저장된 remote profile의 UI 연결, remote profile을 AccountProfile/Session에 연결하는 production write 경로, remote profile 대상 신규 follow 생성, remote profile posts fetch.
...
- **ActivityPub profile의 API 경계는 기능별 실제 invariant로 정한다.** 저장된 ActivityPub profile은 GraphQL `Profile` object로 읽을 수 있고, account가 소유한 active/non-`SUSPENDED` profile이면 instance kind와 무관하게 active profile로 선택하거나 session에서 복원할 수 있다. 기존 `profileByHandle`은 configured local 조회로 유지하고, `followProfile`은 ActivityPub delivery 없이 신규 remote 관계를 만들지 않도록 local target만 허용한다. 반면 follow graph와 viewerFollow는 저장된 visible 관계를 그대로 반영하고, `unfollowProfile`은 visible target의 기존 관계를 instance kind와 무관하게 제거할 수 있다. ActivityPub `Follow`/`Undo`는 발송하지 않으며, ActivityPub profile의 `Profile.posts`는 빈 connection으로 응답하고 remote actor/post/collection fetch를 시도하지 않는다. 현재 production 경로가 remote profile을 AccountProfile이나 Session에 연결하지 않는다는 사실만으로 별도 kind guard를 추가하지 않는다.
...
- **remote profile에 지원되지 않는 workflow가 붙은 것처럼 보일 수 있음** → 신규 remote follow 생성은 profile not found 오류로 닫아 delivery 없는 관계를 만들지 않는다. 선택·session restore는 ownership/visibility, 기존 follow 관계 조회·unfollow는 visibility를 실제 invariant로 사용하고, `Profile.posts`는 빈 connection으로 응답해 remote post fetch가 있는 것처럼 보이지 않게 한다.
