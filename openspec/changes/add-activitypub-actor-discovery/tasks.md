...
- [ ] 3.1 `Profile.displayHandle` 필드를 추가해 현재 local-only profile 모델에서는 `@handle`을 반환한다.
...
- [ ] 3.6 GraphQL schema를 재생성하고 `Profile.displayHandle`이 `apps/api/schema.graphql`에 반영되는지 확인한다.
