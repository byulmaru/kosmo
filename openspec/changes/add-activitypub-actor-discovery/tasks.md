...
- [x] 3.4 active profile selection과 session restore가 account ownership + active/non-SUSPENDED 기준으로 remote profile도 허용하고, follow graph와 viewerFollow는 저장된 visible 관계를 반영하며, 신규 remote follow 생성은 차단하고 unfollow는 visible profile이면 instance kind와 무관하게 동작하도록 구현한다. 저장된 active remote profile의 Post는 일반 게시글 공개 범위를 적용해 읽되, `Profile.posts` 조회에서 federation fetch나 Post materialization은 수행하지 않는다.
