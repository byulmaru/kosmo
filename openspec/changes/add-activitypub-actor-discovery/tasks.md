...
- [x] 3.4 active profile selection과 session restore가 account ownership + active/non-SUSPENDED 기준으로 remote profile도 허용하고, follow graph와 `viewerState.follow`는 저장된 visible 관계를 반영하며, 신규 remote follow 생성은 차단하고 unfollow는 visible profile이면 instance kind와 무관하게 동작하도록 구현한다. 저장된 active remote profile의 Post는 일반 게시글 공개 범위를 적용해 읽되, `Profile.posts` 조회에서 federation fetch나 Post materialization은 수행하지 않는다.
...
- [ ] 6.4 GraphQL 테스트로 `relativeHandle` local 표시와 profile-instance 관계 도입 이후 configured local/non-configured local/remote 표시, remote profile Node 조회, `profileByHandle` local-only 동작, 다른 instance 동일 handle의 local profile 생성 허용과 configured local duplicate handle conflict, unowned remote target active profile selection profile not found, owned remote target active profile selection/session restore 성공, remote follow profile not found, remote unfollow 허용, remote target에 저장된 follow 관계가 없을 때 `viewerState.follow` 없음 응답, 저장된 active remote profile Post가 일반 공개 범위로 Node와 `Profile.posts`에서 조회되는 동작을 검증한다.
