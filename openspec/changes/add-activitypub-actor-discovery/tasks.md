...
- [x] 3.1b `Profile.origin`은 별도 중복 enum 없이 프로필이 속한 instance의 `InstanceKind`를 반환하도록 구현한다.
- [x] 3.2 `profileByHandle(handle:)`가 bare/configured-local handle은 configured local instance에서 조회하고, 다른 domain-qualified handle은 저장된 active ActivityPub profile에서만 조회하며 remote fetch/materialization을 시도하지 않도록 구현한다.
- [x] 3.3 Node ID 기반 `Profile` load가 active local profile과 저장된 active remote profile을 직접 조회할 수 있게 접근 정책을 정렬한다.
- [x] 3.4 domain-qualified `profileByHandle` 조회 외의 active profile selection, follow graph, follow/unfollow mutation, viewerFollow, `Profile.posts`가 remote profile로 확장되지 않고 local profile 기준으로 동작하는지 구현한다.
