...
- [x] 1.4 `packages/fedify`가 federation request 판별, framework-neutral federation instance/request handler 구성요소, actor dispatcher, WebFinger handle mapper/JRD 응답 조립, key pair dispatcher를 export할 수 있는 모듈 구조를 만든다.
...
- [ ] 3.6 GraphQL schema를 재생성하고 `Profile.relativeHandle`이 `apps/api/schema.graphql`에 반영되는지 확인한다.
