...
PROD-639는 clipboard payload에 하나 이상의 image item이 있으면 지원 이미지만 첨부하고 함께 제공된 Plain Text·링크·HTML은 본문에 삽입하지 않도록 확정했다. image item이 없는 payload만 browser의 기본 Plain Text·링크 paste를 유지한다.

## Goals / Non-Goals

**Goals:**

- Web Composer 본문에 focus한 image paste를 기존 picker Media 목록과 upload lifecycle에 연결한다.
- image와 텍스트가 함께 있는 payload는 지원 이미지만 첨부하고 본문·selection을 보존한다.
...
- 혼합 image+text payload는 browser가 같은 clipboard 내용을 여러 표현으로 제공할 수 있다. image 후보를 처리하면서 기본 paste까지 허용하면 이미지와 텍스트가 함께 반영되어 승인된 결과와 달라진다.
...
3. 하나 이상의 image 후보가 있는 payload는 남은 슬롯을 계산한 뒤 event 기본 동작을 막고 함께 제공된 Plain Text·링크·HTML을 사용하지 않는다. 수용한 File마다 object URL preview와 MIME metadata를 가진 기존 upload 입력 형태로 정규화한다. picker와 clipboard가 같은 item 추가·업로드 함수로 들어가게 해 source별 lifecycle 분기를 만들지 않는다.
...
6. 순수 함수 수준에서는 clipboard item 순서·필터·남은 슬롯과 preview cleanup을 검증한다. Storybook browser test는 focus된 editor의 image paste, image+text payload의 이미지 전용 처리, picker와의 혼합 순서, 실패·재시도·제거, text-only/link paste를 검증한다. Playwright compose E2E는 실제 DOM paste event로 image File과 텍스트를 함께 전달해 upload mutation/PUT/완료와 제출 Media 순서, 본문 비변경을 확인하고 Composer 밖 paste 회귀를 확인한다.
...
- image item이 있는 payload의 기본 paste를 함께 허용해 Plain Text·링크·HTML을 본문에 삽입하지 않는다.
...
- [혼합 payload가 이미지와 텍스트를 중복 반영함] → 하나 이상의 image 후보를 처리할 때 기본 paste를 취소하고 본문·selection 비변경을 component/E2E에서 검증한다.
