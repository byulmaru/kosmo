...
- GraphQL은 `addReaction(postId: ID!, type: String!)`과 `AddReactionPayload.reaction: Reaction!` 계약을 유지하며 신규 생성 여부를 공개하지 않는다.
- Notification 생성은 PROD-413 범위이며 PROD-404는 side effect를 호출하지 않는다. service는 후속 연결을 위해 신규/기존 관계를 내부적으로 구분할 수 있다.

**Verification**

- 성공, 허용되지 않은 Type, Local/상태/membership/Post 권한 실패, 반복·동시 요청과 rollback을 database-backed test로 검증한다.
- GraphQL payload·Node와 validation/`NOT_FOUND` error 계약을 integration test로 검증한다.
