...
- GraphQL `usingProfile` entry point는 Active Account와 Account–Profile membership을 검증하고, core service는 검증된 actor Profile identity의 Active/Normal Local 상태를 검증한다.
...
- 성공, 허용되지 않은 Type, Local/Profile/Instance/Post 상태 실패, 반복·동시 요청과 rollback을 core database-backed test로 검증한다.
