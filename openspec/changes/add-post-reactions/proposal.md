...
- Profile, Post와 안정적인 Reaction Type identity를 참조하는 Reaction 저장 계약을 추가한다.
...
- 이미지형 custom emoji를 수용할 수 있도록 Reaction이 Type identity를 참조하되, custom emoji 업로드·소유 범위·shortcode·asset·비활성화 정책 자체는 후속 계약으로 남긴다.
...
- `data-model`: Reaction, Reaction Type identity와 Reaction Notification source를 저장하는 관계·제약 조건 추가
...
- Linear: PROD-395, PROD-404, PROD-405, PROD-406, PROD-407, PROD-413, PROD-417, PROD-418, PROD-419와 부모 PROD-390
- Core/DB: Reaction Type 및 Reaction schema, migration, 무결성·index 검증
- GraphQL/Core service: 생성·삭제 mutation, count와 Profile connection, Reaction Notification concrete type와 loader
- Universal client: Reaction selector·summary component, Relay mutation/cache·pagination, component/integration 검증
...
- Excluded systems: ActivityPub federation, custom emoji 실제 구현, 범용 Notification 재설계, 공통 Post Action Bar rollout
