...
- GraphQL의 create 계열 mutation은 `fieldWithInput`, concrete Node global ID와 simple payload object를 사용한다. `addReaction`은 이 관례를 따라 Post global ID와 Type 문자열을 받고 Reaction Node와 현재 Post를 반환한다.
...
- add/delete mutation payload는 현재 Post의 `viewerReactions`와 `reactionCounts`를 반환하고 Relay가 이를 authoritative 상태로 정규화한다. client는 수동 updater, local count delta 또는 targeted refetch를 사용하지 않는다. mutation callback은 요청을 시작한 Relay Environment와 actor token 안에서만 현재 surface UI를 변경한다.
...
8. PROD-417은 기존 `PostActionBar`에서 private `ReactionAction`과 `ReactionPopover`를 유지하고 private `PostReactionController`가 fixed 여섯 option, `viewerReactions`, Type별 pending/error와 mutation/cache를 공급하게 한다. Quick Picker와 summary token은 같은 toggle을 사용한다. 선택/count는 optimistic하게 바꾸지 않고 add/delete payload Post의 `viewerReactions`와 `reactionCounts`를 Relay가 authoritative하게 정규화한다. partial payload·actor 격리 계약을 유지한다.
...
- mutation payload의 Post가 같은 Post의 `viewerReactions`와 `reactionCounts`를 완전히 정규화해야 한다. client 수동 updater나 후속 refetch를 fallback으로 추가하지 않는다.
...
- mutation 성공 전 count를 바꾸거나 actor 전환 뒤 이전 mutation callback으로 새 actor UI 상태를 덮어쓰지 않는다.
- delete의 nullable `reactionId`를 실패로 취급하거나 payload Post 대신 client에서 선택/count 상태를 추측하지 않는다.
...
- [mutation payload가 늘어남] → add/delete가 이미 주소화한 Post의 현재 `viewerReactions`와 `reactionCounts`만 함께 반환해 별도 client 상태·network round trip을 제거한다.
