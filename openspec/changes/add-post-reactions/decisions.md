...
- Confirmation / Follow-up: PROD-450 Storybook/component interaction에서 supplied order, 현재 여섯 fixture, 선택·해제·복수 Type, option별 border·radius·pending overlay·error와 전체 disabled 미렌더링 및 callback을 검증한다. PROD-417은 실제 Relay data shape, mutation 성공·실패 복구와 selected Profile 전환을 별도로 검증한다.
