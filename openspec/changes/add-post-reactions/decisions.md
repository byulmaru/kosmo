...
- Confirmation / Follow-up: production updater seam을 직접 호출하는 unit test에서 add same-Type/different-ID replacement, same-ID 중복 방지, 다른 Type 보존, payload/Post/field 부재, delete authoritative list·`post: null` fallback·nullable `reactionId`, add 성공 뒤 delete, reverse completion과 actor 전환 stale callback을 검증한다. Storybook integration은 Type별 pending/error, 실패·재시도, unmount와 actor 전환을 검증한다.
