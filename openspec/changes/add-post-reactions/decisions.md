...
- Authority / Provenance:
  ...
  - `PROD-449`
- Status: Active
- Context / Problem: canonical Reaction 계약은 viewer-independent Type별 count의 server 내림차순과 viewer-filtered Profile 목록을 요구하지만, PROD-449의 독립 전달 범위는 실제 Relay query·connection·modal/route 조립 전의 프레젠테이션과 상태 검증이다.
- Decision Outcome: PROD-449는 공급된 count entry를 받은 순서대로 렌더하는 props-only `ReactionSummary`와 props-only `ReactionProfileList`를 제공한다. component는 zero-count Type을 만들거나 제거·정렬·필터링하지 않고, supplied loading/empty/error/populated 상태와 selection·retry·pagination callback만 노출한다. Profile row는 기존 `ProfileListItem`에 Relay `Profile` fragment ref를 전달해 재사용하며, Storybook은 raw `$key` cast 없이 Relay mock fragment ref를 사용한다.
...
- Consequences: 실제 `Post` count query와 `reactionProfiles` connection, modal/route, selected Profile/viewer cache 통합은 PROD-418에 남는다. supplied order는 server가 제공한 count 내림차순과 동률 무보장 계약을 그대로 보존하며, component는 이를 재해석하지 않는다.
- Confirmation / Follow-up: PROD-449는 fixture state, 복수 Type·동률, 기존 Profile row, callback interaction과 Relay mock fragment Storybook을 component 수준에서 검증한다. PROD-418은 실제 query/connection, zero-count와 modal/route UX, cache 통합 및 end-state pagination 검증을 후속으로 수행한다.
