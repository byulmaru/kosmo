...
- 새 Reply가 조상 행의 형제 분기이거나 현재 pagination 범위 밖이면 targeted refetch 뒤에도 현재 thread에 나타나지 않을 수 있다. 이 경우에도 성공 snackbar의 `보기` action으로 결과 Reply에 접근할 수 있어야 하며, 이를 이유로 자동 이동하거나 thread 정렬·membership을 합성하지 않는다.
