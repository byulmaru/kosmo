...
현재 main의 `GraphQLErrorBoundary`, `RouteBoundary`, `SessionFailOpenBoundary`는 각각 React `Component`를 직접 상속한다. PR #375는 Web Sentry를 연결하기 위해 첫 구현을 `GraphQLErrorBoundaryBase`로 옮기고 세 경계에서 error와 `componentStack`을 reporter로 전달하는 구조를 추가하고 있으므로, 이 선행 변경은 같은 reporter 계약을 함수형 조합 지점으로 제공해야 한다.
...
- 기존 fallback, retry/reset, reset key, Suspense와 선택적 reporter 동작을 검증한다.
...
- PR #375는 Web에서만 reporter를 Sentry에 연결하고 공용/Native 경계는 해당 모듈을 import하지 않아야 한다.
- class fixture를 일반 `.tsx`로 두면 정상 lint가 항상 실패하므로 회귀 테스트가 명시적으로 source를 읽어 ESLint API에 전달해야 한다.

### Recommended Approach

세 경계가 `react-error-boundary`의 `ErrorBoundary`를 직접 조합하고 함수형 fallback renderer에서 `resetErrorBoundary`를 사용한다. 공용 경계 props에는 선택적 `onError`를 두고 라이브러리가 전달하는 error와 React error info를 그대로 전달한다. GraphQL/route는 `onReset`에서 기존 callback을 실행하고, session은 `resetKeys`로 기존 reset key 계약을 표현한다.
...
- PR #375의 Sentry 구현을 이 선행 PR로 가져오거나 공용 코드에서 Web 모듈을 import하지 않는다.
...
3. #375에서 추가한 class base/context wiring을 함수형 경계의 선택적 reporter prop에 맞게 정리하고 Web Sentry 수집 테스트를 재검증한다.
