...
만들며, `PROD-527`이 같은 component를 연결한다. Follow Approval Policy는 `PROD-531`로 분리되어 이 화면에서
읽거나 저장하지 않는다.
...
- displayName, bio와 avatar/header 관계를 검증하고 원자적으로 저장한다.
...
- Follow Approval Policy 변경, Profile Link·handle과 Profile lifecycle
...
- presentation component가 Expo Router나 Relay Environment를 직접 읽으면 Storybook state와 후속 wrapper 재사용이
  어렵다.
- current updateProfile input과 resolver는 임의 Profile id·followPolicy를 포함하고 Media 관계를 지원하지 않는다.
...
1. `ProfileEditScreen`과 `ProfileEditForm`을 React Native primitive와 theme token으로 만들고 값, validation,
   image state, save state와 callback을 controlled prop으로 받는다. 현재 avatar/header를 초기 draft로 두고 각
   preview의 편집 control이 자기 field만 갱신하게 한다. optional submit callback이 없거나 draft가 초기값과 같으면
   저장을 disabled로 둔다.
2. 표시 이름·bio와 Profile Tag의 로컬 입력·추가·제거·validation만 presentation 경계에 둔다. Profile Tag
...
5. update service를 selected Profile identity 기준으로 정렬하고 displayName·bio와 avatar/header 관계를 한
...
- Follow Approval Policy를 기존 update input에 있다는 이유로 Profile edit form에 다시 넣지 않는다.
...
2. PROD-492가 API/Core authorization·Media 관계와 protected route를 연결하고 production entrypoint를 활성화한다.
...
5. rollback 시 production entrypoint를 먼저 비활성화하고 additive API/Media 관계 변경은 해당 migration 정책에
   따라 보존한다.
