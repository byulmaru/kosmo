...
- **프로필 이동은 `ProfileListItem`의 정보 영역 링크로 제공한다.** 검색 결과 항목 안에는 `FollowButton`이 함께 있으므로 row 전체를 `<a>`로 감싸지 않는다. 아바타·이름·handle·bio 영역만 `/@{handle}` 링크가 되고, 팔로우 버튼은 독립 버튼으로 유지한다.
