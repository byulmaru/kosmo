...
- follow 성공 시 열린 connection에 새 edge를 직접 삽입하지 않는다. established unfollow는 relation/count/button만 optimistic하게 전환하고 connection row를 진행 중 유지하며, 성공 payload에서만 기존 edge를 제거하고 실패 시 유지된 행에서 rollback 오류를 표시하는 경계를 Relay-backed Storybook과 Web E2E로 검증한다.
