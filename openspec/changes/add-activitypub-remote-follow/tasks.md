...
- [ ] 4.2 Reject를 exact row와 expected generation 조건으로 삭제한다.
- [ ] 4.3 actor/object/recipient mismatch와 SUSPENDED/UNRESPONSIVE 상태를 검증한다.

## 5. PROD-245/263 Graph와 Web Action
...
- [ ] 5.2 PROD-263이 remote follow action과 양쪽 count/viewer state cache 갱신을 구현한다.

## 6. PROD-282와 PROD-235 통합 Gate
...
- [ ] 6.1 PROD-282가 existing/no-relation SUSPENDED unfollow의 NotFound와 relation/count 불변을 GraphQL에서 검증한다.
- [ ] 6.2 PROD-235가 Follow/Undo/Accept/Reject, graph와 Web action 통합 검증을 수행한다.
- [ ] 6.3 전체 task 완료 후 delta spec을 동기화하고 `openspec validate --all --strict`를 통과한 뒤 change를 archive한다.
