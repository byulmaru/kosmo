...
#### Scenario: Read accepted followers for local or remote profile
...
#### Scenario: Read accepted following for local or remote profile
...
#### Scenario: Count accepted follows for local or remote profile
...
#### Scenario: Retry rejected remote follow

- **WHEN** active profile이 있는 인증자가 `REJECTED` 상태의 기존 ActivityPub remote profile follow를 다시 요청한다
- **THEN** 시스템은 기존 `ProfileFollow`를 새 remote follow 시도로 전환하거나 대체할 수 있다
- **AND** 새 시도는 새 outbound Follow activity identity를 사용해야 한다
- **AND** 기존 `REJECTED` 관계의 unique 제약 때문에 재요청이 막히면 안 된다
- **AND** mutation은 새 시도의 `ProfileFollow`를 `FollowProfilePayload.profileFollow`로 반환한다
...
- **WHEN** active profile이 있는 인증자가 없는 대상 프로필, 비활성인 대상 프로필, 또는 `SUSPENDED`/`UNRESPONSIVE` instance의 remote profile unfollow를 요청한다
- **THEN** 시스템은 profile not found 오류를 반환한다
