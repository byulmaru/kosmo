...
시스템은 도메인 상태와 정책 값을 제한된 enum 값으로 저장해야 하며, 성립된 팔로우 관계의 상태 값은 저장하지 않아야 한다(MUST).

#### Scenario: enum 값 사용

- **WHEN** 계정, 프로필, 세션, OAuth token, 애플리케이션, 게시물, 팔로우 정책, 계정-프로필 역할, 미디어가 저장된다
...
- **AND** `ProfileFollow` 행의 존재 자체가 established follow 관계를 의미하므로 `ProfileFollowState` enum은 저장하지 않는다
- **AND** pending follow 요청은 `ProfileFollowRequest`로 표현하고 `ProfileFollowState`로 표현하지 않는다
