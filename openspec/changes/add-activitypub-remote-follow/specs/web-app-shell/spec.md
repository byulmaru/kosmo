...
- **WHEN** 대상 ActivityPub remote profile이 자기 자신이거나, 비활성 profile이거나, `SUSPENDED`/`UNRESPONSIVE` instance에 속하거나, `followPolicy`가 `APPROVAL_REQUIRED`이고 request flow가 아직 제공되지 않는다
- **THEN** 시스템은 local profile 대상의 기존 self/blocked/unsupported 정책과 같은 방식으로 follow action을 숨기거나 사용할 수 없게 한다
