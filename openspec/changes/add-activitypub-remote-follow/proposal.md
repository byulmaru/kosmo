...
- remote Accept/Reject activity가 저장된 outbound Follow와 actor/object 기준으로 일치하면 follow 관계 또는 pending request를 생성/삭제한다.
...
- `data-model`: remote follow activity correlation metadata와 `ProfileFollowRequest` 연계 요구사항을 추가한다.
...
- `packages/core/db`: outbound Follow activity identity와 inbound Follow correlation metadata를 `ProfileFollow` 또는 `ProfileFollowRequest`와 연결할 수 있어야 한다.
