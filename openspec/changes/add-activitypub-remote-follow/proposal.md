...
- `profile`: remote profile 대상 follow/unfollow, viewerFollow, viewerState follow, profile origin의 remote follow action, known follow graph visibility와 저장된 profile count 계약을 확장한다.
...
- `apps/app`: 공용 Relay `FollowButton`과 profile/list/search route가 `Profile.origin = ACTIVITYPUB`인 대상에서도 Android/iOS/Web에서 같은 상태와 mutation으로 동작한다.
