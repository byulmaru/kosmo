...
- remote `Profile`의 ActivityPub followers/following collection은 full mirror하지 않고, GraphQL connection은 kosmo DB의 known `ProfileFollow` 관계만 노출한다. `followersCount`/`followingCount`는 actor materialization/refresh 때 DB에 저장한 remote collection count를 사용한다.
...
- `profile`: remote profile 대상 follow/unfollow, viewerFollow, viewerState follow, known follow graph visibility와 저장된 remote collection count 계약을 확장한다.
