...
- `Profile`은 local/remote 여부와 관계없이 followers/following count를 DB에 저장하고, GraphQL `followersCount`/`followingCount`는 저장된 profile count를 사용한다. Remote `Profile`의 ActivityPub followers/following collection은 full mirror하지 않고, GraphQL connection은 kosmo DB의 known `ProfileFollow` 관계만 노출한다.
...
- `profile`: remote profile 대상 follow/unfollow, viewerFollow, viewerState follow, known follow graph visibility와 저장된 profile count 계약을 확장한다.
