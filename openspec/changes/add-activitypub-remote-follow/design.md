...
- **SUSPENDED remote target의 follow action은 차단한다.** `SUSPENDED` instance의 remote profile은 GraphQL action 대상이 아니므로 기존 관계 유무와 관계없이 follow/unfollow에서 `Profile not found`를 반환한다. 기존 `ProfileFollow`와 저장 count는 instance가 다시 활성화될 때까지 보존하며 ActivityPub activity를 발송하지 않는다.
...
- **노출 불가 전환은 follow row를 보존한다.** profile을 `DISABLED`로 바꾸거나 remote instance를 `SUSPENDED`로 바꿔도 관련 `ProfileFollow` row를 삭제하지 않는다. `SUSPENDED` 동안 GraphQL follow/unfollow action은 관계와 저장 count를 변경하지 않으며, instance가 다시 활성화되면 보존된 관계를 그대로 사용한다.
