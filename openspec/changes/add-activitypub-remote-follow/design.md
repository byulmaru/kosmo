...
- **unresponsive instance에는 outbound follow delivery를 하지 않는다.** `UNRESPONSIVE` instance는 저장된 stale profile 조회만 허용하고 outbound federation을 억제하므로 새 remote follow 관계 생성은 차단한다. 이미 존재하는 remote follow를 다시 follow하면 idempotent하게 기존 `ProfileFollow`를 반환하고, unfollow할 때는 local `ProfileFollow`를 제거하되 `Undo(Follow)`는 발송하지 않는다.
