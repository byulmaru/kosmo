...
- **projection handler가 연결되기 전에는 inbox delivery를 실패시킨다.** transport slice가 endpoint를 먼저 열더라도 placeholder handler는 성공으로 삼키지 않고 예외를 던져 Fedify가 non-2xx를 반환하게 한다. 실제 Follow/Undo/Accept/Reject handler가 연결될 때 placeholder를 교체하며, 그전까지 원격 서버가 delivery를 재시도할 수 있어야 한다.
- **follow transport 인자는 송수신 역할을 이름으로 구분한다.** `senderProfileId`는 activity를 서명해 보내는 local profile이고 `recipientActor`는 activity를 받을 remote actor다. Undo의 원본 Follow와 Accept의 수신 Follow는 각 actor/object가 이 delivery endpoint pair와 교차 일치하는지 전송 전에 검증한다.
- **shared inbox의 authenticated document loader identity는 후속 capability다.** 현재 shared inbox는 공개적으로 조회 가능한 remote actor/key 문서의 signature verification을 지원한다. Authorized-fetch/secure-mode 문서를 조회할 때 사용할 stable local signing identity와 Fedify `setSharedKeyDispatcher()` 연결은 임의 local profile을 암묵적으로 선택하지 않고 PROD-355에서 follow/post inbox 공통 정책으로 구현한다.
