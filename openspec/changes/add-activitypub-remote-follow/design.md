...
- **Accept correlation은 actor/object fallback을 허용하지만 Reject 삭제는 현재 Follow identity를 요구한다.** 저장된 outbound Follow id는 Undo(Follow) 구성과 후속 Fedify transport retry에 사용한다. inbound `Accept`는 embedded/typed Follow의 id가 없거나 kosmo outbound Follow URI가 아니어도 actor/object를 검증해 established projection에 idempotent하게 대응시킬 수 있다. 반면 inbound `Reject`는 local `ProfileFollow`를 삭제하는 destructive side effect이므로 embedded/typed Follow의 kosmo outbound Follow URI 또는 IRI-only kosmo outbound Follow URI가 현재 저장된 outbound Follow identity와 일치할 때만 관계를 제거한다. `Reject.object`가 id 없거나 non-kosmo id인 embedded/typed Follow이면 actor/object가 일치해도 late Reject로 새 refollow를 지울 수 있으므로 side effect 없이 무시한다.
...
- **inbound Undo(Follow)는 현재 inbound Follow id와 일치해야 삭제한다.** remote actor가 Follow/Undo/Follow를 반복하면 늦게 도착한 이전 `Undo(Follow)`가 새 관계를 지울 수 있다. 따라서 embedded/typed `Undo.object`의 Follow id가 현재 established `ProfileFollow` 또는 pending `ProfileFollowRequest`에 저장된 inbound Follow id와 일치할 때만 관계/request를 제거한다. IRI-only object와 id 없는 embedded/typed Follow는 이번 change에서 현재 generation을 검증할 수 없으므로 side effect 없이 무시한다.
