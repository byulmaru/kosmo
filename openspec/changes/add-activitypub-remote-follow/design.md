...
- **profile 비활성화는 follow row를 보존하고 count만 조정한다.** profile을 `DISABLED`로 바꿀 때 관련 `ProfileFollow` row를 삭제하지 않고, 남은 active 상대 profile의 저장 count에서 비활성화된 profile과의 관계를 제외한다.
