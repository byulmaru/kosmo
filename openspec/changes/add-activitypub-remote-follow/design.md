...
- **inbox listener는 실제 handler를 소유한 slice에서 등록한다.** transport slice는 actor-scoped inbox와 shared inbox 경로만 설정하고 placeholder listener를 두지 않는다. Follow/Undo/Accept/Reject projection을 구현하는 slice가 해당 activity type의 실제 listener를 싱글톤 federation에 직접 추가한다.
