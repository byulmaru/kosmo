...
- **unknown remote actor는 lookup 후 저장한다.** inbound follow protocol activity가 저장되지 않은 remote actor를 참조하면 actor URI만으로 `Profile`을 만들지 않고, actor URI host를 `acct:` domain으로 신뢰하지 않는다. Fedify/WebFinger lookup이 실제로 반환한 federated handle과 canonical actor URI가 inbound activity actor URI를 검증할 때만 #182의 materialization 경로로 저장한다.
