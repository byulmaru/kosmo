...
- **`UNRESPONSIVE`는 follow API action을 차단하지 않는다.** `UNRESPONSIVE`는 remote actor materialization/refresh 같은 내부 federation 상태이며, GraphQL `followProfile`/`unfollowProfile` 대상 eligibility는 이 상태로 분기하지 않는다.
