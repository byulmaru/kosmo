...
- **Accept/Reject correlation은 actor/object 기준이다.** 저장된 outbound Follow id는 Undo(Follow) 구성과 transport retry에 사용하지만, inbound Accept/Reject가 반드시 같은 Follow id를 참조해야 하는 것은 아니다. `Accept.object` 또는 `Reject.object`로 전달되거나 참조된 Follow의 actor/object가 저장된 outbound Follow의 local follower actor URI와 remote followee actor URI pair에 대응하면 같은 follow intent로 처리한다.
