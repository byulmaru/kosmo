...
- **AND** Mention node는 UUID `profileId` 하나만 attrs로 가지며, inbound target·anchor URI와 원문 anchor 표시 문자열을 저장하지 않는다
...
- **AND** GraphQL이 제공한 파생 `bodyText`를 Profile 재조회 없이 plain text fallback으로 표시한다
