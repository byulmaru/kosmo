...
- 사용자는 `PROD-910` renderer 구현을 먼저 진행하고 2.x 호환성 증거는 후속으로 검증하도록 결정했다. 따라서 이 deferred 상태는 `PROD-910`의 renderer·Profile route·접근성 구현과 통합 검증을 막지 않으며, reader evidence가 없는 상태에서 compatibility 완료나 writer activation 완료를 주장하지 않는다.
