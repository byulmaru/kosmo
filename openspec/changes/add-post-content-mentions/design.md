...
5. 문제 발생 시 신규 Mention 쓰기를 중지하면서 기존 Mention 읽기·본문 파생 지원과 기존 DB/data를 유지하고, 안전 link/text 경로로 수신을 계속한다. pre-Mention server binary 전체 rollback은 기존 bodyText 파생기가 Mention label을 잃을 수 있으므로 별도 호환·데이터 대응 증거 없이는 수행하지 않는다. 재활성화 조건은 통합 검증 기록에 남긴다.
