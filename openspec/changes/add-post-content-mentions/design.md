...
- target·anchor identity 증거가 저장 Profile stable identity와 어긋나거나 해석되지 않거나 표시 label이 안전한 표시·구조 검증을 통과하지 못한 tag를 안전한 link 또는 text로 보존하고 신규 원격 Profile lookup/materialization을 피한다. 표시 label이 Profile 이름·handle과 같다는 사실만으로 identity를 확정하지 않는다.
...
- `packages/fedify/src/inbound-create-note.ts`는 Note identity·visibility와 content/media projection을 호출한다. `to`/`cc` actor URI는 audience이고 `tag` Mention은 별도 입력이므로, 두 경로를 합쳐서 Mention을 추론하면 안 된다.
...
1. inbound adapter는 Fedify vocabulary에서 raw typed `Mention` candidate(type marker, target URI, safe label)만 수집해 core projection에 전달한다. `projectRemoteNoteContent`의 순수 HTML/plain-text parser는 candidate와 본문의 safe anchor label/href를 대조해 초기 Mention candidate 또는 link/text를 만들며 DB·stored Profile identity를 조회하지 않는다. 그 뒤 `createPost` transaction의 공통 저장 경계가 `{ target, href, label }`을 canonicalize하고 normalized target/href를 실제 `ActivityPubActors.uri`와 exact match해 같은 stored Profile identity일 때만 relation을 저장한다. Profile domain·handle이나 추측한 remote URL을 조합하지 않으며 원격 lookup을 수행하지 않는다. 표시 label이 Profile 이름·handle과 같은지만으로 연결하지 않으며, 일반 anchor와 audience는 기존 의미로 둔다.
...
- [stable identity 검증이 너무 느슨해 다른 Profile을 연결함] → target·anchor identity 증거를 저장 Profile stable identity와 검증하고 표시 label은 안전한 표시·구조만 확인하며, 실패 시 link/text fallback으로 낮춘다.
