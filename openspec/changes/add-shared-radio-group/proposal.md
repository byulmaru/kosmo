...
FeedbackForm과 ProfileDefaultPostVisibilityControl이 radio option의 semantics, 선택 상태와 Web keyboard 동작을 서로 다르게 구현한다. PROD-753에서 승인한 공용 계약으로 두 소비처를 수렴해 접근성과 상태 표현을 일관되게 유지한다.
...
- Feedback 종류와 기본 게시 공개 범위 선택을 공용 primitive로 이관한다.
- Feedback mutation·dirty/submitting, 공개 범위 저장·Relay actor와 각 option layout은 기존 소비처에 유지한다.
...
- FeedbackForm, ProfileDefaultPostVisibilityControl 소비처
