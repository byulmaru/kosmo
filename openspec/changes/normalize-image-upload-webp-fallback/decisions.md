> **2026-09-10 문서 정정:** 실제 Safari Web 앱 빌드의 fallback → PNG → signed PUT → `complete` 성공 검증을 선택적 evidence로 읽을 수 있던 표현을 필수 완료 검증으로 바로잡는다. 이는 2026-09-09 승인된 PNG fallback 제품 정책이나 범위를 변경하는 새 결정이 아니다.
...
- Confirmation / Follow-up: 공통 경계의 지원·미지원 실행 결과와 2048px/no-upscale을 검증한다. 실제 Safari Web 앱 빌드에서 fallback → PNG byte → `image/png` signed PUT → `complete` 성공을 확인하고 evidence를 기록해야 한다. WebKit smoke는 보조 증거이며, 실제 Safari evidence가 없으면 change를 완료로 보지 않는다.
...
- Consequences: 구현은 실제 WebP encoding 미지원 신호를 좁게 판별해야 한다. 검증은 공통 실행 테스트와 WebKit smoke를 보조로 수행하고, 실제 Safari Web 앱 빌드에서 fallback → PNG → signed PUT → `complete` 성공 evidence를 필수로 확보한다. Native 기기 결과를 Web 결과로 간주하지 않는다.
- Confirmation / Follow-up: WebP 성공, PNG fallback, 일반 오류와 body/MIME 불일치 방지의 실행 테스트를 통과시키고, 실제 Safari Web 앱 빌드의 fallback → PNG → signed PUT → `complete` 성공을 확인한다.
