> **2026-09-10 문서 정정:** 실제 Safari full-flow evidence를 선택적으로 기록하는 것처럼 읽힐 수 있던 표현을 완료 필수 검증으로 바로잡는다. 이는 2026-09-09 승인된 PNG fallback 제품 정책이나 범위를 변경하지 않는다.
...
5. WebP 지원 브라우저와 WebKit/Safari 유사 환경에서 차원, body, Content-Type, PUT/complete 순서를 실행 검증한다. 실제 Safari Web 앱 빌드에서 fallback → PNG byte → `image/png` signed PUT → `complete` 성공을 반드시 확인하고 evidence를 기록한다. WebKit 결과만으로 실제 Safari 검증을 충족한 것으로 간주하지 않는다.
...
- [브라우저가 WebP 미지원 여부를 변환 오류로 다르게 표현할 수 있음] → 현재 Web 구현이 반환하는 실제 MIME mismatch 신호를 기준으로 좁게 판별하고, Safari 실제 Web 앱 흐름과 WebP 지원 브라우저를 함께 검증한다.
