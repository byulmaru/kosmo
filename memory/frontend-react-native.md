...
- 새 게시글의 Home/Profile 목록 membership 소유권은 producer에 따라 나눈다. 현재 actor가 호출한 `createPost` 성공 결과는 서버가 Post List Policy로 판정해 반환한 canonical edge만 요청 actor Store의 이미 로드된 managed connection에 최신순·중복 없이 반영하고, 클라이언트가 후보·cursor를 합성하거나 광범위하게 refetch하지 않는다. 다른 Profile·remote ingestion 등 다른 producer의 장기 실시간 membership은 PROD-644 Relay subscription이 소유한다. actor Environment가 전환된 뒤 늦게 완료된 이전 요청은 이전 Store에만 적용하고 새 actor UI를 변경하지 않는다.
