...
- web shell은 `768px`와 `1280px`, native shell은 safe area를 기준으로 한다. 값은 `apps/app/src/theme/tokens.ts`의 `breakpoints`를 사용하며 컴포넌트마다 같은 숫자를 다시 쓰지 않는다.
...
- 현재 TipTap 계약은 doc/paragraph/text subset이다. `apps/app/src/lib/tiptap.ts`의 pure TypeScript adapter로 plain text를 변환하고 DOM TipTap runtime을 native bundle에 넣지 않는다.
