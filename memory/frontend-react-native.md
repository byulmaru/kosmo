...
- 새 게시글의 현재 actor Home/Profile 목록은 producer별로 별도 갱신한다. 현재 actor가 호출한 `createPost` 성공 결과의 normalized `post`만 요청 actor Store의 이미 로드된 managed connection에 Relay `@prependNode` directive로 최신순·중복 없이 반영하고, Post List 후보·정책 필드·cursor를 클라이언트에서 합성하거나 광범위하게 refetch하지 않는다. Reply Parent가 없으면 Home과 작성자 Profile connection ID를, Reply면 Home ID만 mutation에 전달하며 로드되지 않은 connection은 Relay가 만들지 않는다. 다른 producer의 membership 전달은 이 흐름의 계약이 아니다. actor Environment가 전환된 뒤 늦게 완료된 이전 요청은 이전 Store에만 적용하고 새 actor UI를 변경하지 않는다.
