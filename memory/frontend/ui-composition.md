...
- 개별 action은 대상의 fragment·mutation·pending/error 상태와 Relay/cache 갱신을 소유한다. 개별 action이 unrelated item과 전체 메뉴 trigger를 받아 메뉴 조합까지 소유하지 않는다. 같은 관계의 중복 fragment/prop·ID를 받아 합치지 않으며, 메뉴·리스트·화면은 action lifecycle 대신 viewer 방향의 노출·순서·layout·조합을 소유한다.
...
- 공개 callback은 실제 production 조정이 필요하거나 명시적인 controlled/presentation 계약일 때만 둔다. UI close나 toast 같은 후속 표시 callback은 허용하지만, 테스트 계측용 lifecycle callback이나 아직 production caller가 없는 미래 mutation callback을 공개 API로 올리지 않는다. callback 때문에 서버 상태 변경 책임을 메뉴·리스트·화면으로 옮기지 않는다.
