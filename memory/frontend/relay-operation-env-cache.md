...
- Relay fetch·refetch·pending·disposal은 표준 hook·fragment·normalized store·directive를 먼저 사용한다. 수동 `fetchQuery.subscribe`로 state 수명주기를 wrapper에 옮기지 않으며, actor 변경처럼 실제 Environment 교체가 필요한 경계, 요청 실패, 접근성 계약은 각자의 소유자가 보존한다.
