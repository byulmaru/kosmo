...
- mutation 응답은 영향받는 Node의 `id`와 변경된 필드를 선택해 Relay normalized store를 갱신한다. connection membership는 Relay connection directive를 우선하고, 표현할 수 없는 좁은 변경만 updater로 처리한다. payload가 부족하면 raw `RecordSource`/전역 cache 보정 대신 API 계약을 보완하며, 서버 재판정은 해당 query·connection만 invalidate/refetch한다.
...
- `Session.selectedProfile.id`가 바뀌면 Relay Environment와 Store를 새로 만든 뒤 Session과 현재 route query를 새 actor 기준으로 실행한다. 새 actor query가 준비되면 shell의 active profile UI는 `currentSession.selectedProfile` 결과에 수렴한다. 이전 actor Store를 재사용하지 않으며, `homeTimeline`과 canonical viewer-relative 상태인 `Profile.viewerState`를 필드 목록으로 invalidate하지 않는다. 관계 mutation의 성공·실패를 이유로 `resetActor()`를 호출하거나 전체 Store를 교체하지 않는다.
- Relay fetch·refetch·mutation·pending·disposal은 표준 Relay hook, normalized store, declarative directive를 사용한다. 기능 코드는 low-level Environment/Store 조작, mutation commit을 감싼 Promise, 별도 `pending`/`inFlight` 상태로 Relay lifecycle을 재구현하지 않는다. mutation document와 `useMutation`은 mutation을 실행하는 실제 action owner에 colocate하고 `useMutation`의 in-flight 상태를 사용한다. 표준 API로 표현할 수 없는 좁은 connection membership updater와 actor Environment provider boundary만 예외로 하며, actor 변경·요청 실패·접근성 계약은 각자의 소유자가 보존한다.
...
- 프로필 선택이 성공하면 응답의 `selectProfile.profile.id`를 actor reset input으로 사용하고 새 actor query가 준비되면 UI를 갱신한다. mutation payload의 부분 `Session.selectedProfile`을 이전 actor Store에 게시하지 않으며, GraphQL 오류 응답이나 network 오류에서는 actor reset을 시작하지 않고 기존 picker 오류 계약을 유지한다.
