...
- 로컬 `pnpm dev`는 `127.0.0.1:54328`의 영속 PostgreSQL과 Docker Temporal `start-dev`의 health를 기다리고 runtime·queue principal의 실제 인증과 DB/role/ACL 경계를 preflight한 뒤 API, Web, App, Worker, Fedify consumer를 실행한다. Application DB는 canonical `PG*`와 `kosmo_runtime`, queue는 별도 URL/password와 `kosmo_fedify_queue`를 사용하며 `DATABASE_URL`이나 owner/bootstrap credential을 child process에 전달하지 않는다. DB를 사용하지 않는 Expo App에는 PG/queue credential도 전달하지 않는다.
