...
- 로컬 `pnpm dev`와 `pnpm dev:worker`는 Docker Temporal `start-dev`의 health를 기다린 뒤 `TEMPORAL_ADDRESS=127.0.0.1:7233`, `TEMPORAL_NAMESPACE=default`로 Worker를 실행한다. Local Temporal은 개발용 임시 저장소이며 persistent SQLite volume을 사용하지 않는다. Worker health 포트는 admin과 겹치지 않는 `127.0.0.1:8081`이다.
