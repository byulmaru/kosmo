...
- `post_content`: body revision, storing a plain-text projection plus TipTap JSON body, optional HTML body and Content Warning, and eventually `(post_id, revision_number)` uniqueness when revisioning lands. The DB/API domain name is `content_warning`/`contentWarning`; ActivityPub adapters map it to Note `summary`.
