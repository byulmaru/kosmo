...
- `post_content`: body revision, storing a schema version and canonical ProseMirror document JSON plus optional Content Warning, and eventually `(post_id, revision_number)` uniqueness when revisioning lands. Plain Text is derived rather than a second canonical stored body, and executable HTML is never stored. The DB/API domain name is `content_warning`/`contentWarning`; ActivityPub adapters map it to Note `summary`.
