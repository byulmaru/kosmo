...
- service는 production database implementation이 하나인 현재 구조에서 `@kosmo/core/db`의 shared `db`를 내부 사용한다. test seam이나 복수 implementation 요구 없이 호출자마다 `Database`를 전달하는 추상화는 만들지 않는다.
