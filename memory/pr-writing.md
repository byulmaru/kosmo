...
- 이미 OpenSpec에서 accepted durable decision으로 승인된 내용은 전문을 반복하지 않고 해당 decision과 이 PR의
  적용 결과를 링크한다. 구현 중 새로운 durable decision이 필요해졌다면 PR에만 남기지 않고 Linear와 OpenSpec을
  먼저 갱신한다.
- 새로 내린 중요한 결정이 없다면 `없음. 승인된 OpenSpec 결정을 변경 없이 적용했다.`처럼 명시한다. 사소한
  구현 선택이나 대안이 없었던 기계적 변경을 형식적인 결정으로 만들지 않는다.
...
- 관련 근거: <Linear, OpenSpec decision 또는 관련 문서 링크>
```
