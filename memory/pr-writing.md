...
- `이번 PR의 주요 결정`에는 관찰 가능한 행동, 데이터 의미, 보안, 호환성, rollout 또는 가역성에 영향을 주는
  중요한 선택만 기록한다. 단순한 함수명·파일·자료구조·helper·테스트 배치 같은 구현 선택은 결정으로
  포장하지 않는다.
...
- PR 본문에 기록한 구현 선택이나 OpenSpec session-harness 메모는 구현 맥락일 뿐 durable authority가 아니다.
  새로운 비즈니스·보안·호환성·rollout 결정이 필요해졌다면 PR이나 OpenSpec에만 남기지 말고 적용되는
  canonical 문서와 Linear를 먼저 갱신한다.
- 새로 내린 중요한 결정이 없다면 `없음`이라고 명시한다. 세션 하네스의 메모를 적용했다는 형식적인 문구로
  사소한 구현 선택에 권위를 부여하지 않는다.
...
- 관련 근거: <canonical 문서와 최신 Linear 링크; 필요하면 현재 session-harness 메모>
```
