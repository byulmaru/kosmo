...
- 이미 열린 이슈의 완료 과정에서 발견된 작은 후속 수정·hotfix가 같은 목표와 검증 책임 안에 있으면 새
  Linear 이슈를 만들지 않고 `<issue-id>-<short-description>` 브랜치를 사용한다. 새 deliverable, 독립된
  우선순위·소유권 또는 별도 완료 조건이 생긴 경우에만 이슈를 분리한다.
