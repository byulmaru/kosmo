...
- 프로필 handle 표시 정책은 컴포넌트마다 ad hoc 처리하지 않는다. GraphQL에서는 표시용 문자열로 `Profile.displayHandle`을 사용하고, `Profile.handle`은 URL lookup, local handle validation, route 생성용 bare handle로 둔다.
