...
- core service는 caller들이 공유하는 Profile/Post lifecycle, 관계 구조, visibility·eligibility, domain error, transaction, persistence, uniqueness와 idempotency를 소유한다. 여러 진입점이 같은 entity를 materialize하면 공통 검증·정책과 idempotent persistence를 한 경계에서 재사용하고 최종 insert만 공유하지 않는다. 진입점별 envelope·inbox relevance·signature/auth 검증은 각 entry boundary에 남긴다.
- 예상 가능한 domain policy 거절은 각 caller의 observability·retry 경계에서 internal failure와 구분한다. 선언된 거절 관측은 공통 경계 또는 우회할 수 없는 caller가 맡아 같은 처리에서 누락·중복 기록하지 않는다. 실제 내부 오류의 전파·재시도 계약은 유지한다.
