...
- 계약을 바꿀 때는 기존 requirement 변경은 해당 capability의 완전한 `MODIFIED` delta에 반영하고, 영향을 받는 모든 capability와 경쟁하는 active delta, `proposal.md`·`decisions.md`·`tasks.md`를 함께 대조한다. `openspec validate --all --strict`는 구조·형식 검증이지 의미 충돌이나 authority 합의를 판정하지 않으므로, 상세 gate는 [`memory/issue-openspec-workflow.md`](../issue-openspec-workflow.md)를 따른다.
