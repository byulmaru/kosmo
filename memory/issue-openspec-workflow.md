...
이 문서는 새 제품 행동·공개 계약·durable decision을 이슈와 OpenSpec으로 계획하거나 그 lifecycle을 갱신할 때
적용한다. 제품 행동·공개 계약·durable decision을 바꾸지 않는 문서 정리, 기존 계약을 그대로 적용하는 구현과
행동 변화 없는 정리는 이 흐름을 새로 시작하지 않고 해당 canonical 문서와 필요한 검증만 사용한다. 파일 확장자나
변경 파일 수만으로 적용 여부를 판단하지 않는다.

현재 gate 안에서는 요청된 artifact와 focused validation을 끝까지 진행한다. 사용자의 명시적 지시나 최신
canonical·Linear와 독립 대조한 기존 결정은 다시 묻지 않는다. 범위, 관찰 가능한 행동, 공개 계약,
보안·롤아웃, 소유권 또는 되돌릴 수 없는 외부 상태를 바꾸는 미결정만 다음 gate로 넘기기 전에 사람에게 제시한다.
...
gate의 입력으로 사용해도 된다는 전환 승인이다. 사용자가 이미 해당 다음 단계와 범위를 명시적으로 승인했다면
같은 승인을 반복해서 요구하지 않는다.
...
- 새로 확정하거나 변경한 요구사항·결정, 포함·제외 범위와 남은 미결정
- 적용한 canonical 문서, 이슈 또는 OpenSpec artifact와 각 이슈의 구현·검증·archive 책임
- 의존성, 이전 승인과 달라진 점, 승인 뒤 착수할 다음 단계
...
Domain Gate를 넘길 때는 위 승인 목록에 변경한 요구사항, 닫힌 결정, 남은 미결정과 독립 전달 결과를 포함한다.
명시적 승인 전에는 Issue Gate로 넘어가지 않는다.
...
Issue Gate를 넘길 때는 생성·수정한 이슈의 제목, 전달 결과, 포함·제외 범위, 관계, blocker, 적용되는 OpenSpec과
각자의 구현·테스트·통합·archive 책임을 위 승인 목록으로 정리한다. 이슈를 미리 생성했더라도 명시적 승인
전에는 OpenSpec이 필요한 작업의 OpenSpec Gate로 넘어가지 않는다.
...
- 계획·구현 중 최신 canonical·Linear와 독립 대조한 결정이나 사용자의 명시적 지시로 정해지지 않은 선택이
  관찰 가능한 행동, 공개 계약, 데이터, 보안, 호환성, rollout, 가역성 또는 후속 구현 방향을 바꾸면 대안과
  trade-off를 제시하고 그 결정 경계에서 멈춘다. 그 밖의 routine 선택은 현재 범위와 계약 안에서 계속 진행하며,
  이미 정해진 선택을 다시 묻지 않는다.
- 구현 PR을 열거나 갱신할 때는 실제로 확정된 중요한 결정만 기록한다. 근거가 없는 선택이나 이유를 새 결정처럼
  추론하지 않는다.
...
각 구현 이슈의 완료 보고에는 실제 구현 결과, 중요한 구현 결정, 담당 테스트와 검증 증거, canonical
문서·Linear·OpenSpec 변경 여부와 남은 위험을 나열한다. 수정 요청이 있으면 PR과 관련 artifact를 갱신하고
필요한 검증을 다시 실행한다.

## 5. Completion Gate

완료는 첫 구현이 동작하는 시점이 아니라, 이슈가 소유한 scoped 결과와 필요한 문서·검증·수정이 끝난 시점이다.
다음 작업이나 선택적 개선은 후속 범위로 분리하고, 아래 조건을 충족하는 데 필요한 검증과 수정은 중간 리뷰를
기다리지 않고 계속 진행한다.
