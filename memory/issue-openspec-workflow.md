...
승인을 받는다. 승인을 기다리기 전에 문서와 OpenSpec 초안을 수정하거나 Linear 이슈를 생성할 수 있지만,
생성했다는 사실을 승인으로 간주하거나 다음 gate 착수 근거로 사용하지 않는다.
...
OpenSpec Gate 승인 요청은 requirement와 scenario, accepted/deferred decision, 구현 이슈별 task ownership,
권장 접근과 허용 대안, 검증 계획, canonical 문서·Linear와 달라진 점을 나열한다. 명시적 승인과 strict
validation 통과 전에는 Implementation Gate로 넘어가지 않는다.
