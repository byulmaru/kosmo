...
- disposable PostgreSQL에서 실제 package entrypoint와 repository migration 경로를 확인하려면 `pnpm --filter @kosmo/core test:migrate:smoke`를 실행한다. 이 smoke는 `scripts/test-db.mjs`로 빈 database를 만든 뒤 `pnpm --filter @kosmo/core db:migrate`를 두 번 실행하고, history 전체 prefix/count와 대표 최종 schema를 확인한다. Pending suffix resume은 `migrate.test.ts`의 직접 integration 검증으로 중복하지 않는다.
