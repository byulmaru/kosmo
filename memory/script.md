...
- Native test distribution 공통 기반은 `apps/app/mise.toml`, `apps/app/Gemfile`/lock과 `apps/app/fastlane`을 사용한다. 앱 하위 mise 설정은 root Node/pnpm 설정을 상속하고 Ruby만 앱 범위에 추가한다. PR·merge queue는 credential 없이 Android/iOS 입력 계약만 검증하고, 실제 Firebase 인증 smoke는 `main` 수동 실행과 protected `native-test-distribution` environment에서만 GitHub OIDC→WIF 단기 ADC로 수행한다.
