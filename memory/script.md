...
- `--optional-secret-path <path>`는 primary path 위에 package 전용 값을 덧씌우며 Vault KV의 missing status만 빈 overlay로 취급한다. API `dev`는 공용 local path와 optional `secret/kubernetes/kosmo/local/api`를 함께 읽어 API-only secret을 Web·Expo process와 분리한다.
