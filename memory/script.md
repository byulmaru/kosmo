...
- 로컬 `pnpm dev`와 `pnpm dev:worker`는 `pnpm temporal:up`으로 임시 저장소를 쓰는 Docker Temporal 개발 서버의 health를 기다린 뒤 Vault를 읽고 `TEMPORAL_ADDRESS=127.0.0.1:7233`, `TEMPORAL_NAMESPACE=default`를 명시적으로 주입한다. `dev`는 worker도 포함하며 worker health 포트는 admin과 겹치지 않는 `127.0.0.1:8081`이다.
