...
- Android test distribution은 `main`에서 수동으로만 실행하는 Android 전용 Fastlane lane이 clean CNG prebuild, fixed-key Release APK signing, APK 검증, 실제 ADB install/update/launch, Firebase App Distribution upload를 함께 소유한다. PR compile gate, iOS lane, 공통 Fastlane/WIF 기반 또는 read-only ADC smoke로 분리하지 않는다. CI versionCode는 `GITHUB_RUN_NUMBER * 100 + GITHUB_RUN_ATTEMPT`를 사용하고, 각 build의 `KOSMO_ANDROID_VERSION_CODE`를 CNG config에 주입한다. Signing material은 protected environment와 runner 임시 경로에만 둔다.
