...
- Native test distribution 공통 기반은 `apps/app/mise.toml`, `apps/app/Gemfile`/lock과 `apps/app/fastlane`을 사용한다. 앱 하위 mise 설정은 root Node/pnpm 설정을 상속하고 Ruby만 앱 범위에 추가한다. 관련 workflow·mise·Gemfile/lock·Fastlane 파일이 바뀐 PR만 credential 없이 Android/iOS 입력 계약을 검증하고, 실제 Firebase 인증 smoke는 `main` 수동 실행과 protected `native-test-distribution` environment에서만 GitHub OIDC→WIF 단기 ADC로 수행한다.
- Native distribution build number는 `GITHUB_RUN_NUMBER * 100 + GITHUB_RUN_ATTEMPT`를 기본으로 하고 attempt 100 이상을 거부한다. Firebase Android/iOS app·tester group과 Google WIF·service account·IAM 인프라는 PROD-303, signed build/upload는 PROD-285/286, 자동 trigger와 통합 smoke는 PROD-287이 소유한다.
