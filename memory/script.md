...
- Kubernetes의 PKI 모드는 Helm에서 opt-in이다. 같은 issuer의 server certificate, API와 web에 각각 발급하는 `kosmo_runtime` leaf certificate, 별도 `kosmo_migration` certificate 및 CNPG `streaming_replica` certificate를 VSO로 동기화하고 CNPG `cert` 인증에 연결한다. API와 web은 private key와 회전 경계만 분리하고 PostgreSQL 로그인은 공유한다. 첫 sync에서는 `postgresTls.enabled=true`, `clientAuthEnabled=false`로 VSO Secret과 CNPG TLS를 준비해 기존 PreSync migration과 workload가 password를 유지하고, 준비 완료 뒤 두 번째 sync에서 `clientAuthEnabled=true`로 client certificate 인증을 활성화한다. Vault PKI issuer·role·ACL provisioning과 실제 rollout은 이 저장소 밖의 인프라 선행조건이다. 외부 client CA Secret에는 private key가 없으므로 `replicationTLSSecret`도 반드시 Vault에서 공급한다.
