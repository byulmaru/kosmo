...
4. Independently re-read applicable product-canonical `docs/domain` documents and accepted ADRs, plus current Linear issue bodies, relations, and contract-changing comments. Read `docs/design` as the visual design source for visual structure/state/accessibility claims; its authority does not extend to product policy, permissions, state transitions, or server behavior. Check every requirement, active or legacy accepted decision, task Deliverable, and Guardrail against the appropriate authority. A missing or conflicting authority, unresolved `Blocked` decision, or non-superseded `Upstream Change Required` decision stops archival.
