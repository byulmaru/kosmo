...
2. Before writing artifacts, read the applicable product-canonical `docs/domain` documents and accepted ADRs, and independently verify the current Linear issue bodies, relations, and contract-changing comments. Read `docs/design` when the change has visual structure, state presentation, or accessibility scope, and record it as the visual design source for those presentation decisions; its authority does not extend to product policy, permissions, domain state transitions, or server behavior. Confirm the Domain and Issue Gates; OpenSpec cannot prove either gate.
