...
const addLocalReaction = (input: Omit<Parameters<typeof addReaction>[0], 'origin'>) =>
  addReaction({ ...input, origin: 'LOCAL' });
...
test('여섯 built-in Type을 정확한 Unicode 문자열로 저장하고 서로 공존시킨다', async () => {
  ...
  for (const type of reactionTypes) {
    results.push((await addLocalReaction({ ...input, type })).reaction);
  }
...
test('반복·동시 추가는 하나의 Reaction을 반환한다', async () => {
  ...
  const concurrent = await Promise.all(
    Array.from({ length: 4 }, () => addLocalReaction({ ...input, type: '🎉' })),
  );
  const repeated = await addLocalReaction({ ...input, type: '🎉' });
...
test('addReaction 결과는 새 source만 구분한다', async () => {
  const { input } = await createFixture();
  const first = await addLocalReaction({ ...input, type: '🎉' });
  const repeated = await addLocalReaction({ ...input, type: '🎉' });
...
test('core는 entry에서 검증된 actor의 Profile/Instance 상태를 다시 조회하지 않는다', async () => {
  ...
  for (const fixture of fixtures) {
    const { reaction } = await addLocalReaction({ ...fixture.input, type: '👀' });
...
test('활성 Post가 아니면 Reaction을 만들지 않는다', async () => {
  ...
  await assert.rejects(addLocalReaction({ ...deletedPost.input, type: '☘️' }), NotFoundError);
  assert.equal(await countReactions(deletedPost.post.id), 0);

  const actor = await createFixture();
  await assert.rejects(
    addLocalReaction({ ...actor.input, postId: crypto.randomUUID(), type: '🌈' }),
    NotFoundError,
  );
  assert.equal(
    await db
...
test('caller transaction이 rollback되면 ActivityPub Reaction도 남지 않는다', async () => {
  const { input } = await createFixture({ instanceKind: InstanceKind.ACTIVITYPUB });

  await assert.rejects(
    db.transaction(async (tx) => {
      await addReaction({ ...input, origin: 'ACTIVITYPUB', type: '🌈' }, tx);
...
test('Owner는 Post가 unavailable해져도 Post와 Type으로 Reaction을 삭제한다', async () => {
  ...
  const { reaction } = await addLocalReaction({ ...fixture.input, type: '❤️' });
...
test('Reaction 삭제는 실제 삭제된 ID의 Notification만 정리한다', async () => {
  ...
  const { reaction } = await addLocalReaction({
...
test('없는 조합은 no-op이고 오래된 Post/Type 재시도는 재생성된 Reaction을 제거한다', async () => {
  ...
  const { reaction: first } = await addLocalReaction({ ...fixture.input, type: '👀' });
  await deleteReaction({
    actorProfileId: fixture.profile.id,
    postId: fixture.post.id,
    type: first.type,
  });
  const { reaction: recreated } = await addLocalReaction({ ...fixture.input, type: '👀' });
...
test('다른 Profile의 Reaction은 삭제하지 않는다', async () => {
  ...
  const { reaction } = await addLocalReaction({ ...owner.input, type: '🌈' });
