...
test('pair 조회와 승인은 request를 relation으로 원자적으로 전환한다', async () => {
  ...
  const found = await db
    ...
    .where(
      and(
        eq(ProfileFollowRequests.followeeProfileId, followee.id),
  ...
  assert.equal(approved.profileFollow.id, found.id);
  assert.equal(approved.profileFollow.createdAt.toString(), found.createdAt.toString());
...
test('승인은 기존 relation을 재사용하고 count를 중복 증가시키지 않는다', async () => {
  ...
  assert.equal(approved.followerProfile.followingCount, 1);
  assert.equal(approved.followeeProfile.followersCount, 1);
  assert.deepEqual(await readNotifications(existing.id), []);
});
...
test('원격 Accept는 비활성 local participant의 pending을 보존한다', async () => {
...
test('원격 Reject는 비활성 local participant의 pending을 보존한다', async () => {
  const fixture = await createRemotePendingRequest();
  await db
    .update(Profiles)
    .set({ state: ProfileState.DISABLED })
...
test('원격 Accept 동시 처리는 pending을 한 번만 relation으로 승격한다', async () => {
  ...
  const relation = await db
    .select()
    .from(ProfileFollows)
    .where(eq(ProfileFollows.id, fixture.request.id))
    .then(firstOrThrow);
  assert.equal(relation.createdAt.toString(), fixture.request.createdAt.toString());
  assert.deepEqual(await readRemotePairCounts(fixture), {
    followeeFollowers: 1,
    followerFollowing: 1,
  });
