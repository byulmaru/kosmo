...
/**
 * Creates the application federation with the production listeners and dispatchers.
 *
 * Loader overrides are intentionally limited to the test harness boundary. Production callers
 * use the exported singleton below and therefore retain Fedify's default public-URL validation.
 */
export const createKosmoFederation = (
  ...
  federation
    .setFollowersDispatcher(
      ...
      async (context, identifier, cursor) =>
        cursor == null && (await findActiveLocalProfile(context, identifier))
          ? { items: [] }
          : null,
    )
    .setCounter(async (context, identifier) =>
      findActiveLocalProfile(context, identifier).then(
  ...
  federation
    ...
    .setCounter(async (context, identifier) =>
      findActiveLocalProfile(context, identifier).then(
        (profile) => profile?.followingCount ?? null,
      ),
    );
